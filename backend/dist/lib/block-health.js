export function cropHealthFromTone(tone) {
...
export function soilHealthMeta(raw) {
    const health = String(raw ?? 'good').toLowerCase();
    if (health === 'good')
        return { soilHealthLabel: 'Good', soilHealthStatus: 'stable' };
    if (health === 'medium')
        return { soilHealthLabel: 'Average', soilHealthStatus: 'monitor' };
    return { soilHealthLabel: 'Need assistance', soilHealthStatus: 'alert' };
}
//# sourceMappingURL=block-health.js.map