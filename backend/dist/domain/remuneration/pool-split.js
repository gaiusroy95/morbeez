export function resolvePoolSplit(input) {
    ...
    const partnerMaxPct = partnerRaw == null || !Number.isFinite(Number(partnerRaw))
        ? poolPct
        : Math.round(Math.max(0, Number(partnerRaw)) * 100) / 100;
    const agronomistMaxPct = agroRaw == null || !Number.isFinite(Number(agroRaw))
        ? 0
        : Math.round(Math.max(0, Number(agroRaw)) * 100) / 100;
    return { poolPct, agronomistMaxPct, partnerMaxPct };
}
export function validatePoolSplit(split) {
...
//# sourceMappingURL=pool-split.js.map