export const DEFAULT_PARTNER_KPI_WEIGHTS = [
...
export const DEFAULT_AGRONOMIST_KPI_WEIGHTS = [
...
export function parameterScore(actual, target) {
    const t = Number(target);
    if (!Number.isFinite(t) || t <= 0)
        return 0;
    return clampScore((Number(actual) / t) * 100);
}
export function validateWeights(parameters) {
    if (!parameters.length)
        throw new Error('KPI weights cannot be empty');
    ...
    for (const p of parameters) {
        if (!p.key)
            throw new Error('Each KPI parameter needs a key');
        if (Number(p.weightPct) < 0)
            throw new Error(`Negative weight on ${p.key}`);
        if (!(Number(p.target) > 0))
            throw new Error(`Target must be > 0 on ${p.key}`);
    }
}
export function scoreWeightedKpi(parameters, actuals) {
...
//# sourceMappingURL=weighted-kpi.js.map