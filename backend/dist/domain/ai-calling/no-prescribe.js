...
const BLOCK_RE = new RegExp(`\\b(${CHEMICAL_BLOCKLIST.map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'i');
export function findPrescriptionLeak(text) {
    const m = text.match(BLOCK_RE);
    return m ? m[1] : null;
}
export function assertNoPrescription(text, context) {
    const leak = findPrescriptionLeak(text);
    if (leak) {
        throw new Error(`${context}: calling script must not prescribe "${leak}"`);
    }
}
export function stripPrescription(text) {
    return text.replace(BLOCK_RE, 'the recommended application');
}
//# sourceMappingURL=no-prescribe.js.map