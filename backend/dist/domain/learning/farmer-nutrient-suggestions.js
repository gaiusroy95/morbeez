...
export function extractAllFarmerNutrientLabels(raw) {
    const t = raw.trim();
    if (!t)
        return [];
    const found = [];
 ...
 */
export function extractAllFarmerSuggestedDiagnoses(raw) {
    const t = raw.trim();
    if (!t)
        return [];
        ...
        if (listMatch?.[1]) {
            ...
            for (const p of parts) {
                ...
                if (nutrientFromPart.length) {
                    for (const d of nutrientFromPart)
                        push(d);
                }
                else {
 ...
 */
export function summarizeFarmerNutrientSuggestion(raw) {
...
export function mapFarmerSuggestionInput(raw) {
    const t = raw.trim();
    if (!t)
        return undefined;
    const lower = t.toLowerCase();
...
export function isFarmerSuggestionButtonId(raw) {
    const lower = raw.trim().toLowerCase();
    if (lower === FARMER_SUGGEST_OTHER_BUTTON_ID)
        return true;
...
export function getFarmerSuggestedDiagnosesFromStored(params) {
    const refined = params.metadata?.farmer_refined_assessment;
...
//# sourceMappingURL=farmer-nutrient-suggestions.js.map