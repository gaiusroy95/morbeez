...
export const opportunityIntelligenceAlertsService = {
    ...
    async dismiss(alertId, adminUserId) {
        const { error } = await supabase
            .from('opportunity_intelligence_alerts')
            .update({
            status: 'dismissed',
    ...
    async generateDailyAlerts() {
        ...
        const { data: atRiskRows, error: atRiskErr } = await supabase
            ...
            .select('farmer_id, risk_band, days_since_last_inbound, retention_score, farmers(name, phone)')
        ...
        for (const row of atRiskRows ?? []) {
            ...
            const alertType = band === 'churned' ? 'farmer_churned' : 'farmer_at_risk';
            if (alertType === 'farmer_churned')
                farmerChurned += 1;
            else
                farmerAtRisk += 1;
        ...
        for (const emp of employees ?? []) {
            ...
            if (farmerIds.length < MIN_ATTRIBUTED_FARMERS_FOR_LEADERBOARD)
                continue;
            const { data: retention } = await supabase
                .from('farmer_retention_tracking')
                .select('risk_band')
                .in('farmer_id', farmerIds);
            const atRiskCount = (retention ?? []).filter((r) => r.risk_band === 'at_risk' || r.risk_band === 'churned').length;
            const pct = atRiskCount / farmerIds.length;
            if (pct < cohortThreshold)
                continue;
            employeeCohort += 1;
            const prof = emp.employee_profiles;
            const result = await upsertAlert({
        ...
        logger.info({ created, skipped, farmerAtRisk, farmerChurned, highOpportunityIdle, employeeCohort }, 'Opportunity intelligence daily alerts generated');
        return {
            created,
            skipped,
            farmerAtRisk,
    ...
    async enqueueRetentionTasks(limit = 40) {
        const config = await opportunityIntelligenceConfigService.get();
        ...
        const retentionAlerts = alerts.filter((a) => (a.alertType === 'farmer_at_risk' || a.alertType === 'farmer_churned') && a.farmerId);
        let tasksCreated = 0;
...
//# sourceMappingURL=opportunity-intelligence-alerts.service.js.map