...
import { DEFAULT_ATTRIBUTION_WEIGHTS, } from './employee-attribution.types.js';
function mapRow(row) {
    return {
        id: String(row.id),
...
export const employeeAttributionService = {
    ...
    async listForFarmer(farmerId, activeOnly = true) {
        let q = supabase
            .from('employee_farmer_attribution')
            .select('*')
    ...
    async listEligibleForConversion(farmerId, windowDays = 180) {
    ...
    async listForEmployee(employeeProfileId, activeOnly = true) {
        let q = supabase
            .from('employee_farmer_attribution')
            .select('*')
...
//# sourceMappingURL=employee-attribution.service.js.map