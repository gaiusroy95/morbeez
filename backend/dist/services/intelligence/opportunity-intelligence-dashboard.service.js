...
import { buildFarmerScorePresentation, } from './intelligence-score-presentation.service.js';
import { fetchOpportunityScoresByFarmerIds, fetchRetentionByFarmerIds, } from './intelligence-farmer-score-queries.util.js';
function daysAgoIso(days) {
    return new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
}
function tierFromRatio(points, max, high, medium, low) {
    if (max <= 0)
        return low;
...
export function buildFarmerSummary(score, retention) {
    if (!score)
        return null;
    const c = score.components;
    const retentionRiskLabel = retention?.riskBand === 'healthy'
        ? 'Low'
...
export const opportunityIntelligenceDashboardService = {
    async getOverview(periodDays = 30) {
        ...
        const [{ count: farmersScored }, { data: scoreRows }, { data: retentionRows }, { count: events30d }, { count: conversions30d }, { count: activeAttributions }, { count: employeesScored }, { data: lastFarmer }, { data: lastEmployee },] = await Promise.all([
        ...
        const avgOpportunityScore = scores.length > 0 ? Math.round((scores.reduce((a, b) => a + b, 0) / scores.length) * 10) / 10 : 0;
    ...
    async getDistrictHeatmap(limit = 40) {
    ...
    async listAtRiskFarmers(limit = 50) {
        const { data, error } = await supabase
            .from('farmer_retention_tracking')
            .select('farmer_id, risk_band, days_since_last_inbound, farmers(name, phone, district)')
    ...
    async listTopFarmers(opts) {
        ...
        if (opts?.minScore != null)
            q = q.gte('opportunity_score', opts.minScore);
    ...
    async getFarmerProfile(farmerId) {
        ...
        const retentionDto = retention
            ? {
                ...
                daysSinceLastInbound: retention.days_since_last_inbound != null
                    ? Number(retention.days_since_last_inbound)
                    : null,
                calculatedAt: String(retention.calculated_at),
            }
            : null;
        const presentation = score != null
    ...
    async getEmployeeProfileForDashboard(employeeProfileId) {
    ...
    async getEventVolumeByType(periodDays = 30, types) {
        const since = daysAgoIso(periodDays);
        let q = supabase
            .from('farmer_events')
            .select('event_type')
...
//# sourceMappingURL=opportunity-intelligence-dashboard.service.js.map