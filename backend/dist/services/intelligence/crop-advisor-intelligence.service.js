...
import { buildEmployeeScorePresentation, } from './intelligence-score-presentation.service.js';
function displayName(farmer) {
    if (!farmer)
        return 'Farmer';
    if (farmer.name)
...
export const cropAdvisorIntelligenceService = {
    async getWorkspaceIntelligence(agentEmail) {
        ...
        if (farmerIds.length > 0) {
            ...
            const scoreByFarmer = new Map((scores ?? []).map((s) => [String(s.farmer_id), Number(s.opportunity_score)]));
            const retentionByFarmer = new Map((retention ?? []).map((r) => [String(r.farmer_id), r]));
    ...
    async enrichLeadRows(leads) {
        ...
        const scoreMap = new Map((scores ?? []).map((s) => [String(s.farmer_id), Number(s.opportunity_score)]));
...
//# sourceMappingURL=crop-advisor-intelligence.service.js.map