...
export const opportunityScoreStoreService = {
    ...
    async upsertFarmerScore(farmerId, components, factors) {
        const opportunityScore = Math.min(100, components.engagement +
            components.trust +
            components.acreSize +
    ...
    async getEmployeeScore(employeeProfileId) {
        const { data, error } = await supabase
    ...
    async upsertEmployeeScore(employeeProfileId, components, factors, attributedFarmerCount) {
        const performanceScore = Math.min(100, components.engagementGrowth +
...
//# sourceMappingURL=opportunity-score-store.service.js.map