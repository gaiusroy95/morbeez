...
export function scoreEngagement(signals) {
    ...
    if (signals.inboundCount30d <= 2)
        raw = 5;
    else if (signals.inboundCount30d <= 5)
        raw = 10;
    else if (signals.inboundCount30d <= 10)
        raw = 15;
    else
        raw = 20;
...
export function scoreTrust(signals) {
...
export function scoreAcreSize(totalAcreage) {
    ...
    if (totalAcreage < 1)
        score = 3;
    else if (totalAcreage < 3)
        score = 7;
    else if (totalAcreage < 10)
        score = 11;
    else
        score = 15;
    factors.push({
...
export function scoreAcrePotential(profile) {
...
export function scoreRelationship(signals) {
...
export function scoreAdvisoryCooperation(signals) {
...
export function scoreCropValue(primaryCrop) {
...
export function scoreReferralInfluence(signals) {
...
export function applyFarmerBehavioralModifiers(components, input) {
    const next = { ...components };
    const quietButBought = input.trust.ordersConverted180d > 0 && input.engagement.inboundCount30d <= 2;
    if (quietButBought) {
        next.trust = Math.min(next.trust, Math.floor(FARMER_OPPORTUNITY_WEIGHTS.trust * 0.55));
        next.relationship = Math.min(next.relationship, Math.floor(FARMER_OPPORTUNITY_WEIGHTS.relationship * 0.45));
        next.engagement = Math.min(next.engagement, Math.floor(FARMER_OPPORTUNITY_WEIGHTS.engagement * 0.5));
    }
    const highlyEngaged = input.engagement.inboundCount30d >= 8 &&
        (input.engagement.richMediaCount30d >= 2 || input.trust.roiEntryCount90d >= 1);
    if (highlyEngaged) {
        next.engagement = Math.max(next.engagement, Math.min(FARMER_OPPORTUNITY_WEIGHTS.engagement, 16));
    ...
    if (ref && !['whatsapp', 'organic', 'api', 'phone'].includes(ref)) {
        next.relationship = Math.min(FARMER_OPPORTUNITY_WEIGHTS.relationship, next.relationship + 2);
        next.trust = Math.min(FARMER_OPPORTUNITY_WEIGHTS.trust, next.trust + 1);
    }
    if (input.trust.recommendationsApplied90d >= 1 &&
        input.advisory.recommendationsCommunicated90d >= 1) {
        next.advisoryCooperation = Math.min(FARMER_OPPORTUNITY_WEIGHTS.advisoryCooperation, Math.max(next.advisoryCooperation, 7));
    }
    return next;
}
export function computeRetentionRisk(daysSinceLastInbound) {
...
//# sourceMappingURL=farmer-opportunity-scoring.util.js.map