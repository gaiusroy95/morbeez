...
function isMissingTableError(err) {
    ...
    return (msg.includes('farmer_events') &&
        (msg.includes('does not exist') || msg.includes('PGRST204') || msg.includes('schema')));
}
async function resolveLeadAssigneeEmail(farmerId) {
...
function mapOnboardSource(source) {
...
function mapWhatsAppMessageType(msgType, direction) {
    const t = msgType.toLowerCase();
    if (direction === 'outbound')
        return 'MESSAGE_SENT';
    if (t.includes('image') || t === 'sticker')
        return 'IMAGE_UPLOAD';
    if (t.includes('audio') || t.includes('voice') || t === 'ptt')
        return 'VOICE_NOTE';
    if (t.includes('video'))
        return 'IMAGE_UPLOAD';
...
export const farmerEventCaptureService = {
    ...
    async captureWhatsAppInteraction(params) {
        ...
        const assignee = params.employeeEmail ?? (params.direction === 'inbound' ? await resolveLeadAssigneeEmail(params.farmerId) : null);
    ...
    async recordReactivationIfNeeded(farmerId, assigneeEmail) {
