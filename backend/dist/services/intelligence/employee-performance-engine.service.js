...
import { computeEmployeeScoreComponents, } from './employee-performance-scoring.util.js';
function daysAgoIso(days) {
    return new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
}
const TRUST_EVENT_TYPES = ['ROI_ENTRY', 'RECOMMENDATION_APPLIED', 'RECOMMENDATION_APPROVED'];
async function loadSignals(employeeProfileId, agentEmail) {
    ...
    const conversionAssists180d = attributions.filter((a) => a.attributionType === 'conversion_assist' && a.lastTouchAt >= since180).length;
    const reactivations90d = attributions.filter((a) => a.attributionType === 'reactivation' && a.lastTouchAt >= since90).length;
    ...
    let avgFarmerRelationshipScore = null;
    let avgFarmerOpportunityScore = null;
    let healthyRetentionPct = null;
    if (farmerIds.length > 0) {
        ...
        const { data: retention } = await supabase
            ...
            .in('farmer_id', farmerIds);
        if (retention?.length) {
            const healthy = retention.filter((r) => ['healthy', 'watch'].includes(String(r.risk_band))).length;
...
export const employeePerformanceEngineService = {
    async scoreEmployee(employeeProfileId, agentEmail) {
        ...
        const components = opportunityIntelligenceConfigService.applyEmployeeWeightOverrides(rawComponents, config.effectiveEmployeeWeights);
        return opportunityScoreStoreService.upsertEmployeeScore(employeeProfileId, components, factors, signals.attributedFarmerCount);
    },
    async listProfileIdsForBatch(limit = 200) {
...
//# sourceMappingURL=employee-performance-engine.service.js.map