...
export const protocolDefinitionService = {
    ...
    async updateDraft(id, input) {
        ...
        if (input.issueLabel != null)
            patch.issue_label = input.issueLabel;
        if (input.stages != null)
            patch.stages = input.stages;
...
export const applicationHistoryService = {
    async listForFarmer(farmerId, blockId) {
        let q = supabase
...
export const experimentDefinitionService = {
    ...
    async assignOnVisitClose(fieldFindingId) {
        const running = await this.list('running');
        if (!running.length)
            return null;
        const exp = running[0];
        const rawVariants = exp.variants ?? [];
        const keys = rawVariants.map((v) => typeof v === 'string' ? v : String(v.key ?? 'variant'));
...
//# sourceMappingURL=enterprise-ops.service.js.map