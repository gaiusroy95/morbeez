...
export const plotDigitalTwinService = {
    async buildSnapshot(blockId, farmerId) {
        ...
        for (const row of soilReports ?? []) {
            ...
            if (!Number.isNaN(n))
                soilTrend.nitrogen?.push(n);
            if (!Number.isNaN(k))
                soilTrend.potassium?.push(k);
            if (!Number.isNaN(ph))
                soilTrend.ph?.push(ph);
        }
        ...
        const satelliteOverlays = satelliteRows.map((row) => ({
            ...
            provider: String(row.metadata?.provider ?? 'stub'),
        }));
    ...
    async getLatest(blockId) {
        ...
        return {
            ...
            outcomeHistory: (data.outcome_summary?.items) ?? [],
            visitCount12m: Number(data.trends?.visitCount12m ?? 0),
            soilTrend: data.trends?.soilTrend,
        };
    },
    formatForPrompt(trends) {
        if (!trends)
            return 'No plot memory on file';
...
//# sourceMappingURL=plot-digital-twin.service.js.map