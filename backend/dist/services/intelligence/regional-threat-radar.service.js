...
export const regionalThreatRadarService = {
    async computeForDistrict(district, cropType) {
        ...
        for (const [issueLabel, caseCount7d] of counts) {
            const threatLevel = caseCount7d >= 8 ? 'critical' : caseCount7d >= 5 ? 'high' : caseCount7d >= 3 ? 'medium' : 'low';
            if (threatLevel === 'low')
                continue;
            threats.push({
                district,
    ...
    async refreshAndPersist(district, cropType) {
    ...
    async listActive(opts) {
        let q = supabase
            ...
            .limit(opts?.limit ?? 50);
        if (opts?.district)
            q = q.ilike('district', opts.district);
        if (opts?.cropType)
            q = q.eq('crop_type', opts.cropType.toLowerCase());
        ...
        return (data ?? []).map((r) => ({
            ...
            trendDirection: String(r.trend_direction ?? 'stable'),
            reasoning: String(r.reasoning ?? ''),
            computedAt: String(r.computed_at),
        }));
    },
    async riskFlagsForFarmer(farmerId, cropType) {
...
//# sourceMappingURL=regional-threat-radar.service.js.map