...
export const farmerTerminologyMemoryService = {
    ...
    async lookup(params) {
        ...
        const ranked = rows
            ...
            if (crop && wantCrop && crop !== wantCrop)
                return null;
            if (district && wantDistrict && district !== wantDistrict)
                return null;
            if (crop && !wantCrop)
                return null;
            if (district && !wantDistrict)
                return null;
            let score = 0;
            if (district && wantDistrict)
                score += crop && wantCrop ? 100 : 80;
            else if (crop && wantCrop)
                score += 60;
            else if (!crop && !district)
                score += 40;
            else
                return null;
            return { row, score };
    ...
    async listForFarmer(params) {
        ...
        return (data ?? []).map((row) => mapOverride(row));
    },
};
//# sourceMappingURL=farmer-terminology-memory.service.js.map