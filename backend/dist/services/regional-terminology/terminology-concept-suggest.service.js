...
async function nextConceptCode(category) {
...
export const terminologyConceptSuggestService = {
    async suggestForTask(params) {
        ...
        if (!tokens.length)
            return { conceptId: null, conceptName: null, confidence: 0 };
        ...
        for (const t of terms ?? []) {
            ...
            for (const label of labels) {
                ...
                if (score >= 0.35) {
                    const rawConcept = t.agronomy_concepts;
    ...
    async attachSuggestionToTask(taskId) {
        ...
        if (!suggestion.conceptName || suggestion.confidence < 0.4)
            return;
        await supabase
            .from('terminology_review_tasks')
            .update({
...
//# sourceMappingURL=terminology-concept-suggest.service.js.map