...
async function freezeAttribution(orderId, farmerId) {
    const { data: order } = await supabase
        .from('commerce_orders')
        .select('attributed_partner_id, attributed_agronomist_email, farmer_id')
        .eq('id', orderId)
        .maybeSingle();
    if (order?.attributed_partner_id && order.attributed_agronomist_email)
        return order;
...
export const eligibleSaleEngine = {
    ...
    async onReturnOrRefund(orderId, reason) {
        await supabase
            .from('commerce_orders')
            .update({
    ...
    async refresh(orderId, facts) {
        const { data: order } = await supabase
            .from('commerce_orders')
            .select('id, farmer_id, total_amount, oms_status, delivered_at, payment_status, attributed_partner_id, attributed_agronomist_email, incentive_eligibility, incentive_eligible_at, incentive_excluded_reason')
            .eq('id', orderId)
            .maybeSingle();
        if (!order?.id)
            return null;
        ...
        if (result.status === 'eligible') {
            await this.creditIncentives(order);
        }
        return result;
    },
    async creditIncentives(order) {
    ...
    async scanDue(limit = 80) {
        ...
        for (const row of data ?? []) {
            await this.refresh(String(row.id)).catch((err) => logger.warn({ err, orderId: row.id }, 'Eligible-sale refresh failed'));
        }
        return { scanned: data?.length ?? 0 };
    },
};
//# sourceMappingURL=eligible-sale.engine.js.map