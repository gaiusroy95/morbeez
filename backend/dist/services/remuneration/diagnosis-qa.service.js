...
import { DEFAULT_DIAGNOSIS_QA_RULE, diagnosisAccuracyPct, diagnosisQaSampleSize, pickSample, } from '../../domain/remuneration/diagnosis-qa.js';
...
export const diagnosisQaService = {
    ...
    async summary(month, agronomistEmail) {
        let q = supabase
            .from('diagnosis_qa_samples')
            .select('status, agronomist_email')
            .eq('period_month', month);
        if (agronomistEmail)
            q = q.eq('agronomist_email', agronomistEmail.toLowerCase());
        const { data } = await q;
        const rows = data ?? [];
    ...
    async draw(month, force = false) {
        ...
        if (!picked.length)
            return { drawn: 0, existing: 0, month, sampleSize: size };
        const { error } = await supabase.from('diagnosis_qa_samples').insert(picked.map((row) => ({
            period_month: month,
    ...
    async ensureSample(month) {
        return this.draw(month, false);
    },
    async audit(id, input) {
        ...
        if (!row)
            throw new NotFoundError('QA sample not found');
        if (row.status !== 'pending')
            throw new ConflictError('This sample was already audited');
...
//# sourceMappingURL=diagnosis-qa.service.js.map