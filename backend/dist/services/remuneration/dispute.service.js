...
export const disputeService = {
    async list(filter = {}) {
        ...
        if (filter.partyType)
            q = q.eq('party_type', filter.partyType);
        if (filter.partyId)
            q = q.eq('party_id', filter.partyId);
        const { data } = await q;
        return data ?? [];
    },
    async open(input) {
        const { data: existing } = await supabase
            ...
            .maybeSingle();
        if (existing?.id)
            throw new ConflictError('This earning already has an open dispute');
    ...
    async resolve(id, status, actor, notes) {
...
async function writeAdjustment(row) {
    ...
    if (source === 'partner_ledger') {
        const { data: original } = await supabase
            ...
            .maybeSingle();
        if (!original)
            throw new NotFoundError('Original partner earning not found');
        ...
        if (adj.amountInr >= 0)
            throw new ValidationError('Dispute amount must recover a positive earning');
    ...
    const { data: original } = await supabase
        ...
        .maybeSingle();
    if (!original)
        throw new NotFoundError('Original agronomist earning not found');
    const adj = adjustmentRow({
        originalId: String(original.id),
