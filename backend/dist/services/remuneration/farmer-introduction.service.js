...
function hoursBetween(fromIso, to = new Date()) {
    const t = new Date(fromIso).getTime();
    if (Number.isNaN(t))
        return 0;
    return (to.getTime() - t) / 36e5;
}
async function farmerAcreage(farmerId) {
...
async function collectFacts(input) {
    const { data: farmer } = await supabase
        .from('farmers')
        .select('id, phone, name, village, district, created_at, assigned_crop_advisor, enrollment_owner_partner_id, source')
    ...
    if (input.introductionId)
        dupQuery = dupQuery.neq('id', input.introductionId);
    ...
    const farmerVerified = Boolean(phone && name && !/^Farmer\s+\d{4}$/i.test(name) && (farmer.village || farmer.district));
    ...
    const enrollmentPartner = farmer.enrollment_owner_partner_id
        ...
        : '';
    const existingOwnedByOther = Boolean(enrollmentPartner) && enrollmentPartner !== input.partnerId && !newFarmer;
    return {
        phone,
...
export const farmerIntroductionService = {
    ...
    async refresh(introductionId) {
        ...
        if (!intro?.id)
            return null;
        if (['rejected', 'fraud_hold'].includes(String(intro.qualification_status)))
            return intro;
        ...
        const patch = {
            ...
            cash_reward_amount: alreadyEligible || result.cashEligible
    ...
    async refreshForFarmer(farmerId) {
    ...
    async scanPending(limit = 80) {
        const { data } = await supabase
            ...
            .limit(limit);
        for (const row of data ?? []) {
            await this.refresh(String(row.id)).catch((err) => logger.warn({ err, id: row.id }, 'Introduction refresh failed'));
        }
    ...
    async list(opts) {
        let q = supabase
            .from('farmer_introductions')
            .select('id, partner_id, farmer_id, qualification_status, acreage, cash_reward_amount, product_reward_max, product_reward_used, product_reward_balance, pending_reasons, created_at, farmer_mobile, location')
            .order('created_at', { ascending: false })
            .limit(opts?.limit ?? 100);
        if (opts?.partnerId)
            q = q.eq('partner_id', opts.partnerId);
        if (opts?.status)
            q = q.eq('qualification_status', opts.status);
        const { data } = await q;
        return data ?? [];
    },
    async summaryForPartner(partnerId) {
        const { data } = await supabase
            .from('farmer_introductions')
            .select('qualification_status, cash_reward_amount, cash_rewarded_at, product_reward_max, product_reward_used, product_reward_balance')
            .eq('partner_id', partnerId);
        const rows = data ?? [];
        const introduced = rows.length;
        const verified = rows.filter((r) => ['eligible', 'review'].includes(String(r.qualification_status))).length;
...
//# sourceMappingURL=farmer-introduction.service.js.map