...
import { blocksPayout } from '../../domain/remuneration/fraud-hold.js';
async function refreshHolds(partyType, partyId) {
    ...
    for (const row of rows ?? []) {
        ...
        if (Boolean(row.fraud_hold) === hold)
            continue;
...
export const fraudFlagService = {
    ...
    async open(input) {
        ...
        if (data)
            await refreshHolds(input.partyType, input.partyId);
        return data;
    },
    async setStatus(id, status, actor) {
        ...
        await refreshHolds(String(row.party_type), String(row.party_id));
        return data;
    },
    async scan(limit = 80) {
        let opened = 0;
...
//# sourceMappingURL=fraud-flag.service.js.map