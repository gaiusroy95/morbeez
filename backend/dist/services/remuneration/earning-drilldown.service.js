...
import { addToBucket, emptyBuckets, lastNMonths, } from '../../domain/remuneration/earning-drilldown.js';
import { settlementService } from './settlement.service.js';
export const earningDrilldownService = {
    lastThreeMonths(asOf = new Date()) {
        return lastNMonths(3, asOf);
    },
    async forParty(partyType, partyId, months = lastNMonths(3)) {
        ...
        for (const row of settlements ?? []) {
            ...
            if (row.fraud_hold)
                addToBucket(buckets, month, 'held', amount);
            else if (row.status === 'paid')
                addToBucket(buckets, month, 'paid', amount);
...
//# sourceMappingURL=earning-drilldown.service.js.map