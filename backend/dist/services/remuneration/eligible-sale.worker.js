...
async function poll() {
    ...
    if (scanned > 0)
        logger.info({ scanned }, 'Eligible-sale worker refreshed orders');
    const intros = await farmerIntroductionService.scanPending(80);
    if (intros.scanned > 0)
        logger.info({ scanned: intros.scanned }, 'Introduction eligibility refreshed');
    ...
    if (cases.scanned > 0)
        logger.info({ scanned: cases.scanned, month }, 'Qualified cases scanned');
...
export function startEligibleSaleWorker() {
...
//# sourceMappingURL=eligible-sale.worker.js.map