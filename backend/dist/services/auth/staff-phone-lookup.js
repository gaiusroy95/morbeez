...
export function tenDigitPhone(raw) {
    const digits = raw.replace(/\D/g, '');
    if (digits.length === 12 && digits.startsWith('91'))
        return digits.slice(2);
    return digits.slice(-10);
}
function profilePhoneMatches(profile, ten) {
    for (const field of ['personal_mobile', 'company_whatsapp', 'alternate_mobile']) {
        const value = profile[field];
        if (value && tenDigitPhone(String(value)) === ten)
            return true;
    }
    return false;
}
export async function findActiveAdminByPhone(phoneRaw) {
    ...
    const profile = (profiles ?? []).find((row) => profilePhoneMatches(row, ten));
    if (!profile?.admin_user_id) {
        throw new UnauthorizedError('No staff account is linked to this mobile number. Ask your manager to add it in HR.');
    }
...
//# sourceMappingURL=staff-phone-lookup.js.map