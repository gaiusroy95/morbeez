import { applyEvidenceDeltas, buildHypothesisDistribution, distributionThresholdReached, DEFAULT_TARGET_CONFIDENCE, } from '../../domain/visit-ai/confidence-distribution.js';
import { resolveConfidenceAction } from '../../domain/ai-training/confidence-routing.js';
import { supabase } from '../../lib/supabase.js';
import { throwIfSupabaseError } from '../../lib/supabase-errors.js';
import { NotFoundError } from '../../lib/errors.js';
async function getCaseOrThrow(aiCaseId) {
    ...
    if (!data)
        throw new NotFoundError('AI case not found');
    return data;
}
function loadDistributionFromMeta(meta, hypotheses) {
    const stored = meta.confidenceDistribution;
    if (stored?.hypotheses?.length)
        return stored;
    return buildHypothesisDistribution(hypotheses.map((h) => ({ label: h.label, confidence: h.confidence })), meta.targetConfidence ?? DEFAULT_TARGET_CONFIDENCE);
}
async function persistDistribution(aiCaseId, dist, meta) {
    const nextMeta = {
        ...meta,
...
function inferEvidenceDeltas(questionText, answer, hypotheses) {
    const q = questionText.toLowerCase();
    const a = answer.toLowerCase().trim();
    if (!a || a === 'unknown' || a === 'not checked')
        return null;
    ...
    if (/thrips|silver streak|pest|mite/i.test(q)) {
        ...
        if (yes && pestLabel) {
            ...
            if (top && pestLabel !== top)
                deltas.push({ label: top, delta: -4 });
            return deltas;
        }
        if (no && pestLabel) {
...
async function loadQuestions(aiCaseId) {
...
function nextUnanswered(questions) {
...
function buildConfidenceState(dist, questions) {
...
export const visitAiConfidenceEngineService = {
    async getConfidenceState(aiCaseId) {
        ...
        const { data: hypothesisRows } = await supabase
            ...
            .eq('visit_ai_case_id', aiCaseId)
            .order('sort_order', { ascending: true });
        const dist = loadDistributionFromMeta(meta, (hypothesisRows ?? []).map((h) => ({
    ...
    async initializeFromHypotheses(aiCaseId, hypotheses) {
        const caseRow = await getCaseOrThrow(aiCaseId);
        const meta = caseRow.metadata ?? {};
    ...
    async applyAnswer(aiCaseId, questionId, answer) {
        const caseRow = await getCaseOrThrow(aiCaseId);
        const meta = caseRow.metadata ?? {};
        ...
        const { data: hypothesisRows } = await supabase
            ...
            .eq('visit_ai_case_id', aiCaseId)
            .order('sort_order', { ascending: true });
        let dist = loadDistributionFromMeta(meta, (hypothesisRows ?? []).map((h) => ({
            label: String(h.label),
            confidence: Number(h.confidence),
        })));
        const ruleDeltas = inferEvidenceDeltas(String(qRow.question_text), answer, dist.hypotheses);
...
//# sourceMappingURL=visit-ai-confidence-engine.service.js.map