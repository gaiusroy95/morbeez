...
import { appendAuditEntry, formatAuditLabel, readAuditLog, } from './recommendation-audit.util.js';
import { canApproveRecommendations } from '../../lib/console-roles.js';
const EDITABLE_STATUSES = ['draft', 'pending_approval'];
const LIST_SELECT = '*, farmers(id, name, phone, preferred_language), farm_blocks(id, name, crop_type, plot_label)';
...
export const recommendationApprovalsService = {
    ...
    async getDetail(id) {
        ...
        return {
            ...
            canEdit: EDITABLE_STATUSES.includes(String(row.status)),
    ...
    async update(id, patch, editorEmail, editorRole) {
        ...
        const changed = [];
        if (patch.issueDetected !== undefined)
            changed.push('issue');
        if (patch.recommendationText !== undefined)
...
//# sourceMappingURL=recommendation-approvals.service.js.map