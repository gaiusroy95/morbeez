...
export function buildApprovedRecommendationMessage(row, extras) {
    ...
    const productLines = extras?.products?.map((p) => {
...
export const recommendationCommunicationService = {
    ...
    async previewVisitMessages(input) {
        const { data: farmer } = await supabase
            ...
            .maybeSingle();
        const lang = String(farmer?.preferred_language ?? 'en').toLowerCase();
        const allProducts = input.recommendationGroups?.flatMap((g) => g.materials.map((m) => ({
        ...
        return input.issues.map((issue, issueIndex) => {
            ...
            const issueProducts = allProducts.filter((p) => p.issueIndex === undefined || p.issueIndex === issueIndex);
            const productsForIssue = issueProducts.length > 0
                ? issueProducts
                : allProducts.length
                    ? allProducts
