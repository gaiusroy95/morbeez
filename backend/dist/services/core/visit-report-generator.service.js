...
export const visitReportGeneratorService = {
    async generate(findingId) {
        const { data, error } = await supabase
            .from('crm_field_findings')
            .select(`id, farmer_id, block_id, crop_type, observations, disease_pest, visited_at,
        ...
        const farmerRaw = data.farmers;
        const blockRaw = data.farm_blocks;
        const farmer = (Array.isArray(farmerRaw) ? farmerRaw[0] : farmerRaw);
        const block = (Array.isArray(blockRaw) ? blockRaw[0] : blockRaw);
        const farmerName = [farmer?.first_name, farmer?.last_name].filter(Boolean).join(' ') ||
...
//# sourceMappingURL=visit-report-generator.service.js.map