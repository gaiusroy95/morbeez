...
export const outcomeHumanRoutingService = {
    async decide(params) {
        ...
        if (level === 'worse')
            reasons.push('crop_worsened');
        if (level === 'no_improvement')
            reasons.push('no_improvement');
        ...
        if (repeatFailures >= 2)
            reasons.push('repeat_failed_outcomes');
        if (params.kpi.repeatedIssue)
            reasons.push('repeated_issue_reported');
        const autoSuccess = (level === 'fully_improved' || level === 'slight_improvement') &&
            aiConf >= 0.65 &&
            !reasons.some((r) => ['crop_worsened', 'no_improvement', 'high_severity_case', 'repeat_failed_outcomes'].includes(r));
    ...
    async countRecentFailedOutcomes(farmerId) {
...
//# sourceMappingURL=outcome-human-routing.service.js.map