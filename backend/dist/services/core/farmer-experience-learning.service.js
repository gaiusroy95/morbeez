...
function mapRow(r) {
    return {
        ...
        crop_experience_years: r.crop_experience_years != null ? Number(r.crop_experience_years) : null,
...
export const farmerExperienceLearningService = {
    ...
    async updateCapture(id, patch) {
        ...
        if (patch.farmer_prior_product !== undefined)
            row.farmer_prior_product = patch.farmer_prior_product;
        if (patch.farmer_prior_outcome !== undefined)
            row.farmer_prior_outcome = patch.farmer_prior_outcome;
        if (patch.crop_experience_years !== undefined) {
            row.crop_experience_years = patch.crop_experience_years;
        }
        if (patch.capture_step !== undefined)
    ...
    async getById(id) {
        const { data, error } = await supabase
            ...
            .single();
        if (error || !data)
            throw new NotFoundError('Farmer feedback not found');
        return mapRow(data);
    },
    async listPendingReview(limit = 40) {
        const { data, error } = await supabase
            .from('farmer_advisory_feedback')
            .select(`*, farmers(name, phone, district), ai_advisory_sessions(crop_type, symptoms_text, crop_stage)`)
        ...
        return (data ?? []).map((row) => {
            ...
            const session = r.ai_advisory_sessions;
            return {
                ...mapRow(r),
                farmer: farmer
                    ? {
    ...
    async getDetail(id) {
        ...
        const { data: farmerRow } = await supabase
            ...
            .eq('id', fb.farmer_id)
            .maybeSingle();
        const lang = (farmerRow?.preferred_language ?? 'en');
        ...
        if (fb.block_id) {
            ...
            if (b)
                block = { name: b.name, crop_type: b.crop_type, dap: b.dap };
        }
        else {
            const primary = await blockService.getPrimaryBlock(fb.farmer_id);
            if (primary)
                block = { name: primary.name, crop_type: primary.crop_type, dap: primary.dap };
        }
        ...
        return {
            ...
            farmerProfile: farmerRow
                ? {
                    cropExperienceYears: farmerRow.crop_experience_years != null
    ...
    async review(id, body, agentEmail) {
        const fb = await this.getById(id);
        ...
        const finalDx = body.agronomistFinalDiagnosis?.trim() ||
    ...
    async getVerifiedRegionalHints(farmerId, cropType) {
        ...
        let query = supabase
            .from('farmer_advisory_feedback')
            .select('agronomist_final_diagnosis, farmer_prior_product, farmer_suggested_diagnosis, ai_advisory_sessions(crop_type)')
            .eq('status', 'approved')
            .order('created_at', { ascending: false })
            .limit(5);
        ...
        const lines = data
            .filter((row) => {
            const sess = row.ai_advisory_sessions;
    ...
    async saveCropExperienceYears(farmerId, years, feedbackId) {
    ...
    async promoteToVerifiedLearning(fb, agentEmail) {
        if (!fb.session_id || !fb.agronomist_final_diagnosis)
            return;
        const updatedRec = typeof fb.metadata?.updated_recommendation === 'string'
        ...
        const summaryEn = updatedRec ||
            `Verified guidance: ${fb.agronomist_final_diagnosis}.${fb.farmer_prior_product ? ` Farmer reported ${fb.farmer_prior_product} helped.` : ''}`;
        const summaryMl = updatedRec
            ? updatedRec
            : `സ്ഥിരീകരിച്ച മാർഗ്ഗനിർദേശം: ${fb.agronomist_final_diagnosis}`;
        const sessionQ = await verifiedAdvisoryLearningService.loadSessionQuestionSources(fb.session_id);
        ...
        const { data: pmFarmer } = await supabase
            ...
            .eq('id', fb.farmer_id)
            .maybeSingle();
        const pinRow = pmFarmer?.pincode_master;
...
//# sourceMappingURL=farmer-experience-learning.service.js.map