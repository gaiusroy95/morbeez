...
async function planVisitGroundedQuestions(params) {
    ...
    const userPrompt = [
        ...
        .join('\n');
    try {
        const result = await openaiJsonCompletion(VISIT_GROUNDED_QA_SYSTEM, userPrompt, 768);
        const drafts = [];
        const seen = new Set();
...
export const visitAiQuestionsService = {
    async buildVisitFollowUpQuestions(params) {
        ...
        if (drafts.length < 3 && env.OPENAI_API_KEY && !hasPhotoContext) {
            try {
                const extra = await openaiJsonCompletion('Return JSON {"questions":[{"text":"...","answerType":"yes_no_unknown|number|text"}]} with 2-3 short agronomy follow-up questions grounded in the case context.', `Crop: ${params.cropType}, DAP: ${params.context.dap}, Diagnosis: ${params.selectedHypothesis}, Observation: ${params.observation ?? 'none'}`, 512);
