...
export function isValidPlotCoordinate(lat, lon) {
    if (!Number.isFinite(lat) || !Number.isFinite(lon))
        return false;
    return lat >= 6 && lat <= 37.5 && lon >= 68 && lon <= 97.5;
}
export const plotLocationService = {
    async updateBlockLocation(blockId, input) {
        ...
        if (input.farmerId)
            q = q.eq('farmer_id', input.farmerId);
        const { error } = await q;
        throwIfSupabaseError(error, 'Could not save plot GPS');
    },
    async resolveWeatherCoords(farmerId, blockId) {
        ...
        const { data: farmer, error } = await supabase
            .from('farmers')
            .select('district, village, pincode_id, pincode_master(pincode, district, latitude, longitude, village)')
            .eq('id', farmerId)
            .maybeSingle();
...
//# sourceMappingURL=plot-location.service.js.map