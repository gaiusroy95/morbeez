...
import { VISIT_ASSISTANT_CONTRACT_VERSION, validateVisitAssistantProposalResponse, } from '@morbeez/shared/visit-assistant';
...
const operationSchemas = [
    ...
    ...['category', 'issueName', 'severity', 'observation', 'status', 'finalDiagnosis'].map((field) => object({
...
function proposalContainers(operation) {
    const raw = operation;
    const nested = raw.proposed ?? raw.issue ?? raw.changes ?? raw.group ?? raw.material ?? raw.item ?? raw.followUp;
    if (!nested || typeof nested !== 'object' || Array.isArray(nested))
        return [];
    return 'value' in nested
        ? [nested]
        : Object.values(nested).filter((v) => Boolean(v && typeof v === 'object' && !Array.isArray(v) && 'value' in v));
}
export function normalizeVisitAssistantProposal(value, context) {
    const errors = [];
    if (value.baseRevision !== context.snapshot.revision)
        errors.push('baseRevision does not match the snapshot revision');
    if (value.messages.length > 4 || value.operations.length > 20 || value.clarifications.length > 5
        || value.unresolvedFields.length > 10)
        errors.push('response exceeds collection limits');
    ...
    for (const operation of normalized.operations) {
        ...
        if (operation.kind === 'issue.update' && !issueRefs.has(operation.issueRef))
            errors.push(`unknown issueRef: ${operation.issueRef}`);
        if (operation.kind === 'recommendation.group.update' && !groups.has(operation.groupRef))
            errors.push(`unknown groupRef: ${operation.groupRef}`);
        if (operation.kind === 'recommendation.material.add') {
            if (!groups.has(operation.groupRef))
                errors.push(`unknown groupRef: ${operation.groupRef}`);
            if (!issueRefs.has(operation.material.issueRef))
                errors.push(`unknown issueRef: ${operation.material.issueRef}`);
        }
        if (operation.kind === 'recommendation.material.update') {
            const group = groups.get(operation.groupRef);
            if (!group)
                errors.push(`unknown groupRef: ${operation.groupRef}`);
            else if (!group.materials.some((m) => m.ref === operation.materialRef))
                errors.push(`unknown materialRef: ${operation.materialRef}`);
        }
        if (operation.kind === 'monitoring.add' && !issueRefs.has(operation.item.issueRef))
            errors.push(`unknown issueRef: ${operation.item.issueRef}`);
        if (operation.kind === 'monitoring.update' && !monitoringRefs.has(operation.monitoringRef))
            errors.push(`unknown monitoringRef: ${operation.monitoringRef}`);
        ...
        if (issueFields?.issueName) {
            ...
            if (match) {
                ...
                if (issueFields.category)
                    issueFields.category.value = match.category;
            }
        }
        for (const proposed of proposalContainers(operation)) {
            ...
            for (const item of evidenceItems) {
                if (!item || typeof item !== 'object')
                    continue;
                const ev = item;
                if (ev.kind === 'message' && !messageIds.has(String(ev.messageId)))
                    errors.push(`unknown message evidence: ${String(ev.messageId)}`);
                if (ev.kind === 'measurement' && !measurementKeys.has(String(ev.key)))
                    errors.push(`unknown measurement evidence: ${String(ev.key)}`);
                if (ev.kind === 'issue' && !issueRefs.has(String(ev.issueRef)))
                    errors.push(`unknown issue evidence: ${String(ev.issueRef)}`);
...
export function buildVisitAssistantFallback(snapshot, target = 'field_note') {
    const fieldTarget = { kind: target };
    return {
...
//# sourceMappingURL=visit-assistant.service.js.map