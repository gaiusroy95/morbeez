...
export const visitWizardDraftService = {
    ...
    async listByAgent(agronomistEmail, limit = 20) {
        const email = agronomistEmail.trim().toLowerCase();
        const { data, error } = await supabase
            .from('visit_wizard_drafts')
            .select(`id, session_id, farmer_id, block_id, current_step, wizard_version, updated_at, saved_at,
    ...
    async assertSessionOwner(sessionId, agronomistEmail) {
        const { data, error } = await supabase
...
//# sourceMappingURL=visit-wizard-draft.service.js.map