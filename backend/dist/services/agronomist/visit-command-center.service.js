...
function recoveryDayFromJobType(jobType) {
...
async function loadMonitoringRecoveryByFarmer(farmerIds) {
    const map = new Map();
    if (!farmerIds.length)
        return map;
...
export const visitCommandCenterService = {
    async updatePriority(findingId, priority) {
        ...
        return { id: String(data.id), priority: String(data.visit_priority) };
    },
    async getCommandCenter(agentEmail) {
        ...
        const priorityQueue = (priorityFindings.data ?? []).map((row) => {
            const farmerRaw = row.farmers;
            const blockRaw = row.farm_blocks;
            ...
            return {
                ...
                priority: (row.visit_priority ?? 'normal'),
...
//# sourceMappingURL=visit-command-center.service.js.map