import { validateVisitAssistantProposalResponse, } from '@morbeez/shared/visit-assistant';
...
import { VISIT_ASSISTANT_RESPONSE_SCHEMA, buildVisitAssistantFallback, normalizeVisitAssistantProposal, } from './visit-assistant.service.js';
...
const stringArray = { type: 'array', maxItems: 12, items: text(500) };
...
function mapValidation(lines) {
    return lines.map((line) => ({
        text: line,
        status: /check|verify|incompatible|before recommending/i.test(line) ? 'warning' : 'ok',
    }));
}
function buildPreview(workflow) {
    const evidenceRequired = Array.isArray(workflow.evidenceRequired)
    ...
    for (const received of evidenceReceived) {
        const match = evidenceRequired.find((item) => item.label.toLowerCase() === received.label.toLowerCase());
        if (match) {
    ...
    return {
        ...
        diagnosisStatus: workflow.diagnosisStatus ?? 'pending_confirmation',
...
export const visitCopilotExtractionService = {
    async extract(input) {
        ...
        try {
            ...
            const workflow = extracted.workflow;
            const validationRaw = (workflow.validation ?? {});
            return {
                ...
                reminders: Array.isArray(workflow.reminders)
                    ...
                    : [],
                validation: {
                    compatibility: mapValidation(Array.isArray(validationRaw.compatibility) ? validationRaw.compatibility.map(String) : []),
                    weather: mapValidation(Array.isArray(validationRaw.weather) ? validationRaw.weather.map(String) : []),
                    followUp: mapValidation(Array.isArray(validationRaw.followUp) ? validationRaw.followUp.map(String) : []),
                },
            ...
            return {
                assistantMessage: 'I could not fully structure that instruction. Please try again with diagnosis, evidence to collect, treatment dose, and monitoring interval.',
                preview: {
                    workingDiagnosis: '',
...
//# sourceMappingURL=visit-copilot-extraction.service.js.map