...
export const partnerPayoutService = {
    async generateMonth(month) {
        ...
        for (const row of due) {
            ...
            if (row.earning_id)
                cur.earningIds.push(String(row.earning_id));
            cur.total += payable;
        ...
        for (const [partnerId, group] of byPartner) {
            ...
            if (!batchId) {
                ...
                if (!created?.id)
                    throw new ValidationError('Could not create payout batch');
    ...
    async list(month) {
        ...
        const partners = partnerIds.length === 0
            ? []
            : (await supabase.from('partners').select('id, full_name, partner_code').in('id', partnerIds)).data ?? [];
    ...
    async approve(batchId, actorEmail) {
        ...
        if (!batch)
            throw new ValidationError('Payout batch not found');
        if (batch.status !== 'pending')
            throw new ValidationError('Only pending batches can be approved');
        await supabase
            .from('partner_earnings_ledger')
    ...
    async markPaid(batchId) {
        ...
        if (batch.status !== 'approved')
            throw new ValidationError('Approve the batch before marking paid');
        await supabase
            .from('partner_earnings_ledger')
        ...
        const { data: settlements } = await supabase
            ...
            .eq('payout_batch_id', batchId)
            .in('status', ['pending', 'approved']);
        await settlementService.markPaid((settlements ?? []).map((s) => String(s.id)), `payout-batch:${batchId}`);
        const { data, error: updErr } = await supabase
            .from('partner_payout_batches')
...
//# sourceMappingURL=partner-payout.service.js.map