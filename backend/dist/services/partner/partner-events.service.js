...
export const partnerTerritoryService = {
    ...
    async upsertPincode(partnerId, pincode, isPrimary = false) {
        const { data, error } = await supabase
            .from('partner_territory_pincodes')
            .upsert({
    ...
    async partnerIdsForPincode(pincode) {
        const { data, error } = await supabase
            .from('partner_territory_pincodes')
            .select('partner_id')
...
//# sourceMappingURL=partner-events.service.js.map