...
async function partnerKpiScore(partnerId, month) {
    ...
    const { data: snap } = await supabase
        ...
        .limit(1)
        .maybeSingle();
    if (snap?.performance_score != null)
        return Number(snap.performance_score);
...
export const commissionEngineService = {
    ...
    async computeForOrder(input) {
        ...
        const { data: orderLines } = await supabase
            ...
            .select('sku, product_title, qty_ordered, unit_price, channel_pool_pct, channel_pool_agronomist_pct, channel_pool_partner_pct, channel_pool_version_id, channel_pool_version_label')
            .eq('commerce_order_id', input.orderId);
        const categoryKey = input.categoryKey?.trim() ||
            dominantCategory((orderLines ?? []).map((line) => ({
        ...
        if (kpiScore < Number(rule.requires_reliability_min ?? 50))
            return null;
        ...
        let commission = 0;
        let usedPool = false;
        let snapshotPct = null;
        let snapshotAgroPct = null;
        let snapshotPartnerPct = null;
        let snapshotVersionId = null;
        ...
        for (const line of orderLines ?? []) {
            ...
            const snap = await channelPoolService.snapshotForLine({
                ...
                existing: {
                    ...
                    channelPoolAgronomistPct: line.channel_pool_agronomist_pct == null ? null : Number(line.channel_pool_agronomist_pct),
                    channelPoolPartnerPct: line.channel_pool_partner_pct == null ? null : Number(line.channel_pool_partner_pct),
        ...
        if (!usedPool) {
            if (rule.rule_type === 'fixed_inr')
                commission = Number(rule.fixed_inr ?? 0) * kpiFactor;
                ...
                commission = Number(rule.fixed_inr ?? 500) * kpiFactor;
        }
        else {
