...
export const partnerLeadAllocationService = {
    async scorePartner(partnerId) {
        ...
        const headroom = partner.maxActiveFarmers > 0
            ? (partner.maxActiveFarmers - partner.currentActiveFarmers) / partner.maxActiveFarmers
            : 0;
        if (headroom <= 0)
            return 0;
        const score = partner.reliabilityScore * relW +
    ...
    async allocateLeadToPartners(leadId, farmerId, limit = 3) {
        let territoryPartnerIds = null;
        if (farmerId) {
    ...
    async respond(allocationId, partnerId, action) {
...
//# sourceMappingURL=partner-lead-allocation.service.js.map