...
export const partnerEnrollmentService = {
    async resolvePartnerFromCode(partnerCode, qrToken) {
        if (qrToken) {
            const byQr = await partnerService.getByQrToken(qrToken);
            if (byQr && ['active', 'certified', 'training'].includes(byQr.status))
                return byQr;
        }
        if (partnerCode?.trim()) {
            const byCode = await partnerService.getByCode(partnerCode);
            if (byCode && ['active', 'certified', 'training'].includes(byCode.status))
                return byCode;
        }
        return null;
    },
    async enrollFarmerWithPartner(input) {
        ...
        if (!partner)
            return { enrolled: false, partner: null };
        ...
        await partnerAttributionCaptureService.trackEnrollment(input.farmerId, partner.id, input.enrollmentSource ?? 'partner_qr');
    ...
    async enrollByPhone(input) {
        ...
        if (!farmerId) {
            ...
            if (!created?.id)
                throw new ValidationError('Could not create farmer');
...
//# sourceMappingURL=partner-enrollment.service.js.map