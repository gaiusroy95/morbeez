...
export const partnerKpiService = {
    async computeMonthlySnapshot(partnerId, periodStart, periodEnd) {
        ...
        const { count: tasksDone } = await supabase
            ...
            .gte('created_at', start)
            .lte('created_at', end);
        const visitCompletionPct = (tasksTotal ?? 0) > 0 ? Math.round(((tasksDone ?? 0) / (tasksTotal ?? 1)) * 100) : 0;
        const { data: partner } = await supabase
            .from('partners')
            .select('reliability_score, performance_score, current_active_farmers')
            .eq('id', partnerId)
            .single();
        const performanceScore = Math.min(100, Math.round((Number(partner?.reliability_score ?? 70) * 0.4 +
