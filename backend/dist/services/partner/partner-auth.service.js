...
export const partnerAuthService = {
    async sendOtp(phoneRaw, ipAddress) {
        ...
        const { data: partner } = await supabase
            ...
            .maybeSingle();
        if (!partner)
            throw new ValidationError('No partner account found for this number');
    ...
    async verifyOtp(phoneRaw, codeRaw) {
        ...
        if (!profile)
            throw new UnauthorizedError('Partner account not found');
        const token = createPartnerToken(profile.id, phone);
        return { token, partner: profile };
    },
    async loginWithPassword(phoneRaw, password) {
        if (!isValidIndianPhone(phoneRaw)) {
            throw new ValidationError('Enter a valid 10-digit Indian mobile number');
        }
        const phone = normalizePhone(phoneRaw);
...
//# sourceMappingURL=partner-auth.service.js.map