...
export const financeService = {
    async getDashboard() {
        ...
        const dailySales = (ordersRes.data ?? []).reduce((sum, o) => sum + (Number(o.total_amount) || 0), 0);
        const gstLiability = (invoicesRes.data ?? []).reduce((sum, i) => sum + (Number(i.cgst) || 0) + (Number(i.sgst) || 0) + (Number(i.igst) || 0), 0);
    ...
    async refreshDailySnapshot(date) {
