...
export const pickListService = {
    ...
    async resolveRackLocationForLine(line) {
        const formatLoc = (loc) => loc ? warehouseService.formatLocationDisplay(loc) : null;
        if (line.allocation_id) {
            const { data: alloc } = await supabase
                .from('order_line_allocations')
                .select('location_id, batch_id, inventory_batches(location_id, warehouse_locations(zone, rack, shelf, bin, location_code))')
                .eq('id', line.allocation_id)
                .maybeSingle();
            if (alloc) {
                const batchRaw = alloc.inventory_batches;
                const batch = Array.isArray(batchRaw) ? batchRaw[0] ?? null : batchRaw;
                const locRaw = batch?.warehouse_locations;
                const joinedLoc = Array.isArray(locRaw) ? locRaw[0] ?? null : locRaw ?? null;
                const fromJoin = formatLoc(joinedLoc);
                if (fromJoin)
                    return fromJoin;
                const locationId = alloc.location_id ??
                    batch?.location_id ??
                    null;
                if (locationId) {
    ...
    async refreshRackLocations(pickListId) {
        ...
        let updated = 0;
        for (const line of lines ?? []) {
            await inventoryService.applyWarehouseLocationToItemBatches(String(line.inventory_item_id));
