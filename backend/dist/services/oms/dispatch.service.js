...
export const dispatchService = {
    async startSession(commerceOrderId) {
    ...
    async scanAwb(dispatchSessionId, scannedCode, actorEmail) {
        ...
        throwIfSupabaseError(error, 'Dispatch session');
        if (!session)
            throw new NotFoundError('Dispatch session not found');
    ...
    async getSessionForOrder(commerceOrderId) {
        const { data, error } = await supabase
            .from('dispatch_sessions')
    ...
    async confirmDispatch(commerceOrderId, actorEmail) {
