...
export function buildLabelQrPayload(labelId, orderName) {
    const name = (orderName ?? '').replace(/\|/g, '').trim();
    return name ? `LBL|${labelId}|${name}` : `LBL|${labelId}`;
}
export function parseLabelQrPayload(code) {
    const trimmed = code.trim();
    if (!trimmed.toUpperCase().startsWith('LBL|'))
        return null;
...
export const employeeBatchService = {
    ...
    async listAssignableOrders(limit = 80) {
        const { data, error } = await supabase
            ...
            .select('id, order_name, shopify_order_id, oms_status, courier_name, tracking_awb, assigned_employee_name, assigned_batch_id, created_at, pick_lists(id, picker_id)')
            .is('deleted_at', null)
    ...
    async getBatchDetail(batchId) {
        ...
        if (!batch)
            throw new NotFoundError('Label batch not found');
        const { data: labels, error: labelsErr } = await supabase
            .from('shipping_labels')
            .select('*, commerce_orders(id, order_name, shopify_order_id, oms_status, tracking_awb, label_url, courier_name, dispatch_rack)')
    ...
    async printBatch(batchId, actorEmail) {
        ...
        for (const label of labels) {
            ...
            const order = label.commerce_orders;
            ...
            stack.push({
                ...
                orderName: order?.order_name ?? order?.shopify_order_id ?? orderId,
    ...
    async startPicking(commerceOrderId, employeeId) {
    ...
    async verifyShippingLabel(input) {
        ...
        const order = label?.commerce_orders;
        const expectedName = order?.order_name ?? order?.shopify_order_id ?? expectedOrderId;
...
//# sourceMappingURL=employee-batch.service.js.map