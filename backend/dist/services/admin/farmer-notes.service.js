...
export const farmerNotesService = {
    ...
    async create(farmerId, authorEmail, noteText) {
        const text = noteText.trim();
        if (!text)
            throw new ValidationError('Note text required');
        const { data, error } = await supabase
            .from('farmer_notes')
            .insert({
            farmer_id: farmerId,
...
//# sourceMappingURL=farmer-notes.service.js.map