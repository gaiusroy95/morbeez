...
export const crmFarmerService = {
    ...
    async getBlockWorkspace(farmerId, blockId) {
        ...
        const [soilRes, findingsRes, recsRes, soilList, visits, blockRecs, followUps, applicationRows] = await Promise.all([
            ...
            supabase
                ...
                .select(`id, issue_detected, recommendation_text, status, application_status, outcome, metadata,
        ...
        return {
            ...
            applicationTracking: applicationRows.map((row) => {
                ...
                const app = (Array.isArray(row.recommendation_applications)
    ...
    async getHumanCrmInteractionDetail(farmerId, _leadId, interactionId) {
        ...
        const operationalChain = await loadOperationalChain(log);
        const findingLabel = operationalChain?.fieldFinding?.issue ??
        ...
        return {
            ...
            fields: [
                ...
                field('Recommendation', operationalChain?.recommendation?.summary ?? log.recommendation_summary),
    ...
    async createInteraction(farmerId, leadId, input) {
        ...
        if (input.addFieldFinding) {
            ...
            if (!input.findingType || !input.severity || !confirmedIssue) {
                throw new ValidationError('Structured field finding requires type, severity, and confirmed issue');
            }
        }
    ...
    async resolveAgronomistEmail(farmerId) {
        const { data } = await supabase
            .from('farmer_agronomist_assignments')
            .select('email')
            .eq('farmer_id', farmerId)
    ...
    async scheduleVisit(farmerId, leadId, input) {
        ...
        const agronomistEmail = input.assignedAgronomist?.trim().toLowerCase() ||
...
function deriveFieldFindingTone(findingType, severity) {
    if (findingType === 'growth_observation' && severity === 'mild')
        return 'healthy';
    if (severity === 'severe' || findingType === 'disease' || findingType === 'pest')
        return 'danger';
    return 'warning';
}
async function loadOperationalChain(log) {
    const chain = {};
    ...
    if (findingId) {
        ...
        if (finding) {
            chain.fieldFinding = {
                ...
                affectedAreaPct: finding.affected_area_pct != null ? Number(finding.affected_area_pct) : null,
            };
        }
    }
