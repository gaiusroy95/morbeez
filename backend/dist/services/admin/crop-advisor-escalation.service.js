 ...
 */
export const cropAdvisorEscalationService = {
    async escalateFromInteraction(params) {
        const reason = `[CropAdvisor] ${params.interactionType}: ${params.summary}`.slice(0, 500);
        const { data: open } = await supabase
            ...
            .maybeSingle();
        if (open?.id) {
            await escalationAdminService.addEscalationComment(String(open.id), reason, params.agentEmail, 'crop_advisor');
            await supabase
                .from('agronomist_escalations')
                .update({
                reason,
        ...
        const { data: session, error: sessionErr } = await supabase
            ...
            metadata: {
                source: 'crop_advisor_interaction',
...
//# sourceMappingURL=crop-advisor-escalation.service.js.map