...
export const agronomistWorkflowService = {
    async listReviewQueue(limit = 40) {
        const { data: findings, error } = await supabase
            ...
            .select('id, farmer_id, block_id, block_name, crop_type, agronomist_name, observations, disease_pest, disease_tone, action_taken, follow_up_at, photo_urls, visited_at, farmers(id, name, phone, preferred_language, district, source), farm_blocks(id, name, crop_type, plot_label, planting_date, created_at)')
            .is('archived_at', null)
        ...
        const recByFinding = new Map((recs ?? []).map((r) => [String(r.field_finding_id), r]));
        const items = (findings ?? [])
            .filter((f) => {
            const farmer = f.farmers;
            if (farmer?.source === 'demo_seed')
                return false;
    ...
    async generateAiSuggestion(findingId) {
        const { finding, recommendation } = await this.getFindingDetail(findingId);
        const farmer = finding.farmers;
        if (!farmer?.id)
            throw new NotFoundError('Farmer not found for finding');
        const lang = String(farmer.preferred_language ?? 'en').slice(0, 2) || 'en';
        const symptoms = [
            ...
            .filter(Boolean)
            .join('\n');
        const block = finding.farm_blocks;
        ...
        const recText = lang === 'ml' && advisory.farmerSummaryMl
            ? advisory.farmerSummaryMl
            : advisory.farmerSummaryEn || advisory.probableIssue;
        const dosage = formatDosage(advisory.dosageGuidance);
        const precautions = advisory.precautions.length > 0 ? advisory.precautions.join(' ') : undefined;
        return {
    ...
    async submitForApproval(recommendationId, reviewedBy) {
    ...
    async listAgronomistSubmissions(status, limit = 50, createdBy) {
...
//# sourceMappingURL=agronomist-workflow.service.js.map