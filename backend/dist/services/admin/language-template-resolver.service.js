...
import { renderLanguageTemplate, } from './language-template-variables.js';
export const languageTemplateResolverService = {
    async getApprovedBody(templateKey, language, variables) {
        ...
        const { data: def, error: defErr } = await supabase
            ...
            .maybeSingle();
        if (defErr)
            throwIfSupabaseError(defErr, 'Could not load template definition');
        if (!def)
            return null;
    ...
    async getMetaTemplateName(templateKey) {
        const { data, error } = await supabase
            .from('whatsapp_template_definitions')
        ...
        if (!data || data.channel !== 'meta_template')
            return null;
        return data.meta_template_name ? String(data.meta_template_name) : templateKey;
    },
};
//# sourceMappingURL=language-template-resolver.service.js.map