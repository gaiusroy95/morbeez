...
async function verifyStaffPassword(email, password) {
...
export const farmerRoiAdminService = {
    async listEntries(farmerId, limit = 100) {
        const { data, error } = await supabase
    ...
    async staffEditEntry(params) {
        ...
        const { data: existing, error: fetchErr } = await supabase
            ...
            .maybeSingle();
        if (fetchErr)
            throwIfSupabaseError(fetchErr, 'Could not load entry');
        if (!existing)
            throw new NotFoundError('ROI entry not found');
        ...
        const category = (params.patch.category ?? existing.entry_type);
        ...
        if (amount <= 0)
            throw new ValidationError('Debit or credit amount is required');
        const updates = {
            entry_type: category,
...
//# sourceMappingURL=farmer-roi-admin.service.js.map