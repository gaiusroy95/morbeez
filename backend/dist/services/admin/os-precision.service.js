...
export const osPrecisionService = {
    ...
    async getModulePrecision(days = 30) {
        ...
        const openaiCount = rows.filter((r) => ['crop_doctor_openai', 'conversational_openai'].includes(String(r.module_source))).length;
        ...
        for (const s of sessionRows) {
            ...
            if (meta.reusedFrom || meta.reuseCaseId)
                reusedFromCache += 1;
        ...
        const evRows = events ?? [];
        const avgConfidence = evRows.length > 0
            ? Math.round((evRows.reduce((s, e) => s + Number(e.confidence ?? 0), 0) / evRows.length) * 100)
            : 0;
        const topCrops = new Map();
        for (const r of rows) {
        ...
        return {
            ...
            kpis: {
                ...
                uspHeadline: modularCount >= openaiCount
...
//# sourceMappingURL=os-precision.service.js.map