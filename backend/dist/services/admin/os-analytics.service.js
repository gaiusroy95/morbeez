...
export const osAnalyticsService = {
    ...
    async getAiAccuracyTrends(days = 30) {
        ...
        for (const row of events ?? []) {
            ...
            if (Number(row.confidence ?? 0) < 0.7)
    ...
    async getDistrictHeatmap(days = 30) {
        ...
        const farmerDistrict = new Map();
        for (const f of farmers ?? []) {
            farmerDistrict.set(String(f.id), resolveDistrict(f));
        }
        const agg = new Map();
        function touch(district) {
            if (!agg.has(district)) {
                agg.set(district, {
                    district,
                    farmers: 0,
        ...
        for (const f of farmers ?? []) {
            ...
            if (pin)
                a.pincodes.add(String(pin));
        }
        for (const b of blocks ?? []) {
            const row = b;
            const d = row.pincode_master != null
        ...
        return {
            ...
            pincodeFirstNote: 'Districts resolved from pincode_master when farmers have pincode_id; otherwise farmers.district.',
        };
    },
    async getPincodeBreakdown(district, days = 30) {
        const since = daysAgoIso(days);
        ...
        const { data: recs } = await supabase
            ...
            .select('id, farmer_id')
            .gte('created_at', since);
        const farmerPin = new Map();
        for (const f of farmers ?? []) {
            if (f.pincode_id)
                farmerPin.set(String(f.id), String(f.pincode_id));
        }
        const byPin = new Map();
        ...
        for (const f of farmers ?? []) {
            ...
            if (pid && byPin.has(pid))
                byPin.get(pid).farmers += 1;
        }
        for (const r of recs ?? []) {
            const pid = farmerPin.get(String(r.farmer_id));
            if (pid && byPin.has(pid))
                byPin.get(pid).recommendations += 1;
        }
        return {
            district,
    ...
    async getRetention(days = 30) {
        ...
        for (const f of login7 ?? [])
            active7.add(String(f.id));
        for (const i of interactions7 ?? [])
            active7.add(String(i.farmer_id));
        const active30 = new Set();
        for (const f of login30 ?? [])
            active30.add(String(f.id));
        for (const i of interactions30 ?? [])
            active30.add(String(i.farmer_id));
    ...
    async getRecommendationSuccess(days = 30) {
        ...
        for (const r of rows) {
            ...
            if (['communicated', 'applied', 'outcome_recorded'].includes(st))
                communicated += 1;
            if (r.outcome) {
                withOutcome += 1;
                const o = String(r.outcome);
                byOutcome[o] = (byOutcome[o] ?? 0) + 1;
                if (o === 'better' || o === 'partial')
                    positiveOutcome += 1;
            }
        }
    ...
    async getMaiosKpis(days = 30) {
        ...
        for (const s of sessions ?? []) {
            ...
            if (mc?.evidence?.eqs != null)
                eqsSum += mc.evidence.eqs;
            if (mc?.outcomes?.length) {
                recoveryStarted++;
                if (mc.outcomes.some((o) => o.day >= 14))
                    recoveryCompleted++;
            }
            if (mc?.failureType) {
