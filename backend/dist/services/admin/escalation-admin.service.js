...
export const escalationAdminService = {
    ...
    async list(params) {
        ...
        let query = supabase
            .from('agronomist_escalations')
            .select(`id, session_id, farmer_id, reason, confidence_at_escalation, priority, status, assigned_to, created_at, updated_at, resolved_at,
        ...
        const items = (data ?? []).map((row) => {
            ...
            return {
                ...
                resolvedLabel: formatDt(row.resolved_at),
            };
        });
    ...
    async clearCompleted(agentEmail) {
        const now = new Date().toISOString();
        const { data, error } = await supabase
            .from('agronomist_escalations')
            .update({
