...
export const telecallerEscalationService = {
    async escalateFromInteraction(params) {
        ...
        const { data: open } = await supabase
            ...
            .maybeSingle();
        if (open?.id) {
            await escalationAdminService.addEscalationComment(String(open.id), reason, params.agentEmail, 'telecaller');
            await supabase
                .from('agronomist_escalations')
                .update({
                reason,
...
//# sourceMappingURL=telecaller-escalation.service.js.map