...
function isActiveFarmer(row, lastOrderAt) {
    const now = Date.now();
    ...
    if (row.last_login_at) {
        ...
        if (!Number.isNaN(login) && now - login <= activeMs)
            return true;
    }
    if (lastOrderAt) {
        const order = new Date(lastOrderAt).getTime();
        if (!Number.isNaN(order) && now - order <= activeMs)
            return true;
    }
    if (!row.last_login_at && row.created_at) {
        const created = new Date(String(row.created_at)).getTime();
        if (!Number.isNaN(created) && now - created <= newMs)
            return true;
    }
    return false;
}
async function loadCropsByFarmerIds(ids) {
    const map = new Map();
    if (!ids.length)
        return map;
...
async function loadOrderMetaByPhones(phones) {
    const wanted = new Set(phones.map(normalizePhoneDigits).filter((p) => Boolean(p)));
    const lastOrder = new Map();
    const orderCount = new Map();
    if (!wanted.size) {
        return { lastOrder, orderCount, repeatBuyerPhones: new Set() };
    }
    const { data, error } = await supabase
    ...
    for (const row of data ?? []) {
        ...
        if (!lastOrder.has(key))
            lastOrder.set(key, created);
    }
    const repeatBuyerPhones = new Set();
    for (const [phone, count] of orderCount) {
        if (count >= 2)
            repeatBuyerPhones.add(phone);
    }
    return { lastOrder, orderCount, repeatBuyerPhones };
}
function enrichRow(row, crops, lastOrderAt, orderCount) {
...
export const farmersAdminService = {
    ...
    async list(query) {
        ...
        if (query.search?.trim()) {
            ...
            const q = `%${term}%`;
            builder = builder.or(`email.ilike.${q},name.ilike.${q},phone.ilike.${q},first_name.ilike.${q},last_name.ilike.${q},state.ilike.${q},district.ilike.${q}`);
        }
        if (stateFilter) {
        ...
        if (!needsStatusFilter) {
            ...
            const { lastOrder, orderCount } = await loadOrderMetaByPhones(rows.map((r) => r.phone));
            const farmers = rows.map((row) => {
                const phoneKey = normalizePhoneDigits(String(row.phone ?? ''));
                const crops = cropsMap.get(String(row.id)) ?? [];
                return enrichRow(row, crops, phoneKey ? lastOrder.get(phoneKey) ?? null : null, phoneKey ? orderCount.get(phoneKey) ?? 0 : 0);
            });
            const total = count ?? 0;
            return {
                stats,
        ...
        const cropsMap = await loadCropsByFarmerIds(ids);
        const { lastOrder, orderCount } = await loadOrderMetaByPhones(allRows.map((r) => r.phone));
        ...
        if (status === 'active')
            enriched = enriched.filter((f) => f.status === 'active');
        if (status === 'inactive')
            enriched = enriched.filter((f) => f.status === 'inactive');
    ...
    async get(id) {
        ...
        const row = data;
        const cropsRaw = row.farmer_crops ?? [];
        ...
        const phoneKey = normalizePhoneDigits(String(row.phone ?? ''));
        return {
            farmer: enrichRow(row, crops, phoneKey ? lastOrder.get(phoneKey) ?? null : null, phoneKey ? orderCount.get(phoneKey) ?? 0 : 0),
            crops,
        };
    },
    async create(input) {
    ...
    async update(id, patch) {
        ...
        if (patch.district !== undefined)
            updates.district = patch.district;
        ...
        if (patch.firstName !== undefined || patch.lastName !== undefined) {
            const { data: existing } = await supabase
                ...
                .eq('id', id)
                .single();
            const fn = (patch.firstName ?? existing?.first_name ?? '');
            const ln = (patch.lastName ?? existing?.last_name ?? '');
...
//# sourceMappingURL=farmers-admin.service.js.map