...
export const adminDashboardService = {
    async getOverview() {
        ...
        const [farmersCount, farmersWeek, farmersPrevWeek, productCount, ordersCount, paidCheckouts, revenueResult, revenueWeek, revenuePrevWeek, ordersWeek, ordersPrevWeek, pendingCheckouts, aiSessionsCount, aiSessionsWeek, recentFarmers, recentOrders, recentCheckouts, paidSessionsForChart, allProductsList,] = await Promise.all([
        ...
        const revenuePaise = (revenueResult.data ?? []).reduce((sum, row) => sum + (Number(row.amount_paise) || 0), 0);
        const revenueWeekPaise = (revenueWeek.data ?? []).reduce((sum, row) => sum + (Number(row.amount_paise) || 0), 0);
        const revenuePrevWeekPaise = (revenuePrevWeek.data ?? []).reduce((sum, row) => sum + (Number(row.amount_paise) || 0), 0);
        ...
        const conversionRate = farmersTotal > 0 ? Math.round((totalOrders / farmersTotal) * 10000) / 100 : 0;
        ...
        for (const row of paidSessionsForChart.data ?? []) {
            ...
            if (idx >= 0)
                salesByDay[idx] += (Number(row.amount_paise) || 0) / 100;
        }
        ...
        return {
            kpis: {
                ...
                aiTrend: pctChange(aiSessionsWeek.error ? 0 : aiSessionsWeek.count ?? 0, 0),
                compareLabel,
            },
            alerts: {
