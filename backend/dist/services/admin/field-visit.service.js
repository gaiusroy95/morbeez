...
export const fieldVisitService = {
    ...
    async submitStructuredVisitForPartner(input, agentEmail, partnerId, agentDisplayName) {
        ...
        if (input.recApproved && input.compatibilityOverrideReason?.trim()) {
            const materials = (input.recommendationGroups ?? []).flatMap((g) => g.materials.map((m) => ({ technicalName: m.technicalName })));
        ...
        if (input.selectedRecommendationOptionId && primaryIssue) {
            const issueLabel = primaryIssue.finalDiagnosis?.trim() || primaryIssue.issueName.trim() || 'Field issue';
