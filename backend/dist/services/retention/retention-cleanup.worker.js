...
async function redactWebhookPayloads() {
...
async function redactInteractionPayloads() {
    const cutoff = new Date(Date.now() - RETENTION_DAYS * 24 * 60 * 60 * 1000).toISOString();
    const { data, error } = await supabase
...
export function startRetentionCleanupWorker() {
    if (env.NODE_ENV === 'test')
        return;
    if (!env.ENABLE_RETENTION_CLEANUP)
        return;
    if (interval)
        return;
...
//# sourceMappingURL=retention-cleanup.worker.js.map