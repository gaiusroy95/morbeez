...
export const nearbyCasesService = {
    async summarize(farmerId, cropType) {
    ...
    formatForPrompt(summary) {
        ...
        if (summary.recentIssues.length) {
            lines.push('Nearby farmers (same pincode/district, last 30 days): ' +
                summary.recentIssues.map((i) => `${i.issueLabel} (${i.count})`).join('; '));
        }
        if (summary.verifiedReuseHits > 0) {
            lines.push(`Verified Morbeez reuse cases for this crop in region: ${summary.verifiedReuseHits}.`);
        }
        return lines.join('\n');
    },
};
//# sourceMappingURL=nearby-cases.service.js.map