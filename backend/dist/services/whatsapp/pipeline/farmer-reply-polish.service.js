...
import { logOpenAiQuotaInsufficient, parseOpenAiHttpError, } from '../../ai/openai-quota.service.js';
...
function buildCompatibilityLockedFacts(lookup, pair) {
    const a = lookup.productA ?? pair.productA;
    const b = lookup.productB ?? pair.productB;
    const verdict = lookup.compatible === true
    ...
    if (lookup.notes?.trim())
        lines.push(`Notes: ${lookup.notes.trim()}`);
    return lines.join('\n');
}
export function buildDiagnosisLockedFacts(advisory) {
    const lines = [
        `Probable issue: ${advisory.probableIssue}`,
        `Confidence: ${Math.round(advisory.confidence * 100)}%`,
    ];
    if (advisory.uncertain)
        lines.push('Status: uncertain — advise caution');
...
export const farmerReplyPolishService = {
    isEnabled() {
        return (env.ENABLE_WHATSAPP_REPLY_POLISH &&
...
Write the final WhatsApp message body only (no JSON).`;
    ...
    async polishCompatibilityReply(params) {
        const draft = compatibilityLookupService.formatFarmerReply(params.lookup, params.language, params.pair);
        return this.polish({
            factualDraft: draft,
    ...
    async polishDiagnosisSummary(params) {
        const summary = params.language === 'ml' && params.advisory.farmerSummaryMl
...
//# sourceMappingURL=farmer-reply-polish.service.js.map