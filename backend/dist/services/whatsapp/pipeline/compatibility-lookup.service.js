...
import { CALCIUM_NITRATE_MIX_WARNING, CALCIUM_NITRATE_PRODUCT, lookupCalciumNitratePair, } from './calcium-nitrate-tank-mix.knowledge.js';
import { responseComposerService } from './response-composer.service.js';
/** Extract two product names from farmer tank-mix questions. */
export function parseProductPairFromText(text) {
    const t = text.trim();
    if (t.length < 8)
        return null;
    const patterns = [
...
function cleanProductToken(raw) {
    return raw
...
export const compatibilityLookupService = {
    ...
    async lookup(productA, productB) {
        ...
        for (const row of data) {
            ...
            const matchDirect = (keyA.includes(ra) || ra.includes(keyA)) && (keyB.includes(rb) || rb.includes(keyB));
            const matchSwap = (keyA.includes(rb) || rb.includes(keyA)) && (keyB.includes(ra) || ra.includes(keyB));
    ...
    formatFarmerReply(lookup, language, parsed) {
        ...
        if (!lookup.found) {
            ...
            return responseComposerService.compose({
                body,
                validationQuestion: language === 'ml'
        ...
        const interval = lookup.minIntervalHours != null ? `${lookup.minIntervalHours} hours` : null;
        let body;
        if (language === 'ml') {
            body = compatible
                ? `${a} + ${b}: ഡാറ്റാബേസിൽ പൊരുത്തപ്പെടുന്നു എന്ന് രേഖപ്പെടുത്തിയിട്ടുണ്ട്.`
                : `${a} + ${b}: ഡാറ്റാബേസിൽ ഒരുമിച്ച് മിശ്രണം ശുപാർശ ചെയ്യുന്നില്ല.`;
            if (interval && !compatible)
                body += `\nകുറഞ്ഞ ഇടവേള: ${interval}.`;
            if (lookup.notes)
                body += `\n${lookup.notes}`;
        }
        else {
            const source = a === CALCIUM_NITRATE_PRODUCT || b === CALCIUM_NITRATE_PRODUCT
...
//# sourceMappingURL=compatibility-lookup.service.js.map