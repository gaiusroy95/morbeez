...
function buildDiagnosisBody(params) {
    ...
    if (params.plotLabel)
        body = `📍 ${params.plotLabel}\n\n${body}`;
    if (params.reuseNote)
        body += `\n\n${params.reuseNote}`;
    if (params.escalateNote)
        body += `\n\n${params.escalateNote}`;
    if (params.safetyNote)
        body += `\n\n${params.safetyNote}`;
    return body;
}
...
async function applyLanguageSelection(params) {
    ...
    await whatsappScenarioRouter.startMinimalOnboarding(params.phone, params.farmerId, params.language, params.send);
}
...
export const whatsappInboundPipeline = {
    async process(msg, send, _hooks) {
        ...
        if (!session.preferred_language) {
            ...
            if (isLanguageMenuEcho(msg.text ?? '') && !hasInteractiveUserReply(msg)) {
                logger.info({ farmerId: captured.farmerId, messageId: msg.messageId }, 'Ignored echoed language-menu webhook (no interactive reply)');
                return;
            }
        ...
        if (routeResult.handled && 'runDiagnosis' in routeResult && routeResult.runDiagnosis) {
            ...
            if (routeResult.symptomsText || routeResult.postIntake) {
                ...
                const diagnosisImages = batchPaths.length > 0
                    ? await Promise.all(batchPaths.map(async (entry) => {
    ...
    async replyToText(msg, captured, sendText) {
        ...
        const hasPendingImage = batchPending > 0 ||
            Boolean(sessCtxEarly.pendingDiagnosisImagePath ||
    ...
    async runDiagnosis(params) {
        try {
            ...
            if (maiosCase &&
                ...
                await params.sendText(params.phone, evidenceQualityService.missingSlotPrompt(pack, params.language, missing));
            ...
            if (env.ENABLE_WHATSAPP_DIAGNOSIS_POLISH &&
            ...
            const kb = await knowledgeFallbackService.tryReplyWithModule({
                ...
                language: params.language,
                memory,
                hasMedia: Boolean(params.imageBase64 ||
    ...
    async deliverPendingDiagnosis(params) {
        ...
        const advisory = normalizeStructuredAdvisory(latest?.raw_response ?? {
            probableIssue: '',
        ...
        const productRecs = session.ai_product_recommendations ?? [];
        const productBlock = shopifyLinksService.formatRecommendationsForWhatsApp(productRecs.map((r) => ({
            shopifyProductHandle: r.shopify_product_handle ?? undefined,
            productTitle: r.product_title ?? '',
            reason: r.reason ?? '',
            dosageSchedule: typeof r.dosage_schedule === 'object' && r.dosage_schedule
                ? r.dosage_schedule
                : undefined,
