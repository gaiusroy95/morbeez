const INSECT_TERMS = /\b(insect|pest|bug|caterpillar|larvae|larva|egg|chewing|borer|thrips|aphid|whitefly|mite|hopper|beetle)\b/i;
const WEED_TERMS = /\b(weed|weeds|unwanted plant|grass spread|herbicide)\b/i;
const ROOT_TERMS = /\b(root|roots|rhizome|nematode|soft root|drainage|root rot)\b/i;
const COMPATIBILITY_TERMS = /\b(mix|tank mix|compatible|compatibility|jar test|spray together|combine|phytotoxic)\b/i;
const CULTIVATION_TERMS = /\b(irrigation|fertilizer|fertigation|harvest|spray timing|intercrop|labour|labor|dose|dosage)\b/i;
const DISEASE_TERMS = /\b(disease|fungus|fungal|bacterial|blight|spot|wilt|yellow|chlorosis|mildew)\b/i;
...
export const inputClassifierService = {
    ...
    mergeWithVision(textResult, vision) {
        if (!vision)
            return textResult;
...
//# sourceMappingURL=input-classifier.service.js.map