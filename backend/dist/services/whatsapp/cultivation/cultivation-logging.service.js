...
export const cultivationLoggingService = {
    ...
    async scheduleApplicationPrompt(params) {
        if (!env.ENABLE_CULTIVATION_FOLLOW_UPS)
            return;
        const scheduledAt = new Date(Date.now() + (params.daysFromNow ?? applicationDays()) * 24 * 60 * 60 * 1000).toISOString();
    ...
    async scheduleResultValidation(params) {
        if (!env.ENABLE_CULTIVATION_FOLLOW_UPS)
            return;
        const scheduledAt = new Date(Date.now() + (params.daysFromNow ?? resultValidationDays()) * 24 * 60 * 60 * 1000).toISOString();
        await supabase.from('advisory_automation_jobs').insert({
            farmer_id: params.farmerId,
    ...
    async handleApplied(farmerId, _phone, lang) {
    ...
    async handleNotYet(farmerId, lang) {
        await conversationSessionService.patchContext(farmerId, {
            pendingCultivationPrompt: undefined,
        });
        return notYetReminder(lang);
    },
    async handleOutcome(farmerId, phone, lang, outcome) {
        const ctx = await conversationSessionService.getContext(farmerId);
        const activityId = ctx.pendingResultActivityId;
        const resolvedOutcome = outcome === 'agronomist' ? 'no_improvement' : outcome;
        if (activityId) {
            await supabase
                .from('cultivation_activities')
                .update({
    ...
    async escalateNoImprovement(farmerId, phone, _lang, activityId) {
        const ctx = await conversationSessionService.getContext(farmerId);
    ...
    async handleSprayCompletedText(farmerId, _phone, lang, text) {
        const ctx = await conversationSessionService.getContext(farmerId);
    ...
    isSprayCompletedMessage(text) {
        const t = text.trim().toLowerCase();
        return (/\b(spray|sprey|തളി|தெளி|ಸ್ಪ್ರೇ|स्प्रे).*(done|complete|finished|ചെയ്ത|முடிஞ|ಮುಗಿ|हो गया|കഴിഞ്ഞ)/i.test(t) ||
    ...
    async handleInboundAction(params) {
        ...
        if (action === 'cult.help' || action === 'cult.need_help') {
            const reply = await callbackFlowService.createCallback(farmerId, lang, 'Help applying recommendation');
            await whatsappService.sendText(phone, reply);
            return { handled: true };
        }
        ...
        if (action === 'cult.no_improve' ||
            action === 'cult.agronomist' ||
            /no improvement/i.test(action)) {
            const reply = await this.handleOutcome(farmerId, phone, lang, action === 'cult.agronomist' ? 'agronomist' : 'no_improvement');
            await whatsappService.sendText(phone, reply);
            return { handled: true };
        }
...
//# sourceMappingURL=cultivation-logging.service.js.map