...
export function soilGatePreface(language) {
    return language === 'ml'
...
export const nutrientSoilGateService = {
    ...
    async shouldGateBeforeFertilizerAdvice(farmerId, advisory) {
        if (!suggestsNutrientDeficiency(advisory))
            return false;
    ...
    async deliverPending(params) {
        ...
        if (params.extraFooter)
            body += `\n\n${params.extraFooter}`;
        const productBlock = shopifyLinksService.formatRecommendationsForWhatsApp([], params.language);
        if (productBlock)
            body += `\n\n${productBlock}`;
...
//# sourceMappingURL=nutrient-soil-gate.service.js.map