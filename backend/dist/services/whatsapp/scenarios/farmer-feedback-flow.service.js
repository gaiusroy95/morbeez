...
function askProduct(lang) {
    if (lang === 'ml')
        return 'ഏത് ഉൽപ്പന്നം / സ്പ്രേ ഉപയോഗിച്ചിരുന്നു?';
    return 'Which product or spray did you use? (or type "none")';
}
function askOutcome(lang) {
    if (lang === 'ml')
...
export function parseFarmerOutcomeAnswer(text) {
    ...
    if (t.length > 48)
        return null;
    if (/\b(applied|fertiliz|fertilis|spray|labou?r|kg|paid|per acre)\b/i.test(t))
        return null;
    if (/^(improved|better|good|recovered|fine)\b/i.test(t) || /^(മെച്ചം|சரி)$/u.test(t) || /^हो गया/.test(t)) {
        return 'improved';
    }
    if (/^(partial|partially|some improvement)\b/i.test(t) ||
        /^(ഭാഗിക|कुछ हद)/u.test(t)) {
        return 'partial';
    }
    if (/^(no change|same|not improved|worse|no improvement)\b/i.test(t) ||
        /^(ഇല്ല|नहीं)$/u.test(t)) {
        return 'no_change';
    }
    if (/^[1１]$/.test(t))
...
export const farmerFeedbackFlowService = {
    ...
    async tryHandleCapture(params) {
        ...
        if (step === 'experience_years') {
            ...
            await farmerExperienceLearningService.saveCropExperienceYears(params.farmerId, years, feedbackId);
            await farmerExperienceLearningService.updateCapture(feedbackId, {
...
//# sourceMappingURL=farmer-feedback-flow.service.js.map