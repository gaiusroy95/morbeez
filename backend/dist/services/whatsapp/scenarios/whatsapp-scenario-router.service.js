...
import { isMainMenuGreeting, mainMenuCopy, moreMenuCopy, normalizeMenuId, } from './whatsapp-menu.service.js';
...
function resolveMenuAction(text) {
    ...
    if (!t)
        return null;
    if (t.startsWith('menu.') || MENU_IDS.has(t))
        return t.startsWith('menu.') ? t : t;
    const lower = t.toLowerCase();
    if (/^(weather|കാലാവസ്ഥ|வானிலை|ಹವಾಮಾನ|मौसम)$/i.test(lower))
        return 'menu.weather';
    ...
    if (/^(call back|callback|കോൾബാക്ക്|கால்பேக்|ಕಾಲ್|कॉलबैक)/i.test(lower))
        return 'menu.expert';
    if (/^(soil test|soil|മണ്ണ്|மண்|ಮಣ್ಣು|मिट्टी)/i.test(lower))
        return 'menu.soil';
    return null;
}
...
function parseAcreageBucket(text) {
    const t = text.trim().toLowerCase();
    if (t === 'acreage.0_1' || /^0\s*-\s*1/.test(t) || /0\s*to\s*1/.test(t))
        return '0_1';
    if (t === 'acreage.2_5' || /^2\s*-\s*5/.test(t) || /2\s*to\s*5/.test(t))
        return '2_5';
    if (t === 'acreage.5_plus' || /^5\+/.test(t) || /more than 5|5 plus/.test(t))
        return '5_plus';
    return null;
}
function acreageValue(bucket) {
    if (bucket === '0_1')
        return 1;
    if (bucket === '2_5')
        return 3.5;
...
function parsePlantingDateDDMMYYYY(text) {
...
export const whatsappScenarioRouter = {
    ...
    async tryRoute(msg, captured, session, send) {
        ...
        const pendingRecId = ctxEarly.pendingRecommendationRecordId ??
        ...
        if (pendingRecId &&
            ...
            const photoAck = await recommendationFollowUpService.handleOutcomePhotoUpload(captured.farmerId, pendingRecId);
            if (photoAck) {
        ...
        if (text && /^(improved|better|partial|no improvement|worse|worsening|[1-4])$/i.test(text)) {
            ...
            if (recId && ctx.pendingRecommendationFollowUp === 'outcome') {
                const interpreted = await recommendationFollowUpService.interpretAndHandleOutcomeText(captured.farmerId, recId, text, false);
                if (interpreted) {
                    await send.text(msg.phone, interpreted);
                    return { handled: true };
                }
            ...
            if (outcome === 'no_improvement' || outcome === 'worsened') {
                ...
                await send.text(msg.phone, outcome === 'worsened'
        ...
        const inOnboardingCropFlow = session.state === 'onboarding_minimal' &&
            ...
            if (plots.length >= 2 && !activePlotId) {
                if (msg.text?.trim()) {
                    ...
                    if (selectedFromText) {
                        ...
                        const requested = await cropSelectionService.resolveSelection(captured.farmerId, msg.text);
                        const requestedCrop = requested && requested.kind !== 'other' ? requested.slug : null;
