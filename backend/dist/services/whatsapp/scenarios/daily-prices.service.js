...
export const dailyPricesService = {
    async formatForFarmer(farmerId, language) {
        ...
        const weeklyPct = await computeWeeklyTrendPct(crop, String(primary.market_name), date, Number(primary.price_per_kg));
        return this.formatRows(language, crop, date, rows, weeklyPct);
    },
    formatRows(language, crop, date, rows, weeklyPct) {
        const primary = rows[0];
        const price = Number(primary.price_per_kg);
        const arrow = primary.last_year_price_per_kg != null
