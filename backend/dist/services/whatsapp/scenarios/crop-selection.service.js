...
export const DEFAULT_CROP_KEYS = ['ginger', 'banana', 'cardamom', 'pepper'];
const CROP_TITLES = {
    ginger: 'Ginger',
    banana: 'Banana',
    cardamom: 'Cardamom',
...
function readCustomCrops(metadata) {
    ...
    for (const item of raw) {
        if (typeof item === 'string') {
            ...
            if (slug)
                out.push({ slug, label: displayLabel(item) });
            continue;
        }
...
export const cropSelectionService = {
    ...
    async registerCustomCrop(farmerId, cropName) {
        ...
        const defaultSet = new Set(DEFAULT_CROP_KEYS);
    ...
    parseSelection(text) {
        ...
        if (!t)
            return null;
        if (t === 'crop.other' || t === 'others' || t === 'other')
            return { kind: 'other' };
        if (t.startsWith('crop.')) {
            const slug = normalizeSlug(t.slice(5));
            if (!slug || slug === 'other' || slug === 'others')
                return { kind: 'other' };
            if (DEFAULT_CROP_KEYS.includes(slug)) {
...
//# sourceMappingURL=crop-selection.service.js.map