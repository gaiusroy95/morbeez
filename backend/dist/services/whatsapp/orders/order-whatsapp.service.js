...
function displayTrackingId(order) {
    if (order.tracking_awb)
        return order.tracking_awb;
    if (order.order_name)
        return order.order_name.replace('#', 'MBZ');
    return `MBZ${String(order.shopify_order_id ?? '').slice(-6)}`;
}
export const orderWhatsappService = {
    normalizePhone,
    async resolveFarmerByPhone(phone) {
        ...
        if (!farmer) {
            ...
            if (alt) {
                return {
                    farmerId: alt.id,
                    language: (alt.preferred_language ?? 'en'),
    ...
    async linkOrderToFarmer(shopifyOrderId, phone) {
        if (!phone)
            return;
    ...
    async alreadyNotified(referenceKey) {
    ...
    async sendDispatched(params) {
        ...
        if (await this.alreadyNotified(referenceKey))
            return false;
    ...
    async sendPaymentFailed(params) {
        ...
        const amountInr = params.amountPaise != null ? `₹${(params.amountPaise / 100).toFixed(0)}` : undefined;
    ...
    async getLatestOrderForPhone(phone) {
        const normalized = normalizePhone(phone);
        const last10 = normalized.slice(-10);
        const { data } = await supabase
            .from('commerce_orders')
            .select('id, order_name, shopify_order_id, payment_status, fulfillment_status, tracking_awb, tracking_url, expected_delivery_at, total_amount, created_at')
...
//# sourceMappingURL=order-whatsapp.service.js.map