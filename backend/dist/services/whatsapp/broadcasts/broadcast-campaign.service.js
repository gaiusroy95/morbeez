...
async function farmerIdsMatchingAudience(filter, limit = 10000) {
...
export const broadcastCampaignService = {
    ...
    async updateCampaign(id, patch) {
        ...
        if (patch.audienceJson != null)
            payload.audience_json = patch.audienceJson;
        if (patch.messageTitle != null)
            payload.message_title = patch.messageTitle;
        if (patch.messageBody != null)
            payload.message_body = patch.messageBody;
        if (patch.languageMode != null)
            payload.language_mode = patch.languageMode;
        if (patch.mediaUrls != null)
            payload.media_urls = patch.mediaUrls;
        if (patch.status != null)
            payload.status = patch.status;
        if (patch.scheduledAt !== undefined)
            payload.scheduled_at = patch.scheduledAt;
        if (patch.approvedBy != null)
            payload.approved_by = patch.approvedBy;
        const { data, error } = await supabase
            .from('whatsapp_broadcast_campaigns')
            .update(payload)
            .eq('id', id)
    ...
    async previewAudience(filter) {
        ...
        const samples = await Promise.all(sampleIds.map(async (farmerId) => loadFarmerVariableContext(farmerId)));
        return { count: ids.length, sample: samples };
    },
    async previewMessage(campaignId, farmerId) {
        const campaign = await this.getCampaign(campaignId);
        const ids = await farmerIdsMatchingAudience(campaign.audienceJson, 1);
        const targetId = farmerId ?? ids[0];
        if (!targetId)
            throw new ValidationError('No farmers match this audience');
    ...
    async sendCampaign(id, opts) {
        ...
        const kind = CATEGORY_TO_KIND[campaign.category] ?? 'custom_campaign';
    ...
    async cloneTemplateToCampaign(templateId, createdBy) {
        ...
        return this.createCampaign({
            ...
            category: tpl.category ?? 'custom_message',
            messageTitle: tpl.title ? String(tpl.title) : undefined,
            messageBody: String(tpl.body),
            mediaUrls: (tpl.media_urls ?? []),
            templateId,
            createdBy,
        });
    },
    async exportDeliveriesCsv(opts) {
        let q = supabase
    ...
    async getCampaignAnalytics(opts) {
        ...
        for (const row of rows) {
            ...
            if (!byKind[kind])
                byKind[kind] = { sent: 0, failed: 0, skipped: 0 };
            if (status === 'sent')
                byKind[kind].sent++;
            else if (status === 'failed')
                byKind[kind].failed++;
    ...
    async getFarmerPreferences(farmerId) {
        const { data, error } = await supabase
            .from('farmer_broadcast_preferences')
            .select('*')
        ...
        if (!data) {
            return { farmerId, optedOutAll: false, optedOutCategories: [] };
        }
        return {
            farmerId,
            optedOutAll: Boolean(data.opted_out_all),
            optedOutCategories: (data.opted_out_categories ?? []),
        };
    },
    async updateFarmerPreferences(farmerId, patch) {
        const payload = {
            farmer_id: farmerId,
...
//# sourceMappingURL=broadcast-campaign.service.js.map