...
export async function loadFarmerVariableContext(farmerId) {
    ...
    const name = [farmer?.first_name, farmer?.last_name].filter(Boolean).join(' ').trim() ||
        String(farmer?.name ?? '').trim() ||
        'Farmer';
    const crop = String(block?.crop_type ?? 'crop');
    const dap = block?.planting_date != null
...
export function renderBroadcastMessage(template, ctx) {
    return template.replace(PLACEHOLDER_RE, (_, key) => {
        const k = key.toLowerCase();
        if (k === 'farmername')
            return ctx.farmerName;
        if (k === 'crop')
            return ctx.crop;
        if (k === 'dap')
            return ctx.dap;
...
//# sourceMappingURL=broadcast-variables.js.map