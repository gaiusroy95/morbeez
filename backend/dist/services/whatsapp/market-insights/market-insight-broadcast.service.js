...
async function upsertSnapshot(params) {
    ...
    if (error)
        logger.warn({ err: error, farmerId: params.farmerId }, 'Market insight snapshot upsert failed');
}
    ...
    async buildSnapshots(options) {
        ...
        for (const farmer of farmers ?? []) {
            ...
            if (!built.ok || !built.payload) {
                ...
                if (built.error)
                    result.errors.push(`${farmerId}: ${built.error}`);
                continue;
            }
            if (options?.dryRun) {
                result.built++;
    ...
    async sendSnapshots(options) {
        ...
        if (farmerIds.length) {
            ...
            for (const f of farmers ?? []) {
                if (f.phone)
                    phoneByFarmer.set(String(f.id), String(f.phone).replace(/\D/g, ''));
            }
        }
