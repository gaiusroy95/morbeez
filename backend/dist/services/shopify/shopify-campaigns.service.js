...
function parseDiscountLabel(label) {
    ...
    if (flatMatch && /off/i.test(trimmed)) {
        ...
        if (value > 0)
            return { valueType: 'fixed_amount', value };
    }
    return null;
}
export const shopifyCampaignsService = {
    ...
    async syncCoupon(input) {
        ...
        try {
            const ruleRes = await shopifyAdmin('/price_rules.json', {
                method: 'POST',
    ...
    async syncFlashSalePrice(input) {
        const productId = input.shopifyProductId.trim();
        if (!productId)
            return { ok: false, reason: 'No Shopify product linked' };
        try {
            const res = await shopifyAdmin(`/products/${productId}.json`);
            const variants = res.product.variants ?? [];
            if (!variants.length)
                return { ok: false, reason: 'Product has no variants' };
            const flash = input.flashPrice.toFixed(2);
            const compareAt = input.originalPrice.toFixed(2);
            await Promise.all(variants.map((variant) => shopifyAdmin(`/variants/${variant.id}.json`, {
                method: 'PUT',
                body: JSON.stringify({
                    variant: {
                        id: variant.id,
...
//# sourceMappingURL=shopify-campaigns.service.js.map