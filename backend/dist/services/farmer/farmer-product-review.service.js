...
function productKey(title, variantId, sku) {
    ...
    if (sku?.trim())
        return `sku:${sku.trim().toLowerCase()}`;
    return `title:${title.trim().toLowerCase().slice(0, 120)}`;
}
function lineItemsWithKeys(order) {
    return order.lineItems.map((li) => {
        const variantId = li.shopifyVariantId ?? null;
        const productId = li.shopifyProductId ?? null;
...
function canReviewOrder(order, omsStatus) {
...
async function loadCommerceOms(order) {
...
export const farmerProductReviewService = {
    ...
    async submitReview(farmerId, orderId, input) {
        ...
        if (!line)
            throw new NotFoundError('Product not found on this order');
        ...
        if (!data)
            throw new NotFoundError('Review not saved');
        return {
            id: String(data.id),
    ...
    async aggregateForProduct(shopifyProductId) {
        const { data, error } = await supabase
            .from('farmer_product_reviews')
...
//# sourceMappingURL=farmer-product-review.service.js.map