...
function healthFromSoil(raw) {
    const v = String(raw ?? '').toLowerCase();
    if (v.includes('critical') || v.includes('poor'))
        return { status: 'critical', label: 'Critical' };
    if (v.includes('alert') || v.includes('watch'))
        return { status: 'alert', label: 'Needs attention' };
    if (v.includes('monitor'))
        return { status: 'monitor', label: 'Monitor' };
...
function parseRecommendationProducts(raw) {
    if (Array.isArray(raw)) {
        return raw
            ...
            if (p && typeof p === 'object') {
                const o = p;
                const title = String(o.tradeName ?? o.productTitle ?? o.title ?? o.name ?? o.technicalName ?? 'Product').trim();
...
export const farmerPortalMobileService = {
    ...
    async updateBlock(farmerId, blockId, input) {
    ...
    async getBlockDetail(farmerId, blockId) {
        ...
        const { data: soil } = await supabase
            ...
            .limit(1)
            .maybeSingle();
        const metrics = soil?.metrics;
        const macro = (metrics?.macro ?? metrics);
    ...
    async getBlockTimeline(farmerId, blockId) {
        ...
        for (const s of scans.data ?? []) {
            const out = s.ai_advisory_outputs?.[0];
            items.push({
                id: `scan-${s.id}`,
                type: 'scan',
    ...
    async runScan(farmerId, input) {
        ...
        const storagePath = await advisoryImageStorageService.uploadFromBase64(farmerId, input.imageData, input.mimeType ?? 'image/jpeg');
        ...
        const { data: recRow } = await supabase
            ...
            .maybeSingle();
        const summary = lang === 'ml' && result.advisory.farmerSummaryMl
    ...
    async getScan(sessionId, farmerId) {
        const session = await cropDoctorService.getSession(sessionId);
        if (String(session.farmer_id) !== farmerId)
            throw new NotFoundError('Scan not found');
        const output = session.ai_advisory_outputs?.[0];
    ...
    async listScans(farmerId, query) {
    ...
    async listRecommendations(farmerId) {
    ...
    mapRecommendation(r) {
        ...
        return {
            id: String(r.id),
            kind: products.length ? 'product' : 'technical',
    ...
    async getRecommendation(farmerId, id) {
        const { data, error } = await supabase
            .from('crm_recommendations')
            .select('*, farm_blocks(name, crop_name)')
            .eq('farmer_id', farmerId)
        ...
        const base = this.mapRecommendation(data);
    ...
    async markRecommendationApplied(farmerId, id) {
        const { data, error } = await supabase
            .from('crm_recommendations')
    ...
    async listActivities(farmerId, query) {
        const blocks = await blockService.listByFarmer(farmerId);
        ...
        if (query.type)
            q = q.eq('activity_type', query.type);
        if (query.from)
            q = q.gte('applied_at', query.from);
        if (query.to)
            q = q.lte('applied_at', query.to);
        const { data, error } = await q;
    ...
    async createActivity(farmerId, input) {
        const block = await blockService.getById(input.blockId, farmerId);
        if (!block)
            throw new NotFoundError('Field not found');
        const activityType = input.activityType === 'irrigation' ? 'other' : input.activityType;
        const label = input.productUsed?.trim() ||
    ...
    async createRoiEntry(farmerId, input) {
    ...
    async getWeather(farmerId, blockId) {
        const block = blockId
    ...
    async getMarketPrices(farmerId, crop) {
    ...
    async getRoiDashboard(farmerId) {
        ...
        for (const b of active.breakdown) {
            ...
            if (l.includes('labour') || l.includes('labor'))
                breakdownLegacy.labor += b.value;
            else if (l.includes('fert') || l.includes('spray') || l.includes('purchase'))
                breakdownLegacy.inputs += b.value;
            else if (l.includes('irrig') || l.includes('mach'))
                breakdownLegacy.operations += b.value;
            else
                breakdownLegacy.other += b.value;
        }
        return {
