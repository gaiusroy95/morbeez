...
function dapLabelFromPlanting(plantingDate, at) {
    if (!plantingDate)
        return null;
...
function parseRecommendationBullets(text) {
    if (!text?.trim())
        return [];
    return text
        .split(/\n|•|·|;/)
...
export const farmerPortalService = {
    ...
    async getSummary(farmerId) {
        ...
        const latestRec = recsRes.data?.[0];
        const blockJoin = latestRec?.farm_blocks;
        const nextAdvisory = latestRec?.follow_up_at && new Date(String(latestRec.follow_up_at)) > new Date()
    ...
    async listSoilReports(farmerId) {
        const { data, error } = await supabase
            .from('crm_soil_reports')
    ...
    async createSoilReport(farmerId, input) {
        ...
        const metrics = buildMetricsFromForm(input.macro ?? {}, input.micro ?? {}, undefined, input.remarks);
        ...
        if (input.imageData?.trim()) {
            const path = await advisoryImageStorageService.uploadFromBase64(farmerId, input.imageData, input.mimeType ?? 'image/jpeg');
            if (!path)
                throw new NotFoundError('Could not upload report — try a smaller image (max 8MB)');
            pdfUrl = path;
        }
    ...
    async getRoi(farmerId) {
        ...
        const acreage = (blocksRes.data ?? []).reduce((sum, b) => sum + (Number(b.acreage_decimal) || 0), 0);
        return {
            summary: {
    ...
    async listNotifications(farmerId, limit = 12) {
        const items = [];
    ...
    async listFieldPhotos(farmerId) {
        const { data, error } = await supabase
            .from('crop_images')
        ...
        const rows = await Promise.all((data ?? []).map(async (r) => {
            const url = await resolveAdvisoryImageUrl(r.storage_path ? String(r.storage_path) : r.external_url ? String(r.external_url) : null);
            return {
                id: String(r.id),
                uploadedAt: formatDateTime(String(r.created_at)),
                crop: r.crop ? String(r.crop) : null,
                status: r.review_status === 'reviewed'
