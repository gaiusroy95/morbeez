...
export const roiAggregationService = {
    async resolveActiveSeasonIds(farmerId, filter) {
        let q = supabase
            .from('crop_seasons')
        ...
        if (filter.blockId)
            q = q.eq('block_id', filter.blockId);
        if (filter.crop)
            q = q.ilike('crop', filter.crop);
        const { data, error } = await q;
        throwIfSupabaseError(error, 'Could not load active seasons');
        return (data ?? []).map((r) => String(r.id));
    },
    async getSummary(farmerId, filter) {
        const blocks = await blockService.listByFarmer(farmerId).then((rows) => rows.map((b) => ({
            id: b.id,
        ...
        for (const seasonId of seasonIds) {
            ...
            const { data: entries } = await supabase
                ...
                .select('id, entry_date, entry_type, amount_inr, debit_inr, credit_inr, comments, income_subtype, season_id, block_id, category_id, roi_expense_types(expense_name, color), farmer_roi_categories(name, color)')
            ...
            for (const e of entries ?? []) {
                ...
                if (!isIncome) {
                    const roiCat = e.farmer_roi_categories;
                    const expCat = e.roi_expense_types;
                ...
                recentRows.push({
                    ...
                    label: e.farmer_roi_categories?.name ??
        ...
        const fixedRoi = hasIncome && expenseInr > 0 && profitInr != null
    ...
    async getContext(farmerId, filter) {
        const blocks = await blockService.listByFarmer(farmerId).then((rows) => rows.map((b) => ({
            id: b.id,
            name: blockDisplayName(b),
            crop: b.crop_type,
...
//# sourceMappingURL=roi-aggregation.service.js.map