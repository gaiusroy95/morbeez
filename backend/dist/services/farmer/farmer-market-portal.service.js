...
function trendFromYoY(price, last) {
    if (last == null)
        return null;
    if (price > last * 1.02)
        return 'up';
...
function weeklyTrendPct(current, previous) {
    if (previous == null || previous <= 0)
        return null;
    return Math.round(((current - previous) / previous) * 100);
}
async function listActiveCrops() {
...
async function fetchPriceRows(crop, today, marketName) {
    let query = supabase
    ...
    if (marketName)
        fallbackQuery = fallbackQuery.eq('market_name', marketName);
    const { data: fallback } = await fallbackQuery;
    if (!fallback?.length)
        return { date: today, rows: [] };
    return { date: String(fallback[0].price_date), rows: fallback };
}
async function computeDailyChange(crop, marketName, date, price) {
...
async function computeWeeklyTrendPct(crop, marketName, date, todayPrice) {
...
async function fetchMonthlySeries(crop, marketName, insightDate) {
    ...
    for (let m = 1; m <= 12; m += 1) {
        ...
        if (byMonth[key].cy == null && cyHist != null)
            byMonth[key].cy = cyHist;
        if (byMonth[key].py == null && pyHist != null)
            byMonth[key].py = pyHist;
    }
...
function buildTrendInsights(crop, points) {
    const insights = [];
    ...
    if (withBoth.length) {
        ...
        if (avgCy > avgPy * 1.03)
            insights.push('Prices stronger than last year.');
        else if (avgCy < avgPy * 0.97)
            insights.push('Prices weaker than last year.');
        else
            insights.push('Prices tracking close to last year.');
    }
    const rising = points.filter((p) => p.currentYear != null && p.previousYear != null && p.currentYear > p.previousYear * 1.05);
    if (rising.length >= 2) {
    ...
    if (!insights.length)
        insights.push(`Check ${cropLabel(crop)} prices regularly for seasonal movement.`);
    return insights.slice(0, 3);
}
function opportunitySignal(yoyPct, weeklyPct) {
...
export const farmerMarketPortalService = {
    ...
    async listMarkets(_farmerId, crop) {
        const { data } = await supabase
            .from('markets')
    ...
    async getDashboard(farmerId, crop, market) {
        ...
        const weeklyTrendPctVal = top != null ? await computeWeeklyTrendPct(cropType, top.marketName, date, top.pricePerKg) : null;
        const lastYearSameDayPricePerKg = top?.lastYearPricePerKg ?? null;
        const differenceInr = top && lastYearSameDayPricePerKg != null
            ? Math.round(top.pricePerKg - lastYearSameDayPricePerKg)
            : null;
        const yesterdayPrice = top && dailyChangeInr != null ? top.pricePerKg - dailyChangeInr : null;
        const dailyChangePct = dailyChangeInr != null && yesterdayPrice != null && yesterdayPrice > 0
        ...
        return {
            ...
            priceDirection: top?.yoyPct != null
                ? top.yoyPct > 3
                    ? 'strong'
                    : top.yoyPct < -3
                        ? 'weak'
    ...
    async getTrends(farmerId, crop, range, market) {
        const block = await blockService.getPrimaryBlock(farmerId);
        const crops = await listActiveCrops();
        const cropType = (crop ?? block?.crop_type ?? crops[0]?.cropName ?? 'ginger').toLowerCase();
        const today = istDateKey();
    ...
    async getMandiComparison(farmerId, crop, market) {
        const block = await blockService.getPrimaryBlock(farmerId);
        ...
        for (const m of mapped) {
            if (m.pricePerKg > (highest?.pricePerKg ?? 0))
                highest = m;
        }
    ...
    async getMultiCropComparison(farmerId, market) {
        const block = await blockService.getPrimaryBlock(farmerId);
        const defaultMarket = market ?? 'Kochi';
        const crops = await listActiveCrops();
        const today = istDateKey();
        const favoriteCrop = block?.crop_type?.toLowerCase() ?? null;
        const items = await Promise.all(crops.map(async (c) => {
    ...
    async adminArchiveCropMarket(id) {
...
//# sourceMappingURL=farmer-market-portal.service.js.map