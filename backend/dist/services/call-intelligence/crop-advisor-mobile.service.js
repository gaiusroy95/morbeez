...
export const cropAdvisorMobileService = {
    ...
    async buildActionQueue(agentEmail) {
        const [overdue, dueToday, hot, escalated] = await Promise.all([
            cropAdvisorLeadQueueService.listOperationalLeads({ scope: 'mine', smartFilter: 'overdue', limit: 3 }, agentEmail),
            cropAdvisorLeadQueueService.listOperationalLeads({ scope: 'mine', smartFilter: 'due_today', limit: 3 }, agentEmail),
            cropAdvisorLeadQueueService.listOperationalLeads({ scope: 'mine', smartFilter: 'hot_leads', limit: 3 }, agentEmail),
            cropAdvisorLeadQueueService.listOperationalLeads({ scope: 'mine', smartFilter: 'escalated', limit: 3 }, agentEmail),
        ]);
        const items = [];
    ...
    async getWorkspaceSummary(leadId) {
        ...
        return {
            ...
            lead: {
                ...
                stageLabel: cropAdvisorAdminService.stageLabels[String(detail.lead.stage ?? '')] ?? String(detail.lead.stage ?? ''),
...
//# sourceMappingURL=crop-advisor-mobile.service.js.map