...
async function resolveLead(leadId) {
...
async function logCallEvidence(agentEmail, durationSeconds) {
    try {
...
export const callIntelligenceService = {
    ...
    async processCall(callId) {
        ...
        try {
            ...
            const interaction = await crmFarmerService.createInteraction(String(call.farmer_id), call.lead_id ? String(call.lead_id) : null, {
    ...
    async applySoilTestAutomation(leadId, farmerId, summary, agentEmail) {
    ...
    async runDiagnosis(callId, input) {
        const call = await this.getCall(callId);
        const transcript = call.transcript ? String(call.transcript) : '';
        if (!transcript)
            throw new ValidationError('Transcript required for diagnosis');
    ...
    async getLeadTimeline(leadId) {
        ...
        for (const c of calls.data ?? []) {
            if (items.some((i) => i.meta?.callId === c.id))
                continue;
            items.push({
                id: `call-${c.id}`,
                type: 'call',
...
//# sourceMappingURL=call-intelligence.service.js.map