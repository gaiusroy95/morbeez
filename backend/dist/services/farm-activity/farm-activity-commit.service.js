...
function moneyAmount(field) {
    const value = fieldValue(field);
    return value && Number.isFinite(value.amount) ? Number(value.amount) : null;
}
function mapActivityType(label) {
    const text = (label ?? '').toLowerCase();
    if (/spray|pesticide|fungicide|insecticide/.test(text))
        return 'spray_applied';
    if (/fertigat|fertiliz|nutrition|manure|urea/.test(text))
        return 'fertigation';
    if (/drench/.test(text))
        return 'drench';
    if (/scout|inspect|observe|monitor/.test(text))
        return 'scouting';
    return 'other';
}
function buildProvenance(draft, modality) {
    return {
...
async function resolveBlockId(farmerId, blockRef, fallbackBlockId) {
    if (fallbackBlockId)
...
export const farmActivityCommitService = {
    ...
    async commitDraft(input) {
        ...
        const begin = (beginData ?? {});
        if (begin.status === 'confirmed'
        ...
        if (draftRow.status === 'confirmed' && draftRow.metadata) {
            ...
            if (existing?.status === 'confirmed') {
                return {
                    ...
                    revision: draft.revision,
                    commandId,
                    activityIds: (existing.committed_activity_ids ?? []).map(String),
                    roiEntryIds: (existing.committed_roi_entry_ids ?? []).map(String),
                    harvestIds: (existing.committed_harvest_ids ?? []).map(String),
                };
            }
        }
    ...
    async writeSubEvent(input) {
        ...
        if (event.kind === 'labour' || event.kind === 'purchase' || event.kind === 'expense') {
            const amount = event.kind === 'labour'
            ...
            const occurredOn = fieldValue(event.occurredOn) ?? new Date().toISOString().slice(0, 10);
            const blockId = await resolveBlockId(farmerId, event.kind === 'purchase' || event.kind === 'labour' ? null : null, fallbackBlockId);
            const entryType = event.kind === 'labour' ? 'labour' : event.kind === 'purchase' ? 'purchase' : 'misc';
            const comments = event.kind === 'labour'
...
//# sourceMappingURL=farm-activity-commit.service.js.map