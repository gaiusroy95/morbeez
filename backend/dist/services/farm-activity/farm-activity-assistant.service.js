...
import { FARM_ACTIVITY_ASSISTANT_CONTRACT_VERSION, } from '@morbeez/shared/farm-activity-assistant';
...
import { applyClarificationAnswer, resolveBlockRefFromBlocks, } from './farm-activity-clarification.service.js';
...
const ACTIVITY_INTENT_RE = /\b(spray|sprayed|fertiliz|fertilis|labour|labor|harvest|harvested|bought|purchase|purchased|expense|irrigat|weeded|plough|plow|applied|drench|scouting|workers?|wage|cost|spent|income|paid)\b|സ്പ്രേ|വളം|തൊഴിലാളി|വിളവെടുപ്പ്|വാങ്ങി|ചെലവ്|തൊழிலாளர்|அறுவடை|வாங்கி|खरीदा|खर्च|मजदूर|ಕೊಂಡು|ಕೂಲಿ|ಸಿಂಪಡಣೆ/i;
function looksLikeFarmActivityIntent(text) {
...
function isEditButton(text) {
    return text === EDIT_ID || /^edit$/i.test(text);
}
function isCancelButton(text) {
...
async function setAssistantPointer(farmerId, draft, clarificationAttempts, state) {
    await conversationSessionService.patchContext(farmerId, {
...
export const farmActivityAssistantService = {
    ...
    looksLikeIntent(text) {
        return looksLikeFarmActivityIntent(text);
    },
    isFarmActivityState(state) {
    ...
    async processUtterance(input) {
        ...
        try {
            ...
            const clarificationAttempts = Number(pointer?.clarificationAttempts
                ?? existing?.metadata?.clarificationAttempts
                ?? 0);
            const pendingClarification = existing?.draft_json?.clarifications?.[0];
            const inClarify = input.sessionState === 'farm_activity_clarify' || Boolean(pendingClarification);
            if (inClarify && pendingClarification && existing?.draft_json) {
                ...
                if (pendingClarification.field === 'blockRef') {
                    ...
                    if (!blockRef) {
                        await input.send.text(input.phone, input.language === 'ml'
                ...
                if (clarified) {
                    ...
                    if (clarified.clarifications.length > 0) {
                        await setAssistantPointer(input.farmerId, clarified, clarificationAttempts + 1, 'farm_activity_clarify');
                        await input.send.text(input.phone, clarified.clarifications[0]?.question
                            ?? (input.language === 'ml' ? 'കൂടുതൽ വിവരം ആവശ്യമാണ്.' : 'I need one more detail.'));
                        return true;
                    }
                    await setAssistantPointer(input.farmerId, clarified, clarificationAttempts, 'farm_activity_confirm');
    ...
    async sendConfirmPrompt(input) {
        ...
        await input.send.text(input.phone, `${body}\n\nReply: Confirm / Edit / Cancel`);
    },
    async presentInvoiceDraft(input) {
        if (!this.enabled() || !env.ENABLE_FARM_ACTIVITY_INVOICE_OCR)
            return false;
        const subEvents = [
    ...
    async handleAction(input) {
        const ctx = await conversationSessionService.getContext(input.farmerId);
        const pointer = ctx.farmActivityAssistant;
        const draftRow = (pointer?.draftId ? await farmActivityDraftService.getById(pointer.draftId) : null)
            ?? (await farmActivityDraftService.getActiveForFarmer(input.farmerId));
        if (!draftRow) {
            await input.send.text(input.phone, input.language === 'ml'
        ...
        if (pointer && pointer.revision !== draftRow.revision) {
            ...
            await setAssistantPointer(input.farmerId, draftRow.draft_json, Number(draftRow.metadata?.clarificationAttempts ?? 0), 'farm_activity_confirm');
            await this.sendConfirmPrompt({
                phone: input.phone,
                language: input.language,
        ...
        if (isCancelButton(input.text)) {
            ...
            await clearAssistantPointer(input.farmerId);
            const { farmerFeedbackFlowService } = await import('../whatsapp/scenarios/farmer-feedback-flow.service.js');
            ...
            if (!resumed) {
                await input.send.text(input.phone, input.language === 'ml' ? 'ഡ്രാഫ്റ്റ് റദ്ദാക്കി.' : 'Draft cancelled.');
            }
            return true;
        }
        if (isEditButton(input.text)) {
            await setAssistantPointer(input.farmerId, draftRow.draft_json, Number(draftRow.metadata?.clarificationAttempts ?? 0), 'farm_activity_clarify');
...
//# sourceMappingURL=farm-activity-assistant.service.js.map