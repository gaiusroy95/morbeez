import { validateFarmActivityAssistantDraft, } from '@morbeez/shared/farm-activity-assistant';
...
function collectFields(event) {
    return Object.entries(event)
        .filter(([key, value]) => !['id', 'kind', 'sequence', 'sourceRefs'].includes(key)
        && value
        && typeof value === 'object'
        && 'value' in value)
        .map(([key, value]) => [key, value]);
}
export function summarizeDraftFieldConfidence(draft) {
...
export function resolveDraftStatus(draft) {
    if (draft.clarifications.length > 0)
        return 'clarifying';
    if (draft.subEvents.length === 0)
        return 'open';
    const hasUnresolved = draft.subEvents.some((event) => collectFields(event).some(([, field]) => field.value === null));
    return hasUnresolved ? 'open' : 'awaiting_confirm';
}
function modalitiesFromDraft(draft) {
    const modalities = new Set();
    if (draft.source.text)
        modalities.add('text');
    if (draft.source.transcript.length)
        modalities.add('voice');
    for (const media of draft.source.media)
        modalities.add(media.kind);
    return [...modalities];
}
export const farmActivityDraftService = {
    schemaAvailable: null,
    async isSchemaAvailable() {
    ...
    async getById(draftId) {
        if (!(await this.isSchemaAvailable()))
            return null;
    ...
    async getActiveForFarmer(farmerId, channel = 'whatsapp') {
        if (!(await this.isSchemaAvailable()))
            return null;
        const { data, error } = await supabase
            .from('farm_activity_drafts')
...
//# sourceMappingURL=farm-activity-draft.service.js.map