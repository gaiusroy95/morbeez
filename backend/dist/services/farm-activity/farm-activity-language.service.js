 ...
 */
export function detectFarmActivityLanguage(transcript, storedPreference) {
    const text = transcript.normalize('NFKC');
    const words = text.toLowerCase().match(/[a-z]+/gu) ?? [];
    const scores = new Map([['en', 0]]);
    ...
    for (const [code, pattern] of Object.entries(SCRIPT_PATTERNS)) {
        ...
        if (score > 0)
            observedScripts.push(code);
    }
    let englishSignals = 0;
    for (const word of words) {
        if (ENGLISH_TERMS.has(word))
            englishSignals += 1;
        for (const [code, terms] of Object.entries(ROMANIZED_TERMS)) {
            if (terms.has(word))
                scores.set(code, (scores.get(code) ?? 0) + 2);
        }
    }
...
//# sourceMappingURL=farm-activity-language.service.js.map