...
export const seoProductService = {
    async list(opts) {
        ...
        const rows = await Promise.all(products.map(async (p) => {
            ...
            const canonicalUrl = String(seo.canonicalUrl ?? `${seoSyncService.storefrontBase()}/products/${urlSlug || p.handle}`);
    ...
    async update(shopifyProductId, input, adminId) {
        const intel = await productIntelligenceService.get(shopifyProductId);
        const seo = { ...intel.seo };
        if (input.seoTitle !== undefined)
            seo.seoTitle = input.seoTitle;
        if (input.seoDescription !== undefined)
            seo.seoDescription = input.seoDescription;
        if (input.urlSlug !== undefined)
            seo.urlSlug = slugify(input.urlSlug);
        if (input.focusKeywords !== undefined)
            seo.focusKeywords = input.focusKeywords;
        if (input.canonicalUrl !== undefined)
            seo.canonicalUrl = input.canonicalUrl;
    ...
    async syncToShopify(shopifyProductId) {
        return seoSyncService.syncProductToShopify(shopifyProductId);
    },
};
//# sourceMappingURL=seo-product.service.js.map