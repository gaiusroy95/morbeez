...
import { addCalendarDays, currentAndPrevious, derivePoolStatus, indiaToday, isNoOpPoolChange, resolveVersionOnDate, snapshotFromVersion, validatePoolPct, versionLabel, } from './channel-pool.util.js';
function mapRow(row) {
    return {
        id: String(row.id),
...
function withDerivedStatus(row, asOf) {
    return { ...row, status: derivePoolStatus(row, asOf) };
}
async function loadVariantVersions(variantId) {
...
async function actorName(adminId, fallbackEmail) {
    ...
    const { data: admin } = await supabase
        ...
        .eq('id', adminId)
        .maybeSingle();
    return ((admin?.full_name ? String(admin.full_name).trim() : '') ||
        fallbackEmail ||
        admin?.email ||
        'Admin');
}
export const channelPoolService = {
    async listForProduct(productId) {
        ...
        for (const raw of data ?? []) {
            const row = withDerivedStatus(mapRow(raw), asOf);
    ...
    async resolve(input) {
        const asOf = typeof input.asOf === 'string' && /^\d{4}-\d{2}-\d{2}/.test(input.asOf)
    ...
    async snapshotForLine(input) {
        if (input.existing?.channelPoolVersionId && input.existing.channelPoolPct != null) {
            const stub = {
                ...
                agronomistMaxPct: input.existing.channelPoolAgronomistPct == null
                    ? null
                    : Number(input.existing.channelPoolAgronomistPct),
                partnerMaxPct: input.existing.channelPoolPartnerPct == null
                    ? null
            ...
            return {
                ...
                channelPoolAgronomistAmount: input.existing.channelPoolAgronomistAmount ?? snap.channelPoolAgronomistAmount,
                channelPoolPartnerAmount: input.existing.channelPoolPartnerAmount ?? snap.channelPoolPartnerAmount,
            };
        }
    ...
    async createVersion(input) {
        ...
        if (effectiveFrom < today) {
            throw new ValidationError('Effective From cannot be in the past. Historical orders keep their snapshotted pool.');
        }
        ...
        const initialStatus = effectiveFrom > today ? 'pending' : 'active';
        for (const row of existing) {
            if (row.effectiveTo && row.effectiveTo < effectiveFrom)
                continue;
        ...
        return withDerivedStatus(mapRow(data), today);
    },
};
export { versionLabel };
//# sourceMappingURL=channel-pool.service.js.map