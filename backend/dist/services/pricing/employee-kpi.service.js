...
function computeCollectionEfficiency(rows) {
    const byQuote = new Map();
    for (const r of rows) {
        const qid = r.commerce_quote_id ? String(r.commerce_quote_id) : null;
        if (!qid)
            continue;
        const set = byQuote.get(qid) ?? new Set();
...
async function computeRepeatCustomerMetrics(rows, monthStartIso) {
    const farmerIds = new Set();
    for (const r of rows) {
        if (r.farmer_id)
            farmerIds.add(String(r.farmer_id));
    }
    ...
    if (leadIds.length && farmerIds.size < leadIds.length) {
        ...
        for (const l of leads ?? []) {
            if (l.farmer_id)
                farmerIds.add(String(l.farmer_id));
        }
    }
    if (!farmerIds.size)
    ...
    const priorFarmers = new Set();
    for (const r of priorPaid ?? []) {
        if (r.farmer_id)
            priorFarmers.add(String(r.farmer_id));
    }
    let repeatCount = 0;
    for (const fid of farmerIds) {
        if (priorFarmers.has(fid))
            repeatCount += 1;
    }
    return { count: repeatCount, score: Math.min(5, repeatCount) };
}
async function computeReturnComplaintCount(rows, start, end) {
    const ledgerReturns = rows.filter((r) => r.status === 'returned').length;
    const orderIds = [...new Set(rows.map((r) => r.commerce_order_id).filter(Boolean))];
    if (!orderIds.length)
        return ledgerReturns;
...
export const employeeKpiService = {
    async recomputeMonthlyKpi(employeeProfileId, monthYear) {
        ...
        const profitContributionScore = profitTarget > 0 ? Math.min(20, (grossProfit / profitTarget) * 20) : grossProfit > 0 ? 10 : 0;
    ...
    async recomputeQuarterlyBonus(employeeProfileId, quarterKey) {
        const config = await pricingConfigService.getConfig();
        ...
        const avgRealization = list.reduce((s, r) => s + Number(r.avg_realization_pct), 0) / list.length;
    ...
    async getDashboard(monthYear, filter) {
        ...
        let employees = (scores ?? []).map((s) => {
            ...
            return {
                ...
                profitLabel: Number(s.gross_profit_inr) >= 50000
                    ? 'High'
        ...
        employees = employees.map((e, i) => ({
            ...
            rank: i + 1,
            isTopPerformer: i < 3,
            isUnderPerformer: e.grade === 'Risk' || e.grade === 'C' || e.salesAchievementPct < 80,
        }));
        ...
        return {
            ...
            summary: {
                ...
                avgScore: allEmployees.length > 0
                    ? allEmployees.reduce((s, r) => s + r.totalScore, 0) / allEmployees.length
                    : 0,
            },
        };
    },
    async recomputeAllForMonth(monthYear) {
        const my = monthYear ?? monthYearFromDate(new Date());
...
//# sourceMappingURL=employee-kpi.service.js.map