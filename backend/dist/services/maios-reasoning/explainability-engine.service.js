...
export const maiosExplainabilityEngineService = {
    build(params) {
        ...
        for (const item of params.evidence) {
            if (item.key.startsWith('photo:missing'))
                continue;
            const rules = topLabel
                ? maiosKnowledgeService.listLikelihoodRatios(params.pkg, item.key).filter((r) => r.diseaseLabel === topLabel && r.lr >= 1.2)
                : [];
...
//# sourceMappingURL=explainability-engine.service.js.map