...
function pushFeature(out, feature, confidence, value = 'present') {
    out.push({ feature, value, confidence: clampConfidence(confidence) });
}
function inferGingerFromText(t, conf) {
    const out = [];
    if (/blast|pyricularia|spindle|diamond/.test(t)) {
        pushFeature(out, 'spindle_shape', conf);
        pushFeature(out, 'grey_center', conf * 0.88);
        if (/black|dot|speck|pycnid/.test(t))
            pushFeature(out, 'black_dots', conf * 0.9);
    }
    if (/rot|pythium|rhizome|waterlog|foul/.test(t))
        pushFeature(out, 'soft_rot', conf);
    if (hasPestSilverStreakEvidence(t, 'ginger'))
        pushFeature(out, 'silver_streak', conf);
    return out;
}
function inferTomatoFromText(t, conf) {
    const out = [];
...
function inferCoconutFromText(t, conf) {
    const out = [];
    ...
    if (/yellow|chlorosis|nutrient/.test(t))
        pushFeature(out, 'yellowing', conf * 0.85);
    return out;
}
function inferBrinjalFromText(t, conf) {
    const out = [];
    if (/bacterial wilt|ralstonia|sudden wilt/.test(t))
        pushFeature(out, 'wilt_collapse', conf);
    if (/borer|leucinodes|shoot borer|fruit borer/.test(t))
        pushFeature(out, 'borer_hole', conf);
    if (/alternaria|leaf spot|concentric|target/.test(t))
        pushFeature(out, 'concentric_rings', conf);
    if (/yellow|chlorosis/.test(t))
        pushFeature(out, 'yellowing', conf * 0.85);
    return out;
}
function inferGenericFromText(t, conf) {
    return [
...
export const plantIdVisionFeaturesService = {
    ...
    inferFromObservationText(text, cropType) {
...
function parseStructuredJson(raw) {
    ...
    return obs
        ...
        if (!feature || !Number.isFinite(confidence))
            return null;
        if (!present(value) && confidence < 0.55)
            return null;
        return {
...
//# sourceMappingURL=plant-id-vision-features.service.js.map