function buildFarmerSummary(params) {
    ...
    if (params.explanation.diagnosis) {
        parts.push(`Most likely problem: ${params.explanation.diagnosis} (${Math.round(params.explanation.confidence * 100)}% confidence).`);
    }
...
function buildAgronomistSummary(params) {
    ...
    if (params.explanation.diagnosis) {
        lines.push(`Diagnosis: ${params.explanation.diagnosis} (${Math.round(params.explanation.confidence * 100)}%)`);
    }
    ...
    if (params.management) {
        ...
        if (params.management.chemical.length) {
            lines.push(`Chemical classes: ${params.management.chemical.map((c) => c.activeIngredientClass).join(', ')}`);
        }
    }
    if (params.safety) {
        lines.push(`Safety: ${params.safety.status}`);
        for (const r of params.safety.rejectReasons)
            lines.push(`  - ${r}`);
    }
    return lines.join('\n');
}
