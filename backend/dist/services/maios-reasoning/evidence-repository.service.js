...
function visionKey(label) {
    const t = label.toLowerCase();
    if (/blast|pyricularia|spindle/.test(t))
        return 'vision:blast';
    if (/rot|pythium|rhizome|bud rot|spear rot|crown rot/.test(t))
        return 'vision:rot';
    if (/thrip/.test(t))
        return 'vision:thrips';
    if (/sigatoka|leaf spot|mycosphaerella/.test(t))
        return 'vision:sigatoka';
    if (/panama|fusarium|wilt/.test(t))
        return 'vision:wilt';
    if (/weevil|borer|beetle/.test(t))
        return 'vision:borer';
    if (/alternaria|early blight|target spot/.test(t))
        return 'vision:blast';
    if (/late blight|phytophthora infestans/.test(t))
        return 'vision:rot';
    return null;
}
function visionFeatureToEvidence(obs) {
    ...
    if (!present && obs.confidence < 0.55)
        return null;
    const map = {
...
function answerIsNo(answer) {
    const a = answer.toLowerCase().trim();
    return a === 'no' || a === 'false';
}
export const maiosEvidenceRepositoryService = {
    merge(input) {
        const repo = [...input.contextItems];
        ...
        const capturedSlots = new Set(input.photos.filter((p) => p.status === 'captured').map((p) => p.slot));
    ...
    missingPhotoSlots(input) {
        const captured = new Set(input.photos.filter((p) => p.status === 'captured').map((p) => p.slot));
        return input.pack.photoSlots.map((s) => s.id).filter((id) => !captured.has(id));
    },
};
//# sourceMappingURL=evidence-repository.service.js.map