...
export function normalizeDiagnosisLabel(label) {
    return label
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
        .trim();
...
export function diagnosisLabelsMatch(a, b) {
...
function findCandidate(candidates, label) {
    return candidates.find((c) => diagnosisLabelsMatch(c.label, label));
}
function upsertCandidate(map, label, patch) {
...
function computeFusedScore(candidate, params) {
    const post = candidate.posterior ?? 0;
    const llm = candidate.llmProbability ?? 0;
    let score = post * 0.48 + llm * 0.47;
    if (candidate.llmPrimary && llm >= 0.6)
        score += 0.06;
    if (params.bayesianLocked && params.topPosteriorLabel) {
        if (diagnosisLabelsMatch(candidate.label, params.topPosteriorLabel))
            score += 0.1;
    }
    if (llm > post + 0.12 && llm >= 0.55)
        score += 0.05;
...
export function pickFusedPrimary(params) {
    ...
    if (locked && params.reasoning.decision.topLabel) {
        const lockedRow = findCandidate(sorted, params.reasoning.decision.topLabel) ?? top;
        return {
    ...
    if (llmTop?.label && (llmTop.probability ?? 0) >= 0.5) {
        ...
        const beatsIssue = topProb > llmConf + 0.05 ||
        ...
        if (llmTopRow && beatsIssue && !diagnosisLabelsMatch(llmTop.label, llmIssue ?? '')) {
            return {
                ...
                confidence: Math.max(topProb, llmTopRow.llmProbability ?? 0, llmTopRow.posterior ?? 0, llmTopRow.fusedScore),
            };
        }
    }
...
export function alternativesBelowPrimary(primaryLabel, items) {
    return items
        .filter((d) => d.label?.trim() && !diagnosisLabelsMatch(d.label, primaryLabel))
        .sort((a, b) => (b.probability ?? 0) - (a.probability ?? 0));
}
//# sourceMappingURL=diagnosis-fusion.service.js.map