...
function recoveryBlock(advisory) {
...
function weatherLines(ctx) {
    ...
    if (!pack)
        return {};
    const humidity = pack.avgHumidityPct != null ? `${Math.round(pack.avgHumidityPct)}%` : undefined;
    const temperature = pack.maxTempCToday != null ? `${Math.round(pack.maxTempCToday)}°C` : undefined;
    const rainfall = pack.rainMmToday != null ? `${pack.rainMmToday} mm (today)` : undefined;
    const weatherParts = [];
    if (pack.heavyRainLikely)
        weatherParts.push('Heavy rain likely');
    if (pack.highHumidityLikely)
        weatherParts.push('High humidity');
    if (pack.highHeatLikely)
        weatherParts.push('High heat');
    if (!weatherParts.length && pack.seasonPhase)
        weatherParts.push(`${pack.seasonPhase} season`);
    const soilMoisture = pack.drainageRisk === 'high'
...
function pickActivity(fromDb, advisoryLabel, advisoryDate, advisoryDays, emptyLabel) {
...
function buildFarmerReport(advisory, ctx) {
    ...
    const fertilizer = pickActivity(ctx.lastFertilizer, advisory.lastFertilizer, advisory.lastFertilizerDate, advisory.lastFertilizerDaysAgo, 'No recent fertilizer recorded');
    const foliar = pickActivity(ctx.lastFoliarSpray, advisory.lastFoliarSpray, advisory.lastFoliarSprayDate, advisory.lastFoliarSprayDaysAgo, 'No recent foliar spray recorded');
    const drench = pickActivity(ctx.lastDrench, advisory.lastDrench, advisory.lastDrenchDate, advisory.lastDrenchDaysAgo, 'No recent drench recorded');
    const monitor = advisory.monitorAdvice?.trim() ||
    ...
    sections.push('', '---', '', '📌 Why We Think This', '', ...whyWeThink(advisory), '', '---', '', ...buildTreatmentSection(advisory), '', '---', '', '🌱 Recovery', '', `${recovery.emoji} ${recovery.label}`, '', recovery.reason, '', '---', '', '⚠ Monitor', '', monitor, '', '---', '', '🚨 Precautions', '', ...(advisory.precautions?.length
        ? advisory.precautions.slice(0, 3).map((p) => `• ${p.replace(/^•\s*/, '')}`)
        : ['• Follow label instructions for any spray.', '• Avoid spraying during rain.']), '', '---', '', '👨‍🌾 Agronomist Review', '', 'This diagnosis is generated using your field records, weather, crop history and image analysis. Our agronomist team will review if required.');
    return sections.join('\n').replace(/\n{3,}/g, '\n\n').trim();
}
function buildTechnicalReport(advisory, reasoning) {
    const lines = ['🔬 Technical Details (Agronomist View)', ''];
    ...
    if (alternatives.length) {
        for (const alt of alternatives) {
            ...
            const qualifier = pct == null ? '' : pct >= 40 ? ' (Moderate possibility)' : pct >= 20 ? '' : ' (Low possibility)';
    ...
    if (contradicting.length) {
        for (const c of contradicting.slice(0, 6))
            lines.push(`• ${c}`);
    }
    else {
    ...
    lines.push(`Disease Pressure: ${diseaseWatch ? 'High' : advisory.confidence >= 0.7 ? 'Moderate' : 'Low'}`);
    const nutrientStress = advisory.nutrientDeficiency?.some((n) => n.likelihood !== 'low')
        ? 'Moderate'
        : 'Low';
    lines.push(`Nutrient Stress: ${nutrientStress}`);
    lines.push(`Yield Risk: ${advisory.severity === 'severe' ? 'High' : advisory.severity === 'moderate' ? 'Moderate' : 'Low'} (if untreated)`);
    lines.push('');
    ...
    if (reasoning?.explanation.missing?.length) {
        ...
        for (const q of reasoning.explanation.missing.slice(0, 4))
            lines.push(`• ${q}`);
    }
    return lines.join('\n').trim();
}
...
//# sourceMappingURL=crop-doctor-farmer-report.service.js.map