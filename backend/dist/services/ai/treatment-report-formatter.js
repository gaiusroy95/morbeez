export const CONNECTED_PREVENTION_NONE_MESSAGE = 'No connected preventive measures are currently recommended because no moderate or high secondary risks were identified.';
/** Farmer-facing "What To Do Now" block — primary treatment + mandatory connected prevention. */
export function buildTreatmentSection(advisory) {
    const lines = ['💊 What To Do Now', '', '🎯 Primary Treatment', ''];
    ...
    const prevention = (advisory.connectedPrevention ?? []).filter((item) => item.connectedRisk?.trim() &&
    ...
    if (prevention.length) {
        ...
        for (const item of prevention.slice(0, 4)) {
            lines.push(`${item.connectedRisk}\t${item.preventiveProduct}\t${item.dose}\t${item.method}\t${item.reason}`);
        }
    }
    else {
        const noneNote = advisory.connectedPreventionNoneNote?.trim() || CONNECTED_PREVENTION_NONE_MESSAGE;
        lines.push(`> ${noneNote}`);
    }
    if (prevention.length && advisory.tankMixRecommendation?.trim()) {
        ...
        lines.push('', '> ✅ Recommended Tank Mix: Combine compatible foliar products into a single spray to reduce labour and application cost.');
    }
...
export function formatVisitRecommendationText(advisory) {
...
//# sourceMappingURL=treatment-report-formatter.js.map