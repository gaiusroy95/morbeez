import { draftNeedsDilutionClarification, mergeExpertCaseDraft, } from '@morbeez/shared/expert-case';
...
const DILUTION_VOLUME_RE = /\b(\d+(?:\.\d+)?)\s*(?:-\s*)?(?:liter|litre|liters|litres|ltr|l)\b(?:\s*(?:of\s+)?water)?/i;
const DOSE_RE = /(\d+(?:\.\d+)?\s*(?:kg|g|gm|grams?|ml|l|litre|liter|ltr)s?)/i;
const INTERVAL_RE = /\bevery\s+(\d+\s+days?)\b/i;
export function parseDilutionVolumeL(text) {
...
export function detectTreatmentMethod(segment) {
...
export function splitCompositeTreatmentMessage(text) {
    const normalized = text.replace(/\s+/g, ' ').trim();
    if (!normalized)
        return [];
    const byAnd = normalized.split(/\s+and\s+(?=(?:spray|drench|foliar|soil|apply)\b)/i);
    if (byAnd.length > 1) {
...
function extractDoseFromSegment(segment) {
    const doses = [...segment.matchAll(new RegExp(DOSE_RE.source, 'gi'))].map((m) => m[1].trim());
    if (!doses.length)
        return null;
    return doses.join(' + ');
}
function extractProductFromSegment(segment) {
    let working = segment
...
export function parseTreatmentActivitiesFromMessage(message) {
    ...
    for (const segment of segments) {
        ...
        activities.push({
            ...
            dose,
            dilutionVolumeL,
            dilutionNotes: dilutionVolumeL != null ? `${dilutionVolumeL} L spray volume` : null,
...
export function supplementTreatmentDraft(draft, latestMessage) {
    ...
    if (activities.length > 0) {
        const foliar = activities.find((row) => /spray|foliar/i.test(String(row.method ?? ''))) ?? activities[0];
        const drench = activities.find((row) => /drench/i.test(String(row.method ?? '')));
        if (foliar?.product && !draft.treatmentProduct)
            patch.treatmentProduct = foliar.product;
        if (foliar?.dose && !draft.dosage)
            patch.dosage = foliar.dose;
        if (foliar?.method && !draft.applicationMethod)
            patch.applicationMethod = foliar.method;
    ...
    if (volume != null) {
        if (draft.sprayVolumeL == null)
            patch.sprayVolumeL = volume;
        if (!String(draft.dilutionNotes ?? '').trim()) {
            patch.dilutionNotes = `${volume} L spray volume`;
        }
        if (patch.treatmentActivities?.length) {
            patch.treatmentActivities = patch.treatmentActivities.map((row) => /spray|foliar/i.test(String(row.method ?? '')) && row.dilutionVolumeL == null
                ? {
                    ...row,
...
//# sourceMappingURL=expert-case-treatment-extraction.service.js.map