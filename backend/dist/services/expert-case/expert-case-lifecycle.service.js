...
import { buildExpertCaseKey, normalizeIssueFingerprint, priorityTierFromPriority, queueWeightForPriority, slaMinutesForPriority, } from '../../domain/expert-case/types.js';
...
export const expertCaseLifecycleService = {
    ...
    async ensureFromAdvisory(input) {
        if (!this.enabled())
            return null;
        try {
            const blockId = input.blockId ??
                (await blockService.getPrimaryBlock(input.farmerId).then((b) => b?.id ?? null));
            const fingerprint = normalizeIssueFingerprint(input.issueLabel ?? input.reason?.replace(/^Advisory review:\s*/i, '') ?? null);
            ...
            if (this.recurrenceEnabled()) {
                ...
                if (recentClosed) {
                    const previousGeneration = Number(String(recentClosed.case_key ?? '').match(/:g(\d+)$/)?.[1] ?? 1);
    ...
    async findOpenCase(params) {
        let q = supabase
            ...
            .limit(1);
        if (params.blockId)
            q = q.eq('block_id', params.blockId);
        else
    ...
    async findRecentClosedRecurrence(params) {
        const since = new Date(Date.now() - env.EXPERT_CASE_RECURRENCE_DAYS * 24 * 60 * 60 * 1000).toISOString();
        let q = supabase
            .from('expert_cases')
            .select('id, case_key')
            .eq('farmer_id', params.farmerId)
    ...
    async listOpen(params) {
        ...
        if (params?.ownerEmail)
            q = q.eq('owner_email', params.ownerEmail.toLowerCase());
...
//# sourceMappingURL=expert-case-lifecycle.service.js.map