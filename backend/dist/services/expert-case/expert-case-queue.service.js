...
export function scoreExpertCaseQueue(row) {
    const now = Date.now();
    let score = 0;
    if (row.priority_tier === 'emergency')
        score += 1000;
    else if (row.priority_tier === 'sla_risk')
        score += 500;
    else
        score += 100;
    if (row.sla_due_at) {
...
export const expertCaseQueueService = {
    ...
    async listBuckets(ownerEmail) {
        ...
        const my_work = rows.filter((r) => String(r.owner_email ?? '').toLowerCase() === email && r.assignment_status !== 'intervention_required');
        const intervention = rows.filter((r) => r.assignment_status === 'intervention_required');
        const at_risk = rows.filter((r) => !r.owner_email &&
            r.assignment_status === 'queued' &&
            r.sla_due_at &&
            String(r.sla_due_at) <= riskIso);
        const available = rows.filter((r) => !r.owner_email &&
    ...
    async matchExpertsForCase(caseId) {
        ...
        for (const cap of capacityRows ?? []) {
            if (Number(cap.active_weight) >= Number(cap.max_active_weight))
                continue;
            ...
            for (const req of requirements ?? []) {
                if (!req.required)
                    continue;
                requiredCount += 1;
                const hit = (specialties ?? []).find((s) => s.specialty_type === req.specialty_type &&
            ...
            if (caseRow.crop_type) {
                const cropHit = (specialties ?? []).find((s) => s.specialty_type === 'crop' &&
    ...
    async autoAssignBatch(limit = 10) {
        if (!this.enabled() || !env.ENABLE_EXPERT_COPILOT_AUTO_ASSIGN)
            return 0;
    ...
    async markAwaitingCapacity(caseId) {
        await supabase
            .from('expert_cases')
            .update({
...
//# sourceMappingURL=expert-case-queue.service.js.map