...
function mapActivityType(method) {
    const m = String(method ?? '').toLowerCase();
    if (/drench|soil/.test(m))
        return 'drench';
    if (/drip|fertigation|nutrition/.test(m))
        return 'fertigation';
    if (/spray|foliar/.test(m))
...
export const expertCaseCommitMapperService = {
    async persistStructuredOutputs(params) {
        ...
        const activities = (params.draft.treatmentActivities?.length ?? 0) > 0
        ...
        const recommendationText = params.draft.recommendationText ||
            activities
                .map((row) => [row.method, row.product, row.dose, row.dilutionNotes].filter(Boolean).join(' · '))
                .join(' | ');
        if (recommendationText) {
            try {
                const rec = await crmFarmerService.createRecommendation(params.farmerId, null, {
                    ...
                    followUpAt: params.draft.followUpDays != null
...
//# sourceMappingURL=expert-case-commit-mapper.service.js.map