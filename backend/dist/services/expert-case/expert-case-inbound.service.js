import { mergeExpertCaseDraft, emptyExpertCaseDraft, } from '@morbeez/shared/expert-case';
import { supabase } from '../../lib/supabase.js';
import { enrichDraftAfterExtraction, parseFarmerAnswerMessage, } from './expert-case-copilot-simulation.service.js';
...
export const expertCaseInboundService = {
    async tryHandleFarmerReply(params) {
        ...
        const { data: previewIntent } = await supabase
            ...
            .limit(1)
            .maybeSingle();
        if (previewIntent && !previewIntent.payload?.farmerConfirmed) {
            const caseId = String(previewIntent.case_id ?? previewIntent.aggregate_id);
            if (YES_RE.test(text)) {
                const payload = previewIntent.payload ?? {};
                await supabase
                    .from('communication_intents')
                    .update({
                ...
                await persistFarmerTurn({
                    caseId,
                    content: text,
                    draft: payload.draft ?? emptyExpertCaseDraft(),
                    assistantMessage: copilotMsg(payload.language ?? 'en', 'farmerConfirmThanks'),
            ...
            if (EDIT_RE.test(text)) {
                await persistFarmerTurn({
                    ...
                    content: text,
                    draft: previewIntent.payload.draft ?? emptyExpertCaseDraft(),
                    assistantMessage: 'Thanks — your agronomist will review your feedback and send an updated recommendation.',
                    baseRevision: Number(previewIntent.payload.revision ?? 0),
                });
                return true;
            }
        }
        ...
        const { data: pendingDraft } = await supabase
            ...
            .eq('status', 'pending')
            .maybeSingle();
        if (!pendingDraft?.draft_json)
            return false;
        const currentDraft = pendingDraft.draft_json;
        if (!currentDraft.farmerQuestionsSent)
            return false;
        ...
        if (!farmerAnswers && dilutionVolumeL == null)
            return false;
...
//# sourceMappingURL=expert-case-inbound.service.js.map