...
import { applyAnnotationIntent, applyLabelDoseIntent, applyOpenImagesIntent, applySendFarmerQuestionsIntent, detectCopilotIntent, enrichDraftAfterExtraction, loadExpertCaseBriefing, mergeExpertCaseDraft, parseFarmerAnswerMessage, } from './expert-case-copilot-simulation.service.js';
...
import { buildExpertCaseNavigation, formatCaseListMessage, } from './expert-case-navigation.service.js';
...
function validateDraftExtraction(value) {
    if (!value || typeof value !== 'object')
        return { ok: false, errors: ['not an object'] };
    const row = value;
    if (typeof row.assistantMessage !== 'string')
        return { ok: false, errors: ['assistantMessage'] };
...
async function nextTurnIndex(caseId) {
...
export const expertCaseChatService = {
    ...
    async assertOwner(params) {
        ...
        if (data.review_flag !== 'open')
            throw new ConflictError('Case is not open for chat');
    ...
    async postMessage(input) {
        if (!this.enabled())
            throw new UnauthorizedError('Expert Copilot chat is disabled');
        const owned = await this.assertOwner(input);
        const content = input.content.trim();
        if (!content)
            throw new ConflictError('Message is empty');
        ...
        let currentDraft = priorPending?.draft_json ??
            emptyExpertCaseDraft();
        const briefing = await loadExpertCaseBriefing({
            expertCase: owned,
            links: links.data ?? [],
        });
        const farmerLocale = normalizeCopilotLocale(farmerRow.data?.preferred_language ?? locale);
        ...
        if (farmerAnswers) {
            ...
            if (farmerAnswers.soilPh || farmerAnswers.soilEc) {
                currentDraft = mergeExpertCaseDraft(currentDraft, {
                    unresolvedFields: (currentDraft.unresolvedFields ?? []).filter((f) => !['soilPh', 'soilEc', 'soil'].includes(f)),
                });
            }
        }
        if (dilutionVolumeL != null) {
            const activities = (currentDraft.treatmentActivities ?? []).map((row) => /spray|foliar/i.test(String(row.method ?? '')) && row.dilutionVolumeL == null
                ? {
                    ...row,
                    dilutionVolumeL,
            ...
            currentDraft = mergeExpertCaseDraft(currentDraft, {
                ...
                unresolvedFields: (currentDraft.unresolvedFields ?? []).filter((field) => field !== 'dilutionVolume'),
            });
        }
        const intent = detectCopilotIntent(content, currentDraft);
        if (intent === 'nav_next_case' ||
        ...
        let clarification = clarificationCount < maxClarifications ? extraction.clarification : null;
        if (draft.validations?.dosage?.askLabelDose &&
        ...
        else if ((draft.farmerQuestions?.length ?? 0) > 0 &&
        ...
        try {
            const result = await openaiStrictJsonSchemaCompletion({
                ...
                userPrompt: JSON.stringify({
                    ...
                    clarificationAlreadyAsked: (params.clarificationCount ?? 0) >= (params.maxClarifications ?? 8),
                }),
    ...
    async approveDraft(params) {
        ...
        if (env.ENABLE_EXPERT_CASE_VERSION_LOCK &&
        ...
        if (!pending)
            throw new NotFoundError('No pending draft');
        if (Number(pending.base_revision) !== params.expectedBaseRevision) {
            throw new ConflictError('stale_base_revision');
        }
        const merged = mergeExpertCaseDraft(pending.draft_json, params.draftPatch ?? {});
...
//# sourceMappingURL=expert-case-chat.service.js.map