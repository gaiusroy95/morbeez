...
function communicationText(payload) {
    ...
    const precautions = Array.isArray(payload.precautions)
        ? payload.precautions.filter(Boolean).join('; ')
        : '';
    const cultural = Array.isArray(payload.culturalPractices)
        ? payload.culturalPractices.filter(Boolean).join('; ')
        : '';
...
export class ExpertCopilotWorker {
    timer = null;
    start() {
        if (this.timer || env.NODE_ENV === 'test')
            return;
    ...
    async reconcileCapacity() {
        ...
        for (const capacity of capacities ?? []) {
            ...
            if (count !== Number(capacity.active_case_count) ||
    ...
    async processCommunicationIntents(limit = 10) {
        if (!env.ENABLE_RECOMMENDATION_COMMUNICATION_OUTBOX)
            return 0;
        ...
        for (const intent of intents ?? []) {
            ...
            if (attemptNumber > COMMUNICATION_MAX_ATTEMPTS)
                continue;
            ...
            try {
                const recipient = intent.recipient_snapshot ?? {};
                let phone = recipient.phone?.trim() ?? '';
                let language = toAdvisoryLanguage(intent.payload?.language ?? recipient.language);
                ...
                const draftText = communicationText(intent.payload ?? {});
                const purpose = String(intent.purpose ?? 'recommendation');
                const baseLocalized = language === 'en'
                    ? draftText
                ...
                const localized = purpose === 'recommendation_preview'
...
export function startExpertCopilotWorker() {
    expertCopilotWorker.start();
}
//# sourceMappingURL=expert-copilot.worker.js.map