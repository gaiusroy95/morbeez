...
export function buildExpertCaseCommitRequestHash(input) {
    return requestHash({
        caseId: input.caseId,
...
export const expertCaseCommitService = {
    ...
    async commitCaseReview(input) {
        ...
        try {
            ...
            if (caseRow.review_flag === 'closed')
                throw new ConflictError('case_already_closed');
            ...
            const blockers = draftCommitBlockers(input.draft);
            if (blockers.length) {
            ...
            let communicationIntentId = null;
            const draftExtra = input.draft;
            const recommendationBody = input.draft.recommendationText ||
                String(draftExtra.treatmentProduct ?? '') ||
                (draftExtra.treatmentActivities?.length ?? 0) > 0;
...
//# sourceMappingURL=expert-case-commit.service.js.map