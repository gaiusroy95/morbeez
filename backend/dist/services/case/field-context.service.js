...
function measurementsToMap(rows) {
    const m = new Map();
    for (const r of rows) {
        const v = num(r.value);
        if (v != null)
            m.set(r.measurement_key, v);
    }
    return m;
}
function buildFieldMetrics(measurements) {
    ...
    if (spad == null && plantHeight == null && shoots == null)
        return undefined;
    return {
        spad,
...
function buildWaterReading(measurements, metadata) {
...
export const fieldContextService = {
    async loadLatestMeasurements(farmerId, blockId) {
        ...
        return {
            ...
            measurements: measurementsToMap((rows ?? [])),
        };
    },
    buildCanopyAudit(pack, measurements, dap) {
        const bedFloor = measurements.get('bed_floor_visibility') ?? null;
        const weed = measurements.get('weed_pressure') ?? null;
        const canopy = measurements.get('canopy_cover') ?? null;
        if (bedFloor == null && weed == null && canopy == null)
            return undefined;
        return canopyAuditService.build(pack, {
    ...
    async loadFieldContext(params) {
        const [{ findingId, measurements }, inputHistory] = await Promise.all([
            ...
            inputHistoryService.load21Day(params.farmerId, params.blockId),
        ]);
        const fieldMetrics = buildFieldMetrics(measurements);
        const waterReading = buildWaterReading(measurements, params.metadata);
        const canopyAudit = this.buildCanopyAudit(params.pack, measurements, params.dap);
        return {
            findingId,
            fieldMetrics,
            canopyAudit,
...
//# sourceMappingURL=field-context.service.js.map