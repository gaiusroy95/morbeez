...
export async function resolveRecoveryCheckInCondition(params) {
    ...
    if (params.recommendationRecordId) {
        ...
        if (data?.issue_detected)
            return String(data.issue_detected).trim().slice(0, 120);
    }
    if (!params.sessionId)
        return null;
    ...
    if (secondary)
        return secondary.slice(0, 120);
    const ginger = meta.gingerSopCase;
    const gingerIssue = ginger?.issueLabel?.trim() || ginger?.primaryDiagnosis?.trim();
    if (gingerIssue)
        return gingerIssue.slice(0, 120);
    const { data: output } = await supabase
        ...
        .limit(1)
        .maybeSingle();
    if (output?.probable_issue)
        return String(output.probable_issue).trim().slice(0, 120);
    const { data: rec } = await supabase
        ...
        .limit(1)
        .maybeSingle();
    if (rec?.issue_detected)
        return String(rec.issue_detected).trim().slice(0, 120);
    return null;
}
...
//# sourceMappingURL=recovery-checkin-copy.js.map