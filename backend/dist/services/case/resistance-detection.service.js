...
export const resistanceDetectionService = {
    async score(params) {
        ...
        if (qoiSprays.length >= 2)
            warnings.push('qoi_rotation_risk');
        ...
        for (const spray of sprays) {
            for (const product of spray.products) {
                const match = (groups ?? []).find((g) => product.toLowerCase().includes(String(g.technical_name).toLowerCase().slice(0, 6)) ||
                    String(g.technical_name).toLowerCase().includes(product.toLowerCase().slice(0, 6)));
                if (match?.mode_of_action)
                    classes.add(String(match.mode_of_action));
            }
        }
        let score = 15;
...
//# sourceMappingURL=resistance-detection.service.js.map