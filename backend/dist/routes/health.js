...
export async function healthRoutes(app) {
    app.get('/health', async () => {
        ...
        if (env.SHOPIFY_STORE_DOMAIN && env.SHOPIFY_ADMIN_API_ACCESS_TOKEN) {
            try {
                const res = await fetch(`https://${env.SHOPIFY_STORE_DOMAIN}/admin/api/${env.SHOPIFY_API_VERSION}/shop.json`, { headers: { 'X-Shopify-Access-Token': env.SHOPIFY_ADMIN_API_ACCESS_TOKEN } });
                shopifyOk = res.ok;
            }
            catch {
                shopifyOk = false;
