...
export async function farmerPortalRoutes(app) {
    ...
    app.get('/api/v1/farmer/portal/roi/season/:seasonId/entries', async (request, reply) => {
        ...
        const q = request.query;
        const { cropSeasonService } = await import('../../services/farmer/crop-season.service.js');
        const result = await cropSeasonService.listSeasonEntries(farmerId, seasonId, q.page ? Number(q.page) : 1, q.limit ? Number(q.limit) : 30);
        return reply.send({ ok: true, ...result });
    });
}
//# sourceMappingURL=farmer-portal.routes.js.map