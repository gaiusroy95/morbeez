...
export async function osAiCallingRoutes(app) {
    ...
    app.put(`${api}/ai-calling/identities/:slot`, async (request, reply) => {
        ...
        await logAdminMutation({
            ...
            resourceId: identity ? String(identity.id) : String(slot),
            details: { slot, ...body },
        });
    ...
    app.post(`${api}/ai-calling/jobs/:id/click-to-call`, async (request, reply) => {
        ...
        const { data: farmer } = await supabase
            ...
            .maybeSingle();
        if (!farmer?.phone)
            return reply.code(400).send({ ok: false, error: 'Farmer phone missing' });
...
//# sourceMappingURL=os-ai-calling.routes.js.map