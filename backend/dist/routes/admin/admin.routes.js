...
export async function adminRoutes(app) {
    ...
    app.get(`${api}/logistics/pending`, async (request, reply) => {
        requireAdmin(request);
        const q = request.query;
        const result = await shiprocketAdminService.listPending(q.limit ? Number(q.limit) : 25);
        return reply.send({ ok: true, ...result });
    });
    app.get(`${api}/logistics/events`, async (request, reply) => {
        requireAdmin(request);
        const q = request.query;
        const result = await shiprocketAdminService.listRecentEvents(q.limit ? Number(q.limit) : 30);
        return reply.send({ ok: true, ...result });
    });
    app.post(`${api}/logistics/shipments/:shopifyOrderId/retry`, async (request, reply) => {
        ...
        const { shopifyOrderId } = request.params;
    ...
    app.get(`${api}/banners`, async (request, reply) => {
        ...
        const q = request.query;
        const tab = q.tab === 'active' || q.tab === 'upcoming' || q.tab === 'expired' ? q.tab : 'all';
        const placement = q.placement === 'home_hero' ||
    ...
    app.get(`${api}/crop-advisor/leads/:id/field-findings`, async (request, reply) => {
        ...
        const q = request.query;
        const detail = await cropAdvisorAdminService.getLeadDetail(id);
        const result = await cropAdvisorAdminService.listFieldFindings(detail.lead.farmerId, q.page ? Number(q.page) : 1, q.limit ? Number(q.limit) : 10);
        return reply.send({ ok: true, ...result });
    });
