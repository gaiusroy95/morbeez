...
import { executiveCockpitService, weaknessDashboardService, resistanceDashboardService, } from '../../services/intelligence/enterprise-dashboard.service.js';
import { retrainingOpsService, protocolDefinitionService, applicationHistoryService, experimentDefinitionService, } from '../../services/intelligence/enterprise-ops.service.js';
...
export async function osEnterpriseRoutes(app) {
    ...
    app.get(`${api}/product-gaps/:technicalName/inventory-eta`, async (request, reply) => {
        await assertModuleAccess(request, 'intelligence', 'read');
        const { technicalName } = request.params;
    ...
    app.get(`${api}/product-gaps/:technicalName/alternatives`, async (request, reply) => {
        ...
        const { technicalName } = request.params;
        const q = z.object({ crop: z.string().optional() }).parse(request.query ?? {});
        const alternatives = await productGapService.listAlternatives(decodeURIComponent(technicalName), q.crop);
    ...
    app.get(`${api}/experiments/:id`, async (request, reply) => {
        ...
        const variants = await outcomeIntelligenceService.compareVariantsByExperiment(String(experiment.experiment_key)).catch(() => []);
...
//# sourceMappingURL=os-enterprise.routes.js.map