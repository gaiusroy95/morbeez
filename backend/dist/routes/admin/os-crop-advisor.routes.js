...
import { cropAdvisorIntelligenceService } from '../../services/intelligence/crop-advisor-intelligence.service.js';
...
import { cropAdvisorFarmerProfileService } from '../../services/admin/crop-advisor-farmer-profile.service.js';
import { pincodeService } from '../../services/core/pincode.service.js';
import { cropAdvisorLeadQueueService, } from '../../services/admin/crop-advisor-lead-queue.service.js';
...
export async function osCropAdvisorRoutes(app) {
    const api = '/morbeez-staff/api/v1/os/crop-advisor';
    ...
    app.get(`${api}/workspace-intelligence`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const intelligence = await cropAdvisorIntelligenceService.getWorkspaceIntelligence(admin.email);
    ...
    app.get(`${api}/leads/operational`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const q = request.query;
        const result = await cropAdvisorLeadQueueService.listOperationalLeads({
            ...
            opportunityLevel: q.opportunityLevel,
    ...
    app.get(`${api}/leads/queue-summary`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const q = request.query;
        const summary = await cropAdvisorLeadQueueService.getQueueSummary(admin.email, q.scope === 'all' ? 'all' : 'mine');
    ...
    app.get(`${api}/leads/export`, async (request, reply) => {
        ...
        const { csv } = await cropAdvisorLeadQueueService.exportLeads({
            scope: q.scope === 'all' ? 'all' : 'mine',
            stage: q.stage,
            search: q.search,
            district: q.district,
        ...
        reply.header('Content-Disposition', `attachment; filename="leads-export-${new Date().toISOString().slice(0, 10)}.csv"`);
        return reply.send(csv);
    });
    app.post(`${api}/leads/bulk`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'write');
        const body = z
            .object({
            ...
            .parse(request.body);
        const result = await cropAdvisorLeadQueueService.bulkUpdateLeads(body.leadIds, body.action, { owner: body.owner, stage: body.stage, broadcastTag: body.broadcastTag }, admin.email);
        return reply.send({ ok: true, ...result });
    });
    app.get(`${api}/team`, async (_request, reply) => {
        await assertModuleAccess(_request, 'crop_advisor_crm', 'read');
        const team = await cropAdvisorLeadQueueService.listAssignableTeam();
        return reply.send({ ok: true, team });
    });
    app.get(`${api}/table-preferences`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const q = request.query;
        const tableName = z.string().min(1).parse(q.table ?? 'crop_advisor_leads');
    ...
    app.put(`${api}/table-preferences`, async (request, reply) => {
        ...
        const body = z
            .object({
            tableName: z.string().min(1).default('crop_advisor_leads'),
    ...
    app.get(`${api}/leads/:id/farmer-profile`, async (request, reply) => {
        ...
        const { id } = request.params;
        const detail = await cropAdvisorAdminService.getLeadDetail(id);
        const profile = await cropAdvisorFarmerProfileService.getProfile(detail.lead.farmerId);
    ...
    app.patch(`${api}/leads/:id/farmer-profile`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const profile = await cropAdvisorFarmerProfileService.updateProfile(detail.lead.farmerId, body);
        return reply.send({ ok: true, ...profile });
    });
    ...
    app.get(`${api}/leads/:id/agronomist`, async (request, reply) => {
        ...
        const { id } = request.params;
        const detail = await cropAdvisorAdminService.getLeadDetail(id);
        const { cropAdvisorFarmerAgronomistService } = await import('../../services/admin/crop-advisor-farmer-agronomist.service.js');
        const panel = await cropAdvisorFarmerAgronomistService.getPanel(String(detail.lead.farmerId));
    ...
    app.get(`${api}/leads/:id/field-findings`, async (request, reply) => {
        ...
        const q = request.query;
        const detail = await cropAdvisorAdminService.getLeadDetail(id);
        const result = await cropAdvisorAdminService.listFieldFindings(detail.lead.farmerId, q.page ? Number(q.page) : 1, q.limit ? Number(q.limit) : 100);
        return reply.send({ ok: true, ...result });
    });
    app.get(`${api}/leads/:leadId/field-findings/:findingId`, async (request, reply) => {
        ...
        const detail = await cropAdvisorAdminService.getLeadDetail(leadId);
        const finding = await cropAdvisorAdminService.getFieldFinding(String(detail.lead.farmerId), findingId);
    ...
    app.patch(`${api}/field-findings/:id`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const finding = await cropAdvisorAdminService.updateFieldFinding(id, {
    ...
    app.get(`${api}/leads/:id/tasks`, async (request, reply) => {
        await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const { id } = request.params;
        const tasks = await cropAdvisorAdminService.listLeadPendingTasks(id);
        return reply.send({ ok: true, tasks });
    });
    app.get(`${api}/leads/:id/agronomist-tasks`, async (request, reply) => {
        await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const { id } = request.params;
        const tasks = await cropAdvisorAdminService.listLeadAgronomistTasks(id);
        return reply.send({ ok: true, tasks });
    });
    app.get(`${api}/agronomists`, async (request, reply) => {
        await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const agronomists = await cropAdvisorAdminService.listAssignableAgronomists();
    ...
    app.post(`${api}/tasks/:id/comments`, async (request, reply) => {
        ...
        const body = z
            ...
            authorRole: z.enum(['crop_advisor', 'agronomist']).optional(),
        })
            .parse(request.body);
        const role = body.authorRole ?? (admin.role === 'agronomist' ? 'agronomist' : 'crop_advisor');
    ...
    app.get(`${api}/leads/:id/notes`, async (request, reply) => {
        await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const { id } = request.params;
        const result = await cropAdvisorAdminService.listLeadNotes(id);
        return reply.send({ ok: true, ...result });
    });
    app.get(`${api}/leads/:leadId/notes/:noteId`, async (request, reply) => {
        await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const { leadId, noteId } = request.params;
        const note = await cropAdvisorAdminService.getLeadNote(leadId, noteId);
    ...
    app.patch(`${api}/leads/:leadId/notes/:noteId`, async (request, reply) => {
        ...
        const updated = await cropAdvisorAdminService.updateLeadNote(leadId, noteId, note, admin.email);
    ...
    app.patch(`${api}/tasks/:id`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const task = await cropAdvisorAdminService.updateTask(id, body);
    ...
    app.post(`${api}/leads/:id/calls/upload`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const { callIntelligenceService } = await import('../../services/call-intelligence/call-intelligence.service.js');
    ...
    app.get(`${api}/mobile/dashboard`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const { cropAdvisorMobileService } = await import('../../services/call-intelligence/crop-advisor-mobile.service.js');
        const dashboard = await cropAdvisorMobileService.getDashboard(admin.email);
    ...
    app.get(`${api}/mobile/leads`, async (request, reply) => {
        ...
        const q = request.query;
        const { cropAdvisorMobileService } = await import('../../services/call-intelligence/crop-advisor-mobile.service.js');
        const leads = await cropAdvisorMobileService.listLeads(admin.email, {
            scope: q.scope === 'all' ? 'all' : 'mine',
    ...
    app.get(`${api}/mobile/follow-ups`, async (request, reply) => {
        ...
        if (q.grouped === 'true' || q.grouped === '1') {
            const sections = await cropAdvisorMobileService.listFollowUpSections(admin.email);
            return reply.send({ ok: true, sections });
        }
        const tasks = await cropAdvisorMobileService.listFollowUps(admin.email, q.status ?? 'pending');
        return reply.send({ ok: true, tasks });
    });
    app.get(`${api}/mobile/leads/operational`, async (request, reply) => {
        ...
        const q = request.query;
        const { cropAdvisorMobileService } = await import('../../services/call-intelligence/crop-advisor-mobile.service.js');
        const leads = await cropAdvisorMobileService.listOperationalLeads(admin.email, {
            scope: q.scope === 'all' ? 'all' : 'mine',
            search: q.search,
            smartFilter: q.smartFilter,
            sort: q.sort,
    ...
    app.get(`${api}/mobile/leads/:id/workspace-summary`, async (request, reply) => {
        ...
        const { id } = request.params;
        const { cropAdvisorMobileService } = await import('../../services/call-intelligence/crop-advisor-mobile.service.js');
        const summary = await cropAdvisorMobileService.getWorkspaceSummary(id);
        return reply.send({ ok: true, summary });
    });
    app.get(`${api}/mobile/notifications`, async (request, reply) => {
        const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
        const { cropAdvisorMobileService } = await import('../../services/call-intelligence/crop-advisor-mobile.service.js');
        const notifications = await cropAdvisorMobileService.listNotifications(admin.email);
    ...
    app.get(`${api}/mobile/sales-opportunities`, async (request, reply) => {
        ...
        const opportunities = await salesOpportunityService.listForCropAdvisor(admin.email);
    ...
    app.post(`${api}/leads/:id/interactions`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const detail = await cropAdvisorAdminService.getLeadDetail(id);
        const interaction = await crmFarmerService.createInteraction(detail.lead.farmerId, id, { ...body, doneBy: admin.email, doneByRole: 'Crop Advisor' });
    ...
    app.post(`${api}/leads/:id/field-findings`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const detail = await cropAdvisorAdminService.getLeadDetail(id);
        const finding = await cropAdvisorAdminService.createFieldFinding(detail.lead.farmerId, id, body);
    ...
    app.post(`${api}/leads/:id/estimates`, async (request, reply) => {
        ...
        if (body.send) {
            ...
            sendResult = await commerceQuoteService.sendQuote(quote.id, id, [...channels], admin.email);
        }
    ...
    app.put(`${api}/leads/:leadId/estimates/:estimateId`, async (request, reply) => {
        ...
        if (body.send) {
            const channels = body.sendChannels?.length ? body.sendChannels : ['whatsapp'];
            sendResult = await commerceQuoteService.sendQuote(quote.id, leadId, [...channels], admin.email);
        }
    ...
    app.patch(`${api}/masters/:id`, async (request, reply) => {
        const actor = await assertModuleAccess(request, 'crop_advisor_crm', 'write');
        const { id } = request.params;
        const body = z
            .object({
    ...
    app.patch(`${api}/escalations/:id`, async (request, reply) => {
        ...
        const body = z
            ...
            commentRole: z.enum(['crop_advisor', 'agronomist']).optional(),
...
//# sourceMappingURL=os-crop-advisor.routes.js.map