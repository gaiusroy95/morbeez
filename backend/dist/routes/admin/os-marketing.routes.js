...
import { dateRangeFromDays, marketingPerformanceService, } from '../../services/admin/marketing-performance.service.js';
...
const leadChannelEnum = z.enum(LEAD_CHANNELS);
function parseQuery(q) {
    const days = q.days ? Number(q.days) : 30;
    const range = q.from && q.to ? { from: q.from, to: q.to } : dateRangeFromDays(Math.min(Math.max(days, 7), 90));
    return {
        ...range,
        ownerId: q.ownerId,
        campaign: q.campaign,
...
export async function osMarketingRoutes(app) {
...
//# sourceMappingURL=os-marketing.routes.js.map