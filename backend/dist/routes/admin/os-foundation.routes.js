...
import { assertModuleAccess, canApproveRecommendations, getModulesForRole, } from '../../lib/rbac.js';
...
async function assertApprovalsRead(request) {
    try {
...
async function assertApprovalsWrite(request) {
    try {
...
export async function osFoundationRoutes(app) {
    ...
    app.get(`${api}/session`, async (request, reply) => {
        ...
        const canSelfApproveRecommendations = await agronomistTierService.canSelfApproveRecommendations(admin.id, admin.email, admin.role);
        return reply.send({
            ok: true,
    ...
    app.get(`${api}/blocks/:blockId`, async (request, reply) => {
        ...
        if (!block)
            return reply.code(404).send({ ok: false, message: 'Block not found' });
    ...
    app.get(`${api}/recommendations/approvals`, async (request, reply) => {
        ...
        const q = request.query;
        const mineOnly = q.mine === 'true' || q.mine === '1' || !canApproveRecommendations(admin.role);
    ...
    app.post(`${api}/recommendations/:id/approve`, async (request, reply) => {
        ...
        if (!isSuper && !canSelf) {
            throw new UnauthorizedError('Only super admin or an experienced agronomist can approve recommendations');
        }
        const { id } = request.params;
        if (!isSuper) {
