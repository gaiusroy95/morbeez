...
const draftSchema = z
    ...
    .passthrough();
export async function osExpertCasesRoutes(app) {
    ...
    app.post(`${api}/backfill`, async (request, reply) => {
        ...
        const body = z.object({ limit: z.number().int().min(1).max(1000).default(200) }).parse(request.body ?? {});
    ...
    app.post(`${api}/reconcile`, async (request, reply) => {
        await assertModuleAccess(request, 'agronomist', 'write');
        const body = z.object({ limit: z.number().int().min(1).max(2000).default(500) }).parse(request.body ?? {});
    ...
    app.get(`${api}/governance/audit/verify`, async (request, reply) => {
        ...
        const query = z.object({ limit: z.coerce.number().int().min(1).max(5000).default(500) }).parse(request.query);
    ...
    app.get(`${api}/:id`, async (request, reply) => {
        ...
        if (!expertCase)
            return reply.status(404).send({ ok: false, error: 'Expert case not found' });
...
//# sourceMappingURL=os-expert-cases.routes.js.map