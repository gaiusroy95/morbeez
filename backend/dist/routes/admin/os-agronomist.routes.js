...
export async function osAgronomistRoutes(app) {
    ...
    app.get(`${api}/operations/tasks`, async (request, reply) => {
        ...
        const tasks = await cropAdvisorAdminService.listTasksForAgronomist(admin.email, q.status === 'all' ? undefined : { status: q.status ?? 'pending' });
    ...
    app.post(`${api}/routes/:id/stops`, async (request, reply) => {
        ...
        const body = z
            ...
            .parse(request.body);
        const stop = await routePlannerService.addStop({ agentType: 'agronomist', email: admin.email }, id, body.farmerId, body.blockId);
    ...
    app.post(`${api}/routes/:id/optimize`, async (request, reply) => {
        ...
        const route = await routePlannerService.optimizeRoute({ agentType: 'agronomist', email: admin.email }, id, body.lat, body.lng);
