...
function verifyShopifyOAuthQuery(query, secret) {
...
function installSuccessHtml(shop, note) {
 ...
 */
export async function shopifyOAuthRoutes(app) {
    ...
    app.get('/auth/shopify/installed', async (request, reply) => {
        ...
        return reply.type('text/html').send(installSuccessHtml(shop, 'Installation complete. App proxy paths like <code>/apps/morbeez/health</code> should now work on your store.'));
    });
    ...
    app.get('/auth/shopify/callback', async (request, reply) => {
        ...
        if (!clientId) {
            return reply
                .code(500)
                .type('text/html')
                .send('<h1>Server misconfigured</h1><p>Set SHOPIFY_APP_CLIENT_ID on Render (Partner app Client ID).</p>');
        }
        try {
            ...
            if (!tokenRes.ok || !tokenBody.access_token) {
                ...
                return reply
                    .code(502)
                    .type('text/html')
                    .send(`<h1>Could not complete install</h1><p>${tokenBody.error ?? 'Token exchange failed'}. Check redirect URL and app credentials on Render.</p>`);
            }
    ...
    app.get('/auth/shopify/install', async (request, reply) => {
        const shop = request.query.shop ?? env.SHOPIFY_STORE_DOMAIN;
        if (!clientId) {
            return reply
                .code(500)
                .type('text/html')
                .send('<h1>Missing SHOPIFY_APP_CLIENT_ID</h1><p>Add Partner app Client ID to Render env and redeploy.</p>');
        }
...
//# sourceMappingURL=shopify-oauth.routes.js.map