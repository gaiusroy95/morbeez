...
type PublishableUnpublishResult = {
...
async function resolveOnlineStorePublicationId(): Promise<string | null> {
  if (cachedOnlineStorePublicationId) return cachedOnlineStorePublicationId;

  try {
    const data = await shopifyGraphql<PublicationsQuery>(`{
    ...
    const match =
      data.publications.edges.find((e) =>
        String(e.node.name ?? '')
          .toLowerCase()
          .includes('online store')
      ) ?? data.publications.edges[0];
...
async function publishViaRest(productId: string): Promise<void> {
...
async function unpublishViaRest(productId: string): Promise<void> {
  await shopifyAdmin(`/products/${productId}.json`, {
    method: 'PUT',
    body: JSON.stringify({
      product: {
...
export const shopifyPublicationsService = {
  async publishToOnlineStore(productId: string): Promise<void> {
    ...
    try {
      const data = await shopifyGraphql<PublishablePublishResult>(
  ...
  async unpublishFromOnlineStore(productId: string): Promise<void> {
    const publicationId = await resolveOnlineStorePublicationId();
    if (!publicationId) {
    ...
    try {
      const data = await shopifyGraphql<PublishableUnpublishResult>(
  ...
  async syncProductVisibility(productId: string, status: 'active' | 'draft' | 'archived'): Promise<void> {
    if (status === 'active') {
  ...
  async getConnectionStatus(): Promise<{
    ...
    const storefrontUrl = (
      env.SHOPIFY_STOREFRONT_URL ?? `https://${storeDomain}`
    ).replace(/\/$/, '');
    ...
    try {
      const res = await shopifyAdmin<{ count: number }>('/products/count.json');
