...
export const checkoutOmsBridgeService = {
  ...
  async syncToWarehouse(input: { shopifyOrderId: string; razorpayPaymentId?: string }) {
    let order: Awaited<ReturnType<typeof getOrder>>['order'];
    try {
      ({ order } = await getOrder(input.shopifyOrderId));
    } catch (err) {
      logger.warn(
        { err, shopifyOrderId: input.shopifyOrderId },
        'Shopify order not found for checkout warehouse sync'
      );
    ...
    if (input.razorpayPaymentId) patch.razorpay_payment_id = input.razorpayPaymentId;
  ...
  async repairUnsyncedPaidCheckouts(limit = 30) {
    ...
    if (!shopifyIds.length) return { repaired: 0, failed: 0, scanned: 0 };
    ...
    const commerceByShopify = new Map(
      (commerceRows ?? []).map((row) => [String(row.shopify_order_id), row])
    );
    ...
    const needsSync = (sessions ?? []).filter((session) => {
      ...
      if (!commerce || commerce.deleted_at) return true;
    ...
    for (const session of needsSync.slice(0, limit)) {
      try {
        ...
        if (result.commerceOrderId) repaired += 1;
        else failed += 1;
      } catch (err) {
        failed += 1;
        logger.warn(
          { err, checkoutSessionId: session.id, shopifyOrderId: session.shopify_order_id },
          'Paid checkout warehouse repair failed'
        );
      }
    }
