...
export const checkoutPricingService = {
  async validateLineItems(lineItems: CheckoutLineInput[]): Promise<ValidatedCheckoutLine[]> {
    if (!lineItems.length) throw new ValidationError('Cart is empty');
    ...
    for (const li of lineItems) {
      ...
      const res = await shopifyAdmin<{ variant: ShopifyVariantRow }>(
        `/variants/${li.variantId}.json`
      );
