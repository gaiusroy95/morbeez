...
function paiseTotal(lineItems: Array<{ price: number; quantity: number }>): number {
...
export const checkoutService = {
  ...
  async createCodCheckout(
    input: CreateCheckoutInput,
    channel: 'website' | 'mobile' = 'mobile',
    farmerId?: string
  ) {
    const validatedLines = await checkoutPricingService.validateLineItems(input.lineItems);
    ...
    try {
      ...
      logger.error(
        { err, shopifyOrderId: shopifyOrder.shopifyOrderId },
        'Warehouse sync after COD checkout failed'
      );
    }
