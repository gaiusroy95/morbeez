...
export function classifyOrderType(orderTotalInr: number, config: PricingEngineConfig): RetailOrBulk {
  return orderTotalInr > config.bulkOrderThresholdInr ? 'bulk' : 'retail';
}

/** Monthly target achievement → base incentive % */
export function baseIncentivePct(achievementPct: number, config: PricingEngineConfig): number {
  if (achievementPct >= 100) return config.retailBaseIncentive100Plus;
  if (achievementPct >= 80) return config.retailBaseIncentive80_100;
  if (achievementPct >= 50) return config.retailBaseIncentive50_80;
  return config.retailBaseIncentive0_50;
}

/** Avg realization % → multiplier */
export function realizationMultiplier(avgRealizationPct: number, config: PricingEngineConfig): number {
  if (avgRealizationPct >= 95) return config.realizationMult95Plus;
  if (avgRealizationPct >= 90) return config.realizationMult90_95;
  if (avgRealizationPct >= 85) return config.realizationMult85_90;
...
export function gradeFromScore(score: number): 'A+' | 'A' | 'B' | 'C' | 'Risk' {
...
export function computeKpiScore(input: {
  ...
  const totalScore =
    Math.round(
      (scoreSales + scoreRealization + scoreProfit + scoreRepeat + scoreCollection + scoreReturns) * 100
    ) / 100;
  return {
...
export function quarterlyBonusAmount(
  avgMonthlyScore: number,
  avgRealizationPct: number,
  config: PricingEngineConfig
): { grade: 'A+' | 'A' | 'B' | 'C' | 'Risk'; amount: number; eligible: boolean; note: string | null } {
  ...
  if (grade === 'A') return { grade: 'A', amount: config.quarterlyBonusA, eligible: true, note: null };
  if (grade === 'B') return { grade: 'B', amount: 0, eligible: false, note: 'No quarterly bonus for grade B' };
  if (grade === 'C') return { grade: 'C', amount: 0, eligible: false, note: 'Warning — grade C' };
