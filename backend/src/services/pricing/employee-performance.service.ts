...
function statusFromRealization(
  avgPct: number,
  netProfit: number,
  config: Awaited<ReturnType<typeof pricingConfigService.getConfig>>
): EmployeePerformanceRow['status'] {
  if (avgPct >= config.realizationExcellent && netProfit > 0) return 'excellent';
  if (avgPct >= config.realizationGood && netProfit > 0) return 'good';
  if (avgPct >= config.realizationWarning) return 'warning';
  return 'critical';
}

function nextActionStage(current: number, status: EmployeePerformanceRow['status']): number {
  if (status === 'excellent' || status === 'good') return 0;
  if (status === 'warning') return Math.min(2, current + 1);
  return Math.min(5, current + 1);
}

export const employeePerformanceService = {
  async recomputeDailySnapshot(employeeProfileId: string, date: string) {
    ...
    const comp = (profile as { employee_compensation?: { pricing_access_restricted?: boolean; performance_action_stage?: number }[] | { pricing_access_restricted?: boolean; performance_action_stage?: number } | null })?.employee_compensation;
    ...
    if (compRow?.pricing_access_restricted) status = 'restricted';
  ...
  async getDashboard(opts?: { period?: 'daily' | 'weekly'; date?: string }) {
    ...
    const rows: EmployeePerformanceRow[] = (snapshots ?? []).map((s) => {
      const prof = s.employee_profiles as { full_name?: string; employee_code?: string; admin_user_id?: string } | null;
      return {
        ...
        status: String(s.performance_status) as EmployeePerformanceRow['status'],
  ...
  async getMyPerformance(adminUserId: string) {
    const { data: profile } = await supabase
      .from('employee_profiles')
