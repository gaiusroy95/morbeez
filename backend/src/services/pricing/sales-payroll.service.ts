...
export const salesPayrollService = {
  async getMonthlyTotals(employeeProfileId: string, year: number, month: number): Promise<MonthlyPayrollSales> {
    ...
    if (month % 3 === 0) {
      const { data: qb } = await supabase
        ...
        .maybeSingle();
      if (qb?.bonus_eligible) quarterlyBonusInr = Number(qb.bonus_amount) || 0;
    }
