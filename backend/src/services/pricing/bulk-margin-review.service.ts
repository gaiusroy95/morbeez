...
export const bulkMarginReviewService = {
  async isApprovedForQuote(quoteId: string): Promise<boolean> {
  ...
  async approve(reviewId: string, reviewerId: string, notes?: string) {
    const { data, error } = await supabase
      .from('bulk_margin_review_requests')
      .update({
        status: 'approved',
  ...
  async reject(reviewId: string, reviewerId: string, notes?: string) {
    const { data, error } = await supabase
      .from('bulk_margin_review_requests')
      .update({
        status: 'rejected',
