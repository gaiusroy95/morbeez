...
function mapRow(row: Record<string, unknown>): PricingEngineConfig {
  return {
    ...
    hardFloorMarginPctOfGross:
      Number(row.hard_floor_margin_pct_of_gross) || DEFAULTS.hardFloorMarginPctOfGross,
...
export const pricingConfigService = {
  async getConfig(): Promise<PricingEngineConfig> {
    ...
    return data ? mapRow(data as Record<string, unknown>) : DEFAULTS;
  },

  async updateConfig(patch: Partial<PricingEngineConfig>) {
