...
const WEATHER_FILLER_CONTRIB_RE =
...
function contributingFactor(advisory: StructuredAdvisory): string | null {
  ...
  if (fromRanked?.label) {
    if (WEATHER_FILLER_CONTRIB_RE.test(fromRanked.label)) return null;
  ...
  if (raw && WEATHER_FILLER_CONTRIB_RE.test(raw)) return null;
  return raw;
}
...
function buildAskAgainReport(advisory: StructuredAdvisory, ctx: CropDoctorReportContext): string {
