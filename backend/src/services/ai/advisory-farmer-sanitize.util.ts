...
} from '../whatsapp/pipeline/crop-photo-evidence.util.js';

/** Weather/humidity fillers that must not appear as Contributing Factor without lesion evidence. */
const WEATHER_ONLY_DISEASE_RE =
  /\banthracnose\b|\bfungal\s+leaf\s+spot\b|\bcolletotrichum\b/i;

const LESION_EVIDENCE_RE =
  /\b(lesion|spot|spots|blight|anthracnose|necrotic|halo|margin|spore|fungal\s+leaf)\b/i;

function dedupeObservations(list: string[]): string[] {
...
function observationsSupportFungalLesions(observations: string[] | undefined): boolean {
...
function stripWeatherOnlyContributing(advisory: StructuredAdvisory): StructuredAdvisory {
  const next = { ...advisory };
  ...
  const shouldDropLabel = (label: string | undefined | null): boolean => {
    if (!label?.trim()) return false;
    if (!WEATHER_ONLY_DISEASE_RE.test(label)) return false;
  ...
  if (next.diagnosisRanked?.length) {
    next.diagnosisRanked = next.diagnosisRanked.filter((row) => {
      if (row.role !== 'contributing') return true;
      return !shouldDropLabel(row.label);
    });
  }
  if (!usable && next.differentialDiagnosis?.length) {
    next.differentialDiagnosis = next.differentialDiagnosis.filter(
      (d) => !WEATHER_ONLY_DISEASE_RE.test(d.label ?? '')
    );
  }
  return next;
}
 ...
 */
export function sanitizeAdvisoryForFarmerWhatsApp(
  advisory: StructuredAdvisory
): { advisory: StructuredAdvisory; blockDiagnosis: boolean; reason?: string } {
  let next = { ...advisory };
  ...
  if (!usable && (noCrop || (next.imageObservations?.length ?? 0) > 0)) {
    return {
      advisory: {
        ...
        nutrientDeficiency: [],
        farmerSummaryEn:
          'This photo does not clearly show the crop. Please send a close photo of the affected leaves or plant.',
        farmerSummaryMl:
          'ഈ ഫോട്ടോയിൽ വിള വ്യക്തമല്ല. ബാധിത ഇലയുടെ അടുത്ത ഫോട്ടോ അയയ്ക്കുക.',
        agronomistAssessment:
          'Photo rejected for farmer diagnosis: no usable crop tissue visible. Weather priors must not drive a disease label.',
      },
      blockDiagnosis: true,
