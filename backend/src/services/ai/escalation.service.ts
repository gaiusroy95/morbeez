...
function priorityForConfidence(confidence: number): string {
  if (confidence < 0.4) return 'urgent';
...
export const escalationService = {
  async createCaseForReview(
    params: CreateCaseForReviewInput
  ...
  async enqueueWhatsAppInquiry(params: {
    ...
    if (!text || text.length < 4 || !summary) return null;
