...
export function fusePerImageSignals(
  signals: PerImageDiagnosisSignal[],
  totalCount: number
): Pick<MultiImageFusionResult, 'fusedLabel' | 'fusedConfidence' | 'evidenceBlock' | 'analyzedCount' | 'totalCount'> {
  if (!signals.length) {
    return {
      fusedLabel: '',
      fusedConfidence: 0,
  ...
  const byLabel = new Map<string, { label: string; confs: number[]; count: number }>();
...
function pickBestPlantId(
  signals: PerImageDiagnosisSignal[]
): PlantIdHealthResult | null {
  let best: PlantIdHealthResult | null = null;
  let bestProb = -1;
  for (const s of signals) {
...
async function analyzeOneImage(
  image: DiagnosisImageInput,
  index: number,
  ctx?: MultiImageAnalysisContext
): Promise<PerImageDiagnosisSignal | null> {
  let plantIdResult: PlantIdHealthResult | null = null;
  ...
  if (env.OPENAI_API_KEY) {
    try {
      const advisory = await openaiVisionProvider.analyzeVision({
        ...
        temperature: 0,
        systemPrompt:
          'You are a crop disease scout. Analyze ONLY this single field photo. Reply JSON only: {"probableIssue":"...","confidence":0.0-1.0,"imageObservations":["specific visible feature 1","feature 2"]}',
        userPrompt: [
  ...
  if (!plantLabel && !visionLabel) return null;
  ...
  let source: PerImageDiagnosisSignal['source'] = visionLabel && plantLabel ? 'both' : visionLabel ? 'vision' : 'plant_id';

  if (visionLabel && plantLabel) {
    const agree =
      normalizeLabel(visionLabel).includes(normalizeLabel(plantLabel).slice(0, 12)) ||
 ...
 */
export async function analyzeAndFuseDiagnosisImages(
  images: DiagnosisImageInput[],
  ctx?: MultiImageAnalysisContext
): Promise<MultiImageFusionResult | null> {
  const batch = images.filter((img) => img.imageBase64?.length > 100).slice(0, 8);
  if (batch.length <= 1) return null;

  logger.info(
    { photoCount: batch.length, cropType: ctx?.cropType },
    'Multi-image diagnosis: analyzing each photo then fusing'
  );

  const results = await mapInChunks(batch, 4, (img, index) => analyzeOneImage(img, index, ctx));
  const signals = results.filter((s): s is PerImageDiagnosisSignal => Boolean(s));
  if (!signals.length) {
...
export function collectDiagnosisImages(input: {
  ...
  const out: DiagnosisImageInput[] = [];
  const seen = new Set<string>();

  const push = (img: DiagnosisImageInput) => {
