...
export const warehouseService = {
  ...
  async getDefaultWarehouse() {
    ...
    if (!data) throw new NotFoundError('No warehouse configured');
    return data;
  },
  ...
  async createWarehouse(input: { code: string; name: string; state?: string }) {
    const code = input.code.trim().toUpperCase();
    const name = input.name.trim();
    if (!code || !name) throw new ValidationError('Warehouse code and name are required');
  ...
  async updateWarehouse(id: string, input: { code?: string; name?: string }) {
  ...
  async createLocation(input: {
    ...
    if (!rack) throw new ValidationError('Rack is required');
  ...
  async updateLocation(
    id: string,
    input: { rack?: string; shelf?: string; bin?: string; zone?: string }
  ) {
  ...
  async deactivateLocation(id: string) {
    ...
    if (hasStock) {
      throw new AppError(
        'Cannot remove a location that still has stock',
        409,
        'LOCATION_IN_USE'
      );
    }
  ...
  async renameRack(warehouseId: string, oldRack: string, newRack: string) {
    const nextRack = newRack.trim();
    if (!nextRack) throw new ValidationError('Rack name is required');
    ...
    if (!locs?.length) throw new NotFoundError('Rack not found');
  ...
  async deactivateRack(warehouseId: string, rack: string) {
    const { data: locs, error } = await supabase
      .from('warehouse_locations')
      .select('id')
      .eq('warehouse_id', warehouseId)
  ...
  formatLocationDisplay(loc: {
    ...
    if (loc.location_code) return loc.location_code;
