...
type PackagingLookup = {
  ...
  boxes: Map<string, { code: string; name: string }>;
};

async function loadPackagingLookups(
  categoryIds: string[],
  boxIds: string[]
): Promise<PackagingLookup> {
  const categories = new Map<string, string>();
  const boxes = new Map<string, { code: string; name: string }>();
...
function mapPackagingProfile(
  row: Record<string, unknown> | null | undefined,
  lookup?: PackagingLookup
): ProductPackagingProfile | null {
  if (!row) return null;
...
type RawBatchRow = Record<string, unknown> & {
  ...
  warehouse_locations?: Record<string, string> | null;
};

function mapBatchRows(itemBatches: RawBatchRow[]) {
  let available = 0;
  let reserved = 0;
  let damaged = 0;
  let returned = 0;
...
function catalogVariantLabel(variant: CatalogVariantRef): string {
  return (
    variant.option1 ||
...
function catalogDisplayTitle(product: CatalogProductRef, variant: CatalogVariantRef): string {
...
export const inventoryService = {
  async listInventoryItems(opts?: { search?: string }): Promise<InventoryItemRow[]> {
    ...
    const lookup = await loadPackagingLookups(
      [...new Set(rows.map((r) => (r.packaging_category_id ? String(r.packaging_category_id) : '')).filter(Boolean))],
      [...new Set(rows.map((r) => (r.preferred_box_id ? String(r.preferred_box_id) : '')).filter(Boolean))]
    );
    return rows.map((row) => ({
      id: String(row.id),
      sku: String(row.sku),
  ...
  async updateInventoryItem(
    ...
    if (input.itemWeightKg !== undefined) patch.item_weight_kg = input.itemWeightKg;
    if (input.packagingCategory !== undefined) patch.packaging_category = input.packagingCategory;
    ...
    const lookup = await loadPackagingLookups(
      row.packaging_category_id ? [String(row.packaging_category_id)] : [],
      row.preferred_box_id ? [String(row.preferred_box_id)] : []
    );
    return {
      id: String(row.id),
      sku: String(row.sku),
  ...
  async getInventoryItem(id: string): Promise<InventoryItemRow> {
    const { data, error } = await supabase
      .from('inventory_items')
      .select(INVENTORY_PACKAGING_SELECT)
      .eq('id', id)
  ...
  async loadCommerceBatchesByVariant(): Promise<
    Map<string, Array<{ batchCode: string; qty: number }>>
  > {
    const { data, error } = await supabase
    ...
    const map = new Map<string, Array<{ batchCode: string; qty: number }>>();
    for (const row of data ?? []) {
  ...
  async ensureCommerceLinkedItem(
    variantId: string,
    catalogEntry?: { product: CatalogProductRef; variant: CatalogVariantRef } | null
  ): Promise<string> {
    const itemId = await this.ensureInventoryItemForVariant(variantId);
    if (!catalogEntry) return itemId;
  ...
  async getStockSummary(opts?: {
    ...
    const catalogByVariantId = new Map<
      string,
      { product: CatalogProductRef; variant: CatalogVariantRef }
    >();
    const variantIds = new Set<string>(commerceBatchesByVariant.keys());

    for (const product of catalog as CatalogProductRef[]) {
    ...
    const itemRowRecords = (itemRows ?? []) as Record<string, unknown>[];
    const packagingLookup = await loadPackagingLookups(
      [...new Set(itemRowRecords.map((r) => (r.packaging_category_id ? String(r.packaging_category_id) : '')).filter(Boolean))],
      [...new Set(itemRowRecords.map((r) => (r.preferred_box_id ? String(r.preferred_box_id) : '')).filter(Boolean))]
    );
    const itemMetaById = new Map(
      itemRowRecords.map((r) => [
        String(r.id),
        {
          sku: String(r.sku),
    ...
    for (const [variantId, itemId] of itemIdByVariant) {
      ...
      if (!commerceBatches.length && activity === 0) continue;
  ...
  async getStockItemDetail(inventoryItemId: string, opts?: { warehouseId?: string }) {
    ...
    const totals = mapBatchRows((itemBatches ?? []) as RawBatchRow[]);

    const itemRow = item as Record<string, unknown>;
    const detailLookup = await loadPackagingLookups(
      itemRow.packaging_category_id ? [String(itemRow.packaging_category_id)] : [],
      itemRow.preferred_box_id ? [String(itemRow.preferred_box_id)] : []
    );
    return {
  ...
  async createBatchFromGrn(input: {
    ...
    if (input.qty <= 0) throw new AppError('Quantity must be positive', 400, 'VALIDATION');
  ...
  async reserveStock(input: {
    ...
    for (const batch of batches ?? []) {
      ...
      const loc = batch.warehouse_locations as Record<string, string> | null;
      allocations.push({
        ...alloc,
    ...
    if (remaining > 0) {
      throw new AppError(
        `Insufficient stock for item ${input.inventoryItemId} (short by ${remaining})`,
        409,
        'INSUFFICIENT_STOCK'
      );
    }
  ...
  async findBatchByCode(batchCode: string, inventoryItemId?: string) {
    let q = supabase.from('inventory_batches').select('*').eq('batch_code', batchCode.trim());
    if (inventoryItemId) q = q.eq('inventory_item_id', inventoryItemId);
    const { data, error } = await q.limit(1).maybeSingle();
    throwIfSupabaseError(error, 'Find batch');
    return data;
  },

  async releaseOrderAllocations(commerceOrderId: string, actorEmail?: string) {
    ...
    for (const line of lines ?? []) {
      ...
      for (const alloc of allocs ?? []) {
        const batch = alloc.inventory_batches as Record<string, unknown>;
        const reserved = Number(batch.qty_reserved) || 0;
  ...
  async processReturnStock(input: {
    ...
    lines: Array<{ sku?: string; productTitle: string; qty: number; batchCode?: string }>;
    actorEmail?: string;
  }) {
    const warehouse = await warehouseService.getDefaultWarehouse();

    for (const line of input.lines) {
      if (!line.batchCode || !line.sku) continue;
      const { data: item } = await supabase
        .from('inventory_items')
        .select('id')
        .eq('sku', line.sku)
  ...
  async setBatchStatus(batchId: string, status: 'active' | 'quarantine' | 'expired' | 'depleted') {
    const { data, error } = await supabase
      .from('inventory_batches')
  ...
  async pickAllocation(allocationId: string, qty: number) {
  ...
  async finalizePack(allocationId: string) {
    const { data: alloc, error } = await supabase
      .from('order_line_allocations')
      .select('*, inventory_batches(*)')
      .eq('id', allocationId)
  ...
  async findWarehouseLocationIdForInventoryItem(inventoryItemId: string): Promise<string | null> {
    const { data: item } = await supabase
      .from('inventory_items')
    ...
    for (const product of catalog) {
      const variants = (product as { variants?: Array<{ id: string | number }> }).variants ?? [];
      if (variants.some((v) => String(v.id) === variantId)) {
        productId = String((product as { id: string | number }).id);
        break;
      }
    }
    ...
    const { data: intel } = await supabase
      ...
      .maybeSingle();
    const locId = (intel?.basic as Record<string, unknown> | undefined)?.locationId;
    return locId && String(locId).trim() ? String(locId).trim() : null;
  },

  async applyWarehouseLocationToItemBatches(
    inventoryItemId: string,
    locationId?: string | null
  ): Promise<number> {
    const loc =
    ...
    let updated = 0;
    for (const batch of batches ?? []) {
      if (String(batch.location_id ?? '') === loc) continue;
      const { error: updErr } = await supabase
        .from('inventory_batches')
  ...
  async applyProductWarehouseLocationFromIntelligence(shopifyProductId: string) {
    const { data: intel } = await supabase
      .from('product_intelligence')
      .select('basic')
      ...
      .maybeSingle();
    const locId = (intel?.basic as Record<string, unknown> | undefined)?.locationId;
    if (!locId || !String(locId).trim()) return { updatedBatches: 0 };

    const catalog = await shopifyProductsService.getInventoryCatalog();
    const product = catalog.find((p) => String((p as { id: string | number }).id) === shopifyProductId) as
      | { variants?: Array<{ id: string | number }> }
      | undefined;
    if (!product?.variants?.length) return { updatedBatches: 0 };
  ...
  async applyCommerceBatchToWarehouse(
    ...
    if (targetOnHand === whOnHand) return 0;
   ...
   */
  async syncCommerceBatchesToWarehouse(
    inventoryItemId: string,
    opts?: { shopifyFallback?: boolean }
  ) {
    const { data: item, error: itemErr } = await supabase
      .from('inventory_items')
