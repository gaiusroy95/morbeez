...
export const packService = {
  ...
  async scanBarcode(packSessionId: string, scannedCode: string) {
    ...
    if (!lines?.length) {
      await this.logScan(
        ...
        null,
        'wrong_product',
        'Scanned product is not on this pick list'
      );
    ...
    if (batch && line.batch_id && String(batch.id) !== String(line.batch_id)) {
      await this.logScan(
        ...
        String(batch.id),
        'wrong_batch',
        `Expected batch ${line.batch_code ?? line.batch_id}`
      );
      return {
        ok: false,
    ...
    if (!batch && line.batch_code) {
      const expectedBatch = await inventoryService.findBatchByCode(
        String(line.batch_code),
        String(item.id)
      );
      if (expectedBatch && line.batch_id && String(expectedBatch.id) !== String(line.batch_id)) {
        await this.logScan(
          ...
          null,
          'wrong_batch',
          'Batch verification required — scan batch barcode'
        );
    ...
    await this.logScan(
      ...
      trimmed,
      String(item.id),
      line.batch_id ? String(line.batch_id) : batch?.id ? String(batch.id) : null,
      'ok',
      batch ? 'Batch verified' : 'Product scan verified'
    );
  ...
  async completePack(pickListId: string, verifiedBy?: string) {
    ...
    for (const line of lines) {
      if (!line.manually_verified && Number(line.qty_picked) < Number(line.qty_required)) {
        throw new AppError(
          `Line ${line.product_title} not fully picked/verified`,
          400,
          'PACK_INCOMPLETE'
        );
      }
