...
function skuFromLine(line: ManualLineItem, index: number): string {
...
export const manualOrderOmsService = {
  async pushToOms(manualOrderId: string, actorEmail?: string) {
    ...
    if (!manual) throw new NotFoundError('Manual order not found');
    ...
    const farmer = manual.farmers as Record<string, unknown> | null;
    const lineItems = (manual.line_items as ManualLineItem[]) ?? [];
    if (!lineItems.length) throw new AppError('Manual order has no line items', 400, 'VALIDATION');
  ...
  async tryPushOnCreate(manualOrderId: string, actorEmail?: string) {
    try {
