...
export const fulfillmentService = {
  ...
  async getQueue(opts?: { limit?: number; repair?: boolean }) {
    ...
    return (data ?? []).map((row) => {
      const pickLists = (row.pick_lists ?? []) as Array<{
        id: string;
        status: string;
        pick_list_lines: Array<{ id: string }>;
      }>;
      const orderLines = (row.commerce_order_lines ?? []) as Array<{
        qty_ordered: number;
    ...
    if (pick?.commerce_order_id) {
      ...
      if (!rack.data?.dispatch_rack && rack.data?.courier_name) {
        await supabase
          .from('commerce_orders')
          .update({
            dispatch_rack: suggestDispatchRack(rack.data.courier_name as string),
            updated_at: new Date().toISOString(),
          })
          .eq('id', pick.commerce_order_id);
      }
