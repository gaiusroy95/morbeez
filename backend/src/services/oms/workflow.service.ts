...
export const omsWorkflowService = {
  async onOrderPlaced(shopifyOrderId: string, order?: ShopifyOrder) {
    await orderSyncService.syncOrderMetadata(order ?? ({ id: Number(shopifyOrderId) } as ShopifyOrder));
  ...
  async completePacking(pickListId: string, verifiedBy?: string) {
    ...
    const shouldShipLegacy =
      env.ENABLE_SHIPROCKET_AFTER_PACK &&
      !awbRow?.tracking_awb &&
  ...
  async updateStatus(commerceOrderId: string, status: OmsStatus) {
    const patch: Record<string, unknown> = {
      oms_status: status,
      updated_at: new Date().toISOString(),
    };
    if (status === 'shipped') patch.shipped_at = new Date().toISOString();
    if (status === 'delivered' || status === 'completed') patch.delivered_at = new Date().toISOString();
  ...
  async getOrderWorkflow(commerceOrderId: string) {
    const { data: order, error } = await supabase
      .from('commerce_orders')
      .select(
        '*, commerce_order_lines(*), pick_lists(*, pick_list_lines(*)), invoices(*), cod_reconciliation(*)'
      )
      .eq('id', commerceOrderId)
      .single();
  ...
  async listOmsOrders(opts?: { omsStatus?: string; limit?: number }) {
    let q = supabase
      .from('commerce_orders')
      .select(
        'id, shopify_order_id, order_name, oms_status, is_cod, total_amount, created_at, courier_name, tracking_awb, fulfillment_priority, label_url, dispatch_rack'
      )
      .order('created_at', { ascending: false })
      .limit(opts?.limit ?? 50);
    if (opts?.omsStatus) q = q.eq('oms_status', opts.omsStatus);
