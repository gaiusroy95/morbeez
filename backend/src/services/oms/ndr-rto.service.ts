...
export const ndrRtoService = {
  ...
  async resolveException(
    exceptionId: string,
    action: 'reattempt' | 'rto_received' | 'restocked' | 'written_off',
    qcStatus?: 'pass' | 'damage'
  ) {
    ...
    if (!ex) throw new NotFoundError('Exception not found');
  ...
  async restockFromRto(commerceOrderId: string, asDamaged = false) {
    ...
    for (const line of lines ?? []) {
      ...
      if (qty <= 0 || !line.inventory_item_id) continue;
  ...
  async detectFromTrackingStatus(shopifyOrderId: string | undefined, status: string, payload: Record<string, unknown>) {
