...
export const returnWorkflowService = {
  ...
  async markVerificationPending(returnId: string, actorEmail?: string) {
    return this.patchStatus(returnId, 'verification_pending', actorEmail, 'verification_call_scheduled');
  },

  async approveReturn(
    returnId: string,
    input: { refundType: 'full' | 'partial' | 'none'; refundAmount?: number; approvedBy?: string }
  ) {
    const row = await this.get(returnId);
    const total = Number((row.commerce_orders as Record<string, unknown>)?.total_amount) || 0;
    let refundAmount = input.refundAmount ?? 0;
    if (input.refundType === 'full') refundAmount = total;
    if (input.refundType === 'none') refundAmount = 0;
  ...
  async rejectReturn(returnId: string, reason: string, actorEmail?: string) {
    const { data, error } = await supabase
      .from('return_requests')
      .update({
        status: 'rejected',
  ...
  async markReceived(returnId: string, actorEmail?: string) {
    const { data, error } = await supabase
      .from('return_requests')
      .update({
        status: 'received',
  ...
  async processRefund(returnId: string, actorEmail?: string) {
    const row = await this.get(returnId);
