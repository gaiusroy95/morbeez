...
function mapBox(row: Record<string, unknown>): ShippingBox {
  return {
    id: String(row.id),
...
export const shippingBoxService = {
  async listActive(): Promise<ShippingBox[]> {
    ...
    return (data ?? []).map((row) => mapBox(row as Record<string, unknown>));
  },

  async listAll(): Promise<ShippingBox[]> {
    const { data, error } = await supabase
      .from('shipping_boxes')
      .select('*')
      .order('sort_order', { ascending: true });
  ...
  async getById(id: string): Promise<ShippingBox> {
    ...
    if (!data) throw new NotFoundError('Shipping box not found');
    return mapBox(data as Record<string, unknown>);
  },

  async getByCode(code: string): Promise<ShippingBox | null> {
    const { data, error } = await supabase
      .from('shipping_boxes')
      .select('*')
      .eq('code', code.trim())
    ...
    return data ? mapBox(data as Record<string, unknown>) : null;
  },
  ...
  async update(
    ...
    if (patch.breadthCm !== undefined) row.breadth_cm = patch.breadthCm;
    ...
    if (patch.tareWeightKg !== undefined) row.tare_weight_kg = patch.tareWeightKg;
    if (patch.liquidFriendly !== undefined) row.liquid_friendly = patch.liquidFriendly;
    if (patch.packagingType !== undefined) row.packaging_type = patch.packagingType;
