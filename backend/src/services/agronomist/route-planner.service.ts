...
export const routePlannerService = {
  async listRoutes(agentEmail: string, date?: string) {
    ...
    const summaries = await Promise.all(
      (routes ?? []).map((r) => this.getRouteSummary(String(r.id)))
    );
    return summaries;
  },

  async createRoute(agentEmail: string, routeName: string) {
  ...
  async addStop(routeId: string, farmerId: string, blockId?: string) {
  ...
  async optimizeRoute(routeId: string, originLat?: number, originLng?: number) {
  ...
  async getRouteSummary(routeId: string) {
