...
import type { VisitAssistantMessage, VisitAssistantSnapshot } from '@morbeez/shared/visit-assistant';
import {
  detectVisitCopilotIntent,
  emptyVisitCopilotWorkflow,
  looksLikeFarmerEvidenceMessage,
  type VisitCopilotChatResponse,
...
function assistantMessage(content: string): VisitAssistantMessage {
  return {
    id: randomUUID(),
    role: 'assistant',
    content,
...
function formatStructuredPreview(workflow: VisitCopilotWorkflowState): string {
  const lines: string[] = ['### Structured Preview', ''];
  if (workflow.preview.workingDiagnosis) {
    lines.push('**Working Diagnosis**', `* ${workflow.preview.workingDiagnosis}`, '');
  }
  lines.push(
    '**Diagnosis Status**',
    `* ${workflow.preview.diagnosisStatus === 'confirmed' ? 'Confirmed' : workflow.preview.diagnosisStatus === 'suspected' ? 'Suspected' : 'Pending confirmation'}`,
    ''
  );
  if (workflow.preview.confidence != null) {
    const pct =
      workflow.preview.confidence <= 1
  ...
  if (workflow.preview.evidenceRequired.length) {
    ...
    for (const item of workflow.preview.evidenceRequired) {
      const icon =
        item.status === 'received' ? '✅' : item.status === 'not_observed' ? '❌' : '⏳';
...
function formatFarmerQuestions(questions: string[]): string {
...
function formatTreatmentDraft(workflow: VisitCopilotWorkflowState): string {
  ...
  if (workflow.treatment?.activities.length) {
    ...
    for (const act of workflow.treatment.activities) {
      ...
      if (act.intervalDays) lines.push(`  Repeat every ${act.intervalDays} days${act.repeatCount ? ` × ${act.repeatCount}` : ''}`);
    }
    lines.push('');
  }
  if (workflow.treatment?.farmerAdvice.length) {
    lines.push('**Farmer Advice**');
    for (const advice of workflow.treatment.farmerAdvice) lines.push(`* ${advice}`);
    lines.push('');
  }
  if (workflow.treatment?.weatherAdvisory.length) {
    lines.push('**Weather Advisory**');
    for (const note of workflow.treatment.weatherAdvisory) lines.push(`* ${note}`);
    lines.push('');
  }
  return lines.join('\n');
}

function formatValidation(workflow: VisitCopilotWorkflowState): string {
  if (!workflow.validation) return '';
  const lines = ['### Automatic Validation', ''];
  const section = (title: string, items: Array<{ text: string; status: string }>) => {
...
function formatReminders(reminders: VisitCopilotWorkflowState['reminders']): string {
...
export const visitCopilotWorkflowService = {
  async chat(input: {
    ...
    const messages: VisitAssistantMessage[] = [];

    if (intent.kind === 'confirm_send_questions' && prior.farmerQuestions.length) {
      const workflow: VisitCopilotWorkflowState = {
        ...prior,
    ...
    const isFarmerEvidence =
      intent.kind === 'farmer_evidence' || looksLikeFarmerEvidenceMessage(input.message.content);
    ...
    const proposalMessages = extracted.proposal.messages.filter(
