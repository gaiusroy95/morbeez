...
function partnerAppHint(): string {
...
export const partnerOnboardingService = {
  ...
  async createPartnerByAdmin(
    ...
    let activation: Awaited<ReturnType<typeof this.sendActivationInvite>> | null = null;
  ...
  async sendActivationInvite(partnerId: string) {
    ...
    const activationMessage =
      `Welcome to Morbeez Partner Program, ${partner.fullName}!\n\n` +
    ...
    try {
      ...
      logger.warn(
        { err, partnerId, phone: partner.phone },
        'Partner activation WhatsApp failed — returning message for admin'
      );
    }
    ...
    if (!sent || env.NODE_ENV !== 'production') {
      logger.info(
        { partnerId, phone: partner.phone, activationMessage, sent },
        'Partner activation invite created'
      );
    }

    return {
      sent,
      channel: sent ? ('whatsapp' as const) : ('manual' as const),
      expiresAt,
  ...
  async approveApplication(applicationId: string, adminEmail: string) {
    ...
    if (String(app.status) === 'approved' && app.partner_id) {
      ...
      if (existing) return { partner: existing, activation: null };
    }
