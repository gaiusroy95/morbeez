...
export const farmerOwnershipService = {
  async getOwnership(farmerId: string): Promise<FarmerOwnership | null> {
    const { data, error } = await supabase
      .from('farmers')
      .select(
        'enrollment_owner_type, enrollment_owner_partner_id, enrollment_source, enrollment_event_id, customer_owner_type, customer_owner_partner_id, service_model, assigned_partner_id, assigned_crop_advisor_email, assigned_expert_email, partner_code_at_enrollment'
      )
      .eq('id', farmerId)
      .maybeSingle();
  ...
  async syncCropAdvisorAssignment(farmerId: string, cropAdvisorEmail: string | null): Promise<void> {
