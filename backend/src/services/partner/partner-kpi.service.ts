...
export const partnerKpiService = {
  async computeMonthlySnapshot(partnerId: string, periodStart: Date, periodEnd: Date) {
    ...
    const { data: partnerOrders } = await supabase
      ...
      .gte('created_at', startIso)
      .lte('created_at', endIso);
    const delivered = (partnerOrders ?? []).filter((o) =>
      ['delivered', 'completed'].includes(String(o.oms_status ?? ''))
    ).length;
    const collected = (partnerOrders ?? []).filter(
      (o) =>
        ['delivered', 'completed'].includes(String(o.oms_status ?? '')) &&
  ...
  async recomputeAllForMonth(month: string) {
