...
function buildFieldMetrics(measurements: Map<string, number>): MaiosFieldMetrics | undefined {
...
export const fieldContextService = {
  async loadLatestMeasurements(
    farmerId: string,
    blockId?: string | null
  ): Promise<{ findingId: string | null; measurements: Map<string, number> }> {
    let q = supabase
