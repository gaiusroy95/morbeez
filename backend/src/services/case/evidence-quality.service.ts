...
function inferSlotFromCaption(caption: string | undefined, pack: CropPackConfig): string | null {
  ...
  if (/healthy|ആരോഗ്യ|ஆரோக்கிய/.test(t)) return 'healthy_zone';
  if (/field|വയൽ|வயல்|खेत/.test(t)) return 'field_wide';
  if (/affected|ബാധ|பாதிப்ப|बाढ़/.test(t)) return 'affected_zone';
  return null;
}

function baseQualityForChannel(channel: MaiosChannel): number {
...
export const evidenceQualityService = {
  assignPhotosToSlots(params: {
    ...
    const used = new Set<string>(params.existingSlots ?? []);
    const captured: MaiosPhotoEvidence[] = [];

    for (let i = 0; i < params.photoCount; i++) {
      const captionSlot = inferSlotFromCaption(params.captions?.[i], params.pack);
      let slot: string | null = captionSlot;
    ...
    return params.pack.photoSlots.map((def) => {
      ...
      if (hit) return hit;
      return { slot: def.id, status: 'missing' as const, qualityScore: 0 };
    });
  },
  ...
  computeEqs(params: {
    ...
    const tierBonus: Record<EvidenceTier, number> = {
      T0: 0,
      T1: 8,
      T2: 15,
      T3: 22,
    ...
    if (params.hasSoil) eqs += 8;
    if (params.hasRootPhoto) eqs += 10;
    if (params.hasFieldMetrics) eqs += 12;
    if (params.hasWaterData) eqs += 5;
    if (params.hasLabReport) eqs += 7;
