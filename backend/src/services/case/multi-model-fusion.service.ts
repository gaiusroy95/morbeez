...
type FusionInput = {
  ...
  kgCandidates?: Array<{ label: string; relation: string; weight: number }>;
  labReports?: MaiosLabReport[];
};

function labAnomalyHints(reports?: MaiosLabReport[]): string[] {
  const hints: string[] = [];
  for (const r of reports ?? []) {
    if (r.type === 'leaf') {
      const n = Number(r.metrics?.nitrogen_pct ?? r.metrics?.N);
      if (Number.isFinite(n) && n < 2) hints.push('leaf_n_low');
    }
    ...
    if (r.type === 'soil') {
      ...
      if (Number.isFinite(k) && k < 100) hints.push('soil_k_low');
    }
  }
  return hints;
}
...
export const multiModelFusionService = {
  async enrichHypotheses(
    ...
    input: FusionInput
  ): Promise<{ hypotheses: MaiosHypothesis[]; modelAgreement: number }> {
    const enriched = [...hypotheses];

    for (const prior of input.regionalPriors ?? []) {
      if (!prior.issueLabel) continue;
      const existing = enriched.find(
        (h) => h.label.toLowerCase() === prior.issueLabel.toLowerCase()
      );
      if (existing) {
    ...
    const sorted = enriched
      .map((h, i) => ({
        ...h,
        source: i === 0 && h.source === 'ai' ? ('M1' as const) : h.source,
      }))
