...
export type CultivationTaskContext = {
  overdueTasks: Array<{ taskKey: string; title: string; priority: number }>;
  upcomingTasks: Array<{ taskKey: string; title: string; priority: number }>;
  hasOverdue: boolean;
};
...
export const cultivationContextService = {
  async loadForBlock(params: {
    ...
    const overdue: CultivationTaskContext['overdueTasks'] = [];
    const upcoming: CultivationTaskContext['upcomingTasks'] = [];
