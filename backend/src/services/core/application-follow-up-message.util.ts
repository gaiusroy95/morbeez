...
function parseProducts(products: unknown): { technicalName?: string; tradeName?: string } {
...
export function summarizeDosageGuidance(items: DosageItem[] | null | undefined): string {
  ...
  const parts = items.slice(0, 2).map((d) => {
    ...
    const methodHint =
      method && /foliar|spray|leaf|drench|soil/i.test(method) ? method : method || 'foliar spray';
    return [product, rate].filter(Boolean).join(' ') + (methodHint ? ` (${methodHint})` : '');
  });
  return parts.join('; ').slice(0, MAX_SUMMARY_LEN);
}

export function summarizeFromStructuredAdvisory(
  advisory: Partial<StructuredAdvisory>,
  lang: AdvisoryLanguage
): string {
  ...
  const localizedSummary =
    lang === 'ml' ? advisory.farmerSummaryMl : advisory.farmerSummaryEn;
  if (localizedSummary?.trim()) return firstSentence(localizedSummary, MAX_SUMMARY_LEN);
...
export function summarizeFromRecommendationFields(rec: RecommendationFields): string {
  const parts: string[] = [];
  if (rec.dosage?.trim()) parts.push(rec.dosage.trim());
  ...
  if (parts.length) return parts.join(' — ').slice(0, MAX_SUMMARY_LEN);
...
const FALLBACK_ISSUE: Record<AdvisoryLanguage, string> = {
  en: 'crop issue',
  ml: 'വിള പ്രശ്നം',
  ta: 'பயிர் பிரச்சனை',
  kn: 'ಬೆಳೆ ಸಮಸ್ಯೆ',
...
const FALLBACK_SUMMARY: Record<AdvisoryLanguage, string> = {
...
export function contextFromRecommendationRecord(
  rec: RecommendationFields,
  lang: AdvisoryLanguage
): ApplicationFollowUpContext {
...
const COPY: Record<
  AdvisoryLanguage,
  { prompt: string; issue: string; date: string; treatment: string; closing: string }
> = {
  en: {
...
export function formatApplicationCheckMessage(
  lang: AdvisoryLanguage,
  ctx: ApplicationFollowUpContext
): string {
  const labels = COPY[lang] ?? COPY.en;
  const lines = [labels.prompt, '', `${labels.issue}: ${ctx.issueLabel}`];
  if (ctx.recommendedOn) lines.push(`${labels.date}: ${ctx.recommendedOn}`);
  if (ctx.summary) lines.push(`${labels.treatment}: ${ctx.summary}`);
  lines.push('', labels.closing);
  return lines.join('\n');
}

async function loadFromAdvisorySession(
  sessionId: string,
  lang: AdvisoryLanguage
): Promise<ApplicationFollowUpContext | null> {
  const [{ data: session }, { data: outputs }] = await Promise.all([
    ...
    supabase
      ...
      .select(
        'probable_issue, farmer_summary_en, farmer_summary_ml, dosage_guidance, raw_response, created_at'
      )
  ...
  if (!output && !session) return null;

  const raw = (output?.raw_response ?? {}) as Partial<StructuredAdvisory>;
  const advisory: Partial<StructuredAdvisory> = {
    ...raw,
    ...
    dosageGuidance:
      (output?.dosage_guidance as DosageItem[] | undefined) ?? raw.dosageGuidance ?? [],
  };
...
async function loadLatestRecommendationForFarmer(
  farmerId: string,
  lang: AdvisoryLanguage
): Promise<ApplicationFollowUpContext | null> {
  const { data } = await supabase
    ...
    .select(
      `issue_detected, recommendation_text, products, dosage, application_type, technical_name, trade_name,
  ...
  if (!data) return null;
  return contextFromRecommendationRecord(data as RecommendationFields, lang);
}
...
export async function loadApplicationFollowUpContext(params: {
  ...
  if (recommendationRecordId) {
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data) return contextFromRecommendationRecord(data as RecommendationFields, lang);
  }
