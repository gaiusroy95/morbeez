...
export const visitAiQuestionsService = {
  async buildVisitFollowUpQuestions(params: {
    ...
    const drafts: VisitFollowUpQuestionDraft[] = library.map((q) => ({
    ...
    if (drafts.length < 3 && env.OPENAI_API_KEY) {
      try {
        const extra = await openaiJsonCompletion<{ questions: Array<{ text: string; answerType?: string }> }>(
          'Return JSON {"questions":[{"text":"...","answerType":"yes_no_unknown|number"}]} with 2-3 short agronomy follow-up questions.',
