...
import {
  ...
  aiClassificationFromLevel,
  parseImprovementLevelFromText,
...
export const outcomeKpiInterpretationService = {
  async interpretFarmerReply(params: {
    ...
    if (!snippet && !params.hasImage) return null;
    ...
    try {
      ...
      if (!res.ok) return rule ? this.fromRule(rule, snippet) : null;

      const data = (await res.json()) as {
        choices?: Array<{ message?: { content?: string } }>;
      };
      const raw = data.choices?.[0]?.message?.content;
      if (!raw) return rule ? this.fromRule(rule, snippet) : null;

      const parsed = JSON.parse(raw) as { level?: string; confidence?: number };
      const level = parsed.level as ImprovementLevel;
      if (
        !['fully_improved', 'slight_improvement', 'no_improvement', 'worse'].includes(level)
      ) {
  ...
  fromRule(
    rule: { level: ImprovementLevel; confidence: number },
    snippet: string
  ): InterpretedOutcomeKpi {
    return {
      improvementLevel: rule.level,
      confidence: rule.confidence,
      aiClassification: aiClassificationFromLevel(rule.level, rule.confidence),
