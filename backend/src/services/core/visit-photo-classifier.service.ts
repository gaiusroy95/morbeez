...
function isFieldLevelPhotoType(photoType: string): boolean {
  const key = photoType.toLowerCase();
  return /whole|field|plot|overview|drainage|plant|bush|palm/.test(key);
}

function isSymptomPhotoType(photoType: string): boolean {
  const key = photoType.toLowerCase();
  return /leaf|disease|pest|symptom|rhizome|stem|fruit|root|spot|close/.test(key);
}

function pickFromList(raw: string, availableTypes: string[]): string | null {
...
export function mapClassifierCategoryToVisitPhotoType(
  ...
  if (cat === 'root' || cat === 'rhizome') {
    if (RHIZOME_CROPS.test(crop)) return pick('rhizome', 'root');
  ...
  if (cat === 'crop_leaf' || cat === 'leaf' || cat === 'symptom') {
    if (RHIZOME_CROPS.test(crop) && avail.has('leaf')) return 'leaf';
...
function parseVisionPhotoJson(
  text: string,
  availableTypes: string[]
): VisitPhotoClassification | null {
  try {
            ...
            messages: [
              {
                role: 'system',
                content:
                  `Classify a field visit photo for ${params.cropType}. photo_type MUST be one of: ${typeList}. ` +
