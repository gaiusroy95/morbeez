...
function mapLifecycleRow(row: Record<string, unknown>): SessionLifecycle {
  return {
    sessionId: String(row.id),
    confidenceScore: row.confidence_score != null ? Number(row.confidence_score) : null,
    confidenceBand: row.confidence_band
      ? (String(row.confidence_band) as ConfidenceAction)
      : null,
...
export const confidenceLifecycleService = {
  ...
  }): Promise<{ band: ConfidenceAction; needsEscalation: boolean }> {
  ...
  async markAutoSent(sessionId: string, channel = 'whatsapp'): Promise<void> {
    const now = new Date().toISOString();
  ...
  async getLifecycle(sessionId: string): Promise<SessionLifecycle | null> {
    const { data, error } = await supabase
      .from('ai_advisory_sessions')
      .select(
        `id, confidence_score, confidence_band, escalation_recommended,
  ...
  async getRoutingStats(days = 30) {
    ...
    const { data, error } = await supabase
      .from('ai_advisory_sessions')
      .select(
        `confidence_band, auto_sent, human_reviewed, corrected, confidence_score, status, created_at`
      )
    ...
    for (const r of rows) {
      const band = String(r.confidence_band ?? '') as keyof typeof byBand;
      if (band in byBand) byBand[band] += 1;
      if (r.auto_sent) autoSent += 1;
      if (r.human_reviewed) humanReviewed += 1;
    ...
    return {
      ...
      avgConfidencePct:
        confCount > 0 ? Math.round((confSum / confCount) * 1000) / 10 : null,
      thresholds: {
        autoSend: 95,
        employeeReview: 80,
      },
