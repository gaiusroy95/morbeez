...
} from '../whatsapp/pipeline/follow-up-question.types.js';
...
export type CaseReviewFollowUpInput = {
  ...
  intakeQa?: Array<{ question: string; answer: string; kind?: string }>;
};
...
function parseKind(raw: unknown): FollowUpQuestionKind {
  return normalizeFollowUpKind(raw);
}

function mapRow(row: Record<string, unknown>): SavedFollowUpQuestion {
...
function symptomKeysForText(cropType: string, symptomsText: string, issueLabel?: string): string[] {
  ...
  if (issueLabel?.trim()) set.add(buildSymptomKey(`${symptomsText} ${issueLabel}`));
  return [...set].filter(Boolean);
}

function questionAlreadyImplied(questionText: string, symptomsText: string): boolean {
...
export const expertFollowUpLearningService = {
  ...
  localize(q: SavedFollowUpQuestion, lang: AdvisoryLanguage): string {
    if (lang === 'ml' && q.textMl.trim()) return q.textMl.trim();
  ...
  async findForFarmer(params: {
    ...
    const out: SavedFollowUpQuestion[] = [];
    ...
    for (const symptomKey of keys) {
      ...
      for (const row of rows ?? []) {
        ...
        if (questionAlreadyImplied(mapped.textEn, params.symptomsText)) continue;
        seen.add(dedupe);
        out.push(mapped);
  ...
  async recordHit(libraryId: string): Promise<void> {
  ...
  async persistQuestions(params: {
    ...
    for (const q of params.questions) {
      ...
      if (!textKey || textKey.length < 8) continue;
  ...
  async generateQuestionsForCase(input: CaseReviewFollowUpInput): Promise<
    Array<{
      questionId: string;
      kind: FollowUpQuestionKind;
      textEn: string;
  ...
  > {
    ...
    try {
      const raw = await openaiJsonCompletion<{ questions?: unknown[] }>(
      ...
      for (const row of rows.slice(0, 3)) {
        ...
        if (textEn.length < 8) continue;
        if (existingLibrary.some((e) => normalizeTextKey(e) === normalizeTextKey(textEn))) continue;
        if (existingIntake.some((e) => normalizeTextKey(e) === normalizeTextKey(textEn))) continue;
  ...
  async onCaseReviewApproved(input: CaseReviewFollowUpInput): Promise<{ saved: number }> {
    const keys = symptomKeysForText(input.cropType, input.symptomsText, input.issueLabel);
    const primaryKey = keys[0] ?? buildSymptomKey(input.symptomsText);
    let saved = 0;
    let order = 0;
    ...
    for (const extraKey of keys.slice(1, 4)) {
      if (extraKey === primaryKey || !generated.length) continue;
      saved += await this.persistQuestions({
        cropType: input.cropType,
        district: input.district,
        symptomKey: extraKey,
  ...
  async loadIntakeQaFromSession(sessionId: string): Promise<
    Array<{ question: string; answer: string; kind?: string }>
  > {
    const { data } = await supabase
