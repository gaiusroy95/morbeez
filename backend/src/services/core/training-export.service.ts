...
export const trainingExportService = {
  ...
  async exportVisitCaseBundle(fieldFindingId: string) {
    ...
    const [
      ...
      supabase
        .from('visit_ai_cases')
        .select(
          `*, visit_ai_hypotheses(*), visit_ai_questions(*),
    ...
    const { data: callbackRows } = await supabase
      ...
      .order('created_at', { ascending: false })
      .limit(20);
    const callbacks = (callbackRows ?? []) as Record<string, unknown>[];
    ...
    if (recIds.length) {
      ...
      followUps = (fuRows ?? []) as Record<string, unknown>[];
    }
    ...
    return {
      ...
      workflowArtifacts: {
        ...
        followUpQa: (aiCasesRes.data ?? []).flatMap(
          (c) => (c.visit_ai_questions as unknown[]) ?? []
        ),
        recommendations: (aiCasesRes.data ?? []).flatMap(
          (c) => (c.visit_ai_recommendations as unknown[]) ?? []
        ),
