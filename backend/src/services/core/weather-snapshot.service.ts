...
function forecastToSnapshotContext(forecast: WeatherForecast): Record<string, unknown> {
  const alerts: string[] = [];
  if (forecast.heavyRainLikely) alerts.push('heavy_rain_likely');
  if (forecast.highHeatLikely) alerts.push('high_heat_likely');
  if (forecast.highHumidityLikely) alerts.push('high_humidity_likely');
  return {
...
async function resolveLocationForBlock(blockId: string): Promise<{
  farmerId: string | null;
  coords: ReturnType<typeof resolveCoords>;
}> {
  const { data: block } = await supabase
    .from('farm_blocks')
    .select(
      `id, farmer_id, latitude, longitude, plot_label, pincode_id,
  ...
  const farmer = block.farmers as {
    district?: string;
    pincode_master?: { latitude?: number; longitude?: number; pincode?: string; village?: string };
  } | null;
...
async function resolveLocationForFarmer(farmerId: string): Promise<ReturnType<typeof resolveCoords>> {
...
export const weatherSnapshotService = {
  ...
  async capture(params: {
    ...
    eventType: WeatherEventType;
    eventId?: string | null;
  }): Promise<{ snapshotId: string; context: Record<string, unknown> } | null> {
    try {
      ...
      const alerts = (context.disease_alerts as string[]) ?? [];
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          weather_risk_score: forecast.weatherRiskScore,
          disease_alerts: alerts,
          raw_forecast: forecast as unknown as Record<string, unknown>,
        })
        .select('id')
        .single();
  ...
  async getById(id: string): Promise<WeatherSnapshotRow | null> {
    ...
    return {
      ...
      eventType: data.event_type as WeatherEventType,
      ...
      diseaseAlerts: (data.disease_alerts as unknown[]) ?? [],
  ...
  mapSourceToEventType(source: TrainingEventSource): WeatherEventType {
    if (source === 'field_visit') return 'field_finding';
    if (source === 'whatsapp') return 'ai_session';
    if (source === 'crm') return 'recommendation';
    return 'manual';
  },

  mapReviewSurfaceToEventType(surface: string): WeatherEventType {
    if (surface === 'field_finding' || surface === 'image_review') return 'field_finding';
    if (
      surface === 'case_review' ||
  ...
  async linkSnapshotToEvent(snapshotId: string, eventId: string): Promise<void> {
  ...
  }): Promise<{ weatherSnapshotId: string | null; weatherContext: Record<string, unknown> }> {
    if (params.fieldFindingId) {
      ...
      if (ff?.weather_snapshot_id) {
        return {
          ...
          weatherContext: (ff.weather_context as Record<string, unknown>) ?? {},
        };
      }
    }
