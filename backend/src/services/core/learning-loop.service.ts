...
export const learningLoopService = {
  ...
  async promoteRecommendationToReuse(recommendationRecordId: string): Promise<void> {
    const { data: rec } = await supabase
      ...
      .select(
        'id, farmer_id, block_id, ai_session_id, field_finding_id, issue_detected, recommendation_text, products, outcome, status, dap_at_recommendation, source, farm_blocks(crop_type)'
      )
    ...
    if (rec.ai_session_id) {
      ...
      if (output) {
        ...
        await verifiedAdvisoryLearningService.promoteVerifiedAnswer({
          ...
          extraSymptomSources: [rec.issue_detected, rec.recommendation_text].filter(Boolean) as string[],
          global: true,
        });

        logger.info(
          { recommendationRecordId, dapBucket: buildDapBucket(rec.dap_at_recommendation ?? 0) },
          'Promoted AI session to advisory_reuse_cases'
        );
        return;
      }
    }

    const issueSources: string[] = [rec.issue_detected, rec.recommendation_text].filter(Boolean) as string[];
    ...
    if (rec.field_finding_id) {
      ...
      for (const vi of visitIssues ?? []) {
        if (vi.issue_name) issueSources.push(String(vi.issue_name));
        if (vi.observation) issueSources.push(String(vi.observation));
      }
    }
    ...
    logger.info(
      { recommendationRecordId, dapBucket: buildDapBucket(rec.dap_at_recommendation ?? 0) },
      'Promoted field visit to advisory_reuse_cases'
    );
  },
