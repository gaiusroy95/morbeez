...
export const recommendationRecordsService = {
  ...
  async submitForApproval(id: string, reviewedBy?: string) {
    const { data: existing } = await supabase
      .from('recommendation_records')
      .select(
        'metadata, farmer_id, block_id, recommendation_text, dosage, application_type, dap_at_recommendation'
      )
      .eq('id', id)
      .maybeSingle();

    if (env.ENFORCE_SERVER_RECOMMENDATION_SAFETY) {
      if (!existing) throw new AppError('Recommendation not found', 404, 'NOT_FOUND');
      await recommendationSafetyGateService.evaluate({
        ...
        validation: {
          ...
          dap:
            existing.dap_at_recommendation == null
              ? null
        ...
        if (!decision.allowsApproval) {
          throw new AppError(
            `Recommendation safety is ${decision.decision}`,
            409,
            'SAFETY_GATE_BLOCKED'
          );
        }
