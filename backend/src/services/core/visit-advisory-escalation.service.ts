...
const REASON_TITLES: Record<VisitEscalationReason, string> = {
...
const REASON_PRIORITY: Record<VisitEscalationReason, 'normal' | 'high' | 'urgent'> = {
...
function reasonTag(reason: VisitEscalationReason): string {
  return `[visit:${reason}]`;
}
...
export const visitAdvisoryEscalationService = {
  async escalate(params: {
    ...
    leadId?: string | null;
  }): Promise<{ callbackId: string | null; taskCreated: boolean }> {
  ...
  async processMonitoringProgressionJob(job: {
    ...
    if (!progressionConfirmed) return;
    ...
    if (!issue || String(issue.severity) !== 'high') return;
  ...
  async processEscalationJob(job: {
    ...
    const reason = job.payload.reason as VisitEscalationReason | undefined;
