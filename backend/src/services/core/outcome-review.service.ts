...
function mapOutcomeToLearning(outcome: RecommendationOutcome): string {
  if (outcome === 'better') return 'improved';
  if (outcome === 'partial') return 'partial';
  if (outcome === 'no_improvement') return 'no_improvement';
  return 'unknown';
}

function mapRow(r: Record<string, unknown>) {
  const farmer = r.farmers as Record<string, unknown> | null;
  const block = r.farm_blocks as Record<string, unknown> | null;
  return {
    id: String(r.id),
...
export const outcomeReviewService = {
  async listQueue(params: {
    ...
    let followUpByRec = new Map<string, Record<string, unknown>>();
    ...
    if (recIds.length) {
      ...
      for (const fu of followUps ?? []) {
        ...
        if (!followUpByRec.has(rid)) followUpByRec.set(rid, fu as Record<string, unknown>);
      }
    }
  ...
  async getDetail(recommendationId: string) {
    ...
    const { data: row } = await supabase
      .from('recommendation_records')
      .select(
        `*, farmers(name, phone, district, preferred_language), farm_blocks(name, crop_type, plot_label)`
      )
  ...
  async recordOutcome(
    ...
    input: RecordStructuredOutcomeInput,
    agentEmail: string
  ) {
    const rec = await recommendationFollowUpService.loadRecord(recommendationId);
    if (!rec) throw new NotFoundError('Recommendation not found');
    ...
    const issueResolved =
      input.issueResolved ?? (input.outcome === 'better' || input.outcome === 'partial');
