...
function rainfallBand(mm: number | null | undefined): string {
  if (mm == null || Number.isNaN(mm) || mm <= 0) return 'dry';
  if (mm <= 10) return 'light';
  if (mm <= 30) return 'moderate';
  return 'heavy';
}

function isDiseaseFinding(type: string | null, issue: string | null): boolean {
  if (type === 'disease' || type === 'pest' || type === 'weather_stress') return true;
  ...
  async getAnalytics(days = 90) {
    ...
    const [findingsRes, snapshotsRes, eventsRes, imagesRes, activitiesRes] = await Promise.all([
      supabase
        ...
        .select(
          `id, finding_type, final_confirmed_issue, disease_pest, weather_snapshot_id, weather_context,
    ...
    const bandTotals: Record<string, RainBand> = {
    ...
    for (const f of findings) {
      const snapRaw = f.weather_snapshots as Record<string, unknown> | Record<string, unknown>[] | null;
      const snap = Array.isArray(snapRaw) ? snapRaw[0] : snapRaw;
      const rainfall =
        snap?.rainfall_mm != null
          ? Number(snap.rainfall_mm)
          : ((f.weather_context as Record<string, unknown> | undefined)?.rainfall_mm as number | undefined);
      const humidity =
        snap?.humidity_pct != null ? Number(snap.humidity_pct) : null;
      const bandKey = rainfallBand(rainfall);
      const band = bandTotals[bandKey];
      if (!band) continue;

      if (f.weather_snapshot_id || snap) findingsWithWeather += 1;
      ...
      band.findingCount += 1;
      if (findingType === 'disease') band.diseaseCount += 1;
      else if (findingType === 'pest') band.pestCount += 1;
      else if (findingType === 'weather_stress') band.weatherStressCount += 1;
      else if (isDiseaseFinding(findingType, issue)) band.diseaseCount += 1;

      if (humidity != null && humidity >= 80) {
        highHumidityFindings += 1;
        if (isDiseaseFinding(findingType, issue)) highHumidityDisease += 1;
      }
    }

    const rainfallBands: RainBand[] = Object.values(bandTotals).map((b) => ({
      ...b,
    ...
    const heavyDiseaseRate = pct(
      heavyBand.diseaseCount + heavyBand.pestCount,
      heavyBand.findingCount
    );
    ...
    for (const e of eventsRes.data ?? []) {
      ...
      if (meta.weatherSnapshotId || meta.weatherContext) eventsWithWeather += 1;
    }

    const imagesTotal = imagesRes.count ?? 0;
    let imagesWithWeather = 0;
    for (const img of imagesRes.data ?? []) {
      if (img.weather_snapshot_id) imagesWithWeather += 1;
    }

    const activitiesTotal = activitiesRes.count ?? 0;
    let activitiesWithWeather = 0;
    for (const a of activitiesRes.data ?? []) {
      if (a.weather_snapshot_id) activitiesWithWeather += 1;
    }

    const insights: string[] = [];
    if (heavyBand.findingCount >= 3 && heavyDiseaseRate > dryDiseaseRate + 10) {
      insights.push(
        `Disease/pest findings are ${heavyDiseaseRate}% of field visits after heavy rain (>30 mm) vs ${dryDiseaseRate}% in dry conditions — prioritize scouting 3–5 days after rainfall.`
      );
    }
    if (highHumidityFindings >= 3 && highHumidityDisease > 0) {
      insights.push(
        `${pct(highHumidityDisease, highHumidityFindings)}% of high-humidity (≥80%) visits had disease/pest signals — fungal risk window is elevated.`
      );
    }
    if (findingsWithWeather < findings.length * 0.5 && findings.length > 0) {
      insights.push(
        'Less than half of field findings have linked weather snapshots — capture is improving via operational sessions.'
      );
    }
    if (insights.length === 0 && findings.length > 0) {
      insights.push(
        'Collect more field findings with rainfall variation to strengthen disease–weather correlation models.'
      );
    }
