...
async function findGapRow(technicalName: string, cropType: string | undefined, district: string | undefined) {
...
export const productGapService = {
  async incrementFromRecommendation(params: {
    ...
    const samples = Array.isArray(existing?.sample_recommendation_ids)
      ? [...(existing.sample_recommendation_ids as string[])]
      : [];
    ...
    const urgency =
      count >= 25 ? 'critical' : count >= 15 ? 'high' : count >= GAP_THRESHOLD ? 'normal' : 'low';
  ...
  async updateStatus(id: string, status: 'open' | 'reviewing' | 'sourcing' | 'resolved') {
    const { data, error } = await supabase
      .from('product_gap_queue')
  ...
  async getCommerceInventoryEta(technicalName: string): Promise<{
    technicalName: string;
    availableQty: number;
    etaDays: number | null;
    ...
    if (availableQty <= 0 && wmsRows?.length) {
      ...
      for (const line of poLines ?? []) {
        const po = line.purchase_orders as { status?: string; expected_delivery_date?: string } | null;
        if (!po || po.status === 'cancelled') continue;
        const remaining = Number(line.qty_ordered ?? 0) - Number(line.qty_received ?? 0);
        if (remaining <= 0) continue;
        if (po.expected_delivery_date) {
          const days = Math.ceil(
            (new Date(po.expected_delivery_date).getTime() - Date.now()) / (24 * 60 * 60 * 1000)
          );
  ...
  async listAlternatives(technicalName: string, cropType?: string): Promise<
    Array<{ technicalName: string; reason: string; availableQty?: number }>
  > {
    ...
    const alts: Array<{ technicalName: string; reason: string }> = [];
    for (const row of rotation ?? []) {
      const active = String(row.active_ingredient ?? '').toLowerCase();
      if (!active.includes(name) && !name.includes(active)) continue;
      if (cropType && row.crop_type && String(row.crop_type).toLowerCase() !== cropType.toLowerCase()) continue;
      const alternates = Array.isArray(row.alternate_ingredients) ? row.alternate_ingredients : [];
      for (const alt of alternates) {
        const tn = String(alt).trim();
        if (tn.length) alts.push({ technicalName: tn, reason: 'Resistance rotation rule' });
      }
    }
    ...
    if (alts.length < 3) {
      try {
        ...
        for (const p of catalog.products) {
          if (p.title.toLowerCase().includes(name)) continue;
    ...
    const out: Array<{ technicalName: string; reason: string; availableQty?: number }> = [];
    for (const a of alts) {
