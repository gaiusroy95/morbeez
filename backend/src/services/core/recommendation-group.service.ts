...
function mapMaterialRow(row: Record<string, unknown>): RecommendationGroupMaterialRow {
  return {
    id: String(row.id),
...
async function loadMaterialsForGroups(groupIds: string[]): Promise<Map<string, RecommendationGroupMaterialRow[]>> {
  const map = new Map<string, RecommendationGroupMaterialRow[]>();
  ...
  for (const row of data ?? []) {
    ...
    list.push(mapMaterialRow(row as Record<string, unknown>));
    map.set(groupId, list);
  }
  return map;
}

function productsJsonForIssue(
  groups: RecommendationGroupRow[],
  visitIssueId: string
): Array<Record<string, unknown>> {
  const products: Array<Record<string, unknown>> = [];
  for (const group of groups) {
    for (const material of group.materials) {
      if (material.issueId && material.issueId !== visitIssueId) continue;
      products.push({
...
export const recommendationGroupService = {
  ...
  async listByFieldFinding(fieldFindingId: string): Promise<RecommendationGroupRow[]> {
    const { data, error } = await supabase
    ...
    return (data ?? []).map((row) =>
      mapGroupRow(row as Record<string, unknown>, materialsByGroup.get(String(row.id)) ?? [])
    );
  },

  async getById(groupId: string): Promise<RecommendationGroupRow> {
    const { data, error } = await supabase
      .from('recommendation_groups')
      .select('*')
      .eq('id', groupId)
    ...
    if (!data) throw new NotFoundError('Recommendation group not found');

    const materialsByGroup = await loadMaterialsForGroups([groupId]);
    return mapGroupRow(data as Record<string, unknown>, materialsByGroup.get(groupId) ?? []);
  },

  async create(fieldFindingId: string, input: RecommendationGroupInput): Promise<RecommendationGroupRow> {
    ...
    if (!groupRow) throw new NotFoundError('Could not create recommendation group');

    const groupId = String(groupRow.id);
    const materials = await this.insertMaterials(groupId, input.materials);
    return mapGroupRow(groupRow as Record<string, unknown>, materials);
  },

  async update(groupId: string, input: Partial<RecommendationGroupInput>): Promise<RecommendationGroupRow> {
    await this.getById(groupId);

    const patch: Record<string, unknown> = {};
    if (input.applicationType !== undefined) patch.application_type = input.applicationType;
    if (input.applicationDay !== undefined) patch.application_day = input.applicationDay;
  ...
  async replaceForFieldFinding(
    fieldFindingId: string,
    groups: RecommendationGroupInput[]
  ): Promise<RecommendationGroupRow[]> {
    const { error: delErr } = await supabase
      .from('recommendation_groups')
      .delete()
      .eq('field_finding_id', fieldFindingId);
    ...
    const created: RecommendationGroupRow[] = [];
    for (let i = 0; i < groups.length; i++) {
      created.push(
        await this.create(fieldFindingId, {
          ...groups[i]!,
  ...
  async insertMaterials(
    groupId: string,
    materials: RecommendationGroupMaterialInput[]
  ): Promise<RecommendationGroupMaterialRow[]> {
  ...
  primaryApplicationTypeForIssue(groups: RecommendationGroupRow[], visitIssueId: string): string | null {
    for (const group of groups) {
      const hasIssueMaterial = group.materials.some(
        (material) => !material.issueId || material.issueId === visitIssueId
      );
      if (hasIssueMaterial) return group.applicationType;
    }
