...
import type {
  VisitPromptSimilarCase,
  VisitPromptTrainingExample,
} from './visit-ai-prompt-context.service.js';
...
export const visitAiRetrievalService = {
  async findTrainingExamples(params: {
    ...
    const scored = rows
      .map((s) => {
        ...
        let score = 0;
        if (symptomKey && sym.includes(symptomKey.split('|')[0] ?? '')) score += 2;
        if (label.includes(params.issueName.toLowerCase().split(' ')[0] ?? '')) score += 1;
        if (district && String(s.district ?? '').toLowerCase() === district) score += 1;
        return { s, score };
      })
      .filter((r) => r.score > 0)
  ...
  async findVerifiedCases(params: {
    ...
    const { data } = await supabase
      .from('visit_ai_cases')
      .select(
        `issue_name, final_diagnosis, metadata,
    ...
    return matches.slice(0, params.limit ?? 5).map((r) => {
      const recs = r.visit_ai_recommendations as Array<{ review_action?: string; human_text?: string }> | null;
      const rec = recs?.[0];
