...
import {
  hashVisitImageBase64,
  lookupVisitImageDiagnosis,
  storeVisitImageDiagnosis,
} from './visit-image-diagnosis-cache.service.js';
...
function topPlantIdLabel(
  result: Awaited<ReturnType<typeof plantIdProvider.assessHealth>>,
  cropType?: string
): VisitImageSignal | null {
...
function visionLabelFromAdvisory(advisory: { probableIssue?: string; label?: string }): string {
...
async function analyzeOnePhoto(
  ...
  let signal: VisitImageSignal | null = null;
  ...
  if (!signal && env.OPENAI_API_KEY) {
    try {
      ...
      const advisory = await openaiVisionProvider.analyzeVision({
        ...
        temperature: 0,
        systemPrompt:
          `Identify the most likely crop disease, pest, or nutrient problem visible in the photo. Reply JSON only: {"probableIssue":"...","confidence":0.0-1.0,"observations":[{"feature":"${featureList}","value":"present|absent","confidence":0.0-1.0}]}`,
      ...
      const label = visionLabelFromAdvisory(advisory as { probableIssue?: string; label?: string });
      if (label) {
        const structuredObs =
          env.ENABLE_STRUCTURED_VISION !== false
            ...
            : visitVisionObservationsService.inferFromLabel(
                label,
                Number(advisory.confidence ?? 0.65),
                ctx?.cropType
              );
        signal = {
          label,
  ...
  if (!signal.observations?.length) {
    signal.observations = visitVisionObservationsService.inferFromLabel(
      signal.label,
      signal.confidence,
      ctx?.cropType
    );
...
export async function resolveVisitImagePredictions(
  ...
  const batch = sortVisitPhotosForDiagnosis(
    (photos ?? []).filter((p) => p.dataBase64?.length > 100)
  ).slice(0, 8);
  ...
  return {
    ...
    observations: visitVisionObservationsService.merge(
      ...signals.map((s) =>
        s.observations ??
