...
export const productRewardService = {
  async applyToOrder(input: { orderId: string; farmerId: string; grossInr: number }) {
    const purchase = Math.max(0, Number(input.grossInr) || 0);
    if (purchase <= 0) return { consumed: 0 };
    ...
    const { data: already } = await supabase
      ...
      .eq('kind', 'use')
      .maybeSingle();
    if (already?.id) return { consumed: 0 };
    ...
    if (next.consumeInr <= 0) return { consumed: 0 };
