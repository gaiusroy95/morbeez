...
async function poolFromLines(orderId: string) {
  const { data: lines } = await supabase
    .from('commerce_order_lines')
    .select(
      'qty_ordered, unit_price, channel_pool_pct, channel_pool_agronomist_pct, channel_pool_partner_pct'
    )
...
async function monthEligibleTotals(agronomistEmail: string, month: string) {
...
export const agronomistSalesIncentiveService = {
  ...
  async adjustOrder(orderId: string, reason: string) {
    ...
    if (!existing?.id) return { adjusted: 0 };
