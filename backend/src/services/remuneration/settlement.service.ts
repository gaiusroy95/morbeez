...
export const settlementService = {
  ...
  async dueForParty(partyType: 'partner' | 'employee', partyId: string, asOf = new Date()) {
  ...
  async dueTotalInr(partyType: 'partner' | 'employee', partyId: string, asOf = new Date()) {
  ...
  async markPaid(ids: string[], paymentReference?: string) {
  ...
  async applyReturnRecovery(earningSource: 'partner_ledger' | 'agronomist_ledger', earningId: string, recoverInr: number) {
  ...
  async resyncPending(input: {
    ...
    if (rows.some((r) => r.status === 'paid' || r.status === 'approved')) return;
    const rule = await earningRulesService.settlementRule();
  ...
  async attachPayoutBatch(ids: string[], batchId: string) {
    if (!ids.length) return;
    await supabase.from('earning_settlements').update({ payout_batch_id: batchId }).in('id', ids);
  },

  async attachPayrollEntry(ids: string[], payrollEntryId: string) {
    if (!ids.length) return;
    await supabase
      .from('earning_settlements')
  ...
  async dueUnattached(partyType: 'partner' | 'employee', asOf = new Date()) {
    const day = asOf.toISOString().slice(0, 10);
    const { data, error } = await supabase
      .from('earning_settlements')
      .select('*')
