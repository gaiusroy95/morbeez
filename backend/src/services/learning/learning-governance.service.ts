...
export const learningGovernanceService = {
  ...
  async submitCandidate(input: SubmitCandidateInput): Promise<{ id: string; status: string }> {
  ...
  async reviewCandidate(params: {
    ...
    notes?: string;
    reasonCodes?: string[];
  }): Promise<{ candidateId: string; status: string; knowledgeVersionId?: string | null }> {
    ...
    const { data: candidate } = await supabase
      ...
      .maybeSingle();
    if (!candidate) throw new ConflictError('candidate_not_found');
    ...
    let knowledgeVersionId: string | null = null;
  ...
  async publishApprovedCandidate(params: {
    ...
    const payload = (params.candidate.payload as Record<string, unknown>) ?? {};
  ...
  async findApprovedReuse(params: {
    ...
    if (!this.approvedReuseReadsEnabled()) return null;
    ...
    let q = supabase
      ...
      .limit(1);
    if (params.cropType) q = q.eq('crop_type', params.cropType.toLowerCase());
    if (params.district) q = q.eq('district', params.district.toLowerCase());
