...
import type { GingerInputHistoryEntry, GingerInputHistorySummary } from '../../domain/ginger-sop/types.js';
...
function analyzeWarnings(entries: GingerInputHistoryEntry[]): string[] {
  ...
  for (let i = 1; i < sprays.length; i++) {
    ...
    if (gapDays < 3 && prev.products.length && cur.products.length) {
      const overlap = prev.products.some((p) =>
        cur.products.some((c) => c.toLowerCase().includes(p.toLowerCase().slice(0, 6)))
      );
      if (overlap) {
        warnings.push(
          `Possible repeat spray within ${Math.round(gapDays)}d: ${cur.products.join(', ')}`
        );
      }
    }
  }
...
export const gingerSopInputHistoryService = {
  async load21Day(farmerId: string, blockId?: string | null): Promise<GingerInputHistorySummary> {
    ...
    const entries: GingerInputHistoryEntry[] = (rows ?? []).map((r) => ({
