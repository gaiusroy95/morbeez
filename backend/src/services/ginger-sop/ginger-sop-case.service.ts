...
function isGingerCrop(cropType: string): boolean {
  return cropType.toLowerCase().includes('ginger');
}

function buildHypotheses(input: GingerSopBuildInput): GingerHypothesis[] {
  const rows: GingerHypothesis[] = [];
...
export const gingerSopCaseService = {
  ...
  async resolveIdentity(farmerId: string, blockId?: string | null) {
    const block = blockId
  ...
  async buildCase(input: GingerSopBuildInput): Promise<GingerSopCase | null> {
    if (!isGingerCrop(input.cropType)) return null;
    ...
    const evidenceTier = gingerSopEvidenceService.evidenceTier(
      completenessPct,
      Boolean(input.hasSoilReport),
      hasRootPhoto,
      hasFieldMetrics
    );
    ...
    const fusedConfidence = gingerSopConfidenceService.fusedConfidence(
      moduleScores,
      modelConfidence,
      null
    );
  ...
  async persistToSession(sessionId: string, gingerCase: GingerSopCase): Promise<void> {
