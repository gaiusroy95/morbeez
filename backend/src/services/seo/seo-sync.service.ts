...
export const seoSyncService = {
  ...
  async syncProductToShopify(shopifyProductId: string) {
    ...
    const product: Record<string, unknown> = { id: Number(shopifyProductId) };
    if (handle) product.handle = handle.replace(/^\//, '').replace(/^products\//, '');
    ...
    if (metafields.length) product.metafields = metafields;
