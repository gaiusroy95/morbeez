...
export const seoDashboardService = {
  async getDashboard() {
    ...
    let missingSeo = 0;
    let schemaErrors = 0;
    const productTraffic: Array<{ id: string; title: string; handle: string; score: number }> = [];
    ...
    for (const p of productRows.slice(0, 50)) {
      ...
      if (!hasTitle || !hasDesc || !hasSlug) missingSeo += 1;
      if (!hasTitle || !hasDesc) schemaErrors += 1;
      const score =
        (hasTitle ? 40 : 0) + (hasDesc ? 40 : 0) + (hasSlug ? 20 : 0);
