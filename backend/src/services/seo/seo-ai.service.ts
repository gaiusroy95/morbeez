...
export const seoAiService = {
  ...
  async completeJob(jobId: string, output: Record<string, unknown>, failed?: string) {
    await supabase
      .from('seo_ai_jobs')
      .update({
  ...
  async generateProductSeo(shopifyProductId: string, adminId?: string) {
    if (!env.OPENAI_API_KEY) throw new AppError('OpenAI not configured', 503, 'OPENAI_NOT_CONFIGURED');
    ...
    try {
      const result = await openaiJsonCompletion<{
        ...
        altTags: Array<{ description: string }>;
        internalLinkSuggestions: Array<{ anchorText: string; targetHint: string }>;
        schemaNotes: string;
        aiVisibilityTips: string;
      }>(
        SEO_SYSTEM,
        `Generate SEO for product: ${product.title}
...
schemaNotes, aiVisibilityTips`
  ...
  async generateCropProblemContent(
    input: { crop: string; problem: string; stage?: string; region?: string },
    adminId?: string
  ) {
    if (!env.OPENAI_API_KEY) throw new AppError('OpenAI not configured', 503, 'OPENAI_NOT_CONFIGURED');
    ...
    try {
      const result = await openaiJsonCompletion<{
        ...
        internalLinks: Array<{ anchorText: string; targetSlug: string }>;
        aiVisibilityStructure: string;
      }>(
        SEO_SYSTEM,
        `Write a long-form SEO agronomy article for:
Crop: ${input.crop}
...
Return JSON with metaTitle, metaDescription, bodyHtml (HTML), faqs (5+), focusKeywords, relatedProductHints, internalLinks, aiVisibilityStructure`
  ...
  async generateArticle(
    input: { topic: string; crop?: string; region?: string },
    adminId?: string
  ) {
    if (!env.OPENAI_API_KEY) throw new AppError('OpenAI not configured', 503, 'OPENAI_NOT_CONFIGURED');
    ...
    try {
      const result = await openaiJsonCompletion<{
        ...
        SEO_SYSTEM,
        `Write a comprehensive agronomy SEO article: ${input.topic}
