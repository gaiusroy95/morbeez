...
export const opportunityEmployeeLeaderboardsService = {
  async listTopRelationshipBuilders(limit = 25): Promise<SpecializedEmployeeRow[]> {
    const { data, error } = await supabase
      .from('employee_scores')
      .select(
        'employee_profile_id, performance_score, relationship_quality_score, attributed_farmer_count, calculated_at, employee_profiles(full_name, email, role)'
      )
  ...
  async listHighRetentionEmployees(limit = 25): Promise<SpecializedEmployeeRow[]> {
    const { data, error } = await supabase
      .from('employee_scores')
      .select(
        'employee_profile_id, performance_score, retention_quality_score, attributed_farmer_count, calculated_at, employee_profiles(full_name, email, role)'
      )
      .gte('attributed_farmer_count', MIN_ATTRIBUTED_FARMERS_FOR_LEADERBOARD)
