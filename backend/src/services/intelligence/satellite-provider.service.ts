...
export type SatelliteProvider = {
  name: string;
  fetchNdvi(input: SatelliteFetchInput): Promise<{ ndviMean: number; storageUrl?: string } | null>;
};

/** Reference stub — returns deterministic NDVI when no vendor keys configured. */
export const stubSatelliteProvider: SatelliteProvider = {
  name: 'stub',
  async fetchNdvi() {
...
export const sentinelHubProvider: SatelliteProvider = {
  name: 'sentinel',
  async fetchNdvi(input) {
    ...
    const statJson = (await statRes.json()) as { data?: Array<{ outputs?: { default?: { bands?: { B0?: { stats?: { mean?: number } } } } } }> };
...
export function getSatelliteProvider(): SatelliteProvider {
  ...
  if (name === 'sentinel' && process.env.SENTINEL_CLIENT_ID) return sentinelHubProvider;
  return stubSatelliteProvider;
}

export const satelliteProviderService = {
  async refreshBlockNdvi(blockId: string, farmerId: string, bbox?: SatelliteFetchInput['bbox']) {
