...
export const opportunityIntelligenceConfigService = {
  async get(): Promise<OpportunityIntelligenceConfig> {
    ...
    const farmerOverrides =
      (data?.farmer_weight_overrides as Partial<typeof FARMER_OPPORTUNITY_WEIGHTS>) ?? {};
    const employeeOverrides =
      (data?.employee_weight_overrides as Partial<typeof EMPLOYEE_PERFORMANCE_WEIGHTS>) ?? {};
    const rawThresholds = (data?.alert_thresholds as Partial<AlertThresholds>) ?? {};
  ...
  applyFarmerWeightOverrides(
    components: FarmerScoreComponents,
    weights: typeof FARMER_OPPORTUNITY_WEIGHTS
  ): FarmerScoreComponents {
    const keys: Array<keyof FarmerScoreComponents> = [
      'engagement',
      'trust',
      'acreSize',
      'acrePotential',
    ...
    const weightKeyMap: Record<keyof FarmerScoreComponents, keyof typeof FARMER_OPPORTUNITY_WEIGHTS> =
      {
  ...
  applyEmployeeWeightOverrides(
    components: EmployeeScoreComponents,
    weights: typeof EMPLOYEE_PERFORMANCE_WEIGHTS
  ): EmployeeScoreComponents {
    const keys: Array<keyof EmployeeScoreComponents> = [
      'engagementGrowth',
      'relationshipQuality',
      'retentionQuality',
      'trustBuilding',
    ...
    const weightKeyMap: Record<keyof EmployeeScoreComponents, keyof typeof EMPLOYEE_PERFORMANCE_WEIGHTS> =
      {
