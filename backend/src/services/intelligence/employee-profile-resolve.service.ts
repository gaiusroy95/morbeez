...
export const employeeProfileResolveService = {
  async byAdminUserId(adminUserId: string): Promise<string | null> {
  ...
  async resolve(input: {
    ...
    if (input.employeeProfileId) return input.employeeProfileId;
    if (input.adminUserId) return this.byAdminUserId(input.adminUserId);
    if (input.employeeEmail) return this.byEmail(input.employeeEmail);
    return null;
  },
};
