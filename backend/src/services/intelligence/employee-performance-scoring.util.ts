...
export function scoreEngagementGrowth(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
  ...
  const growth =
    signals.inboundEventsPrev30d > 0
...
export function scoreRelationshipQuality(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
...
export function scoreRetentionQuality(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
...
export function scoreTrustBuilding(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
...
export function scoreDelayedConversion(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
  ...
  return {
    score,
    factors:
      signals.conversionAssists180d > 0
        ? [
            {
...
export function scoreFarmerReactivation(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
  ...
  return {
    score,
    factors:
      signals.reactivations90d > 0
        ? [{ code: 'reactivation', label: 'Farmer reactivations credited', delta: score }]
        : [],
  };
}

export function scoreKnowledgeContribution(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
...
export function scoreFarmerSatisfaction(signals: EmployeePerformanceSignals): {
  score: number;
  factors: ScoreFactor[];
} {
  ...
  return {
    score,
    factors:
      signals.positiveOutcomes90d > 0
        ? [{ code: 'satisfaction', label: 'Positive recommendation outcomes', delta: score }]
        : [],
  };
}

export function computeEmployeeScoreComponents(
  signals: EmployeePerformanceSignals
): { components: EmployeeScoreComponents; factors: ScoreFactor[] } {
...
export function performanceBreakdownFromComponents(
  ...
  return [
    ...
    },
    {
      ...
      pct: Math.round(
        (components.relationshipQuality / EMPLOYEE_PERFORMANCE_WEIGHTS.relationshipQuality) * 100
      ),
    },
    {
      label: 'Retention quality',
      pct: Math.round(
        (components.retentionQuality / EMPLOYEE_PERFORMANCE_WEIGHTS.retentionQuality) * 100
      ),
    },
    {
    ...
    },
    {
      ...
      pct: Math.round(
        (components.delayedConversion / EMPLOYEE_PERFORMANCE_WEIGHTS.delayedConversion) * 100
      ),
    },
    {
      label: 'Farmer reactivation',
      pct: Math.round(
        (components.farmerReactivation / EMPLOYEE_PERFORMANCE_WEIGHTS.farmerReactivation) * 100
      ),
    },
    {
      label: 'Knowledge contribution',
      pct: Math.round(
        (components.knowledgeContribution / EMPLOYEE_PERFORMANCE_WEIGHTS.knowledgeContribution) * 100
      ),
    },
    {
      label: 'Farmer satisfaction',
      pct: Math.round(
        (components.farmerSatisfaction / EMPLOYEE_PERFORMANCE_WEIGHTS.farmerSatisfaction) * 100
      ),
    },
  ];
}
