...
function to100(points: number, max: number): number {
  if (max <= 0) return 0;
...
function acrePotentialDisplay100(components: FarmerScoreComponents): number {
...
export function buildFarmerMetrics100(
...
export function classifyFarmer(
  opportunityScore: number,
  metrics: MetricScore100[]
): string {
  const m = Object.fromEntries(metrics.map((x) => [x.key, x.score])) as Record<string, number>;
...
export function farmerBusinessInsight(
  classification: string,
  metrics: MetricScore100[],
  opportunityScore: number
): string {
...
export function detectFarmerSignalsFromEvents(events: FarmerEventRow[]): {
  positive: string[];
  negative: string[];
} {
  ...
  if (types.has('IMAGE_UPLOAD')) positive.push('Uploads crop images regularly');
  else negative.push('Rarely uploads crop images');

  if (types.has('ROI_ENTRY')) positive.push('Uses ROI tracker');
  else negative.push('No ROI tracking');

  if (inbound.length >= 8) positive.push('Active weekly on WhatsApp');
  else if (inbound.length >= 3) positive.push('Moderate WhatsApp replies');
  else negative.push('Rarely replies on WhatsApp');

  if (types.has('VOICE_NOTE')) positive.push('Uses voice notes for doubts');
  if (types.has('RECOMMENDATION_APPLIED')) positive.push('Follows recommendations');
  if (types.has('CALLBACK_REQUESTED')) positive.push('Requests callbacks');
...
export function buildFarmerScorePresentation(input: {
  ...
  const businessInsight = farmerBusinessInsight(
    classification,
    metrics,
...
export function buildEmployeeMetrics100(components: EmployeeScoreComponents): MetricScore100[] {
  return [
    {
...
export function classifyEmployee(performanceScore: number, metrics: MetricScore100[]): string {
...
export function employeeBusinessInsight(classification: string): string {
  switch (classification) {
...
export function buildEmployeeScorePresentation(score: EmployeeScoreSnapshot): EmployeeScorePresentation {
