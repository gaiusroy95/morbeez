...
import {
  ...
  type FarmerAdvisorySignals,
  type FarmerEngagementSignals,
  type FarmerProfileSignals,
  type FarmerReferralSignals,
  type FarmerRelationshipSignals,
  type FarmerTrustSignals,
} from './farmer-opportunity-scoring.util.js';
...
async function loadSignals(farmerId: string) {
  ...
  const farmerAcreage =
    farmerRes.data?.total_acreage != null ? Number(farmerRes.data.total_acreage) : null;
  const totalAcreage = farmerAcreage && farmerAcreage > 0 ? farmerAcreage : blockAcreSum > 0 ? blockAcreSum : null;

  const primaryBlock =
    blocks.find((b) => b.is_primary) ?? blocks[0];
  ...
  const profile: FarmerProfileSignals = {
    totalAcreage,
  ...
  const relationship: FarmerRelationshipSignals = {
  ...
  const advisory: FarmerAdvisorySignals = {
  ...
  const referral: FarmerReferralSignals = {
...
async function recordMetricSnapshots(
  farmerId: string,
  components: ReturnType<typeof computeFarmerScoreComponents>['components'],
  daysSinceLastInbound: number | null
): Promise<void> {
...
export const farmerOpportunityEngineService = {
  async scoreFarmer(farmerId: string): Promise<FarmerScoreSnapshot> {
    ...
    const components = opportunityIntelligenceConfigService.applyFarmerWeightOverrides(
      rawComponents,
      config.effectiveFarmerWeights
    );

    const snapshot = await opportunityScoreStoreService.upsertFarmerScore(
      farmerId,
      components,
      factors
    );
  ...
  async listFarmerIdsForBatch(opts?: { limit?: number; activityDays?: number }): Promise<string[]> {
  ...
  async runBatch(opts?: {
    ...
    dryRun?: boolean;
    farmerId?: string;
  }): Promise<{ scored: number; skipped: number; errors: number; dryRun: boolean }> {
