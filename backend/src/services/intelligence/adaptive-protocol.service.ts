...
export const adaptiveProtocolService = {
  ...
  async listRecentSuggestions(limit = 20): Promise<AdaptiveProtocolSuggestion[]> {
    ...
    const results: AdaptiveProtocolSuggestion[] = [];
    for (const row of data ?? []) {
      const farmerRaw = row.farmers as unknown;
      const farmer = (Array.isArray(farmerRaw) ? farmerRaw[0] : farmerRaw) as { district?: string } | null;
