...
export const cropAdvisorIntelligenceService = {
  async getWorkspaceIntelligence(agentEmail: string): Promise<CropAdvisorWorkspaceIntelligence> {
    ...
    let atRiskCount = 0;
    let churnedCount = 0;
    const priorityFarmers: CropAdvisorPriorityFarmer[] = [];
    ...
    const suggestedActions: CropAdvisorWorkspaceIntelligence['suggestedActions'] = [];
    if (atRiskCount > 0) {
