...
function isMissingTableError(err: unknown): boolean {
  ...
  return (
    msg.includes('employee_farmer_attribution') &&
...
async function resolveStaff(
  ...
  roleHint?: AttributionEmployeeRole
): Promise<{ profileId: string; role: AttributionEmployeeRole } | null> {
...
export const employeeAttributionCaptureService = {
  async upsertSafe(input: UpsertAttributionInput): Promise<void> {
    try {
      await employeeAttributionService.upsertTouch(input);
    } catch (err) {
      if (isMissingTableError(err)) {
        logger.warn(
          'employee_farmer_attribution table missing — run migration 20260637000000_opportunity_intelligence_phase0.sql'
        );
        return;
      }
      logger.warn(
        { err, farmerId: input.farmerId, type: input.attributionType },
        'Attribution capture failed'
      );
    }
  },
  ...
  async trackInboundEngagement(farmerId: string, assigneeEmail: string | null): Promise<void> {
    const email =
      assigneeEmail ??
  ...
  async trackReactivation(farmerId: string, employeeEmail: string | null): Promise<void> {
    ...
    if (!email) {
      ...
      if (lastOutbound?.channel === 'crm' && lastOutbound.content) {
        ...
        if (match?.[1]?.includes('@')) email = match[1].trim();
      }
    }
  ...
  async trackConversionForOrder(
    farmerId: string,
    metadata: { shopifyOrderId: string; orderName?: string | null; total?: string | number | null }
  ): Promise<void> {
    const rows = await employeeAttributionService.listEligibleForConversion(
      farmerId,
      ATTRIBUTION_CONVERSION_WINDOW_DAYS
    );

    const credited = new Set<string>();
    for (const row of rows) {
      if (!CONVERSION_ELIGIBLE_TYPES.includes(row.attributionType)) continue;
      if (credited.has(row.employeeProfileId)) continue;
    ...
    switch (params.eventType) {
      ...
      case 'ORDER_CONVERTED':
        await this.trackConversionForOrder(params.farmerId, {
          ...
          orderName: params.eventValue?.orderName as string | null | undefined,
          total: params.eventValue?.total as string | number | null | undefined,
        });
        break;
