...
export const agronomistIntelligenceService = {
  async getWorkspaceIntelligence(agentEmail: string): Promise<AgronomistWorkspaceIntelligence> {
    ...
    const focusFarmers: AgronomistWorkspaceIntelligence['focusFarmers'] = [];
    ...
    if (farmerIds.length > 0) {
      ...
      for (const fid of farmerIds) {
        ...
        if (s && s.opportunity >= 70) highOpportunityFarmers++;
