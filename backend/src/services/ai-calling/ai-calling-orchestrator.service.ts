...
function whatsappFallbackOn(): boolean {
  return env.AI_CALLING_WHATSAPP_FALLBACK !== false;
}

async function loadPrefs(farmerId: string) {
...
async function ensurePrefs(farmerId: string, farmerLang?: string | null) {
  ...
  const { data, error } = await supabase
    ...
    .select('*')
    .maybeSingle();
  if (error && error.code !== '23505') throwIfSupabaseError(error, 'Could not create call preferences');
  return (data as Record<string, unknown> | null) ?? (await loadPrefs(farmerId));
}

async function loadFarmer(farmerId: string) {
  const { data, error } = await supabase
    .from('farmers')
    .select(
      'id, phone, name, preferred_language, district, state, village, assigned_crop_advisor'
    )
    .eq('id', farmerId)
    .maybeSingle();
...
async function loadIdentities(): Promise<IdentityRow[]> {
  const { data, error } = await supabase
    .from('agronomist_call_identities')
    .select(
      'id, slot_number, agronomist_email, display_name, did_number, backup_identity_id, is_active'
    )
...
async function resolveIdentity(farmer: Record<string, unknown>, prefs: Record<string, unknown> | null) {
  const identities = await loadIdentities();
  const active = identities.filter((i) => i.is_active);
  const ownerEmail = String(
    farmer.assigned_crop_advisor ?? prefs?.assigned_agronomist_email ?? ''
  )
    .trim()
    .toLowerCase();
  ...
  const { data: least } = await supabase
    .from('agronomist_call_identities')
    .select(
      'id, slot_number, agronomist_email, display_name, did_number, backup_identity_id, is_active, last_assigned_at'
    )
    .eq('is_active', true)
  ...
  return (least as IdentityRow | null) ?? active[0] ?? null;
}

async function loadProtocols(): Promise<CropProtocolMatch[]> {
  const { data, error } = await supabase
    .from('crop_call_protocols')
    .select(
      'crop_type, stage_key, stage_label, dap_from, dap_to, prompt_kind, question_en, follow_up_hours_if_no, health_follow_up_days'
    )
    .eq('is_active', true);
  ...
  return (data ?? []).map((row) => ({
    ...
    promptKind: row.prompt_kind as CropProtocolMatch['promptKind'],
    ...
    healthFollowUpDays: Array.isArray(row.health_follow_up_days)
      ? (row.health_follow_up_days as number[])
      : [1, 3, 7],
  }));
}

function snapshotOutcome(parts: Record<string, unknown>): Record<string, unknown> {
  return {
    ...parts,
...
export const aiCallingOrchestrator = {
  ...
  async enqueue(input: EnqueueInput): Promise<{ id: string; reused: boolean } | null> {
    ...
    if (error?.code === '23505') {
      const { data: again } = await supabase
        ...
        .in('status', ['pending', 'calling', 'awaiting_reply', 'queued_for_agent'])
        .maybeSingle();
      if (again?.id) return { id: String(again.id), reused: true };
    }
  ...
  async processJob(jobId: string): Promise<void> {
    ...
    if (!job) return;
    if (!['pending', 'skipped_window'].includes(String(job.status))) return;
    ...
    const language = normalizeCallLanguage(
      String(job.language ?? prefs?.preferred_language ?? farmer.preferred_language ?? 'en')
    );
  ...
  async openStaffScriptSession(params: {
    ...
    const language = normalizeCallLanguage(
      String(params.job.language ?? params.prefs?.preferred_language ?? 'en')
    );
    const script = await this.scriptForJob(params.job, language);
  ...
  async scriptForJob(job: Record<string, unknown>, language: CallLanguage) {
    const payload = (job.payload ?? {}) as Record<string, unknown>;
    const type = String(job.call_type) as CallType;
    const stageQuestion =
      typeof payload.stageQuestion === 'string' ? payload.stageQuestion : null;
  ...
  async tryConsumeInboundReply(farmerId: string, text: string): Promise<{ handled: boolean; reply?: string }> {
    if (!enabled() || !text.trim()) return { handled: false };
  ...
  async applyFarmerReply(params: {
  ...
  }): Promise<{ farmerReply: string; intent: FarmerIntent; actionKind: string }> {
    const { data: session, error } = await supabase
      .from('ai_call_sessions')
      .select('*')
    ...
    if (!session) throw new ValidationError('Calling session not found');
    ...
    const { data: job } = await supabase
      ...
      .maybeSingle();
    const callType = String(job?.call_type ?? session.call_type) as CallType;
    const payload = (job?.payload ?? {}) as Record<string, unknown>;
  ...
  async escalate(params: {
    ...
    let status: 'open' | 'assigned' | 'callback_queue' = 'open';
    let email = assigned?.agronomist_email ?? null;
    let backupId = backup?.id ?? null;
    let ladder = params.ladder;

    if (ladder === 'assigned' && !email) ladder = 'backup';
    if (ladder === 'backup' && !backup) ladder = 'queue';
    if (ladder === 'queue') {
  ...
  async optOut(farmerId: string, reason: string): Promise<void> {
  ...
  async setConsent(farmerId: string, patch: {
    ...
    if (patch.consentOutboundCall !== undefined) updates.consent_outbound_call = patch.consentOutboundCall;
    if (patch.consentWhatsapp !== undefined) updates.consent_whatsapp = patch.consentWhatsapp;
    if (patch.dnd !== undefined) {
      updates.dnd = patch.dnd;
      if (patch.dnd) updates.opted_out_at = new Date().toISOString();
      else {
    ...
    if (patch.bestTimeStart !== undefined) updates.best_time_start = patch.bestTimeStart;
    if (patch.bestTimeEnd !== undefined) updates.best_time_end = patch.bestTimeEnd;
  ...
  async listConsole() {
    const [jobs, sessions, identities, escalations] = await Promise.all([
      supabase
        .from('ai_call_jobs')
        .select(
          'id, farmer_id, call_type, status, scheduled_at, assigned_agronomist_email, language, attempts, last_error, created_at, payload'
        )
      ...
      supabase
        .from('ai_call_sessions')
        .select(
          'id, job_id, farmer_id, call_type, channel, status, language_used, farmer_intent, outcome, summary, started_at, ended_at'
        )
        .order('started_at', { ascending: false })
        .limit(80),
      supabase
        .from('agronomist_call_identities')
        .select(
          'id, slot_number, agronomist_email, display_name, did_number, backup_identity_id, is_active, last_assigned_at, notes'
        )
        .order('slot_number', { ascending: true }),
      supabase
        .from('ai_call_escalations')
        .select(
          'id, farmer_id, assigned_agronomist_email, status, reason, priority, created_at'
        )
    ...
    const farmerIds = [
      ...new Set(
        [...(jobs.data ?? []), ...(sessions.data ?? []), ...(escalations.data ?? [])]
          .map((r) => String((r as { farmer_id?: string }).farmer_id ?? ''))
          .filter(Boolean)
      ),
    ];
    const farmers =
      farmerIds.length === 0
        ? []
        : (
            await supabase.from('farmers').select('id, name, phone, district').in('id', farmerIds)
          ).data ?? [];
    const farmerMap = new Map(farmers.map((f) => [String(f.id), f]));

    const decorate = <T extends { farmer_id?: string }>(row: T) => {
    ...
    const pendingJobs = (jobs.data ?? []).filter((j) =>
      ['pending', 'calling', 'awaiting_reply', 'queued_for_agent', 'skipped_window'].includes(
        String(j.status)
      )
    ).length;
  ...
  async resolveCropDueJob(farmerId: string, cropType: string, plantingDateIso: string) {
