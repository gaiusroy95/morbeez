...
export const agronomistCopilotService = {
  async ask(input: CopilotAskInput): Promise<{ answer: string; citations: string[] }> {
    ...
    if (input.farmerId && input.blockId) {
      ...
      const flags = await regionalThreatRadarService.riskFlagsForFarmer(
        input.farmerId,
        input.cropType ?? 'ginger'
      );
      if (flags.length) {
    ...
    if (input.issueName) {
      ...
      const similar = input.farmerId
        ? await visitAiOrchestratorService.similarCases(
            input.farmerId,
            input.cropType ?? 'ginger',
            input.issueName
          )
    ...
    const result = await openaiJsonCompletion<{ answer: string }>(
      'You are Morbeez Agronomist Copilot. Answer ONLY from provided evidence. Never invent diagnosis labels. Cite what evidence supports the answer. If evidence is insufficient, say so and recommend escalation.',
  ...
  async whyDiagnosis(aiCaseId: string): Promise<{ answer: string; citations: string[] }> {
