...
import type {
  ...
  DiagnosisV17SessionMeta,
} from '../../domain/diagnosis/v17-session.types.js';
...
function sessionMeta(raw: unknown): DiagnosisV17SessionMeta | null {
  if (!raw || typeof raw !== 'object') return null;
  const v17 = (raw as Record<string, unknown>).v17Diagnosis;
  if (!v17 || typeof v17 !== 'object') return null;
  const m = v17 as DiagnosisV17SessionMeta;
  return m.version === '17.0' && m.lastSnapshot ? m : null;
}

function seedHypotheses(cropType: string, suspectedIssue?: string): MaiosHypothesis[] {
...
function photosFromCount(count: number, pack: Awaited<ReturnType<typeof cropPackLoaderService.load>>): MaiosPhotoEvidence[] {
  if (count <= 0) return [];
...
function estimateEqs(photoCount: number, answerCount: number): number {
  return Math.min(88, 40 + photoCount * 14 + answerCount * 6);
}

async function resolveFarmerId(input: { farmerId?: string; phone?: string; name?: string; language?: string }) {
  if (input.farmerId) return input.farmerId;
  if (!input.phone) throw new ValidationError('Provide farmerId or phone');
...
async function runPipeline(meta: Omit<DiagnosisV17SessionMeta, 'lastSnapshot'>): Promise<DiagnosisV17SessionMeta> {
  ...
  const snapshot = await maiosReasoningPipelineService.run({
    ...
    visionObservations: meta.visionLabel
      ? visitVisionObservationsService.inferFromLabel(
          meta.visionLabel,
          meta.visionConfidence ?? 0.75
        )
      : undefined,
...
function toStartResponse(sessionId: string, meta: DiagnosisV17SessionMeta): DiagnosisStartResponse {
...
export const diagnosisV17Service = {
  async start(input: DiagnosisStartInput): Promise<DiagnosisStartResponse> {
    ...
    const baseMeta: Omit<DiagnosisV17SessionMeta, 'lastSnapshot'> = {
      version: '17.0',
  ...
  async submitAnswers(sessionId: string, input: DiagnosisAnswerInput): Promise<DiagnosisStartResponse> {
    const { data: session, error } = await supabase
      .from('ai_advisory_sessions')
    ...
    if (error) throw error;
    if (!session) throw new NotFoundError('Diagnosis session not found');

    const existing = sessionMeta(session.metadata);
    if (!existing) throw new ValidationError('Session is not a v17 diagnosis session');

    const answered = new Set(existing.answeredQuestionIds);
    for (const ans of input.answers) {
      if (ans.questionId) answered.add(ans.questionId);
    }
    ...
    await supabase
      ...
      .update({
        status: meta.lastSnapshot.decision.action === 'LOCK' ? 'completed' : 'processing',
        metadata: { ...(session.metadata as Record<string, unknown>), v17Diagnosis: meta },
  ...
  async getReport(sessionId: string): Promise<DiagnosisReportResponse> {
    const { data: session, error } = await supabase
      .from('ai_advisory_sessions')
      .select('id, metadata')
      .eq('id', sessionId)
