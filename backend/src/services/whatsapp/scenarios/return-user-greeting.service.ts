...
function cropDisplay(cropType: string): string {
  return cropType.charAt(0).toUpperCase() + cropType.slice(1);
}

function issuePrompt(issue: string, lang: AdvisoryLanguage): string {
...
function cropPrompt(ctx: GreetingContext, lang: AdvisoryLanguage): string {
  const crop = cropDisplay(ctx.cropType);
  const cropKey = ctx.cropType.toLowerCase();

  if (cropKey === 'cardamom') {
    const cardamomMap: Record<AdvisoryLanguage, string> = {
...
function salutation(name: string | undefined, lang: AdvisoryLanguage): string {
  const first = name?.trim().split(/\s+/)[0];
  const base: Record<AdvisoryLanguage, string> = {
    en: 'Hi',
    ml: 'ഹായ്',
    ta: 'ஹாய்',
    kn: 'ಹಾಯ್',
...
export const returnUserGreetingService = {
  async buildSmartGreeting(
    farmerId: string,
    language: AdvisoryLanguage
  ): Promise<{ greeting: string; includeTrackOrder: boolean; optionsIntro: string } | null> {
    ...
    if (!hasHistory || !hasCropContext) return null;
