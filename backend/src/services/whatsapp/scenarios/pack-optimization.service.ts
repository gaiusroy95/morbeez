...
export const packOptimizationService = {
  async optimizeQuantity(productKey: string, requiredKg: number): Promise<{
    requiredKg: number;
    assignedKg: number;
    packs: Array<{ packKg: number; count: number }>;
  }> {
    ...
    const result: Array<{ packKg: number; count: number }> = [];
  ...
  formatPacks(packs: Array<{ packKg: number; count: number }>): string {
