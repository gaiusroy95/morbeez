...
function dayLabel(isoDate: string, index: number, lang: AdvisoryLanguage): string {
  if (index === 0) {
    const today: Record<AdvisoryLanguage, string> = {
      en: 'Today',
      ml: 'ഇന്ന്',
      ta: 'இன்று',
      kn: 'ಇಂದು',
  ...
  if (index === 1) {
    const tomorrow: Record<AdvisoryLanguage, string> = {
      en: 'Tomorrow',
      ml: 'നാളെ',
      ta: 'நாளை',
      kn: 'ನಾಳೆ',
...
async function fetchSnapshot(farmerId: string, language: AdvisoryLanguage): Promise<WeatherSnapshot | null> {
...
function snapshotFacts(snapshot: WeatherSnapshot): string {
...
function compactFallback(
  ...
  if (!today) {
    const unavailable: Record<AdvisoryLanguage, string> = {
  ...
  const rainAlert =
    snapshot.heavyRainLikely && language === 'ml'
    ...
    try {
      ...
      if (!snapshot) {
        return compactFallback(
          { locationLabel: 'your area', days: [], heavyRainLikely: false, highHeatLikely: false },
          language,
          ctx.cropType
        );
      }
