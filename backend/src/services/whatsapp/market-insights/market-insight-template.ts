...
function chartPaths(payload: MarketInsightPayload): { cy: string; py: string; yMax: number } {
  const pts = payload.chart.points;
  const values = pts.flatMap((p) => [p.currentYear, p.previousYear].filter((v): v is number => v != null));
  ...
  const toX = (i: number) => left + (i / 11) * w;
  const toY = (v: number) => top + h - (v / yMax) * h;

  const cyParts: string[] = [];
...
export function renderMarketInsightSvg(payload: MarketInsightPayload): string {
