...
async function buildFromManualPrices(
  farmerId: string,
  insightDate: string,
  ...
  const crops = (fallbackProfile.crop_cards as string[]) ?? ['ginger'];
...
export const marketInsightDataService = {
  ...
  async buildForFarmer(farmerId: string, insightDate?: string): Promise<MarketInsightBuildResult> {
    ...
    if (!region) {
      return {
        ok: false,
        error:
          'Farmer region requires a 6-digit PIN (pincode_id or delivery_pincode in pincode_master)',
      };
    }
