...
function yoyPct(current: number, lastYear: number | null | undefined): number | null {
...
function conditionFromRain(rainMm: number, heavyRainLikely: boolean): string {
  if (heavyRainLikely || rainMm >= 15) return 'Heavy Rain';
  if (rainMm >= 5) return 'Moderate Rain';
  if (rainMm >= 1) return 'Light Rain';
  return 'Partly Cloudy';
}

function toCropCards(raw: z.infer<typeof aiResponseSchema>['cropCards']): MarketInsightCropCard[] {
  return raw.map((c) => {
    const cropType = String(c.cropType).toLowerCase();
    const lastYear =
      c.lastYearPricePerKg != null ? Number(c.lastYearPricePerKg) : null;
...
function defaultChartPoints(): MarketInsightChartPoint[] {
...
export const marketInsightAiService = {
  async fetchVerifiedWeather(region: FarmerPincodeRegion) {
  ...
  async getCached(pincode: string, insightDate: string): Promise<MarketInsightPayload | null> {
    ...
    return data.payload as MarketInsightPayload;
  },
  ...
  async fetchForPincode(
    region: FarmerPincodeRegion,
    insightDate: string
  ): Promise<MarketInsightPayload | null> {
