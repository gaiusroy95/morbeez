...
export async function createCropAdvisorTask(params: {
  farmerId: string;
  title: string;
  notes?: string;
  ...
  if (effectivePriority === 'normal') {
    try {
      ...
      if (farmerHealthScoreService.cropAdvisorPriorityFromHealth(health.band) === 'high') {
        effectivePriority = 'high';
  ...
  const healthNote =
    boostReason && effectivePriority !== (params.priority ?? 'normal')
