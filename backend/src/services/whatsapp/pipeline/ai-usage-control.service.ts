...
export type UsageCheckResult =
  | { allowed: true }
  | { allowed: false; reason: 'rate_limit' | 'daily_text' | 'daily_image' | 'daily_voice' | 'voice_too_long' };

function todayDate(): string {
  return new Date().toISOString().slice(0, 10);
}

async function getOrCreateUsage(farmerId: string) {
...
export const aiUsageControlService = {
  async checkAndConsume(params: {
    ...
    if (params.kind === 'text') patch.text_queries = row.text_queries + 1;
    if (params.kind === 'image') patch.image_queries = row.image_queries + 1;
    if (params.kind === 'voice') {
  ...
  async recordBlocked(farmerId: string): Promise<void> {
