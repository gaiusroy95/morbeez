...
export const CALCIUM_NITRATE_TANK_MIX_RULES: TankMixRule[] = [
...
export const CALCIUM_NITRATE_MIX_WARNING =
  'Never mix Calcium Nitrate + Magnesium Sulphate + MKP or Phosphonic Acid in one tank — causes precipitation and clogging.';

export function normalizeTankMixToken(raw: string): string {
  const key = raw
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
...
export function isCalciumNitrateProduct(name: string): boolean {
...
export function lookupCalciumNitratePair(
  productA: string,
  productB: string
): {
  found: boolean;
...
export function calciumNitrateRulesForDb(): Array<{
  product_a: string;
  product_b: string;
  compatible: boolean;
  notes: string | null;
