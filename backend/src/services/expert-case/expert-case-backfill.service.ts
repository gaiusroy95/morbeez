...
export const expertCaseBackfillService = {
  async backfillOpenEscalations(limit = 200): Promise<{
    scanned: number;
    created: number;
    merged: number;
    skipped: number;
    ...
    for (const row of rows ?? []) {
      ...
      try {
        const result = await expertCaseLifecycleService.ensureFromAdvisory({
          ...
          confidence:
            row.confidence_at_escalation != null ? Number(row.confidence_at_escalation) : null,
  ...
  async reconcileLinkedEscalations(limit = 500): Promise<{ checked: number; repaired: number }> {
    if (!expertCaseLifecycleService.enabled()) return { checked: 0, repaired: 0 };
    ...
    for (const link of links ?? []) {
      const { data: esc } = await supabase
        ...
        .maybeSingle();
      if (!esc) continue;
      if (esc.expert_case_id === link.case_id) continue;
      await supabase
        .from('agronomist_escalations')
        .update({
