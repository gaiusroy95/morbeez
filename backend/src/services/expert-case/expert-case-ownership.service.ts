...
export const expertCaseOwnershipService = {
  ...
  async ensureCapacity(email: string) {
  ...
  async claim(params: {
    ...
    ownerEmail: string;
    reason?: string;
  }): Promise<{ caseId: string; leaseToken: string; leaseExpiresAt: string }> {
    if (!this.enabled()) {
    ...
    const { data: row } = await supabase
      ...
      .eq('id', params.caseId)
      .maybeSingle();
    if (!row) throw new NotFoundError('Expert case not found');
    ...
    const { data: updated, error } = await supabase
      ...
      .maybeSingle();
    if (error) throw error;
    if (!updated) throw new ConflictError('Case claim lost a race — retry');
  ...
  async renewLease(params: {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) throw error;
    if (!data) throw new ConflictError('Lease renew failed — ownership changed');
    await supabase.from('expert_case_ownership_events').insert({
      case_id: params.caseId,
      event_type: 'renewed',
      to_owner_email: email,
  ...
  async release(params: {
    ...
    const interruptionCount =
      Number(row.interruption_count ?? 0) + (params.countInterruption ? 1 : 0);
    const intervention =
      params.countInterruption && interruptionCount >= env.EXPERT_CASE_INTERRUPTION_LIMIT;
  ...
  async reaperExpiredLeases(limit = 20): Promise<number> {
    if (!env.ENABLE_EXPERT_COPILOT_LEASE_REAPER) return 0;
