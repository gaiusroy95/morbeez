...
export const maiosReasoningPipelineService = {
  ...
  async run(input: MaiosReasoningPipelineInput): Promise<MaiosReasoningSnapshot | null> {
    ...
    const nextPhoto =
      !nextQuestion || nextQuestion.expectedInformationGain < 8
        ? maiosEvsiEngineService.rankMissingPhoto({ missingSlots, posterior })
        : null;

    const nextEvidence =
      nextQuestion && (!nextPhoto || nextQuestion.expectedInformationGain >= nextPhoto.expectedInformationGain)
        ? nextQuestion
        : nextPhoto;
  ...
  enrichHypotheses(
    ...
    if (snapshot.shadowMode) return hypotheses;
