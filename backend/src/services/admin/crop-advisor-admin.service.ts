...
export interface CropAdvisorListQuery {
  scope?: 'mine' | 'all';
  stage?: string;
  search?: string;
  page?: number;
...
/** CropAdvisor "mine" = assigned to agent OR unassigned (WhatsApp / inbound pool). */
function applyCropAdvisorLeadScope<T extends { or: (f: string) => T; eq: (c: string, v: string) => T }>(
  builder: T,
...
function isDueTodayIso(iso: string | null | undefined): boolean {
  if (!iso) return false;
...
function mapLeadRow(row: Record<string, unknown>) {
  ...
  const stage = String(row.stage ?? 'new_lead') as LeadStage;
  return {
    id: row.id,
    ...
    followUpLabel: formatDateTime(row.follow_up_at as string),
...
async function touchLead(leadId: string, _farmerId: string) {
...
export const cropAdvisorAdminService = {
  stageLabels: STAGE_LABELS,

  async getOverview(agentEmail: string) {
    ...
    const conversionRate =
      (myLeads ?? 0) > 0 ? Math.round(((ordersGenerated / (myLeads ?? 1)) * 100) * 10) / 10 : 0;
  ...
  async listLeads(query: CropAdvisorListQuery, agentEmail: string) {
    ...
    builder = applyCropAdvisorLeadScope(builder, query.scope, agentEmail);
    ...
    let rows = (data ?? []).map((r) => mapLeadRow(r as Record<string, unknown>));
    ...
    const { cropAdvisorIntelligenceService } = await import(
      '../intelligence/crop-advisor-intelligence.service.js'
    );
    const enriched = await cropAdvisorIntelligenceService.enrichLeadRows(rows);
  ...
  async getLeadDetail(leadId: string) {
    ...
    const farmer = data.farmers as Record<string, unknown>;
    ...
    const orders = (ordersRes.data ?? []).filter(
      (o) => normalizePhone(String(o.phone)) === phoneKey
    );
    ...
    for (const c of callsRes.data ?? []) {
      timeline.push({
        ...
        atLabel: formatDateTime(c.created_at as string) ?? '',
      });
    }
    ...
    for (const i of interactionsRes.data ?? []) {
      timeline.push({
        ...
        atLabel: formatDateTime(i.created_at as string) ?? '',
      });
    }
    ...
    return {
      lead: {
        ...
        assignedPartnerName,
        ownership:
          ownership?.customerOwnerType === 'partner'
            ? 'Partner'
      ...
      farmOverview: {
        ...
        blocks: blocks.map((b) => ({
          ...
          name: String((b as { name?: string }).name ?? (b as { plot_label?: string }).plot_label ?? 'Block'),
      ...
      nextFollowUp: nextTask
        ? {
            ...
            dueLabel: formatDateTime(nextTask.due_at as string),
        ...
        : data.follow_up_at
          ? {
              ...
              dueLabel: formatDateTime(data.follow_up_at as string),
              notes: data.notes,
            }
          : null,
      ...
      orders: orders.slice(0, 5).map((o) => ({
        ...
        date: formatDateTime(o.created_at as string),
      })),
      stages: Object.entries(STAGE_LABELS).map(([id, label]) => ({
        id,
        label,
        active: id === lead.stage,
        done: this.stageIndex(id as LeadStage) < this.stageIndex(lead.stage),
      })),
    };
  },
  ...
  async createLead(
    input: {
      ...
      roiEnabled?: boolean;
      farmerNotes?: string;
      preferredMarkets?: Array<{ marketKey: string; cropType?: string }>;
      cropBlocks?: Array<{
        blockName?: string;
        cropName: string;
        acreage?: number;
    ...
    const { cropAdvisorFarmerProfileService } = await import('./crop-advisor-farmer-profile.service.js');
    await cropAdvisorFarmerProfileService.applyProfileOnCreate(result.farmer.id, {
      name: input.name,
      ...
      await whatsappOsAdminService.saveFarmerMarketPreferences(
        {
          ...
          markets: input.preferredMarkets
            ...
            .filter((m): m is { marketName: string; district: string | null } => Boolean(m)),
        }
      );
    }
  ...
  async updateLead(
    ...
    if (patch.stage) {
      await logInteraction(
        data.farmer_id as string,
        'crm',
        `Lead stage updated to ${STAGE_LABELS[patch.stage as LeadStage] ?? patch.stage} by ${agentEmail}`
      );
    }

    if (patch.assignedTo !== undefined) {
      const { farmerOwnershipService } = await import('../partner/farmer-ownership.service.js');
      void farmerOwnershipService.syncCropAdvisorAssignment(
        data.farmer_id as string,
    ...
    if (patch.assignedTo) {
      ...
      void farmerEventCaptureService.trackLeadAssignment(
        data.farmer_id as string,
        patch.assignedTo
      );
    }

    await touchLead(leadId, data.farmer_id as string);
    return this.getLeadDetail(leadId);
  },

  async listLeadNotes(leadId: string) {
    ...
    const rows = (data ?? []).map((n) => {
      ...
      return {
        ...
        note: body,
        author: String(n.author ?? 'Crop Advisor'),
        createdLabel: formatDateTime(n.created_at as string) ?? '—',
  ...
  async getLeadNote(leadId: string, noteId: string) {
    const { data: lead } = await supabase.from('leads').select('farmer_id, notes').eq('id', leadId).single();
    if (!lead) throw new NotFoundError('Lead not found');
  ...
  async updateLeadNote(leadId: string, noteId: string, note: string, agentEmail: string) {
    if (noteId.startsWith('legacy-')) {
    ...
    await logInteraction(lead.farmer_id as string, 'note', `${agentEmail} updated note`);
    await touchLead(leadId, lead.farmer_id as string);
    return data;
  },

  async addNote(leadId: string, note: string, agentEmail: string) {
    ...
    const { error: noteError } = await supabase.from('crop_advisor_notes').insert({
    ...
    await logInteraction(data.farmer_id as string, 'note', `${agentEmail}: ${note}`);
    await touchLead(leadId, data.farmer_id as string);
    return this.getLeadDetail(leadId);
  },
  ...
  async createTask(
    ...
    const isPartnerVisit =
      (input.taskCategory === 'visit_request' || input.taskType === 'visit') &&
    ...
    const insertRow: Record<string, unknown> = {
      ...
      assigned_to_role: isPartnerVisit ? 'partner' : agronomistEmail ? 'agronomist' : 'crop_advisor',
      title: input.title,
      notes,
  ...
  async listLeadAgronomistTasks(leadId: string) {
    const { data: lead } = await supabase.from('leads').select('farmer_id').eq('id', leadId).single();
    if (!lead) throw new NotFoundError('Lead not found');
  ...
  async listTasksForAgronomist(agronomistEmail: string, opts?: { status?: string; limit?: number }) {
    ...
    else if (opts?.status === 'done') query = query.eq('status', 'done');
  ...
  async listScheduledVisitsForAgronomist(agronomistEmail: string, opts?: { limit?: number }) {
    const email = agronomistEmail.trim().toLowerCase();
    const { data, error } = await supabase
      .from('crm_tasks')
      .select(
        `id, title, notes, status, due_at, farmer_id, lead_id, block_id,
  ...
  async addTaskComment(
    taskId: string,
    input: { body: string; authorEmail: string; authorRole: 'crop_advisor' | 'agronomist' | 'system'; authorName?: string }
  ) {
    const { data, error } = await supabase
      .from('crm_task_comments')
      .insert({
    ...
    const prefix = input.authorRole === 'agronomist' ? 'Agronomist' : 'Crop Advisor';
  ...
  async logCall(
    leadId: string,
    input: { outcome?: string; notes?: string; durationSeconds?: number },
    agentEmail: string
  ) {
    const { data: lead } = await supabase.from('leads').select('farmer_id').eq('id', leadId).single();
    if (!lead) throw new NotFoundError('Lead not found');
  ...
  async listLeadPendingTasks(leadId: string): Promise<LeadPendingWorkItem[]> {
    ...
    if (leadErr || !lead) throw new NotFoundError('Lead not found');

    const farmerId = String(lead.farmer_id);
    const farmersRel = lead.farmers as Record<string, unknown> | Record<string, unknown>[] | null;
    ...
    const items: LeadPendingWorkItem[] = [];
    const seenInteractionIds = new Set<string>();
    const crmTaskIds = new Set((tasksRes.data ?? []).map((t) => String(t.id)));

    const partnerIds = new Set<string>();
    for (const t of tasksRes.data ?? []) {
      if (t.assigned_partner_id) partnerIds.add(String(t.assigned_partner_id));
    }
    ...
    for (const e of escRows) {
      if (e.workflowStatus === 'completed') continue;
      items.push({
        id: `esc-${e.id}`,
        itemType: 'escalation',
        category: 'Escalation',
    ...
    for (const r of aiRes.data ?? []) {
      items.push({
        ...
        dueLabel: formatDateTime(r.created_at as string) ?? '—',
        isDueToday: false,
        farmerName,
    ...
    for (const r of crmRecRes.data ?? []) {
      ...
      items.push({
        ...
        dueAt,
        dueLabel: formatDateTime(dueAt) ?? formatDateTime(r.created_at as string) ?? '—',
        isDueToday: isDueTodayIso(dueAt),
        farmerName,
        status: 'pending',
        statusLabel: 'Pending',
    ...
    for (const ix of interactionsPack.interactions ?? []) {
      if (ix.completionStatus !== 'pending') continue;
      const ixId = String(ix.id);
      if (seenInteractionIds.has(ixId)) continue;
      seenInteractionIds.add(ixId);
      if (ix.taskId && crmTaskIds.has(String(ix.taskId))) continue;
  ...
  async listTasks(agentEmail: string, status = 'pending') {
    const { data, error } = await supabase
      .from('crm_tasks')
    ...
    return (data ?? []).map((t) => {
      const farmer = t.farmers as Record<string, unknown> | null;
      const name = farmer ? displayFarmerName(farmer) : 'Farmer';
      return {
        id: t.id,
        title: t.title,
        dueLabel: formatDateTime(t.due_at as string),
        isDueToday: isDueToday(t.due_at as string),
        status: t.status,
        farmerName: name,
        phone: farmer?.phone,
        leadId: t.lead_id,
        stage: (t.leads as { stage?: string } | null)?.stage,
      };
    });
  },

  async listCalls(agentEmail?: string, limit = 50) {
    let q = supabase
      .from('crm_call_logs')
    ...
    return (data ?? []).map((c) => {
      const farmer = c.farmers as Record<string, unknown> | null;
      return {
        id: c.id,
        farmerName: farmer ? displayFarmerName(farmer) : 'Farmer',
        phone: farmer?.phone,
  ...
  async listWhatsAppThreads(limit = 30) {
    ...
    const byFarmer = new Map<string, Record<string, unknown>>();
    for (const row of data ?? []) {
      const fid = String(row.farmer_id);
      if (!byFarmer.has(fid)) {
        const rawFarmer = row.farmers;
        const farmer = (Array.isArray(rawFarmer) ? rawFarmer[0] : rawFarmer) as
        ...
        byFarmer.set(fid, {
          ...
          lastAt: formatDateTime(row.created_at as string),
  ...
  async getWhatsAppMessages(farmerId: string) {
    const { data, error } = await supabase
      .from('interaction_logs')
    ...
    return (data ?? []).map((m) => ({
      ...
      content: m.content,
      atLabel: formatDateTime(m.created_at as string),
      createdAt: m.created_at,
    }));
  },

  async sendWhatsAppMessage(farmerId: string, text: string, agentEmail: string) {
    ...
    if (error || !farmer?.phone) throw new NotFoundError('Farmer phone not found');
    ...
    await logInteraction(
      farmerId,
      'whatsapp',
      `${sent ? 'Sent' : 'Queued'} by ${agentEmail}: ${text.slice(0, 80)}`
    );

    return { messages: await this.getWhatsAppMessages(farmerId), sent };
  },

  async listFieldFindings(farmerId: string, page = 1, limit = 10) {
    const from = (page - 1) * limit;
    const to = from + limit - 1;
  ...
  mapFieldFinding(r: Record<string, unknown>) {
    const params = (r.parameters as { label: string; value: string }[]) ?? [];
  ...
  async getFieldFinding(farmerId: string, findingId: string) {
    const { data, error } = await supabase
      .from('crm_field_findings')
  ...
  async createFieldFinding(
    ...
    input: {
      ...
      diseaseTone?: string;
      actionTaken?: string;
      parameters?: Array<{ label: string; value: string; key?: string }>;
      photoUrls?: string[];
