...
export const staffInviteService = {
  async ensurePendingAdminUser(employeeProfileId: string): Promise<string> {
    ...
    if (profile.admin_user_id) {
      ...
      if (adminRow?.email_verified_at) return String(adminRow.id);
    ...
    if (!created?.id) throw new ValidationError('Could not create pending admin account');
  ...
  async createInvite(input: { employeeProfileId: string; createdBy?: string }) {
  ...
  async previewToken(rawToken: string) {
    ...
    if (!data) throw new ValidationError('Invalid or expired invitation link');
    if (data.used_at) throw new ValidationError('This invitation link has already been used');
  ...
  async completeInvite(input: {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    throwIfSupabaseError(error, 'Could not validate invite token');
    if (!data) throw new ValidationError('Invalid invitation link');
    if (data.used_at) throw new ValidationError('Invitation link already used');
    if (new Date(String(data.expires_at)).getTime() < Date.now()) {
