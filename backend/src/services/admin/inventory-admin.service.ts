  ...
  async list(query: InventoryListQuery) {
    ...
    for (const product of catalog) {
      ...
      for (const v of variants) {
        ...
        const status = stockStatus(stock);
        const variantLabel =
          v.option1 || `${v.packSize || ''} ${v.unit || ''}`.trim() || v.title || 'Default';
    ...
    const productIdsLow = new Set(
      allRows.filter((r) => r.status === 'low_stock').map((r) => r.productId)
    );
    const productIdsOut = new Set(
      allRows.filter((r) => r.status === 'out_of_stock').map((r) => r.productId)
    );
