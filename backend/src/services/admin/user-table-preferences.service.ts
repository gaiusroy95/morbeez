...
function mapRow(row: Record<string, unknown>): UserTablePreferencesRow {
  return {
    userEmail: String(row.user_email),
    tableName: String(row.table_name),
    viewName: String(row.view_name),
    visibleColumns: Array.isArray(row.visible_columns) ? (row.visible_columns as string[]) : [],
    columnOrder: Array.isArray(row.column_order) ? (row.column_order as string[]) : [],
    columnWidths:
      row.column_widths && typeof row.column_widths === 'object'
        ? (row.column_widths as Record<string, number>)
        : {},
    filterState:
      row.filter_state && typeof row.filter_state === 'object'
        ? (row.filter_state as Record<string, unknown>)
        : {},
...
export const userTablePreferencesService = {
  async get(userEmail: string, tableName: string, viewName = 'active') {
  ...
  async listViews(userEmail: string, tableName: string) {
    const { data, error } = await supabase
      .from('user_table_preferences')
  ...
  async deleteView(userEmail: string, tableName: string, viewName: string) {
    if (viewName === 'active') {
