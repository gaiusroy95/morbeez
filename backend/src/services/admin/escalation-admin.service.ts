...
function farmerDisplayName(row: Record<string, unknown> | null): string {
...
function dbStatusFromWorkflow(workflow: EscalationWorkflowStatus): string {
  if (workflow === 'completed') return 'resolved';
  if (workflow === 'agronomist_review') return 'in_review';
  return 'pending';
}
...
export const escalationAdminService = {
  ...
  async listEscalationComments(escalationId: string) {
    ...
    return (data ?? []).map((c) => {
      ...
      return {
        ...
        body: parsed.body,
        createdLabel: formatDt(c.created_at as string) ?? '—',
  ...
  async getById(id: string) {
    const { data, error } = await supabase
      .from('agronomist_escalations')
      .select(
        `*, farmers(*), ai_advisory_sessions(*, ai_advisory_outputs(*), ai_product_recommendations(*))`
      )
      .eq('id', id)
      .single();

    if (error || !data) throw new NotFoundError('Escalation not found');
    ...
    const recs = (session?.ai_product_recommendations as unknown[]) ?? [];
  ...
  async update(
    ...
    if (body.assignedTo !== undefined) patch.assigned_to = body.assignedTo || agentEmail;
    if (body.agronomistNotes !== undefined) patch.agronomist_notes = body.agronomistNotes;
    if (body.resolution !== undefined) patch.resolution = body.resolution;
    if (body.correction !== undefined) patch.correction = body.correction;
