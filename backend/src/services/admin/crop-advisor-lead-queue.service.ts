...
import {
  cropAdvisorAdminService,
  type LeadStage,
  type CropAdvisorListQuery,
} from './crop-advisor-admin.service.js';
import {
  fetchOpportunityScoresByFarmerIds,
  fetchRetentionByFarmerIds,
} from '../intelligence/intelligence-farmer-score-queries.util.js';
...
export type OperationalLeadSmartFilter =
  | 'all'
  | 'pending'
  | 'escalated'
  | 'overdue'
...
export type OperationalLeadSort =
  | 'priority'
  | 'pending_tasks'
  | 'escalations'
  | 'opportunity_score'
...
export interface OperationalLeadListQuery extends CropAdvisorListQuery {
...
export type OperationalLeadRow = Awaited<
  ReturnType<typeof cropAdvisorAdminService.listLeads>
>['leads'][number] & {
...
function daysAfterPlanting(date: string | null | undefined): number | null {
  if (!date) return null;
...
function computePriority(input: {
  ...
  if (input.escalationCount > 0) return 'escalated';
  if (input.hasOverdueTask || isOverdueIso(input.followUpAt)) return 'overdue';
  if (input.hasDueTodayTask || isDueToday(input.followUpAt)) return 'due_today';
  if (input.stage === 'follow_up' || input.followUpAt) return 'follow_up';
  if ((input.opportunityScore ?? 0) >= 70 || input.stage === 'interested') return 'hot_lead';
  if ((input.opportunityScore ?? 0) >= 50) return 'high_opportunity';
  if (input.stage === 'new_lead') return 'new_lead';

  const last = input.lastInteractionAt ? new Date(input.lastInteractionAt).getTime() : 0;
  const inactiveDays = 30 * 86400000;
  if (!last || Date.now() - last > inactiveDays) return 'inactive';

  return 'new_lead';
}

async function loadFarmerMetrics(farmerIds: string[]) {
  const ids = [...new Set(farmerIds.filter(Boolean))];
  const pendingByFarmer = new Map<string, { count: number; overdue: boolean; dueToday: boolean }>();
  const escalationByFarmer = new Map<string, number>();
  const cropByFarmer = new Map<string, { crop: string; acreage: number | null; dap: number | null }>();
  const pincodeByFarmer = new Map<string, string>();
  const languageByFarmer = new Map<string, string>();
  ...
  if (!ids.length) {
    return {
      ...
      relationshipScores: new Map<string, number>(),
  ...
  for (let i = 0; i < ids.length; i += chunk) {
    ...
    for (const t of tasksRes.data ?? []) {
      ...
      if (isOverdueIso(due)) cur.overdue = true;
      if (isDueToday(due)) cur.dueToday = true;
    ...
    for (const b of blocksRes.data ?? []) {
      ...
      if (cropByFarmer.has(fid) && !b.is_primary) continue;
      ...
      cropByFarmer.set(fid, {
        ...
        dap: daysAfterPlanting(b.planting_date as string | null),
      });
    }
    ...
    for (const f of farmersRes.data ?? []) {
      ...
      const pm = f.pincode_master as { pincode?: string } | null;
      if (pm?.pincode) pincodeByFarmer.set(fid, String(pm.pincode));
    }
  }
...
function matchesSmartFilter(row: OperationalLeadRow, filter: OperationalLeadSmartFilter): boolean {
  switch (filter) {
    case 'pending':
...
function sortRows(rows: OperationalLeadRow[], sort: OperationalLeadSort): OperationalLeadRow[] {
  const copy = [...rows];
  copy.sort((a, b) => {
    if (sort === 'priority') {
    ...
    const num = (key: keyof OperationalLeadRow, dir: 'asc' | 'desc' = 'desc') => {
      const av = a[key];
      const bv = b[key];
...
export const cropAdvisorLeadQueueService = {
  priorityMeta: PRIORITY_META,

  async listOperationalLeads(query: OperationalLeadListQuery, agentEmail: string) {
    const base = await cropAdvisorAdminService.listLeads(
      {
        ...query,
        page: 1,
    ...
    let rows: OperationalLeadRow[] = base.leads.map((lead) => {
      ...
      const followUpDueAt = (lead.followUpAt as string | null) ?? null;
      ...
      const priorityBand = computePriority({
        ...
        opportunityScore,
        lastInteractionAt: (lead.lastInteractionAt as string | null) ?? null,
      });
      ...
      return {
        ...
        owner: (lead.assignedTo as string | null) ?? null,
        ...
        createdAtLabel: formatDateLabel(lead.createdAt as string),
      };
    });
    ...
    if (query.opportunityLevel) {
      rows = rows.filter((r) => {
        ...
        if (query.opportunityLevel === 'high') return score >= 70;
        if (query.opportunityLevel === 'medium') return score >= 40 && score < 70;
        return score < 40;
      });
    }
  ...
  async getQueueSummary(agentEmail: string, scope: 'mine' | 'all' = 'mine') {
    const { leads } = await this.listOperationalLeads(
      { scope, limit: 200, smartFilter: 'all', sort: 'priority' },
      agentEmail
    );
  ...
  async listAssignableTeam() {
    const roles = ['crop_advisor', 'manager', 'operations', 'admin', 'super_admin', 'agronomist'];
    const { data, error } = await supabase
      .from('admin_users')
  ...
  async exportLeads(
    query: OperationalLeadListQuery,
    agentEmail: string,
    leadIds?: string[]
  ) {
  ...
  async bulkUpdateLeads(
    ...
    if (action === 'change_stage' && payload.stage) {
      for (const id of leadIds) {
        await cropAdvisorAdminService.updateLead(
          id,
    ...
    if (action === 'add_broadcast_tag' && payload.broadcastTag?.trim()) {
      ...
      if (!farmerIds.length) return { updated: 0 };
