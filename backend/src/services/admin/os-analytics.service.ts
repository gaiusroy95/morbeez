...
export const osAnalyticsService = {
  ...
  async getMaiosTrends(days = 30) {
    ...
    const labels: string[] = [];
    const buckets = new Map<string, { d3: number; d7: number; d14: number }>();
    ...
    for (const job of jobs ?? []) {
      ...
      if (jt.endsWith('_d3')) b.d3++;
      else if (jt.endsWith('_d7')) b.d7++;
      else if (jt.endsWith('_d14')) b.d14++;
    }

    const d3Responses: number[] = [];
    const d7Responses: number[] = [];
    const d14Responses: number[] = [];
