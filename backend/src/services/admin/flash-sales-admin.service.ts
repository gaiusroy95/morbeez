...
function calcDiscount(original: number, flash: number): number {
  if (original <= 0) return 0;
  return Math.round(((original - flash) / original) * 100);
}

function resolveStatus(startsAt: string, endsAt: string): 'live' | 'upcoming' | 'completed' {
  const now = Date.now();
...
function mapFlash(row: FlashRow) {
  const original = Number(row.original_price);
  const flash = Number(row.flash_price);
  const discount =
    row.discount_percent > 0 ? row.discount_percent : calcDiscount(original, flash);
...
export const flashSalesAdminService = {
  async list(query: FlashSalesListQuery) {
    ...
    const all = (data ?? []).map((r) => mapFlash(r as FlashRow));
  ...
  async get(id: string) {
    ...
    if (!data) throw new NotFoundError('Flash sale not found');
