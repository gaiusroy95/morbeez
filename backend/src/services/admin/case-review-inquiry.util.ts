...
export function textsLikelySame(a: string | null | undefined, b: string | null | undefined): boolean {
  if (!a?.trim() || !b?.trim()) return false;
  const na = normalizeTextKey(a);
  const nb = normalizeTextKey(b);
  if (na === nb) return true;
  if (buildLooseSymptomKey(a) === buildLooseSymptomKey(b)) return true;
  if (buildSymptomKey(a) === buildSymptomKey(b)) return true;
  ...
  if (na.length >= 4 && nb.length >= 4) {
    ...
    if (ta.size && tb.size) {
      ...
      if (overlap / Math.min(ta.size, tb.size) >= 0.6) return true;
    }
  }
  return false;
}
...
export function pickLatestOutput(
  ...
  const rows = (outputs ?? []) as Record<string, unknown>[];
...
export function resolveFarmerQuestion(sessionRow: Record<string, unknown> | null): string {
...
export function pickFarmerFacingSummary(
...
export function resolveProbableIssue(
  output: Record<string, unknown> | undefined,
  sessionProbable: string | null | undefined,
  ...
  for (const c of candidates) {
    if (!textsLikelySame(c, farmerQuestion)) return c;
  }
  return null;
}
