...
function parseDateRange(from: string, to: string): { fromIso: string; toIso: string } {
...
function isAttributedLead(row: LeadRow): boolean {
  return Boolean(row.lead_channel && row.campaign_source);
}

function aggregateFunnel(
  leadIds: string[],
  flagsByLead: Map<string, LeadFunnelFlags>
): FunnelCounts {
  ...
  for (const id of leadIds) {
    ...
    if (!f) continue;
    if (f.connected) connected += 1;
    if (f.interested) interested += 1;
...
function suggestedBonus(
  counts: Pick<FunnelCounts, 'connected' | 'booked' | 'paid'>,
  rule: Record<string, unknown> | null
): number {
  if (!rule) return 0;
  const raw =
    counts.connected * Number(rule.flat_connected_inr ?? 0) +
  ...
  if (cap != null && cap > 0) return Math.min(bonus, cap);
  return bonus;
}

async function buildFunnelFlags(leads: LeadRow[]): Promise<Map<string, LeadFunnelFlags>> {
  const leadIds = leads.map((l) => l.id);
  const farmerIds = [...new Set(leads.map((l) => l.farmer_id))];
  const flags = new Map<string, LeadFunnelFlags>();

  if (!leadIds.length) return flags;
  ...
  for (const call of calls ?? []) {
    ...
    if (
      CONNECTED_CALL_OUTCOMES.has(outcome) ||
      (direction === 'outbound' && CONNECTED_CALL_OUTCOMES.has(outcome))
    ) {
      if (call.lead_id) connectedByLead.add(String(call.lead_id));
    }
  }
...
async function loadLeads(query: MarketingPerformanceQuery): Promise<LeadRow[]> {
  const { fromIso, toIso } = parseDateRange(query.from, query.to);
  let q = supabase
    .from('leads')
    .select(
      'id, farmer_id, stage, created_at, last_interaction_at, lead_channel, campaign_source, marketing_owner_id, marketing_owner_name'
    )
    .gte('created_at', fromIso)
    .lte('created_at', toIso);

  if (query.ownerId) q = q.eq('marketing_owner_id', query.ownerId);
  if (query.campaign) q = q.ilike('campaign_source', `%${query.campaign}%`);
  if (query.channel) q = q.eq('lead_channel', query.channel);
...
async function sumSpend(query: MarketingPerformanceQuery): Promise<number> {
  const { fromIso, toIso } = parseDateRange(query.from, query.to);
  let q = supabase
  ...
  if (query.campaign) q = q.ilike('campaign_name', `%${query.campaign}%`);
  if (query.ownerId) q = q.eq('marketing_owner_id', query.ownerId);
  ...
  return (data ?? []).reduce((sum, row) => {
    ...
    if (Number.isNaN(t) || t < fromMs || t > toMs) return sum;
    return sum + Number(row.amount_inr ?? 0);
  }, 0);
}

export const marketingPerformanceService = {
  async getOverview(query: MarketingPerformanceQuery) {
  ...
  async getByMarketer(query: MarketingPerformanceQuery) {
    ...
    const groups = new Map<
      string,
      { marketerKey: string; marketerName: string; marketerId: string | null; leadIds: string[] }
    >();
  ...
  async getByCampaign(query: MarketingPerformanceQuery) {
    const leads = (await loadLeads(query)).filter(isAttributedLead);
    const flagsByLead = await buildFunnelFlags(leads);
    const rule = await loadIncentiveRule();
    ...
    const groups = new Map<
      string,
      { campaign: string; channel: string | null; leadIds: string[] }
    >();
  ...
  async getFunnelDrilldown(leadIds: string[]) {
    if (!leadIds.length) return [];
    const { data, error } = await supabase
      .from('leads')
      .select(
        'id, farmer_id, stage, created_at, last_interaction_at, lead_channel, campaign_source, marketing_owner_id, marketing_owner_name, farmers(phone, name)'
      )
    ...
    const flagsByLead = await buildFunnelFlags(leads);
    return leads.map((l) => {
      const farmer = (l as Record<string, unknown>).farmers as Record<string, unknown> | null;
...
async function sumSpendByCampaign(query: MarketingPerformanceQuery): Promise<Map<string, number>> {
  const { fromIso, toIso } = parseDateRange(query.from, query.to);
  const { data, error } = await supabase
    .from('marketing_spend_entries')
