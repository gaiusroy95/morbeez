...
import type { TemplateLanguage } from './language-template-variables.js';
...
export const languageTemplateTranslateService = {
  async translateBody(params: {
    sourceText: string;
    sourceLanguage: TemplateLanguage;
        ...
        messages: [
          {
            role: 'system',
            content:
              'You translate WhatsApp farmer messages for Indian agriculture. Keep {{VariableName}} placeholders exactly unchanged. Use natural spoken language for the target locale — not literal word-for-word translation. Return ONLY the translated message text, no quotes or explanation.',
          },
          {
            role: 'user',
