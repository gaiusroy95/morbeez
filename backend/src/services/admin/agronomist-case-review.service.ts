...
import {
  ...
  parseEscalationCorrection,
  pickFarmerFacingSummary,
  pickLatestOutput,
  resolveFarmerQuestion,
...
function caseRef(id: string, createdAt: string): string {
...
function buildTopDiagnoses(
  probableIssue: string | null | undefined,
  confidence: number | null | undefined,
  plantIdRaw: unknown,
  stressAnalysis: unknown
): AiDiagnosisRow[] {
  const rows: AiDiagnosisRow[] = [];
  ...
  const plant = plantIdRaw as { diseases?: Array<{ name: string; probability: number }> } | null;
  for (const d of plant?.diseases ?? []) {
    if (!d.name) continue;
    if (rows.some((r) => r.label.toLowerCase() === d.name.toLowerCase())) continue;
  ...
  for (const s of stress.slice(0, 2)) {
    ...
    if (!label || rows.some((r) => r.label === label)) continue;
...
async function fetchOutboundWhatsAppReply(
  farmerId: string,
...
function parseReviewAction(
  raw: string | undefined
): 'approve_ai' | 'correct_ai' | 'partial_match' | 'escalate_urgent' | undefined {
  if (
    raw === 'approve_ai' ||
    raw === 'correct_ai' ||
    raw === 'partial_match' ||
...
export const agronomistCaseReviewService = {
  ...
  async getCaseDetail(escalationId: string) {
    ...
    const outputs = (sessionRow?.ai_advisory_outputs as unknown[]) ?? [];
    ...
    const lang = (esc.farmer?.language ?? 'en') as AdvisoryLanguage;

    let weatherSummary: string | null = null;
    try {
      weatherSummary = await weatherAlertsService.formatForFarmer(esc.farmerId, lang);
      if (weatherSummary.length > 400) weatherSummary = `${weatherSummary.slice(0, 397)}…`;
    } catch {
      weatherSummary = null;
    }
    ...
    const farmerQuestion = resolveFarmerQuestion(
      sessionRow as Record<string, unknown> | null
    );
    const probable = resolveProbableIssue(
      latestOutput,
      esc.session?.probableIssue ? String(esc.session.probableIssue) : null,
      farmerQuestion
    );
    const conf =
      esc.confidence ??
    ...
    const topDiagnoses = buildTopDiagnoses(
      probable,
      conf,
      plantId,
      latestOutput?.stress_analysis
    );
    ...
    if (!whatsappResponse || textsLikelySame(whatsappResponse, farmerQuestion)) {
      const logged = await fetchOutboundWhatsAppReply(
        esc.farmerId,
        sessionCreated ?? String(esc.createdAt)
      );
    ...
    const recMeta = (existingRec?.metadata as Record<string, unknown> | null) ?? {};
    const savedAction = parseReviewAction(
      savedCorrection?.action ?? (recMeta.caseReviewAction as string | undefined)
    );
    const savedSeverity =
      savedCorrection?.severity ??
      mapRecordSeverityToUi(existingRec?.severity ? String(existingRec.severity) : null);
    const draftRecommendation =
      existingRec?.recommendation_text != null
    ...
    return {
      ...
      review: {
        ...
        correctDiagnosis:
          savedCorrection?.correctDiagnosis ??
      ...
      context: {
        ...
        soil: soil
          ? (() => {
              const m = (soil.metrics as Record<string, unknown>) ?? {};
              return {
                ph: m.ph ?? m.pH ?? null,
                ec: m.ec ?? m.EC ?? null,
      ...
      farmerFeedback: feedback
        ? {
            ...
            cropExperienceYears:
              feedback.crop_experience_years != null
      ...
      similarCases: (similar ?? []).map((s) => {
        const snap = s.advisory_snapshot as { probableIssue?: string } | null;
        return {
          id: s.id,
  ...
  async submitReview(
    ...
    agent: { email: string; adminUserId: string; role: string }
  ) {
    const agentEmail = agent.email;
    const { agronomistTierService } = await import('./agronomist-tier.service.js');
    const selfApprove = await agronomistTierService.canSelfApproveRecommendations(
      agent.adminUserId,
      agentEmail,
      agent.role
    );
    ...
    await escalationAdminService.update(
      escalationId,
      {
        status: newStatus,
    ...
    if (body.submitForApproval && recommendationId) {
      const latestOutput = detail.ai as { costEstimate?: Array<{ note?: string }> };
      ...
      const fusedConf = (detail.maiosCase as { diagnostics?: { fusedConfidence?: number } } | null)
      ...
      if (
        ...
        throw new Error(
          `Economic gate blocked auto-approval: ${economic.reason}. Correct diagnosis or escalate.`
        );
      }
      ...
      if (selfApprove) {
        const { recommendationRecordsService } = await import(
          '../core/recommendation-records.service.js'
        );
        ...
        const { recommendationCommunicationService } = await import(
          '../core/recommendation-communication.service.js'
        );
    ...
    if (
      ...
      const { expertFollowUpLearningService } = await import(
        '../core/expert-follow-up-learning.service.js'
      );
  ...
  async listDiagnosisLabels(params: { cropType?: string | null; search?: string | null }) {
    ...
    for (const label of defaultLabels) labels.add(label);
    ...
    for (const row of masters ?? []) {
      ...
      if (!crop || !rowCrop || rowCrop === crop) labels.add(name);
    }
    ...
    let rtq = supabase
      ...
      .neq('status', 'archived')
      .limit(200);
    if (crop) rtq = rtq.eq('crop_type', crop);
  ...
  async createDiagnosisLabel(params: { label: string; cropType?: string | null }) {
    const name = params.label.trim();
    if (!name) throw new Error('Diagnosis label is required');
    const crop = params.cropType?.trim().toLowerCase() || null;
