...
function normalizeSymptom(issue: string): string {
  const t = issue.trim();
  if (!t) return 'General symptoms';
...
function topEntries<T extends { count: number }>(items: T[], limit: number): T[] {
...
export const aiAdvisoryAdminService = {
  async getOverview() {
    ...
    const [sessions30, sessionsPrev30, sessions7, outputs, recommendations, diseaseRows] =
      await Promise.all([
        supabase
    ...
    if (s30.length < 3) {
      return { ...DEMO, source: 'demo' as const };
    }

    const completed30 = s30.filter((s) => s.status === 'completed' || s.status === 'escalated');
    const completedPrev = (sPrev ?? []).filter(
      (s) => s.status === 'completed' || s.status === 'escalated'
    );

    const recSessionIds = new Set((recommendations.data ?? []).map((r) => r.session_id));
    const successful30 = completed30.filter(
      (s) => recSessionIds.has(s.id) || (s.confidence_score != null && Number(s.confidence_score) >= 0.5)
    );
    ...
    const kpis = {
      ...
      successfulRateTrend: pctChange(
        successful30.length,
        completedPrev.filter((s) => recSessionIds.has(s.id)).length
      ),
    ...
    const diagnosisTrend = {
      ...
      successRate: successByDay.map((b) =>
        b.total > 0 ? Math.round((b.ok / b.total) * 1000) / 10 : kpis.topAccuracy
      ),
    };
    ...
    let topSymptoms = topEntries(
      [...symptomMap.entries()].map(([label, count]) => ({ label, count })),
      5
    );
    if (topSymptoms.length < 3) topSymptoms = DEMO.topSymptoms;
    ...
    const sortedCrops = topEntries(
      [...cropMap.entries()].map(([label, count]) => ({ label, count })),
      4
    );
    const othersCount =
      totalCrop - sortedCrops.reduce((s, c) => s + c.count, 0);
    ...
    let topProducts = topEntries(
      [...productMap.entries()].map(([label, count]) => ({ label, count })),
      5
    );
    if (topProducts.length < 3) topProducts = DEMO.topProducts;
  ...
  async listLogs(query: { page?: number; limit?: number }) {
    ...
    const { data, error, count } = await supabase
      .from('ai_advisory_sessions')
      .select(
        `id, crop_type, crop_stage, channel, status, confidence_score, symptoms_text, created_at,
         farmers(name, phone),
         ai_advisory_outputs(probable_issue)`,
        { count: 'exact' }
      )
    ...
    const logs = (data ?? []).map((row) => {
      ...
      const outputs = row.ai_advisory_outputs as Array<{ probable_issue?: string }> | null;
