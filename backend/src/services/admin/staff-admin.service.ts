...
let workspaceCache: { at: number; data: StaffWorkspace } | null = null;
...
function loginDaysAgo(lastLoginAt: string | null): number | null {
  if (!lastLoginAt) return null;
  const ms = Date.now() - new Date(lastLoginAt).getTime();
  return Math.floor(ms / 86400000);
}

function scoreFromMetrics(leads: number, tasksDone: number, daysAgo: number | null): number {
  let score = 62;
...
function computeRoiPct(performanceScore: number, turnoverInr: number): number {
  if (turnoverInr <= 0) return 0;
...
function buildStaffMember(
  ...
  const resolvedTurnover =
    turnoverInr ?? leads * 12500 + pendingTasks * 800;
  const loginMs = params.lastLoginAt ? new Date(params.lastLoginAt).getTime() : null;
  const statusOnline =
    params.active && loginMs != null && Date.now() - loginMs < 15 * 60 * 1000;
...
async function countForEmail(email: string): Promise<EmailMetrics> {
...
async function loadMetricsForEmails(emails: string[]): Promise<Map<string, EmailMetrics>> {
  const map = new Map<string, EmailMetrics>();
...
function emptyInteractionMap(emails: string[]): Map<string, InteractionStats> {
  const map = new Map<string, InteractionStats>();
...
async function loadInteractionStatsBatch(emails: string[]): Promise<Map<string, InteractionStats>> {
  ...
  for (const row of ixRes.data ?? []) {
    ...
    if (!stats) continue;
    stats.month += 1;
    if (new Date(String(row.created_at)).getTime() >= todayMs) stats.today += 1;
  }
...
async function loadInteractionStats(email: string): Promise<InteractionStats> {
  const map = await loadInteractionStatsBatch([email]);
  return map.get(email.toLowerCase()) ?? { today: 0, month: 0 };
}

async function loadLedgerTurnoverByProfile(
  profileIds: string[],
  start: string,
  end: string
...
async function loadTurnoverTrend(profileId: string): Promise<{ labels: string[]; values: number[] }> {
  const now = new Date();
...
async function applyEngineScores(employees: StaffMember[]): Promise<void> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      'employee_profile_id, performance_score, attributed_farmer_count'
    )
    .in('employee_profile_id', profileIds);
...
async function resolveEmployeeRecord(id: string): Promise<StaffMember | null> {
  ...
  if (profile) {
    ...
    let incentive = 0;
    try {
      const bonus = await incentiveCalculatorService.estimateMonthlyIncentive(
        profile.id,
        member.turnoverInr,
...
async function buildWorkspace(): Promise<StaffWorkspace> {
  ...
  for (const e of employees) {
    ...
    if (e.hasProfile) {
      ...
      if (ledger != null && ledger > 0) e.turnoverInr = Math.round(ledger);
    } else {
...
export const staffAdminService = {
  ...
  async getWorkspace(opts?: { skipCache?: boolean }): Promise<StaffWorkspace> {
    if (
      !opts?.skipCache &&
      workspaceCache &&
  ...
  async getEmployeeDetail(id: string) {
    ...
    const farmerIds = [...new Set(recentLeads.map((l) => l.farmer_id).filter(Boolean))] as string[];
