...
function mapVisitFollowupToOutcome(outcome: string): RecommendationOutcome {
  if (outcome === 'improved') return 'better';
  if (outcome === 'worsened') return 'no_improvement';
  if (outcome === 'no_change') return 'partial';
  return 'unknown';
}
...
export const fieldVisitService = {
  ...
  listMeasurementTemplates: fieldFindingsMastersService.listMeasurementTemplates.bind(
    fieldFindingsMastersService
  ),
  ...
  async submitStructuredVisitForPartner(
    input: StructuredFieldVisitInput,
    agentEmail: string,
    ...
    const findingRow = await telecallerAdminService.createFieldFinding(input.farmerId, leadId, {
      ...
      findingType: primaryIssue.category === 'nutrient_deficiency' ? 'nutrient_deficiency' : primaryIssue.category === 'water_stress' ? 'irrigation' : primaryIssue.category === 'weed' ? 'other' : (primaryIssue.category as 'disease' | 'pest' | 'other'),
    ...
    for (let i = 0; i < input.issues.length; i++) {
      ...
      const humanAction = await resolveFieldTrainingAction(
        input.farmerId,
        issue.issueName,
        issue.observation
      );
      ...
      const aiPrediction =
        issue.finalDiagnosis?.trim() || issue.issueName;
      ...
      if (issue.aiCaseId) {
        ...
        const priorMeta = (aiCaseRow?.metadata as Record<string, unknown>) ?? {};
        await supabase
          .from('visit_ai_cases')
          .update({
            metadata: {
          ...
          .eq('id', issue.aiCaseId);
        await visitAiOrchestratorService.linkCaseToVisitIssue(
          issue.aiCaseId,
          findingId,
          visitIssueId
        );
        ...
        if (shouldApprove) {
          ...
          void recommendationCommunicationService
            .sendApprovedRecommendation(String(row.id), {
              ...
              complianceQuestion:
                whatsappOverride?.complianceQuestion?.trim() ||
          ...
          if (issue.aiCaseId && !partnerId) {
            ...
            const intakeQa = (qaRows ?? [])
              ...
              .map((q) => ({
                ...
                kind: (q.metadata as Record<string, unknown>)?.kind as string | undefined,
              }));
...
function mapReviewSeverity(severity: string): 'mild' | 'moderate' | 'severe' {
...
async function resolveFieldTrainingAction(
...
): Promise<'approve_ai' | 'correct_ai' | 'partial_match'> {
  ...
  if (!sessionIds.length) return 'approve_ai';
  ...
  if (!aiLabel) return 'approve_ai';
  if (aiLabel === human || aiLabel === issueName.trim().toLowerCase()) return 'approve_ai';
  if (aiLabel.includes(human) || human.includes(aiLabel)) return 'partial_match';
  return 'correct_ai';
}
