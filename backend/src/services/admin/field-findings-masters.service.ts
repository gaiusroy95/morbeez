...
export const fieldFindingsMastersService = {
  async listIssueMaster(opts?: { category?: IssueCategory; cropType?: string; q?: string; limit?: number }) {
    let query = supabase
      .from('issue_master')
      .select('*')
      .eq('active', true)
  ...
  async deactivateIssueMaster(id: string) {
    const { data, error } = await supabase
      .from('issue_master')
    ...
    if (!data) throw new NotFoundError('Issue master entry not found');
    return { ok: true };
  },

  async listMeasurementTemplates(cropType: string) {
