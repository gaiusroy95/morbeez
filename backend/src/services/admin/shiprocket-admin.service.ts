...
function isPaid(row: OrderRow): boolean {
  const fs = String(row.financial_status ?? '').toLowerCase();
  const ps = String(row.fulfillment_status ?? '').toLowerCase();
  if (fs === 'voided' || fs === 'refunded' || ps === 'cancelled') return false;
  return fs === 'paid' || fs === 'partially_paid';
}

function needsShipment(row: OrderRow): boolean {
  if (!row.shopify_order_id) return false;
  if (!isPaid(row)) return false;
  const ful = String(row.fulfillment_status ?? '').toLowerCase();
  if (ful === 'delivered' || ful === 'fulfilled' || ful === 'cancelled') return false;
  if (row.tracking_awb?.trim()) return false;
  return true;
}

function formatDisplayId(orderName: string | null, id: string): string {
  if (orderName) {
...
export const shiprocketAdminService = {
  getOverview() {
    ...
    return {
      ...
      webhookReady: Boolean(
        env.SHIPROCKET_WEBHOOK_TOKEN?.trim() && apiBase
      ),
    };
  },
  ...
  async listPending(limit = 25) {
    const { data, error } = await supabase
      ...
      .select(
        'id, shopify_order_id, order_name, phone, financial_status, fulfillment_status, tracking_awb, total_amount, created_at'
      )
    ...
    const pending = (data ?? [])
      .filter((r) => needsShipment(r as OrderRow))
      .slice(0, limit)
      .map((r) => {
  ...
  async listRecentEvents(limit = 30) {
    ...
    const shopifyIds = [
      ...new Set(
        (data ?? [])
          .map((e) => e.shopify_order_id as string | null)
          .filter(Boolean) as string[]
      ),
    ];

    const orderByShopify = new Map<string, { orderName: string | null; phone: string | null }>();
  ...
  async retryCreateShipment(shopifyOrderId: string) {
    const id = shopifyOrderId.trim();
    if (!id) throw new ValidationError('Shopify order id is required');
    ...
    if (!data) throw new NotFoundError('Order not found for this Shopify id');
