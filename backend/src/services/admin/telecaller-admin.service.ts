...
export const telecallerAdminService = {
  ...
  async listLeads(query: TelecallerListQuery, agentEmail: string) {
    ...
    const { telecallerIntelligenceService } = await import(
      '../intelligence/telecaller-intelligence.service.js'
    );
  ...
  async updateLead(
    ...
    if (patch.assignedTo !== undefined) {
      ...
      void farmerOwnershipService.syncTelecallerAssignment(
        data.farmer_id as string,
  ...
  async addTaskComment(
    taskId: string,
    input: { body: string; authorEmail: string; authorRole: 'telecaller' | 'agronomist' | 'system'; authorName?: string }
  ) {
    const { data, error } = await supabase
      .from('crm_task_comments')
      .insert({
