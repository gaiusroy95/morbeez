...
function contributionLabel(netContribution: number, grossProfit: number): 'Strong' | 'Good' | 'Moderate' | 'Weak' {
  if (grossProfit <= 0) return 'Weak';
...
function realizationStatus(pct: number, config: Awaited<ReturnType<typeof pricingConfigService.getConfig>>): string {
  if (pct >= config.realizationExcellent) return 'Healthy';
  if (pct >= config.realizationGood) return 'Good';
  if (pct >= config.realizationWarning) return 'Watch';
  return 'Risk';
}

function complaintLevel(returnPct: number): 'Low' | 'Medium' | 'High' {
  if (returnPct >= 5) return 'High';
  if (returnPct >= 2.5) return 'Medium';
  return 'Low';
}

async function loadLedger(start: string, end: string): Promise<LedgerRow[]> {
...
export const superAdminMonitorService = {
  async refreshMonitor(opts?: { date?: string; monthYear?: string }) {
  ...
  async getMonitor(opts?: { date?: string; monthYear?: string }) {
    ...
    const [dayLedger, monthLedger, finance, stockSummary, pendingBulkReviews, shipmentExceptions, loggedAdSpend] =
      await Promise.all([
    ...
    const summarize = (rows: LedgerRow[]) => {
    ...
    const profileMap = new Map(
      (profiles ?? []).map((p) => [String(p.id), { fullName: String(p.full_name), employeeCode: String(p.employee_code ?? '') }])
    );
    ...
    const byEmployee = new Map<
      string,
      {
        ...
        incentive: number;
        returnCost: number;
        realizationWeighted: number;
        realizationSales: number;
        nearFloorOrders: Set<string>;
        discountSum: number;
    ...
    for (const r of monthLedger) {
      ...
      const cur = byEmployee.get(eid) ?? {
        ...
        realizationSales: 0,
        nearFloorOrders: new Set<string>(),
        discountSum: 0,
        discountCount: 0,
        totalLines: 0,
        returnedLines: 0,
      ...
      cur.discountCount += 1;
      if (
        (discount >= 12 || Number(r.realization_pct) < config.realizationWarning) &&
        r.commerce_quote_id
      ) {
    ...
    const adSpendTotal =
      loggedAdSpend > 0 ? round2(loggedAdSpend) : round2(totalMonthSales * 0.02);
    const adSpendSource: 'logged' | 'estimated' = loggedAdSpend > 0 ? 'logged' : 'estimated';
    ...
    const employeeHeadwise = [...byEmployee.entries()]
      ...
      .map(([employeeProfileId, m]) => {
        ...
        const adAllocation =
          totalMonthSales > 0 ? round2((m.sales / totalMonthSales) * adSpendTotal) : 0;
    ...
    const bulkByCustomer = new Map<
      string,
      { customerKey: string; customerName: string; sales: number; gp: number }
    >();

    const bulkQuoteIds = [
      ...new Set(
        monthLedger.filter((r) => r.retail_or_bulk === 'bulk' && r.commerce_quote_id).map((r) => String(r.commerce_quote_id))
      ),
    ];
    ...
    const stockRows = stockSummary as Array<{
      sku: string;
      productTitle: string;
      available: number;
      incoming: number;
      batches: Array<{ qtyOnHand: number; expiryDate: string | null }>;
    }>;
