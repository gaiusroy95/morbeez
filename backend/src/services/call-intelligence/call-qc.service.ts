...
type RubricCriterion = { key: string; label: string; maxPoints: number };

type QcResult = {
  totalScore: number;
  flagged: boolean;
  flagReason: string | null;
  rubric: Record<string, { score: number; maxPoints: number; note?: string }>;
};

export const callQcService = {
  async getActiveRubric(): Promise<RubricCriterion[]> {
  ...
  async getOverview(days: number, agentEmail?: string) {
    ...
    for (const row of rows) {
      const json = row.ai_summary_json as Record<string, unknown> | null;
      if (json?.interestLevel === 'high' || json?.interestLevel === 'medium') interested += 1;
      if (json?.interestedInSoilTest) soilInterest += 1;
    }
  ...
  async listFlaggedCalls(days: number, limit = 50) {
    const since = new Date();
    since.setDate(since.getDate() - days + 1);
    const { data, error } = await supabase
      .from('crm_call_logs')
      .select(
        'id, lead_id, farmer_id, agent_email, qc_score, qc_flag_reason, qc_rubric_json, transcript, ai_summary, created_at, farmers(name, phone)'
      )
      .eq('qc_flagged', true)
      .gte('created_at', since.toISOString())
