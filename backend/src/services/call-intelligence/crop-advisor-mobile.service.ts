...
import { cropAdvisorLeadQueueService } from '../admin/crop-advisor-lead-queue.service.js';
import { cropAdvisorFarmerProfileService } from '../admin/crop-advisor-farmer-profile.service.js';
...
function displayFarmerName(farmer: Record<string, unknown>): string {
...
function categorizeTask(row: Record<string, unknown>): string {
  const taskType = String(row.task_type ?? '').toLowerCase();
  const title = String(row.title ?? '').toLowerCase();
  if (taskType.includes('recommendation') || title.includes('recommendation')) return 'recommendationReviews';
  if (taskType.includes('visit') || title.includes('visit')) return 'visitFollowUps';
  if (taskType.includes('order') || title.includes('order')) return 'orderFollowUps';
  return 'general';
}
...
function mapOperationalLead(row: Awaited<ReturnType<typeof cropAdvisorLeadQueueService.listOperationalLeads>>['leads'][number]) {
  return {
    id: String(row.id),
...
export const cropAdvisorMobileService = {
  ...
  async listLeads(agentEmail: string, query: { scope?: 'mine' | 'all'; limit?: number }) {
    ...
    const result = await cropAdvisorLeadQueueService.listOperationalLeads(
      {
        ...
        search: query.search,
        smartFilter: query.smartFilter as
          | 'all'
          | 'pending'
          | 'escalated'
          | 'overdue'
          ...
          | 'no_engagement'
          | undefined,
        sort: (query.sort as 'priority' | 'recent_interaction' | undefined) ?? 'priority',
  ...
  async getQueueSummary(agentEmail: string, scope: 'mine' | 'all' = 'mine') {
    return cropAdvisorLeadQueueService.getQueueSummary(agentEmail, scope);
  },

  async listFollowUps(agentEmail: string, status = 'pending') {
    return cropAdvisorAdminService.listTasks(agentEmail, status);
  },

  async listFollowUpSections(agentEmail: string) {
    ...
    const sections = {
      today: [] as ReturnType<typeof mapTaskRow>[],
      overdue: [] as ReturnType<typeof mapTaskRow>[],
      upcoming: [] as ReturnType<typeof mapTaskRow>[],
      recommendationReviews: [] as ReturnType<typeof mapTaskRow>[],
      visitFollowUps: [] as ReturnType<typeof mapTaskRow>[],
      orderFollowUps: [] as ReturnType<typeof mapTaskRow>[],
      general: [] as ReturnType<typeof mapTaskRow>[],
    };

    for (const task of rows) {
      if (task.isDueToday) sections.today.push(task);
      else if (task.dueAt && isOverdue(task.dueAt)) sections.overdue.push(task);
      else sections.upcoming.push(task);

      const cat = task.category ?? 'general';
      if (cat === 'recommendationReviews') sections.recommendationReviews.push(task);
      else if (cat === 'visitFollowUps') sections.visitFollowUps.push(task);
      else if (cat === 'orderFollowUps') sections.orderFollowUps.push(task);
      else sections.general.push(task);
    }

    return sections;
  },

  async listTodaysTasks(agentEmail: string) {
  ...
  async countOpenEscalations(agentEmail: string) {
  ...
  async buildActionQueue(agentEmail: string) {
    const [overdue, dueToday, hot, escalated] = await Promise.all([
      cropAdvisorLeadQueueService.listOperationalLeads(
        { scope: 'mine', smartFilter: 'overdue', limit: 3 },
        agentEmail
      ),
      cropAdvisorLeadQueueService.listOperationalLeads(
        { scope: 'mine', smartFilter: 'due_today', limit: 3 },
        agentEmail
      ),
      cropAdvisorLeadQueueService.listOperationalLeads(
        { scope: 'mine', smartFilter: 'hot_leads', limit: 3 },
        agentEmail
      ),
      cropAdvisorLeadQueueService.listOperationalLeads(
        { scope: 'mine', smartFilter: 'escalated', limit: 3 },
        agentEmail
      ),
    ]);
    ...
    try {
      ...
      const opps = await salesOpportunityService.listForCropAdvisor(agentEmail);
      if (opps.length) {
    ...
    if (partnerVisits?.length) {
      const v = partnerVisits[0] as Record<string, unknown>;
      const f = v.farmers as Record<string, unknown> | null;
      const farmerName =
        f?.name ?? [f?.first_name, f?.last_name].filter(Boolean).join(' ') ?? 'Farmer';
  ...
  async getWorkspaceSummary(leadId: string) {
    const detail = await cropAdvisorAdminService.getLeadDetail(leadId);
    const farmerId = String(detail.lead.farmerId);
    const farmerRow = detail.farmer as Record<string, unknown>;

    const [profile, intelligence, blocks, lastVisitRes, openRecsRes, lastOrderRes, ownership] =
      await Promise.all([
      cropAdvisorFarmerProfileService.getProfile(farmerId).catch(() => null),
    ...
    const acreage =
      blocks.reduce((s, b) => s + (b.acreage_decimal ?? 0), 0) ||
    ...
    const pendingTasks = (detail.tasks ?? []).filter((t: Record<string, unknown>) =>
      ['pending', 'open', 'in_progress'].includes(String(t.status ?? ''))
    ).length;

    const profileData = profile as Record<string, unknown> | null;
    const intel = intelligence as Record<string, unknown> | null;
    ...
    return {
      ...
      lead: {
        ...
        stageLabel:
          cropAdvisorAdminService.stageLabels[
            String(detail.lead.stage ?? '') as keyof typeof cropAdvisorAdminService.stageLabels
          ] ?? String(detail.lead.stage ?? ''),
        assignedCropAdvisor: detail.lead.assignedTo ? String(detail.lead.assignedTo) : null,
      ...
      activeCrops: [...new Set(blocks.map((b) => b.crop_type).filter(Boolean))] as string[],
  ...
  async listNotifications(agentEmail: string) {
    ...
    try {
      oppsRes = await supabase
        ...
        .eq('assigned_crop_advisor_email', agentEmail)
    ...
    for (const t of tasksRes.data ?? []) {
      ...
      notifications.push({
        id: `task-${t.id}`,
        category: isOverdue(t.due_at as string) ? 'overdue_task' : isDueToday(t.due_at as string) ? 'due_today' : 'upcoming_task',
        title: String(t.title ?? 'Follow-up'),
        detail: isDueToday(t.due_at as string) ? 'Due today' : formatDateTime(t.due_at as string),
        at: dueAt,
        leadId: t.lead_id ? String(t.lead_id) : undefined,
        taskId: String(t.id),
      });
