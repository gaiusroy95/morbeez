...
function mapOperationalLead(row: Awaited<ReturnType<typeof telecallerLeadQueueService.listOperationalLeads>>['leads'][number]) {
  return {
    id: String(row.id),
...
export const telecallerMobileService = {
  ...
  async buildActionQueue(agentEmail: string) {
    const [overdue, dueToday, hot, escalated] = await Promise.all([
      telecallerLeadQueueService.listOperationalLeads(
