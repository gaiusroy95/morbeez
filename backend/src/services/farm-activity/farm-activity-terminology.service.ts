...
export type FarmActivityTerminologyExpansion = {
  language: string;
  farmerOverrides: Array<{ term: string; resolvedMeaning: string; standardTerm: string | null }>;
  productAliases: Array<{ alias: string; canonicalProductKey: string }>;
  unitAliases: Array<{ alias: string; canonicalUnit: string }>;
};
...
export async function loadFarmActivityTerminologyExpansion(input: {
  ...
  const empty: FarmActivityTerminologyExpansion = {
