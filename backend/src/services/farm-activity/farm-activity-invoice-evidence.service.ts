...
import {
  type FarmActivityAssistantConfidence,
  type FarmActivityAssistantExpenseSubEvent,
  type FarmActivityAssistantField,
  type FarmActivityAssistantMoney,
  type FarmActivityAssistantProvenance,
  type FarmActivityAssistantPurchaseSubEvent,
  type FarmActivityAssistantSource,
  type FarmActivityAssistantSourceMedia,
  type FarmActivityAssistantUnit,
  type FarmActivityAssistantUnresolved,
} from '@morbeez/shared/farm-activity-assistant';
import { env } from '../../config/env.js';
import { AppError } from '../../lib/errors.js';
import { logger } from '../../lib/logger.js';
import {
  openaiStrictJsonSchemaMediaCompletion,
...
const UNIT_ALIASES: Record<string, FarmActivityAssistantUnit> = {
  kg: 'kg',
  kilogram: 'kg',
  kilograms: 'kg',
  g: 'g',
...
const confidenceEnum = enumString(['low', 'medium', 'high'] as const);
const unresolvedReasonEnum = enumString(['missing', 'ambiguous', 'conflicting', 'unsupported'] as const);
...
export const FARM_ACTIVITY_INVOICE_EVIDENCE_SCHEMA: StrictJsonSchema = object({
  documentKind: enumString(['invoice', 'receipt', 'bill', 'unknown'] as const),
...
export type InvoiceEvidenceExtraction = {
  ...
  notes: string | null;
  contentHash: string;
  unresolvedFields: Array<{ field: string; reason: FarmActivityAssistantUnresolved['reason']; detail: string }>;
};
...
export function classifyInvoiceEvidenceMedia(
  mimeType: string,
  fileName?: string
): InvoiceEvidenceMediaKind | null {
  const mime = normalizeMime(mimeType);
  const name = fileName?.trim().toLowerCase() ?? '';
  if (mime === 'application/pdf' || mime.includes('pdf') || name.endsWith('.pdf')) return 'pdf';
  if (SUPPORTED_IMAGE_MIME.has(mime) || mime.startsWith('image/')) return 'image';
  return null;
}

/** Reuse WhatsApp pipeline document/image buffers without router coupling. */
export function mediaFromWhatsAppExtract(
  media: MediaExtractResult,
...
export function hashInvoiceEvidenceContent(buffer: Buffer): string {
...
export function findInvoiceDuplicateCandidates(
  incoming: {
    farmerId: string;
    vendorName: string | null;
    invoiceDate: string | null;
  ...
  existing: InvoiceDuplicateCandidateRecord[]
): InvoiceDuplicateCandidate[] {
  const vendor = normalizeVendor(incoming.vendorName);
  const out: InvoiceDuplicateCandidate[] = [];

  for (const row of existing) {
    if (row.farmerId !== incoming.farmerId) continue;
    const reasons: InvoiceDuplicateCandidate['matchReasons'] = [];
...
function asNullableField(
  value: unknown,
  fieldName: string,
  errors: string[]
): InvoiceEvidenceNullableField<string> | null {
  if (!isRecord(value)) {
...
function asNullableNumberField(
  value: unknown,
  fieldName: string,
  errors: string[]
): InvoiceEvidenceNullableField<number> | null {
  if (!isRecord(value)) {
    errors.push(`${fieldName} must be an object`);
    return null;
  }
...
export function validateInvoiceEvidenceExtraction(
  value: unknown,
  contentHash: string
): { ok: true; value: InvoiceEvidenceExtraction } | { ok: false; errors: string[] } {
  ...
  if (!Array.isArray(value.lineItems)) errors.push('lineItems must be an array');
  ...
  const lineItems: InvoiceEvidenceLineItem[] = [];
  ...
  const unresolvedFields: InvoiceEvidenceExtraction['unresolvedFields'] = [];
  const collect = (
    field: string,
    extracted: InvoiceEvidenceNullableField<string | number> | null
  ) => {
  ...
  return {
    ok: true,
    value: {
      documentKind: documentKind as InvoiceEvidenceExtraction['documentKind'],
      ...
      total: total!,
      lineItems,
      overallConfidence: overallConfidence as FarmActivityAssistantConfidence,
...
function mapUnit(raw: string | null): FarmActivityAssistantUnit | null {
  if (!raw) return null;
  const key = raw.trim().toLowerCase();
  return UNIT_ALIASES[key] ?? 'other';
}

function toContractField<T>(
  extracted: InvoiceEvidenceNullableField<T>,
  sourceRefs: string[],
  provenance: FarmActivityAssistantProvenance[] = ['source_media']
...
): FarmActivityAssistantField<FarmActivityAssistantMoney> {
  if (amount == null) {
    return {
      value: null,
      confidence: 'low',
...
export function buildInvoiceDraftEvidence(params: {
  ...
  const purchaseSubEvents: FarmActivityAssistantPurchaseSubEvent[] = [];
  const expenseSubEvents: FarmActivityAssistantExpenseSubEvent[] = [];
  let sequence = 1;
  ...
  if (params.extraction.lineItems.length) {
    for (const item of params.extraction.lineItems) {
      ...
      purchaseSubEvents.push({
        ...
        unitPrice: moneyField(
          ...
          sourceRefs,
          item.unitPrice.unresolved ?? undefined
        ),
        totalCost: moneyField(
          item.lineTotal.value,
          item.lineTotal.confidence,
          sourceRefs,
          item.lineTotal.unresolved ?? undefined
        ),
      });
    }
    ...
    purchaseSubEvents.push({
      ...
      totalCost: moneyField(
        params.extraction.total.value,
        params.extraction.total.confidence,
        sourceRefs,
        params.extraction.total.unresolved ?? undefined
      ),
    });
  }
  ...
  if (params.extraction.gstAmount.value != null && params.extraction.gstAmount.value > 0) {
    expenseSubEvents.push({
      ...
      amount: moneyField(
        params.extraction.gstAmount.value,
        params.extraction.gstAmount.confidence,
        sourceRefs
      ),
      paidTo: vendorName,
    });
...
function fail(code: string, message: string): InvoiceEvidenceExtractErr {
  return { ok: false, code, message };
}

export async function extractFarmActivityInvoiceEvidence(
  input: InvoiceEvidenceExtractRequest,
  deps?: { provider?: ProviderFn }
  ...
  let extraction: InvoiceEvidenceExtraction;
  try {
    ...
    logger.warn(
      { err: error, farmerId: input.farmerId, mediaKind: kind },
      'Farm activity invoice OCR failed closed'
    );
  ...
  const mediaRef: FarmActivityAssistantSourceMedia = {
    id: mediaId,
