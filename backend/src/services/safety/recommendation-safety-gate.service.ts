...
import {
  validateVisitAssistantRecommendations,
} from '../agronomist/visit-assistant-recommendation-safety.service.js';
...
export const recommendationSafetyGateService = {
  ...
  async evaluate(input: SafetyGateInput): Promise<SafetyGateResult> {
    ...
    let decision: SafetyGateDecision = 'PASS';
    let blockers: unknown[] = [];
    let warnings: unknown[] = [];
    let compatibilityReport: unknown = null;
    let maiosReport: unknown = null;
    ...
    const finalDecision = data.decision as SafetyGateDecision;
    return {
  ...
  async assertAllowsApproval(params: {
    ...
    return {
      decisionId: String(data.id),
      decision: data.decision as SafetyGateDecision,
      contentHash: String(data.content_hash),
      blockers: (data.blockers as unknown[]) ?? [],
  ...
  async override(params: {
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (!existing) throw new ConflictError('safety_decision_not_found');
