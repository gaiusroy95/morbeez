...
function kindFromMime(mime: string): keyof typeof MAX_BYTES | null {
...
export const productMediaStorageService = {
  async upload(input: {
    ...
    if (!raw) throw new ValidationError('File data is required');
    ...
    if (error) {
      throw new AppError(
        `Upload failed: ${error.message}. Ensure storage bucket "product-assets" exists.`,
        502,
