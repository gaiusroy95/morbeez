...
async function fetchLatestRow(farmerId: string, blockId?: string | null) {
  if (blockId) {
...
export const soilReportLoaderService = {
  async loadLatestForBlock(
    farmerId: string,
    blockId?: string | null
  ): Promise<LoadedSoilReport | null> {
    ...
    const summaryLine =
      formatSoilMetricsForAi(metrics, { reportedAt, labName, maxLines: 8 }) ?? '';
