...
const CONSOLE_BUILD_HINT =
  'React console not built. From backend/: run npm run build:console (or npm run build).';
...
export async function buildApp() {
  ...
  if (!consoleBuilt) {
    app.get(STAFF_PORTAL_PREFIX, async (_request, reply) =>
      reply.code(503).type('text/html').send(buildMissingConsoleHtml())
    );
  }
...
function buildMissingConsoleHtml(): string {
