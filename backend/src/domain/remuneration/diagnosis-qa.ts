...
export const DEFAULT_DIAGNOSIS_QA_RULE: DiagnosisQaRule = {
  sampleRatePct: 10,
  sampleCap: 30,
};

/** MIN(rate% of qualified cases, cap). Never sample more than the qualified set. */
export function diagnosisQaSampleSize(
  qualifiedCount: number,
  rule = DEFAULT_DIAGNOSIS_QA_RULE
): number {
...
export function diagnosisAccuracyPct(accurate: number, inaccurate: number): number {
...
export function pickSample<T>(items: T[], size: number, seed: string): T[] {
