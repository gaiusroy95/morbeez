...
const RULES: Array<{ key: CommissionCategoryKey; pattern: RegExp }> = [
  ...
  { key: 'fpo_order', pattern: /\bfpo\b/i },
  {
    key: 'biologicals',
    pattern:
      /tricho|pseudomon|bacillus|mycorrhiz|biological|bio[- ]?fert|consortia|metarhiz|beauveria|paecilomyces|verticillium/i,
  },
  {
...
export function resolveCommissionCategory(input: {
  ...
  if (kind.includes('dealer')) return 'dealer_order';
  if (kind.includes('fpo')) return 'fpo_order';

  const blob = [input.productType, input.sku, input.title].filter(Boolean).join(' ');
  for (const rule of RULES) {
    if (rule.pattern.test(blob)) return rule.key;
  }
  return 'biologicals';
}

export function dominantCategory(
  lines: Array<{ sku?: string | null; title?: string | null; productType?: string | null; salesInr: number }>
): CommissionCategoryKey {
  if (!lines.length) return 'biologicals';
  const scored = new Map<CommissionCategoryKey, number>();
...
export function isCommercialOrder(grossInr: number): boolean {
  return grossInr > 30_000;
}

export function reliabilityHoldPct(score: number): number {
