...
export function buildHypothesisDistribution(
  raw: Array<{ label: string; confidence?: number; weight?: number }>,
  ...
  const cleaned = raw
    ...
    .filter((h): h is HypothesisWeight => h != null && h.weight > 0)
    .slice(0, 5);
  ...
  if (unknownWeight < minUnknown) {
    ...
    const top = cleaned[0];
    if (top) top.weight = clampPercent(top.weight - deficit);
...
export function distributionThresholdReached(dist: HypothesisDistribution): boolean {
...
export function applyEvidenceDeltas(
  ...
  return buildHypothesisDistribution(
    hypotheses.map((h) => ({ label: h.label, weight: h.weight })),
    dist.targetConfidence
  );
}
