...
function m(value: string, unit = ''): { value: string; unit: string } {
  return { value, unit };
}

export const GINGER_ADVISORY_SCENARIOS: GingerAdvisoryScenario[] = [
  {
    id: 'rhizomeRot',
...
export function getGingerAdvisoryScenario(id: GingerAdvisoryScenarioId): GingerAdvisoryScenario {
  const row = GINGER_ADVISORY_SCENARIOS.find((s) => s.id === id);
  if (!row) throw new Error(`Unknown ginger advisory scenario: ${id}`);
  return row;
}
