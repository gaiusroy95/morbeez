...
export function leadChannelFromUtm(utmSource?: string | null, utmMedium?: string | null): LeadChannel {
  const src = String(utmSource ?? '').toLowerCase();
  const medium = String(utmMedium ?? '').toLowerCase();
  if (src.includes('instagram') || medium.includes('instagram')) return 'instagram';
  if (
  ...
  if (src.includes('google') || medium.includes('google')) return 'google';
  if (medium === 'referral' || src.includes('referral')) return 'referral';
  if (!src && !medium) return 'organic';
  return 'other';
}

export function attributionBadge(channel?: string | null, campaign?: string | null): string | null {
  const ch = channel?.trim();
  const camp = campaign?.trim();
  if (!ch && !camp) return null;
  const channelLabel = ch ? ch.charAt(0).toUpperCase() + ch.slice(1) : 'Unknown';
  if (camp) return `${channelLabel} · ${camp}`;
  return channelLabel;
}
