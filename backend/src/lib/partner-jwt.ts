...
export function createPartnerToken(partnerId: string, phone: string, ttlDays = 30): string {
...
export function verifyPartnerToken(token: string): PartnerTokenPayload {
  ...
  let decoded: PartnerTokenPayload;
  try {
    decoded = JSON.parse(
...
export function generateQrToken(partnerCode: string): string {
