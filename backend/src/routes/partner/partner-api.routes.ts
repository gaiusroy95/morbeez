...
export async function partnerApiRoutes(app: FastifyInstance): Promise<void> {
  ...
  await app.register(async (partnerApp) => {
    ...
    partnerApp.post(`${api}/farmers/:farmerId/support-request`, async (request, reply) => {
      ...
      const body = z
        ...
        .parse(request.body);
      const result = await partnerMobileService.createSupportRequest(
        partner.id,
        farmerId,
        body,
        partner.fullName
    ...
    partnerApp.get(`${api}/issue-master`, async (request, reply) => {
      ...
      const q = request.query as { cropType?: string; category?: string };
    ...
    partnerApp.post(`${api}/visits/photos/validate`, async (request, reply) => {
      await requirePartner(request);
      const { z: zod } = await import('zod');
      const { visitPhotoValidationService } = await import(
        '../../services/core/visit-photo-validation.service.js'
      );
      const body = zod
        .object({
  ...
  app.get('/api/v1/enroll/partner/:code', async (request, reply) => {
    ...
    if (!partner) return reply.code(404).send({ ok: false, message: 'Partner not found' });
    return reply.send({
      ok: true,
      partner: {
        partnerCode: partner.partnerCode,
