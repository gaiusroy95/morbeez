...
export async function osKpiControlRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/qualified-cases`, async (request, reply) => {
    ...
    const cases = await qualifiedCaseEngine.list(
      month,
      q.qualified == null ? undefined : q.qualified === 'true'
    );
    return reply.send({
      ok: true,
      month,
  ...
  app.get(`${api}/drilldown`, async (request, reply) => {
    await assertModuleAccess(request, 'partner_program', 'read');
    const { earningDrilldownService } = await import(
      '../../services/remuneration/earning-drilldown.service.js'
    );
    const q = z
      .object({
        partyType: z.enum(['partner', 'employee']),
