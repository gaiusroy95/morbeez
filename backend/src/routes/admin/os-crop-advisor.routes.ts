...
import {
  cropAdvisorLeadQueueService,
  type OperationalLeadSort,
} from '../../services/admin/crop-advisor-lead-queue.service.js';
...
export async function osCropAdvisorRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/leads/queue-summary`, async (request, reply) => {
    ...
    const summary = await cropAdvisorLeadQueueService.getQueueSummary(
      admin.email,
  ...
  app.post(`${api}/leads/bulk`, async (request, reply) => {
    ...
    const body = z
      ...
      .parse(request.body);
    const result = await cropAdvisorLeadQueueService.bulkUpdateLeads(
      body.leadIds,
      body.action,
  ...
  app.get(`${api}/leads/:id/farmer-profile`, async (request, reply) => {
    ...
    const { id } = request.params as { id: string };
    const detail = await cropAdvisorAdminService.getLeadDetail(id);
    const profile = await cropAdvisorFarmerProfileService.getProfile(detail.lead.farmerId as string);
  ...
  app.patch(`${api}/leads/:id/farmer-profile`, async (request, reply) => {
    ...
    const body = z
      ...
      .parse(request.body);
    const profile = await cropAdvisorFarmerProfileService.updateProfile(
  ...
  app.get(`${api}/leads/:id/agronomist`, async (request, reply) => {
    ...
    const { id } = request.params as { id: string };
    const detail = await cropAdvisorAdminService.getLeadDetail(id);
    const { cropAdvisorFarmerAgronomistService } = await import(
      '../../services/admin/crop-advisor-farmer-agronomist.service.js'
    );
  ...
  app.get(`${api}/leads/:leadId/field-findings/:findingId`, async (request, reply) => {
    ...
    const detail = await cropAdvisorAdminService.getLeadDetail(leadId);
    const finding = await cropAdvisorAdminService.getFieldFinding(
      String(detail.lead.farmerId),
      findingId
    );
  ...
  app.get(`${api}/mobile/dashboard`, async (request, reply) => {
    const admin = await assertModuleAccess(request, 'crop_advisor_crm', 'read');
    const { cropAdvisorMobileService } = await import(
      '../../services/call-intelligence/crop-advisor-mobile.service.js'
    );
  ...
  app.post(`${api}/leads/:id/interactions`, async (request, reply) => {
    ...
    const interaction = await crmFarmerService.createInteraction(
      detail.lead.farmerId as string,
      id,
      { ...body, doneBy: admin.email, doneByRole: 'Crop Advisor' }
    );
