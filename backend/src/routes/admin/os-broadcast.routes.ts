...
export async function osBroadcastRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/deliveries/export`, async (request, reply) => {
    await assertModuleAccess(request, 'operations', 'read');
    const q = request.query as { campaignId?: string; limit?: string };
  ...
  app.get(`${api}/analytics`, async (request, reply) => {
    await assertModuleAccess(request, 'operations', 'read');
    const q = request.query as { campaignId?: string; days?: string };
