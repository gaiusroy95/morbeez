...
export async function osFieldRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/farmers/search`, async (request, reply) => {
    ...
    const farmers = await fieldPwaService.searchFarmers(
      q.q ?? '',
  ...
  app.get(`${api}/issue-master`, async (request, reply) => {
    await assertModuleAccess(request, 'agronomist', 'read');
    const q = request.query as { category?: string; cropType?: string; q?: string; limit?: string };
  ...
  app.get(`${api}/blocks/:blockId/plot-intelligence`, async (request, reply) => {
    ...
    if (!trends) trends = await plotDigitalTwinService.buildSnapshot(blockId, q.farmerId);
