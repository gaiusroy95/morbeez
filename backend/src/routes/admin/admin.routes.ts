...
import { osKpiControlRoutes } from './os-kpi-control.routes.js';
...
export async function adminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/auth/me`, async (request, reply) => {
    ...
    let agronomistTier: 'new' | 'experienced' | null = null;
    let canSelfApproveRecommendations = false;
    try {
      const { agronomistTierService } = await import(
        '../../services/admin/agronomist-tier.service.js'
      );
  ...
  app.delete(`${api}/orders/:id`, async (request, reply) => {
    ...
    const source =
      q.source === 'quote' || q.source === 'razorpay_checkout' || q.source === 'shopify'
        ? q.source
        : 'shopify';
  ...
  app.get(`${api}/employees`, async (request, reply) => {
    ...
    const q = request.query as { role?: string; status?: string; search?: string; limit?: string };
  ...
  app.post(`${api}/employees/:id/incentives/preview`, async (request, reply) => {
    ...
    const body = z
      ...
      .parse(request.body);
    const preview = await incentiveCalculatorService.estimateMonthlyIncentive(
      id,
      body.monthlySalesInr,
      body.conversionRatePct
    );
  ...
  app.patch(
    `${api}/products/warehouse-options/warehouses/:warehouseId/racks/:rackName`,
    async (request, reply) => {
      const actor = requireAdmin(request);
      requireAdminRole(request, 'admin', 'manager');
  ...
  app.get(`${api}/products/shopify-connection`, async (request, reply) => {
    requireAdmin(request);
    const { shopifyPublicationsService } = await import(
      '../../services/shopify/shopify.publications.service.js'
    );
  ...
  await app.register(osKpiControlRoutes);
