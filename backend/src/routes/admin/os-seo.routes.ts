...
export async function osSeoRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/products`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const q = request.query as { search?: string; missingOnly?: string };
  ...
  app.get(`${api}/products/:shopifyProductId`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const { shopifyProductId } = request.params as { shopifyProductId: string };
  ...
  app.get(`${api}/pages`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const q = request.query as { pageType?: string; status?: string; crop?: string; search?: string };
  ...
  app.get(`${api}/faqs`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const q = request.query as { pageId?: string; productId?: string };
  ...
  app.get(`${api}/links`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const q = request.query as { sourceType?: string; sourceId?: string };
  ...
  app.post(`${api}/schema/preview`, async (request, reply) => {
    ...
    switch (body.type) {
      case 'product':
        schema = seoSchemaService.buildProductSchema(body.payload as never);
        break;
      case 'faq':
        schema = seoSchemaService.buildFaqSchema(body.payload.faqs as never);
        break;
      case 'breadcrumb':
        schema = seoSchemaService.buildBreadcrumbSchema(body.payload.items as never);
        break;
      default:
        schema = seoSchemaService.buildArticleSchema(body.payload as never);
    }
  ...
  app.get(`${api}/health`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const q = request.query as { resolved?: string; issueType?: string };
  ...
  app.get(`${api}/keywords`, async (request, reply) => {
    await assertModuleAccess(request, 'seo', 'read');
    const q = request.query as { region?: string; search?: string };
