...
import {
  runEmployeePerformanceScoresNow,
  runFarmerOpportunityScoresNow,
} from '../../services/intelligence/farmer-opportunity-score.worker.js';
...
export async function osIntelligenceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${api}/weather-rules`, async (request, reply) => {
    ...
    const q = request.query as { status?: string; crop?: string };
  ...
  app.get(`${api}/opportunity-scores/top`, async (request, reply) => {
    ...
    let query = supabase
      .from('farmer_scores')
      .select(
        'farmer_id, opportunity_score, engagement_score, trust_score, calculated_at, farmers(name, phone, district)'
      )
  ...
  app.post(`${api}/opportunity-scores/recalculate`, async (request, reply) => {
    ...
    if (body.employeeProfileId && !body.dryRun) {
      ...
      const score = await employeePerformanceEngineService.scoreEmployee(
    ...
    const employeeBatch =
      body.includeEmployees !== false && !body.dryRun
        ? await runEmployeePerformanceScoresNow({ limit: body.limit ?? 200, dryRun: body.dryRun })
        : null;

    let businessActions: Record<string, unknown> | null = null;
  ...
  app.get(`${api}/performance-scores/employees/:employeeProfileId`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const { employeeProfileId } = request.params as { employeeProfileId: string };
  ...
  app.get(`${dash}/overview`, async (request, reply) => {
    ...
    const overview = await opportunityIntelligenceDashboardService.getOverview(
      q.days ? Number(q.days) : 30
    );
    const eventVolume = await opportunityIntelligenceDashboardService.getEventVolumeByType(
      q.days ? Number(q.days) : 30
    );
  ...
  app.get(`${dash}/districts`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const q = request.query as { limit?: string };
    const districts = await opportunityIntelligenceDashboardService.getDistrictHeatmap(
  ...
  app.get(`${dash}/farmers/top`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const q = request.query as { limit?: string; minScore?: string; district?: string };
  ...
  app.get(`${dash}/farmers/at-risk`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const q = request.query as { limit?: string };
    const farmers = await opportunityIntelligenceDashboardService.listAtRiskFarmers(
  ...
  app.get(`${dash}/employees`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const q = request.query as { limit?: string };
    const employees = await opportunityIntelligenceDashboardService.listEmployeeLeaderboard(
  ...
  app.get(`${api}/performance-scores/employees/top-relationship-builders`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const q = request.query as { limit?: string };
    const employees = await opportunityEmployeeLeaderboardsService.listTopRelationshipBuilders(
      q.limit ? Number(q.limit) : 25
    );
    return reply.send({
      ok: true,
  ...
  app.get(`${api}/performance-scores/employees/high-retention`, async (request, reply) => {
    await assertModuleAccess(request, 'intelligence', 'read');
    const q = request.query as { limit?: string };
    const employees = await opportunityEmployeeLeaderboardsService.listHighRetentionEmployees(
      q.limit ? Number(q.limit) : 25
    );
    return reply.send({
      ok: true,
  ...
  app.get(`${api}/opportunity-scores/farmers/:farmerId/trend`, async (request, reply) => {
    ...
    const { farmerId } = request.params as { farmerId: string };
    const q = request.query as { limit?: string };
    const trend = await opportunityScoreTrendsService.getFarmerTrend(
      farmerId,
      q.limit ? Number(q.limit) : 12
    );
  ...
  app.get(`${api}/performance-scores/employees/:employeeProfileId/trend`, async (request, reply) => {
    ...
    const { employeeProfileId } = request.params as { employeeProfileId: string };
    const q = request.query as { limit?: string };
    const trend = await opportunityScoreTrendsService.getEmployeeTrend(
      employeeProfileId,
      q.limit ? Number(q.limit) : 12
    );
    return reply.send({ ok: true, trend });
  ...
  app.patch(oppConfig, async (request, reply) => {
    ...
    const admin = (request as { admin?: { id: string } }).admin;
    const config = await opportunityIntelligenceConfigService.update({
      farmerWeightOverrides: body.farmerWeightOverrides as never,
      employeeWeightOverrides: body.employeeWeightOverrides as never,
  ...
  app.post(`${alertsApi}/:alertId/acknowledge`, async (request, reply) => {
    ...
    const admin = (request as { admin?: { id: string } }).admin;
    if (!admin?.id) return reply.status(401).send({ ok: false, error: 'Unauthorized' });
