...
export async function osPartnerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(`${api}/leads/:leadId/allocate`, async (request, reply) => {
    ...
    const allocations = await partnerLeadAllocationService.allocateLeadToPartners(
      leadId,
      body.farmerId
    );
  ...
  app.put(`${api}/commission/:categoryKey`, async (request, reply) => {
    const admin = await assertModuleAccess(request, 'partner_program', 'write');
    const { categoryKey } = request.params as { categoryKey: string };
    const body = z
      .object({
  ...
  app.get(`${api}/events/list`, async (request, reply) => {
    await assertModuleAccess(request, 'partner_program', 'read');
    const q = request.query as { partnerId?: string };
  ...
  app.patch(`${api}/applications/:id/stage`, async (request, reply) => {
    ...
    const body = z
      ...
      .parse(request.body);
    const application = await partnerOnboardingService.advanceStage(
      id,
      body.stage,
      admin.email,
      body.notes
  ...
  app.get(`${api}/control-tower/:farmerId`, async (request, reply) => {
    ...
    let fraudSignals: Array<Record<string, unknown>> = [];
    ...
    if (partnerId) {
      const { data: partnerRow } = await supabase
        ...
        .eq('id', partnerId)
        .maybeSingle();
      partnerReliability = partnerRow as Record<string, unknown> | null;
      const { data: signals } = await supabase
        ...
        .limit(20);
      fraudSignals = (signals ?? []) as Array<Record<string, unknown>>;
    }
    return reply.send({
      ok: true,
      farmer,
