...
export async function osWarehouseRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(`${api}/fulfillment/orders/:id/generate-awb`, async (request, reply) => {
    ...
    try {
      const result = await fulfillmentService.provisionShipment(
        id,
        (request as { adminEmail?: string }).adminEmail,
        { forceRecreate: body.forceRecreate ?? false }
      );
  ...
  app.patch(`${api}/fulfillment/orders/:id/shipping-method`, async (request, reply) => {
    ...
    const body = z
      ...
      .parse(request.body);
    const order = await fulfillmentService.setShippingMethod(
      id,
      body.method,
      (request as { adminEmail?: string }).adminEmail
    );
  ...
  app.post(`${api}/fulfillment/orders/:id/package/confirm`, async (request, reply) => {
    ...
    const estimate = await fulfillmentService.confirmPackage(
      id,
      (request as { adminEmail?: string }).adminEmail,
  ...
  app.post(`${api}/fulfillment/orders/:id/package/override`, async (request, reply) => {
    ...
    const body = z
      ...
      .parse(request.body);
    const estimate = await fulfillmentService.overridePackage(
      id,
      body,
      (request as { adminEmail?: string }).adminEmail
    );
  ...
  app.get(`${api}/masters`, async (request, reply) => {
    ...
    const type = z.string().min(1).parse(q.type ?? 'manual_courier') as MasterType;
