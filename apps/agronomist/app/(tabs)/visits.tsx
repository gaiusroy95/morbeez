...
function isBlock(item: ListItem): item is AgronomistBlockRow {
...
export default function VisitsScreen() {
  ...
  async function search() {
    setError('');
    try {
      const rows = await agronomistClient.searchFarmers(query.trim());
      setFarmers(
        rows.map((f) => ({
          id: f.id,
          name: f.name,
  ...
  async function selectFarmer(f: Farmer) {
    ...
    setBusy(true);
    try {
      setBlocks(await agronomistClient.getFarmerBlocks(f.id));
    } catch (e) {
