...
export function FarmerTeamPanel({ farmerId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setEntries(await agronomistClient.getFarmerTeamTimeline(farmerId));
    } catch (e) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await agronomistClient.addFarmerTeamComment(farmerId, draft.trim());
      setDraft('');
      await load();
    } catch (e) {
