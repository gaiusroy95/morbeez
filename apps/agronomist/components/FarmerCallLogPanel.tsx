...
const FALLBACK_OUTCOMES: DynamicSelectOption[] = [
  { key: 'answered', value: 'answered', label: 'Answered' },
  { key: 'no_answer', value: 'no_answer', label: 'No answer' },
  { key: 'busy', value: 'busy', label: 'Busy' },
  { key: 'callback_requested', value: 'callback_requested', label: 'Callback requested' },
];

function mastersToOutcomeOptions(
...
export function FarmerCallLogPanel({ farmerId, leadId }: Props) {
  ...
  const [outcomeOptions, setOutcomeOptions] = useState<DynamicSelectOption[]>(FALLBACK_OUTCOMES);
  const [outcomesLoading, setOutcomesLoading] = useState(false);
  const [reminderReason, setReminderReason] = useState('');
  const [saving, setSaving] = useState(false);

  const loadOutcomeMasters = useCallback(async () => {
    setOutcomesLoading(true);
    try {
      const items = await agronomistClient.listCrmMasters('interaction_outcome');
      const options = items.length ? mastersToOutcomeOptions(items) : FALLBACK_OUTCOMES;
      ...
      setOutcomeOptions(FALLBACK_OUTCOMES);
      setCallOutcome((prev) => prev || FALLBACK_OUTCOMES[0]?.value || '');
    } finally {
  ...
  useEffect(() => {
    void load();
    void loadOutcomeMasters();
  }, [load, loadOutcomeMasters]);
  ...
  async function submitCallLog() {
    if (!canWrite || !callOutcome.trim()) return;
    setSaving(true);
    setError('');
    try {
  ...
  return (
    <View style={styles.root}>
      ...
      <Panel title="Call log summary">
        ...
        />
        {summary?.lastCallSummary ? <Text style={styles.aiSummary}>{summary.lastCallSummary}</Text> : null}
        {canWrite ? (
        ...
        <View style={styles.modal}>
          ...
          <ScrollView contentContainerStyle={styles.modalBody}>
            <DynamicSelect
              ...
              onChange={(value) => setCallOutcome(value)}
            />
            <Text style={styles.masterHint}>
              Outcomes are managed by super admin in CRM masters for consistent tracking across staff.
            </Text>
            <TextField
              label="Notes / summary"
              value={callNotes}
            ...
            />
            <Btn
              label={saving ? 'Saving…' : 'Save call log'}
              onPress={() => void submitCallLog()}
              disabled={saving || !callOutcome.trim()}
            />
          </ScrollView>
        </View>
      </Modal>
      ...
      <Modal visible={selected != null} animationType="slide" onRequestClose={() => setSelected(null)}>
        <View style={styles.modal}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>{String(selected?.interactionType ?? 'Interaction detail')}</Text>
...
const styles = StyleSheet.create({
  ...
  masterHint: { fontSize: 11, color: tokens.textMuted, marginBottom: 12, lineHeight: 16 },
