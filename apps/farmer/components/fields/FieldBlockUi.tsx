...
import { formatInr, t, tokens, type CultivationActivity, type FieldOverview, type PortalSoilReport } from '@morbeez/shared';
import { Btn, HealthBadge, StageProgressBar } from '@morbeez/ui-native';
      ...
      <View style={styles.summaryTop}>
        ...
        <View style={styles.summaryMain}>
          <View style={styles.summaryTitleRow}>
            ...
            <HealthBadge status={block.healthStatus} label={t('activeStatus', locale)} />
          </View>
          <View style={styles.summaryGrid}>
            ...
            {block.plantingDateLabel ? (
              <SummaryCell label={t('plantingDateLabel', locale)} value={block.plantingDateLabel} />
            ) : null}
            {block.dap != null ? (
              <SummaryCell label={t('dapLabel', locale)} value={`${block.dap} Days`} />
            ) : null}
          </View>
          {block.stage ? (
...
export function ActivityTimeline({
  ...
  if (!activities.length) {
    return <Text style={styles.muted}>{t('noActivitiesYet', locale)}</Text>;
  }
...
export function SoilTestsPanel({
  ...
  return (
    <>
      {latest ? (
        <View style={styles.soilCard}>
          <Text style={styles.soilCardTitle}>
            {t('latestSoilTest', locale)} · {latest.dateLabel}
          </Text>
          {latest.metrics?.length ? (
            ...
            <Text style={styles.muted}>{t('soilReportOnFile', locale)}</Text>
          )}
          {latest.pdfUrl ? (
            <Pressable style={styles.reportBtn} onPress={() => void Linking.openURL(latest.pdfUrl!)}>
              <Text style={styles.reportBtnText}>📄 {t('viewFullReport', locale)}</Text>
            </Pressable>
          ) : null}
        </View>
      ) : (
        <Text style={styles.muted}>{t('noSoilTestsYet', locale)}</Text>
      )}

      {reports.length ? (
        <View style={styles.soilHistory}>
          <Text style={styles.soilHistoryTitle}>{t('allSoilTests', locale)}</Text>
          {reports.map((r) => (
            <Pressable
              key={r.id}
      ...
      <Btn label={`+ ${t('newSoilTest', locale)}`} variant="secondary" onPress={onNewTest} />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  tabLabel: { fontSize: 14, fontWeight: '600', color: tokens.textMuted },
  tabLabelActive: { color: tokens.green800 },
  tabUnderline: { position: 'absolute', bottom: 0, left: 12, right: 12, height: 3, backgroundColor: tokens.green700, borderRadius: 2 },
  summaryCard: {
  ...
  chevron: { fontSize: 22, color: tokens.textMuted, fontWeight: '300' },
