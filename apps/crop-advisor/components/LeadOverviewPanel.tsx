...
import { formatDate, formatPhoneDisplay, cropAdvisorClient, tokens, type LeadWorkspaceTab, type CropAdvisorWorkspaceSummary } from '@morbeez/shared';
...
export function LeadOverviewPanel({ leadId, summary, onNavigate }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const p = await cropAdvisorClient.getLeadFarmerProfile(leadId);
      setProfile(p);
    } catch (e) {
  ...
  return (
    <View style={styles.root}>
      ...
      <Panel title="CRM information">
        ...
        />
        <KeyValueRow label="Assigned cropAdvisor" value={summary.lead.assignedCropAdvisor ?? '—'} />
