...
import { cropAdvisorClient, tokens } from '@morbeez/shared';
import {
  AlertBox,
  Btn,
  DynamicSelect,
...
export function AddInteractionModal({ visible, leadId, onClose, onSaved }: Props) {
  ...
  const loadMasters = useCallback(async () => {
    ...
    try {
      const [types, outcomes, nextActions, rawBlocks] = await Promise.all([
        cropAdvisorClient.listMasters('interaction_type'),
        cropAdvisorClient.listMasters('interaction_outcome'),
        cropAdvisorClient.listMasters('interaction_next_action'),
        cropAdvisorClient.listLeadBlocks(leadId),
      ]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await cropAdvisorClient.createLeadInteraction(leadId, {
      ...
      <View style={styles.modal}>
        ...
        >
          ...
          <DynamicSelect
            ...
            onAdd={async (name) => {
              const item = await cropAdvisorClient.createMaster({
            ...
            {addFieldActivity ? (
              <View style={styles.nested}>
                <DynamicSelect
                  ...
                  onAdd={async (name) => {
                    const type = await cropAdvisorClient.createFieldActivityType(leadId, {
                      activityName: name,
