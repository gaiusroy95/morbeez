...
import { formatDate, cropAdvisorClient, tokens, type CropAdvisorTimelineItem } from '@morbeez/shared';
...
export function LeadInteractionsPanel({ leadId, farmerId }: Props) {
  ...
  const [timeline, setTimeline] = useState<CropAdvisorTimelineItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [tl, wa] = await Promise.all([
        cropAdvisorClient.getLeadTimeline(leadId),
        cropAdvisorClient.listWhatsAppMessages(farmerId).catch(() => []),
      ]);
      setTimeline(tl);
      setWhatsapp(wa);
    } catch (e) {
