...
import { formatDate, cropAdvisorClient, tokens } from '@morbeez/shared';
...
export function LeadRecommendationsPanel({ leadId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setRecs(await cropAdvisorClient.listLeadRecommendations(leadId));
    } catch (e) {
  ...
  const createFollowUp = async () => {
    ...
    setSaving(true);
    try {
      await cropAdvisorClient.createLeadTask(leadId, {
    ...
    <View style={styles.root}>
      ...
      <Panel title="Recommendations">
        <Text style={styles.hint}>
          WhatsApp follow-up automation is handled by the system. Create a crop advisor follow-up when the farmer declines.
        </Text>
      </Panel>
