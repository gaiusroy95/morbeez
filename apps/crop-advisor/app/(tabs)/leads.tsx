...
import { cropAdvisorClient, tokens, type CropAdvisorLeadRow, formatPhoneDisplay, telHref } from '@morbeez/shared';
import { AlertBox, Loading } from '@morbeez/ui-native';

export default function LeadsScreen() {
  const router = useRouter();
  const [leads, setLeads] = useState<CropAdvisorLeadRow[]>([]);
  ...
  const load = useCallback(async (background = false) => {
    ...
    setError('');
    try {
      const rows = await cropAdvisorClient.listLeads({ scope: 'mine', limit: 50 });
      setLeads(rows);
    } catch (e) {
