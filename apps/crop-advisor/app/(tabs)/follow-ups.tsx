...
import { cropAdvisorClient, tokens, type CropAdvisorFollowUpSections, type CropAdvisorTaskRow } from '@morbeez/shared';
import { AlertBox, Btn, Loading, Panel } from '@morbeez/ui-native';

const EMPTY_SECTIONS: CropAdvisorFollowUpSections = {
  today: [],
  overdue: [],
  upcoming: [],
...
export default function FollowUpsScreen() {
  ...
  const [sections, setSections] = useState<CropAdvisorFollowUpSections>(EMPTY_SECTIONS);
  ...
  const snooze = async (taskId: string) => {
    ...
    try {
      await cropAdvisorClient.snoozeTask(taskId, due.toISOString());
      await load(true);
    } catch (e) {
  ...
  const openTask = (task: CropAdvisorTaskRow) => {
