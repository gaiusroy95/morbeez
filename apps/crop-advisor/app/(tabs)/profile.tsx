...
import { useCropAdvisorDashboard } from '@/context/CropAdvisorDashboardContext';

export default function ProfileScreen() {
  const router = useRouter();
  const { admin, logout } = useStaffAuth();
  const { locale, setLocale } = useLocale();
  const { dashboard, loading } = useCropAdvisorDashboard();
  const overview = dashboard?.overview;

  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      <Panel title={admin?.fullName ?? admin?.email ?? t('crop_advisor', locale)}>
