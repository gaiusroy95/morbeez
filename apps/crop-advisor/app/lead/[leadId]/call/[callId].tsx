...
import { formatDate, cropAdvisorClient, tokens, type CropAdvisorCallRow } from '@morbeez/shared';
...
export default function CallDetailScreen() {
  ...
  const [call, setCall] = useState<CropAdvisorCallRow | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setCall(await cropAdvisorClient.getCall(String(callId)));
    } catch (e) {
  ...
  const createFollowUp = async (title: string) => {
    ...
    setSaving(true);
    try {
      await cropAdvisorClient.createLeadTask(String(leadId), { title });
    } catch (e) {
