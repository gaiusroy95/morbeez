...
export default function FindingReviewScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const q = await staffApi<{ ok: boolean; items: QueueItem[] }>(`${AGRO}/queue?limit=80`);
      const match = (q.items ?? []).find((i) => i.finding.id === id);
      if (match) {
        setItem(match);
        setForm({
          recommendationId: match.existingRecommendation?.id ?? '',
          findingId: match.finding.id,
          farmerId: match.finding.farmerId,
          blockId: match.block?.id ?? match.finding.blockId ?? '',
          issueDetected: match.finding.diseasePest ?? '',
          recommendationText: '',
          dosage: '',
          weatherWarning: '',
          language: match.farmer?.preferredLanguage?.slice(0, 2) ?? 'en',
        });
      } else {
        const d = await staffApi<{ ok: boolean; finding: Record<string, unknown>; recommendation: { id: string } | null }>(
          `${AGRO}/findings/${id}`
        );
        const f = d.finding;
        setItem({
          ...
          farmer: null,
          block: null,
          existingRecommendation: d.recommendation ? { id: d.recommendation.id, status: 'draft' } : null,
        });
        setForm((prev) => ({
          ...prev,
  ...
  async function runAiSuggest() {
    ...
    setError('');
    try {
      const d = await staffApi<{ ok: boolean } & AiSuggestion>(
        `${AGRO}/findings/${id}/ai-suggest`,
        { method: 'POST', body: '{}' }
      );
  ...
  async function saveDraft() {
    ...
    setError('');
    try {
      const d = await staffApi<{ ok: boolean; recommendation: { id: string } }>(`${AGRO}/drafts`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  async function submitForApproval() {
    ...
    setError('');
    try {
      await staffApi(`${AGRO}/recommendations/${form.recommendationId}/submit`, {
        method: 'POST',
        body: '{}',
      });
      router.back();
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
      ...
      <Panel title="Finding detail">
        ...
        {finding.diseasePest ? (
          <KeyValueRow label="Issue" value={finding.diseasePest} />
        ) : null}
        ...
        <Panel title="Recommendation">
          {aiMeta?.confidence != null ? (
            <Text style={styles.confidence}>
              AI confidence: {Math.round(aiMeta.confidence * 100)}%
            </Text>
          ) : null}
          <Btn
