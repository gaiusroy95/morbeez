...
export async function renderCropAdvisorWorkspace() {
  const el = $('#main-content');
  el.innerHTML = '<div class="products-loading"><div class="spinner"></div></div>';

  try {
    const q = new URLSearchParams({
      scope: state.cropAdvisor.scope,
      stage: state.cropAdvisor.stage,
      page: String(state.cropAdvisor.page),
      limit: '15',
      ...(state.cropAdvisor.search ? { search: state.cropAdvisor.search } : {}),
    });

    const [overviewRes, leadsRes] = await Promise.all([
      api('/console/api/v1/crop-advisor/overview'),
      api(`/console/api/v1/crop-advisor/leads?${q}`),
    ]);
    ...
    if (!state.cropAdvisor.selectedLeadId && leads[0]) {
      state.cropAdvisor.selectedLeadId = leads[0].id;
    }
    if (state.cropAdvisor.selectedLeadId && !leads.find((l) => l.id === state.cropAdvisor.selectedLeadId)) {
      state.cropAdvisor.selectedLeadId = leads[0]?.id || null;
    }

    const selectedId = state.cropAdvisor.selectedLeadId;
          ...
          <div class="tc-leads-pane">
            <div class="tc-leads-toolbar">
              <div class="tc-scope-tabs">
                <button type="button" class="tc-scope-tab ${state.cropAdvisor.scope === 'mine' ? 'active' : ''}" data-scope="mine">My Leads (${counts.mine})</button>
                <button type="button" class="tc-scope-tab ${state.cropAdvisor.scope === 'all' ? 'active' : ''}" data-scope="all">All Leads (${counts.all})</button>
              </div>
              <div class="tc-leads-filters">
                <select id="tc-stage-filter" class="products-select">
                  <option value="all">All Stages</option>
                  <option value="new_lead" ${state.cropAdvisor.stage === 'new_lead' ? 'selected' : ''}>New Lead</option>
                  <option value="interested" ${state.cropAdvisor.stage === 'interested' ? 'selected' : ''}>Interested</option>
                  <option value="follow_up" ${state.cropAdvisor.stage === 'follow_up' ? 'selected' : ''}>Follow-up</option>
                  <option value="recommendation" ${state.cropAdvisor.stage === 'recommendation' ? 'selected' : ''}>Recommendation</option>
                  <option value="order_placed" ${state.cropAdvisor.stage === 'order_placed' ? 'selected' : ''}>Order Placed</option>
                </select>
                <input type="search" id="tc-lead-search" class="products-search tc-lead-search" placeholder="Search leads…" value="${escapeHtml(state.cropAdvisor.search)}" />
    ...
    const selectLead = async (id) => {
      state.cropAdvisor.selectedLeadId = id;
      state.cropAdvisor.leadTab = state.cropAdvisor.leadTab || 'overview';
    ...
    el.querySelectorAll('[data-scope]').forEach((btn) => {
      btn.addEventListener('click', () => {
        state.cropAdvisor.scope = btn.dataset.scope;
        state.cropAdvisor.page = 1;
        renderCropAdvisorWorkspace();
      });
    });

    $('#tc-stage-filter')?.addEventListener('change', () => {
      state.cropAdvisor.stage = $('#tc-stage-filter').value;
      renderCropAdvisorWorkspace();
    });

    $('#tc-lead-search')?.addEventListener('keydown', (ev) => {
      if (ev.key === 'Enter') {
        state.cropAdvisor.search = $('#tc-lead-search').value.trim();
        renderCropAdvisorWorkspace();
      }
    });
  } catch (err) {
