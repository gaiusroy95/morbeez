...
import { openAddLeadModal } from './crop-advisor-shared.js';
 ...
 */
export function bindCropAdvisorCrmTopbar(pageTitle = 'Crop Advisor CRM Workspace') {
  const topbar = $('.topbar');
  if (!topbar) return;
  ...
  $('#btn-phone-quick')?.addEventListener('click', () => {
    location.hash = 'cropAdvisor/calls';
  });
...
export function refreshCrmTopbarUser() {
  if (document.body.classList.contains('route-crop-advisor') || document.querySelector('.crm-topbar-shell')) {
    fillUserProfile();
  }
}
