...
import { enrichLeadData } from './crop-advisor-lead-demo.js';
import {
  showAddBlockModal,
...
export function renderLeadHeader(d, opts = {}) {
  ...
  const back = opts.inPane
    ? ''
    : `<a href="#cropAdvisor" class="ld-back">${icon('arrowLeft', 'icon-back')} Back to Leads</a>`;
...
function renderBlocksTab(d, blockWs) {
  ...
  const active = state.cropAdvisor.blockId || blocks[0]?.id;
  const block = blocks.find((b) => b.id === active) || blocks[0];
  const sub = state.cropAdvisor.blockTab || 'overview';
...
export async function loadLeadData(leadId, tab) {
  const base = await api(`/console/api/v1/crop-advisor/leads/${leadId}`);
  let data = { ...base };
  let ffData = null;
  let blockWs = null;

  try {
    const crm = await api(`/console/api/v1/crop-advisor/leads/${leadId}/crm`);
  ...
  if (tab === 'interactions') {
    try {
      ...
      const ix = await api(`/console/api/v1/crop-advisor/leads/${leadId}/interactions?${q}`);
    ...
    try {
      const crmIx = await api(`/console/api/v1/crop-advisor/leads/${leadId}/interactions`);
  ...
  if (tab === 'field-findings') {
    const ffLimit = state.cropAdvisor.ffLimit || 10;
    const ffPage = state.cropAdvisor.ffPage || 1;
    ffData = await api(`/console/api/v1/crop-advisor/leads/${leadId}/field-findings?limit=${ffLimit}&page=${ffPage}`);
  }

  if (tab === 'blocks') {
    const blocks = data.blocks || [];
    const blockId = state.cropAdvisor.blockId || blocks[0]?.id;
    if (blockId) {
      state.cropAdvisor.blockId = blockId;
      try {
        blockWs = await api(`/console/api/v1/crop-advisor/leads/${leadId}/blocks/${blockId}/workspace`);
      } catch {
        blockWs = null;
      }
    }
...
export function bindLeadDetail(leadId, onRefresh, dataRef = {}) {
  document.querySelectorAll('.ld-tab').forEach((btn) => {
    btn.addEventListener('click', () => {
      state.cropAdvisor.leadTab = btn.dataset.leadTab;
      onRefresh(leadId);
    });
  });
  ...
  document.querySelectorAll('[data-goto-tab]').forEach((el) => {
    el.addEventListener('click', (e) => {
      e.preventDefault();
      state.cropAdvisor.leadTab = el.dataset.gotoTab;
      onRefresh(leadId);
    });
  });
  ...
  document.querySelectorAll('[data-block-id]').forEach((btn) => {
    btn.addEventListener('click', () => {
      state.cropAdvisor.blockId = btn.dataset.blockId;
      state.cropAdvisor.leadTab = 'blocks';
      onRefresh(leadId);
    });
  });
  ...
  document.querySelectorAll('[data-block-tab]').forEach((btn) => {
    btn.addEventListener('click', () => {
      state.cropAdvisor.blockTab = btn.dataset.blockTab;
      onRefresh(leadId);
    });
  });
  ...
  $('#ld-add-note')?.addEventListener('click', async () => {
    ...
    if (!note?.trim()) return;
    try {
      await api(`/console/api/v1/crop-advisor/leads/${leadId}/notes`, { method: 'POST', body: JSON.stringify({ note: note.trim() }) });
  ...
  $('#ld-change-block')?.addEventListener('change', (ev) => {
    state.cropAdvisor.blockId = ev.target.value;
    onRefresh(leadId);
  });
  ...
  $('#ld-add-soil')?.addEventListener('click', async () => {
    const blockId = state.cropAdvisor.blockId || blocks[0]?.id;
    try {
      await api(`/console/api/v1/crop-advisor/leads/${leadId}/soil-reports`, {
        method: 'POST',
  ...
  $('#ld-ff-limit')?.addEventListener('change', () => {
    state.cropAdvisor.ffLimit = Number($('#ld-ff-limit').value);
    state.cropAdvisor.ffPage = 1;
    onRefresh(leadId);
  });

  document.querySelectorAll('[data-ld-page]').forEach((btn) => {
    btn.addEventListener('click', () => {
      state.cropAdvisor.ffPage = Number(btn.dataset.ldPage);
      onRefresh(leadId);
    });
  });
  ...
  $('#ld-update-stage')?.addEventListener('click', async () => {
    ...
    if (!stage) return;
    try {
      await api(`/console/api/v1/crop-advisor/leads/${leadId}`, { method: 'PATCH', body: JSON.stringify({ stage }) });
  ...
  $('#ld-ix-reset')?.addEventListener('click', () => {
    state.cropAdvisor.crmFilters.interactions = { type: '', status: '', blockId: '' };
    onRefresh(leadId);
  });
  ...
  bindRowMenus(document, {
    'archive-ix': async (el) => {
      ...
      await api(`/console/api/v1/crop-advisor/interactions/${id}/archive`, { method: 'POST', body: '{}' });
      showToast('Archived');
      onRefresh(leadId);
    },
    'edit-rec': async (el) => {
    ...
    'archive-rec': async (el) => {
      await api(`/console/api/v1/crop-advisor/recommendations/${el.dataset.id}/archive`, { method: 'POST', body: '{}' });
      showToast('Archived');
      onRefresh(leadId);
    },
    'archive-ff': async (el) => {
      ...
      await api(`/console/api/v1/crop-advisor/field-findings/${id}/archive`, { method: 'POST', body: '{}' });
      showToast('Archived');
      onRefresh(leadId);
    },
  });
...
export async function renderLeadDetailInto(container, leadId, opts = {}) {
  if (!container) return;
  const tab = state.cropAdvisor.leadTab || 'overview';
