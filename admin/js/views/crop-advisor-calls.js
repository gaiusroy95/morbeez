...
export async function renderCropAdvisorCalls() {
  const el = $('#main-content');
  el.innerHTML = '<div class="products-loading"><div class="spinner"></div></div>';

  try {
    const data = await api('/console/api/v1/crop-advisor/calls');
    ...
    el.innerHTML = `
      <div class="tc-subpage">
        <p class="tc-subpage-desc">Outbound and inbound call history logged by crop advisors.</p>
