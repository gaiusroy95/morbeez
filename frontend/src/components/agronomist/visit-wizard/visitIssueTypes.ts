...
const GLOBAL_FALLBACKS: Record<IssueCategory, string[]> = {
  ...
  nutrient_deficiency: ['Nitrogen', 'Phosphorus', 'Potassium', 'Zinc', 'Magnesium'],
...
const CROP_ISSUE_FALLBACKS: Record<string, Partial<Record<IssueCategory, string[]>>> = {
  ginger: {
    disease: ['Rhizome Rot', 'Soft Rot', 'Leaf Spot', 'Bacterial Wilt'],
    pest: ['Thrips', 'Shoot Borer', 'Rhizome Scale'],
    nutrient_deficiency: ['Magnesium', 'Zinc', 'Nitrogen'],
...
export function getIssueCategoryLabel(category: IssueCategory): string {
  return ISSUE_CATEGORY_OPTIONS.find((o) => o.value === category)?.label ?? category;
}
