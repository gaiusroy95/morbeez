...
import { cropAdvisorClient } from '@morbeez/shared';
...
export function CropAdvisorSalesOpportunitiesPanel({ canWrite, onOpenLead }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setItems(await cropAdvisorClient.listSalesOpportunities());
    } catch (e) {
  ...
  async function updateStatus(id: string, status: string) {
    setBusyId(id);
    try {
      await cropAdvisorClient.updateSalesOpportunityStatus(id, status);
      await load();
    } catch (e) {
