...
import { formatDate, cropAdvisorClient } from '@morbeez/shared';
...
export function CropAdvisorLeadTeamPanel({ leadId, canWrite }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setEntries(await cropAdvisorClient.getLeadTeamTimeline(leadId));
    } catch (e) {
  ...
  async function postComment() {
    ...
    setError('');
    try {
      await cropAdvisorClient.addLeadTeamComment(leadId, draft.trim());
      setDraft('');
      await load();
    } catch (e) {
  ...
  return (
    <div className="tc-team-panel">
      ...
      <p className="tc-team-hint">
        Internal discussion visible to cropAdvisor, partner, expert, and admin — not the farmer.
      </p>
