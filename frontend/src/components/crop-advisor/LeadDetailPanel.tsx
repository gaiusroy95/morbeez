...
import { CropAdvisorLeadTeamPanel } from './CropAdvisorLeadTeamPanel';
...
type Props = {
  leadId: string;
  canWrite: boolean;
  variant?: 'crop_advisor' | 'agronomist';
};
...
export function LeadDetailPanel({ leadId, canWrite, variant = 'crop_advisor' }: Props) {
  ...
  return (
    <div className="tc-detail-root">
      ...
      <div className="tc-detail-body">
        ...
        {tab === 'team' ? (
          <CropAdvisorLeadTeamPanel leadId={leadId} canWrite={canWrite} />
        ) : null}
            ...
            <aside className="tc-wa-session">
              ...
              <label className="block text-xs text-ink-secondary">
                ...
                <StaticSelect
                  ...
                  options={['ai', 'crop_advisor', 'agronomist'].map((o) => ({ value: o, label: o }))}
                />
              </label>
              <label
