import { useCropAdvisorHeaderOptional } from '../../context/CropAdvisorHeaderContext';
...
export function CropAdvisorWorkspaceHeader({ onOpenMenu, onLogout }: Props) {
  const header = useCropAdvisorHeaderOptional();

  const search = header?.search ?? '';
  const setSearch = header?.setSearch;
  const canWrite = header?.canWrite ?? false;
    ...
    <WorkspaceHeader
      ...
      trailingActions={
        <>
          <button
            type="button"
            className="tool-btn tool-btn-wa"
            aria-label="WhatsApp"
            title={selectedPhone ? 'Open WhatsApp' : 'Select a lead first'}
