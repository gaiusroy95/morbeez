...
import { useCropAdvisorHeader } from '../context/CropAdvisorHeaderContext';
import { useAuth } from '../context/AuthContext';
import { api } from '../lib/api';
import { LeadDetailPanel } from '../components/crop-advisor/LeadDetailPanel';
import { LeadOperationsTable } from '../components/crop-advisor/lead-queue/LeadOperationsTable';
import type { OperationalLead } from '../components/crop-advisor/lead-queue/lead-queue-types';
import { EscalationsPanel } from '../components/crop-advisor/EscalationsPanel';
import { AiCallingPanel } from '../components/crop-advisor/AiCallingPanel';
import { CropAdvisorQcDashboard } from '../components/crop-advisor/CropAdvisorQcDashboard';
import { CropAdvisorFollowUpsPanel } from '../components/crop-advisor/CropAdvisorFollowUpsPanel';
import { CropAdvisorSalesOpportunitiesPanel } from '../components/crop-advisor/CropAdvisorSalesOpportunitiesPanel';
import { CropAdvisorIntelligenceBar } from '../components/crop-advisor/CropAdvisorIntelligenceBar';
...
} from '../components/crop-advisor/CropBlockFields';
...
export function CropAdvisorCrmPage({ canWrite }: { canWrite: boolean }) {
  const [searchParams, setSearchParams] = useSearchParams();
  const { patchHeader } = useCropAdvisorHeader();
  const { can } = useAuth();
  ...
  useEffect(() => {
    ...
    const leadsChannel = realtimeClient
      .channel('crop-advisor-leads-live')
      .on(
        'postgres_changes',
    ...
    const tasksChannel = realtimeClient
      .channel('crop-advisor-tasks-live')
      .on(
        'postgres_changes',
    ...
    const escalationsChannel = realtimeClient
      .channel('crop-advisor-escalations-live')
      .on(
        'postgres_changes',
  ...
  return (
    <div className="crop-advisor-page space-y-4">
      {showNotifications ? (
      ...
      {crmView === 'followUps' ? (
        <CropAdvisorFollowUpsPanel canWrite={canWrite} onOpenLead={(id) => openLeadWorkspace(id)} />
      ) : null}

      {crmView === 'qc' ? <CropAdvisorQcDashboard /> : null}
      {crmView === 'escalations' ? (
      ...
      {crmView === 'workspace' ? (
        <>
          <MyEarningsPanel />
          <CropAdvisorIntelligenceBar onSelectLead={(id) => openLeadWorkspace(id)} />
          <CropAdvisorSalesOpportunitiesPanel
            canWrite={canWrite}
      ...
      <div className="max-h-[74vh] space-y-4 overflow-y-auto pr-1">
        <section className="rounded-lg border border-emerald-200 bg-emerald-50/60 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-ink-muted">Complete core fields for cleaner handoff to crop advisor workspace.</p>
            </div>
        ...
        <section className="rounded-lg border border-indigo-200 bg-indigo-50/40 p-3">
          ...
          <p className="mb-2 text-xs text-ink-muted">
            Channel + campaign required for marketer scorecards. CropAdvisor manual leads default to field
            visits.
          </p>
          <div className="grid gap-3 md:grid-cols-2">
            <Field label="Lead channel">
              <StaticSelect
                ...
                options={[
                  ...
                  { value: 'field', label: 'Field / crop advisor' },
